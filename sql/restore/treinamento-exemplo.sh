...
pg_restore --host [IP_SERVIDOR_BANCO] --port 5432 --username "postgres" --dbname "siminc2_tr_new" --disable-triggers -O -x --verbose /var/www/html/siminc2/sql/restore/bkp_prod_dbsiminc.backup

# Executar criação de estrutura de auditoria e mudança de senhas, emails
psql --host [IP_SERVIDOR_BANCO] --port 5432 --username "postgres" --dbname "siminc2_tr_new" -f /var/www/html/siminc2/sql/restore/create_auditoria.sql

# Executar permissções nas tabelas
psql --host [IP_SERVIDOR_BANCO] --port 5432 --username "postgres" --dbname "siminc2_tr_new" -f /var/www/html/siminc2/sql/restore/grants.sql
