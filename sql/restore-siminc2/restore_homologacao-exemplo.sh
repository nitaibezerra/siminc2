...
scp [USUÁRIO_SERVIDOR_BKP]@[IP_SERVIDOR_BKP]:/home/bkp_siminc/bkp_prod_dbsiminc.backup /var/www/html/siminc2/sql/restore-siminc2
...
pg_restore --host [IP_SERVIDOR_BANCO] --port 5432 --username "postgres" --dbname "siminc2_hom_new" --disable-triggers -O -x --verbose /var/www/html/siminc2/sql/restore-siminc2/bkp_prod_dbsiminc.backup

# Executar criação de estrutura de auditoria e mudança de senhas, emails
psql --host [IP_SERVIDOR_BANCO] --port 5432 --username "postgres" --dbname "siminc2_hom_new" -f /var/www/html/siminc2/sql/restore-siminc2/create_auditoria.sql

# Executar permissções nas tabelas
psql --host [IP_SERVIDOR_BANCO] --port 5432 --username "postgres" --dbname "siminc2_hom_new" -f /var/www/html/siminc2/sql/restore-siminc2/grants.sql
