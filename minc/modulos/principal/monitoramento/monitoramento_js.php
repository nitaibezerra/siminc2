...
//		$("#moncpfcnpj_cnpj").mask("99.999.999/9999-99");
...
// 		jQuery('.parceiro1').attr('checked', '');
		...
		jQuery('[name="moncoordmesmodainscr"]').change(function () {
			if (jQuery(this).val() == 't') {
				jQuery('.monnomecoord').val('<?=$dadoscoordenador['nome']?>');
				jQuery('.moncpfcoord').val(mascaraglobal('###.###.###-##', '<?=$dadoscoordenador['cpf']?>'));
				jQuery('.montelcoord').val('<?=$dadoscoordenador['ddd'].$dadoscoordenador['telefone']?>');
				jQuery('.monemailcoord').val('<?=$dadoscoordenador['email']?>');
			} else {
		...
		jQuery('#moncpfcnpj_cnpj').blur(function (e) {
			jQuery.post(window.location.href, {requisicao: 'getPessoaJuridica', cnpj: jQuery('#moncpfcnpj_cnpj').val() }, function (data) {
				jQuery.each(data, function (index, value) {
					jQuery('input[name='+index+']').val(value[0]);
				});
			}, 'json');
		});
	});
	...
	function excluirAnexo(arqid) {
		if (confirm('Deseja realmente apagar este anexo?')) {
			location.href = "minc.php?modulo=principal/monitoramento&acao=A&requisicao=deletaArquivo&arqid=" + arqid;
		}
	}

	function apagarItem(atiid) {
		if (confirm('Deseja realmente apagar este item?')) {
			location.href = "minc.php?modulo=principal/monitoramento&acao=A&apagarItem=" + atiid;
		}
	}
</script>