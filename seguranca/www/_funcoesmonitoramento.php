...
function pegarDadosPorPagina($dados) {
	...
	$resultado = $db->carregar("SELECT COUNT(e.oid) as num,  to_char(estdata, 'DD') as dia  FROM seguranca.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e 
	...
	$resultado = $db->carregar("SELECT COUNT(DISTINCT u.usucpf) as num,  to_char(estdata, 'DD') as dia FROM seguranca.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e
								INNER JOIN seguranca.menu m ON m.mnuid=e.mnuid 
	...
	$resultado = $db->carregar("SELECT ROUND(CAST(AVG(estmemusa) as numeric),2) as num, to_char(estdata, 'DD') as dia FROM seguranca.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e 
						 		INNER JOIN seguranca.menu m ON m.mnuid=e.mnuid
	...
	$resultado = $db->carregar("SELECT ROUND(CAST(AVG(esttempoexec) as numeric),2) as num, to_char(estdata, 'DD') as dia FROM seguranca.estatistica_".sprintf("%04d_%02d", $dados['ano'], $dados['mes'])." e 
						 		INNER JOIN seguranca.menu m ON m.mnuid=e.mnuid
								WHERE e.sisid='".$dados['sisid']."' AND (date_part('year',estdata)::varchar||date_part('month',estdata)::varchar)::varchar='".$dados['ano'].(integer)$dados['mes']."' AND m.mnulink ILIKE '%".$dados['link']."%' 
								GROUP BY to_char(estdata, 'DD') ORDER BY to_char(estdata, 'DD')");
