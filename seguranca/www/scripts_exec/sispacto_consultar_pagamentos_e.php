...
$sql = "select i.iuscpf, lpad(f.fpbmesreferencia::text,2,'0') as fpbmesreferencia, fpbanoreferencia, p.pboid from sispacto.pagamentobolsista p 
		...
		where d.esdid in(".ESD_PAGAMENTO_AGUARDANDO_PAGAMENTO.",".ESD_PAGAMENTO_ENVIADOBANCO.",".ESD_PAGAMENTO_AG_AUTORIZACAO_SGB.") and h.htddata <= (now()- interval '1 months')";
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sispacto_consultar_pagamentos_e.php'";
$db->executar($sql);
$db->commit();
