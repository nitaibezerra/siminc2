...
class TestCurl implements iSenderConsumer {
	...
	public function readUrls() {
		...
		$db->executar("delete from par.empenho_temp where sistema = 'OBRAS'");

		foreach ($limitEmp as $key => $empid) {
			$sql = "insert into par.empenho_temp(codigo, empenho, quantidade, sistema)
					values($key, '$empid', ".sizeof(explode(', ', $empid)).", 'OBRAS')";
			$db->executar($sql);
		}
		$db->commit();
		...
		for ($i=0; $i<$totalUrl; $i++){
			$urls = $_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/seguranca/scripts_exec/par_atualiza_empenho_sigef.php?sistema=OBRAS&cont='.$i;
...
$strAssunto  = SIGLA_SISTEMA. " - Atualização de Empenho OBRA PAR SIGEF";
