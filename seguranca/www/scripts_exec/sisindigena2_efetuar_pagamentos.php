...
$sql = "SELECT * FROM sisindigena2.remessapagamento WHERE remprocessada=FALSE";
...
if($remessapagamento[0]) {
	foreach($remessapagamento as $rem) {
		...
		if($consultarRemessaDePagamentos_obj->remessa->situacao->codigo=='PROCESSADA') {
			...
			$sql = "UPDATE sisindigena2.remessapagamento SET remprocessada=true, remdtprocessamento='".formata_data_sql($consultarRemessaDePagamentos_obj->remessa->situacao->data)."' WHERE remid='".$consultarRemessaDePagamentos_obj->remessa->id."'";
			$db->executar($sql);
			$db->commit();
			...
			$sql = "UPDATE sisindigena2.pagamentobolsista SET remid=null WHERE remid='".$rem['remid']."'";
			$db->executar($sql);
			$sql = "UPDATE sisindigena2.remessapagamento SET remprocessada=true, remdtprocessamento=NOW() WHERE remid='".$rem['remid']."'";
			$db->executar($sql);
			$db->commit();
		}
...
$sql = "SELECT p.pboid FROM sisindigena2.identificacaousuario i 
		INNER JOIN sisindigena2.pagamentobolsista p ON p.iusd = i.iusd 
		INNER JOIN workflow.documento d ON d.docid = p.docid 
		INNER JOIN sisindigena2.remessapagamento r ON r.remid = p.remid 
...
if($reenvios) {
	foreach($reenvios as $pboid) {
		$db->executar("UPDATE sisindigena2.pagamentobolsista SET remid=NULL WHERE pboid='".$pboid."'");
		$db->commit();
	}
}
...
$sql = "SELECT DISTINCT i.iusd, p.pboid, p.pboparcela, i.iuscpf, i.nacid, i.iusnome, i.iusdatanascimento, i.iusnomemae, i.iussexo, m.muncod as co_municipio_ibge_nascimento, m.estuf as sg_uf_nascimento, 
		...
		INNER JOIN sisindigena2.folhapagamento ff ON ff.fpbid = p.fpbid
		INNER JOIN sisindigena2.universidade u ON u.uniid = p.uniid 
		INNER JOIN sisindigena2.universidadecadastro un ON un.uniid = u.uniid  
		INNER JOIN sisindigena2.folhapagamentouniversidade fpu ON fpu.fpbid = ff.fpbid AND CASE WHEN i.picid IS NOT NULL THEN fpu.uncid = un.uncid AND i.picid = fpu.picid ELSE fpu.uncid = un.uncid END
		INNER JOIN workflow.documento d ON d.docid = p.docid 
...
if($_listapagamento) {
	foreach($_listapagamento as $per => $pagamentos) {
		...
		$sql = "INSERT INTO sisindigena2.remessapagamento(remdata) VALUES (NOW()) RETURNING remid;";
		$remid = $db->pegaUm($sql);
		...
		foreach($pagamentos as $pg) {
			...
			$sql = "UPDATE sisindigena2.pagamentobolsista SET remid='".$remid."' WHERE iusd='".$pg['iusd']."'";
			$db->executar($sql);
		}
    	...
    	if($logerro_enviarRemessaDePagamentos=='FALSE') {
			...
			$sql = "UPDATE sisindigena2.remessapagamento SET remano='".$pers[0]."', remmes='".$pers[1]."', remrastreador='".$enviarRemessaDePagamentos_obj->reciboEnvio->rastreador."' WHERE remid='".$remid."'";
			$db->executar($sql);
			$db->commit();
    	} else {
...
if(!$_REQUEST['naoProcessar']) {
		...
		if($consultarSituacaoDePagamentos_obj->situacoes->pagamentos->pagamento) {
			foreach($consultarSituacaoDePagamentos_obj->situacoes->pagamentos->pagamento as $pgs) {
				...
				if($pg->codigo==SGB_AUTORIZADA || $pg->codigo==SGB_HOMOLOGADA || $pg->codigo==SGB_PREAPROVADA || $pg->codigo==SGB_ENVIADOAOSIGEF) {
					$docid = $db->pegaUm("SELECT p.docid FROM sisindigena2.pagamentobolsista p 
										  INNER JOIN workflow.documento d ON d.docid = p.docid 
				...
				if($pg->codigo==SGB_CREDITADA || $pg->codigo==SGB_SACADA) {
					...
					if($esdid_origem && $esdid_origem!=ESD_PAGAMENTO_EFETIVADO) {
					
						$sql = "SELECT aedid FROM workflow.acaoestadodoc WHERE esdidorigem='".$esdid_origem."' and esdiddestino='".ESD_PAGAMENTO_EFETIVADO."'";
						$aedid = $db->pegaUm($sql);
						
						if(!$aedid) {
							echo "Não existe ação do estado #{$esdid_origem} para ".ESD_PAGAMENTO_EFETIVADO."<br>";
						}
	...
	$sql = "update sisindigena2.mensarioavaliacoes x set mavmonitoramento=foo.fatmonitoramento from (
	
			select mm.*, f.*, d.esdid from sisindigena2.mensario m 
			inner join sisindigena2.tipoperfil t on t.iusd = m.iusd and t.pflcod!=849 
			INNER JOIN sisindigena2.fatoresdeavaliacao f ON f.fatpflcodavaliado = t.pflcod
			inner join workflow.documento d on d.docid = m.docid and d.esdid in(657,601)
			inner join sisindigena2.mensarioavaliacoes mm on mm.menid = m.menid 
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sisindigena2_efetuar_pagamentos.php'";
$db->executar($sql);
$db->commit();
