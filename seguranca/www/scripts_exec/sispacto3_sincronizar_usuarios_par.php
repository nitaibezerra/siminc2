...
$_DEPARA_PERFIL = array("674"=> array("pflcod"=>"1386","campo"=>"muncod"),
						"672"=> array("pflcod"=>"1385","campo"=>"estuf")
						);
...
if($lista[0]) {
	foreach($lista as $li) {
		...
		if($espelhamento) {
			...
			$existe = $db->pegaUm("select suscod from seguranca.usuario_sistema where usucpf='".$li['usucpf']."' and sisid='182'");
			
			if(!$existe) {
				
				$sql = "insert into seguranca.usuario_sistema (usucpf, sisid, pflcod, suscod) values ('".$li['usucpf']."', 182, NULL, '".$li['sit_par']."')";
				$db->executar($sql);
				...
				if($existe != $li['sit_par']) {
					...
					$sql = "update seguranca.usuario_sistema set suscod='".$li['sit_par']."' where usucpf='".$li['usucpf']."' and sisid=182";
					$db->executar($sql);
					...
					$sql = "INSERT INTO seguranca.historicousuario(
	    					...
	    					VALUES ('Script automático identificou diferença PAR(status ".$li['sit_par'].") e SISPACTO 2015(status ".$existe.") e espelhou a situação do PAR no SISPACTO', NOW(), '".$li['usucpf']."', 182, '".$li['sit_par']."');";
					$db->executar($sql);
					$atualizacao = true;
			...
			if($atualizacao) {
				...
				$db->executar("DELETE FROM sispacto3.usuarioresponsabilidade WHERE usucpf='".$li['usucpf']."' AND pflcod='".$_DEPARA_PERFIL[$li['pflcod']]['pflcod']."'");
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sispacto3_sincronizar_usuarios_par.php'";
$db->executar($sql);
