...
class TestCurl implements iSenderConsumer {
	...
	public function readUrls() {
		...
		$db->executar("delete from par.pagamento_temp where sistema = 'OBRAS'");

		foreach ($limitEmp as $key => $pagid) {
			$sql = "INSERT INTO par.pagamento_temp(codigo, pagamento, quantidade, sistema)
					VALUES($key, '$pagid', ".sizeof(explode(', ', $pagid)).", 'OBRAS')";
			$db->executar($sql);
		}
		$db->commit();
		...
		for ($i=0; $i<$totalUrl; $i++){
			$urls = $_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/seguranca/scripts_exec/par_atualiza_pagamento_sigef.php?sistema=OBRAS&cont='.$i;
...
$strAssunto  	= SIGLA_SISTEMA. " - Atualização de Pagamento OBRA PAR SIGEF";
