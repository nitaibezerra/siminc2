		...
		if(!$existe) {
			...
			if($existe != $li['sit_par']) {
				...
				$sql = "INSERT INTO seguranca.historicousuario(
    					...
    					VALUES ('Script automático identificou diferença PAR(status ".$li['sit_par'].") e SISPACTO 2014(status ".$existe.") e espelhou a situação do PAR no SISPACTO 2014', NOW(), '".$li['usucpf']."', 174, '".$li['sit_par']."');";
				$db->executar($sql);
				$atualizacao = true;
			}
		}
...
$mensagem->FromName		= "Atualizando usuários - SISPACTO 2014";
$mensagem->From 		= "noreply@mec.gov.br";
$mensagem->AddAddress($_SESSION['email_sistema'], SIGLA_SISTEMA );
$mensagem->Subject = "Atualizando usuários PAR - SISPACTO 2014";
