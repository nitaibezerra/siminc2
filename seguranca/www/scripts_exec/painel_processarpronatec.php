...
//CARREGANDO DADOS ENVIADOS VIA WEBSERVICE
...
$HTML .= processarCargaIndicadores(1);

$HTML .= processarCargaIndicadores(2);

$HTML .= processarCargaIndicadores(3);

$HTML .= processarCargaCursos();
...
function processarCargaIndicadores($tipo){
    // abre conexão com o servidor de banco de dados
    $db = new cls_banco();
     ...
     */
    if($tipo==1){
        $inds = $db->carregar("SELECT i.indid, c.causql, 1 AS cautipoproc, c.caudiames, c.caudiasem
                                from painel.cargaautomatica c
                                INNER JOIN painel.indicador i ON i.indid = c.indid
                                where i.indstatus = 'A'
                                and causql ilike '%sysdbsistec_consulta_simec%'
                                order by c.indid");
    }elseif($tipo==2){
        $inds = $db->carregar("SELECT i.indid, c.causql, 1 AS cautipoproc, c.caudiames, c.caudiasem
                                from painel.cargaautomatica c
                                INNER JOIN painel.indicador i ON i.indid = c.indid
                                where i.indstatus = 'A'
                                and c.indid IN (2401,3201,3202,3204)
                                order by c.indid");
    }elseif($tipo==3){
        $inds = $db->carregar("SELECT i.indid, c.causql, 1 AS cautipoproc, c.caudiames, c.caudiasem
                                from painel.cargaautomatica c
    ...
    if($inds[0]) {
        foreach($inds as $in) {
            if($_REQUEST['indidat']) {
                ...
                $consultasql[$in['indid']] = stripslashes($in['causql']);
            } else {
                ...
                switch($agp['regid']) {
                            ...
                            foreach($agp['qry'] as $con) {
                                if($con['unicod'] && !is_null($con['dshqtde'])) {
                                    ...
                                    if(!$_CACHE_PAINEL[REGIONALIZACAO_UNIVERSIDADE][$con['unicod']]) {
                                        ...
                                        $_CACHE_PAINEL[REGIONALIZACAO_UNIVERSIDADE][$con['unicod']] = $dadosuni;
                    ...
                    case REGIONALIZACAO_ESCOLA:
                        if($agp['qry'][0]) {
                            ...
                            if(!$_CACHE_PAINEL[REGIONALIZACAO_ESCOLA]) {
...
function processarCargaCursos(){
    // abre conexão com o servidor de banco de dados
    $db = new cls_banco();

    $db->executar("TRUNCATE TABLE pde.cockpit_cursos_pronatec");

    $HTML =  "TRUNCATE TBL_cockpit_cursos_pronatec foi processado com sucesso (".date("d/m/Y h:i:s").")<br>";

    $sql = "INSERT INTO pde.cockpit_cursos_pronatec (ccpcurso, ccptipo, ccpmatricula) (
            SELECT
                ccpcurso, ccptipo, ccpmatricula
            FROM dblink (
            'dbname=
            hostaddr=
            user=
            password=
            port=',
            'SELECT UPPER(no_curso) AS ccpcurso, ds_tipo_curso AS ccptipo, SUM(total) AS ccpmatricula
            FROM (
                --BOLSA FORMAÇÃO
                SELECT no_curso, ds_tipo_curso, COUNT(0) AS total
                FROM pronatec.tb_vw_aluno_bolsa_formacao
                WHERE co_grupo IN (2)
                AND co_tipo_situacao_matricula NOT IN (9,17,19,24,29)
                AND substr(dt_inicio_curso,7,4) BETWEEN ''2011'' AND ''2014''
                GROUP BY no_curso, ds_tipo_curso
            UNION ALL
                --SISUTEC
                SELECT no_curso, ''Técnico'' AS ds_tipo_curso, COUNT(0) AS total
                FROM sisutec.tb_vw_aluno_inscrito_matriculado
                WHERE co_grupo IN (2)
                AND co_situacao_matricula NOT IN (9,17,19,24,29)
                AND substr(dt_inicio_aula,7,4) BETWEEN ''2011'' AND ''2014''
                GROUP BY no_curso, ds_tipo_curso
            UNION ALL
                --REDE FEDERAL
                SELECT no_curso, ''Técnico'' AS ds_tipo_curso, COUNT(0) AS total
                FROM public.tb_vw_indicador_rede_federal_ifes
                WHERE SUBSTR(dt_ocorrencia_ciclo,7,4) BETWEEN ''2011'' AND ''2014''
                AND (
                    (co_tipo_curso IN (3) AND tipo_oferta = ''Integrado'') --Técnico Integrado
                    OR
                    (co_tipo_curso IN (3) AND tipo_oferta = ''Concomitante'') --Técnico Concomitante
                    OR
                    (co_tipo_curso IN (3) AND tipo_oferta = ''PROEJA - Integrado'') --Técnico PROEJA integrado
                    OR
                    (co_tipo_curso IN (3) AND tipo_oferta = ''Subsequente'' AND ''modalidade ensino'' = ''Ensino Presencial'') --Técnico Subsequente presencial
                    OR
                    (co_tipo_curso IN (3) AND tipo_oferta = ''PROEJA - Concomitante'') --Técnico PROEJA concomitante
                    OR
                    (co_tipo_curso IN (1,2) AND carga_horaria::integer >= 160 ) --FIC >= 160h
                )
                AND co_simec is not null
                GROUP BY no_curso, ds_tipo_curso
            UNION ALL
                --REDE FEDERAL
                SELECT no_curso, ''FIC'' AS ds_tipo_curso, COUNT(0) AS total
                FROM public.tb_vw_indicador_rede_federal_ifes
                WHERE SUBSTR(dt_ocorrencia_ciclo,7,4) BETWEEN ''2011'' AND ''2014''
                AND (
                    (co_tipo_curso IN (1,2) AND carga_horaria::integer >= 160 ) --FIC >= 160h
                ...
                GROUP BY no_curso, ds_tipo_curso
            UNION ALL
                --DADOS REDE FEDERAL QUE VÃO PARA O ETEC
                SELECT no_curso, ''Técnico'' AS ds_tipo_curso, COUNT(0) AS total
                FROM public.tb_vw_indicador_rede_federal_ifes
                WHERE SUBSTR(dt_ocorrencia_ciclo,7,4) BETWEEN ''2011'' AND ''2014''
                AND (
                    (co_tipo_curso IN (3) AND tipo_oferta = ''Subsequente'' AND ''modalidade ensino'' = ''Ensino a Distância'') --Técnico Subsequente a distância
                )
                AND co_simec is not null
                GROUP BY no_curso, ds_tipo_curso
            UNION ALL
                --ETEC
                SELECT no_curso, ''Técnico'' AS ds_tipo_curso, COUNT(0) AS total
                FROM public.tb_vw_indicador_etec
                WHERE SUBSTR(dt_ocorrencia_ciclo,7,4) BETWEEN ''2011'' AND ''2014''
                AND co_tipo_curso IN (3)
                GROUP BY no_curso, ds_tipo_curso
            UNION ALL
                -- BRASIL PROFISSIONALIZADO
                SELECT no_curso, ''Técnico'' AS ds_tipo_curso, COUNT(0) AS total
                FROM public.tb_vw_brasil_pro_painel_simec
                WHERE SUBSTR(dt_ocorrencia_ciclo,7,4) BETWEEN ''2011'' AND ''2014''
                GROUP BY no_curso, ds_tipo_curso
            --UNION ALL
                -- ACORDO
            ) AS FOO
            GROUP BY ccpcurso, ccptipo;'
            ) AS rs (
            ccpcurso varchar(500),
            ccptipo varchar(10),
            ccpmatricula integer
            )
        )";
    $db->executar($sql);
    $HTML .=  "INSERT TBL_cockpit_cursos_pronatec foi processado com sucesso (".date("d/m/Y h:i:s").")<br>";
