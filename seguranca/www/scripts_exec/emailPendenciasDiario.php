...
SELECT DISTINCT
                ...
                REPLACE(mnmdsc, 'Meta - ', '') as descricao,
                ...
                (SELECT 
				 ...
				 INNER JOIN pde.atividade ati2 ON ati2.atiacaid = aca2.acaid 
				 INNER JOIN pde.atividade proj ON proj.atiid = ati2._atiprojeto 
...
WHERE
	...
	AND dmi.dmidataexecucao <= (current_date + integer '7')
--	AND dmi.dmidataexecucao <= now()::date
	AND doc.esdid in (443,444)
--	AND ati._atiprojeto=145447 --TEMPORÁRIO ATÉ A VALIDAÇÃO DOS DEMAIS PROJETOS
ORDER BY
...
foreach($arrUsu as $u){
	...
	if( $u['projeto'] != '' ){
		$sql = "SELECT 
				...
				WHERE
					ur.pflcod in (732,733) AND rpustatus = 'A' AND atiid = ".$u['projeto'];
	...
	$e->setEmailsDestino(array_unique($arrEmail));
	$e->enviarEmails();	
}