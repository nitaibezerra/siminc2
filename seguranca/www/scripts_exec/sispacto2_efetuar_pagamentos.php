...
$sql = "SELECT * FROM sispacto2.remessapagamento WHERE remprocessada=FALSE";
...
if($remessapagamento[0]) {
	foreach($remessapagamento as $rem) {
		...
		if($consultarRemessaDePagamentos_obj->remessa->situacao->codigo=='PROCESSADA') {
			...
			$sql = "UPDATE sispacto2.remessapagamento SET remprocessada=true, remdtprocessamento='".formata_data_sql($consultarRemessaDePagamentos_obj->remessa->situacao->data)."' WHERE remid='".$consultarRemessaDePagamentos_obj->remessa->id."'";
			$db->executar($sql);
			$db->commit();
			...
			$sql = "UPDATE sispacto2.pagamentobolsista SET remid=null WHERE remid='".$rem['remid']."'";
			$db->executar($sql);
			$sql = "UPDATE sispacto2.remessapagamento SET remprocessada=true, remdtprocessamento=NOW() WHERE remid='".$rem['remid']."'";
			$db->executar($sql);
			$db->commit();
		}
...
$sql = "SELECT p.pboid FROM sispacto2.identificacaousuario i 
		INNER JOIN sispacto2.pagamentobolsista p ON p.iusd = i.iusd 
		INNER JOIN workflow.documento d ON d.docid = p.docid 
		INNER JOIN sispacto2.remessapagamento r ON r.remid = p.remid 
...
if($reenvios) {
	foreach($reenvios as $pboid) {
		$db->executar("UPDATE sispacto2.pagamentobolsista SET remid=NULL WHERE pboid='".$pboid."'");
		$db->commit();
	}
}
...
$sql = "SELECT i.iusd, p.pboid, p.pboparcela, i.iuscpf, i.nacid, i.iusnome, i.iusdatanascimento, i.iusnomemae, i.iussexo, m.muncod as co_municipio_ibge_nascimento, m.estuf as sg_uf_nascimento, 
		...
		INNER JOIN sispacto2.folhapagamento ff ON ff.fpbid = p.fpbid
		INNER JOIN sispacto2.universidade u ON u.uniid = p.uniid 
		INNER JOIN sispacto2.universidadecadastro un ON un.uniid = u.uniid  
		INNER JOIN sispacto2.folhapagamentouniversidade fpu ON fpu.uncid = un.uncid AND fpu.fpbid = ff.fpbid AND pp.pflcod = fpu.pflcod  
		INNER JOIN workflow.documento d ON d.docid = p.docid 
...
if($_listapagamento) {
	foreach($_listapagamento as $per => $pagamentos) {
		...
		$sql = "INSERT INTO sispacto2.remessapagamento(remdata) VALUES (NOW()) RETURNING remid;";
		$remid = $db->pegaUm($sql);
		...
		foreach($pagamentos as $pg) {
			...
			$sql = "UPDATE sispacto2.pagamentobolsista SET remid='".$remid."' WHERE iusd='".$pg['iusd']."'";
			$db->executar($sql);
		}
    	...
    	if($logerro_enviarRemessaDePagamentos=='FALSE') {
			...
			$sql = "UPDATE sispacto2.remessapagamento SET remano='".$pers[0]."', remmes='".$pers[1]."', remrastreador='".$enviarRemessaDePagamentos_obj->reciboEnvio->rastreador."' WHERE remid='".$remid."'";
			$db->executar($sql);
			$db->commit();
    	} else {
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sispacto2_efetuar_pagamentos.php'";
$db->executar($sql);
$db->commit();
