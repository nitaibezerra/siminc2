...
// black list
$pularcpf = $db->carregarColuna("SELECT lnscpf FROM sispacto2.listanegrasgb");


$sql = "SELECT DISTINCT i.iusd, l.logcpf FROM sispacto2.identificacaousuario i 
		LEFT JOIN log_historico.logsgb_sispacto2 l ON l.logcpf = i.iuscpf
...
if($iusds) {
	foreach($iusds as $iusd) {
		...
		$lnsid = $db->pegaUm("INSERT INTO sispacto2.listanegrasgb(lnscpf) VALUES ((SELECT iuscpf FROM sispacto2.identificacaousuario WHERE iusd='".$iusd."')) RETURNING lnsid;");
		$db->commit();
		
		sincronizarDadosUsuarioSGB(array("iusd" => $iusd, "sincronizacao" => true));
		
		$db->executar("DELETE FROM sispacto2.listanegrasgb WHERE lnsid='".$lnsid."'");
		$db->commit();
...
$sql = "SELECT uncid FROM sispacto2.universidadecadastro WHERE (cadastrosgb=false OR cadastrosgb IS NULL)";
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sispacto2_sincronizar_usuarios_sgb.php'";
$db->executar($sql);
$db->commit();
