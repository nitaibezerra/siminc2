	...
	if(is_array($processos) ){
		foreach($processos as $dadosProcesso){
			...
			if( !empty($arrRetorno[0]['numero_do_processo']) ) {
				...
				if( $testeProcesso ){
					$db->executar("UPDATE par.processoobraspar SET prodataconsultasigef = now(), prostatus = 'A' WHERE pronumeroprocesso = '{$nu_processo}'");
					$db->commit();
				}
...
$html = "<span style='color: red;'><b>Detalhes da Execução - Histórico SIGEF:</b><br/><br/>
									<b>Rotina de Atualização de Empenho OBRAS PAR SIGEF ".$dataInicio." a ".$dataFim." realizada com sucesso em ".$intervalos."</b></span>";
$assunto  = SIGLA_SISTEMA. " - Atualiza Empenho OBRAS PAR SIGEF";
