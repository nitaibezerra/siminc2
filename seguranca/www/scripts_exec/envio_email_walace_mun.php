...
<form name="form" id="form" method="post" action="" >
	...
	<input type="submit" name="enviar" id="enviar" value="Enviar e-mail Mun">
</form>
	...
	elseif($_POST['enviar_email'] == 'S'){
		...
		$sql = "
			Select	lstid, 
					...
					email_enviado
			From temporario.email_pde_interativo_mun
		...
		if($us[0]){
			foreach($us as $u){
				...
				if($i <= 10000){ 
					$sql = "UPDATE temporario.email_pde_interativo_mun SET email_enviado = 'S' WHERE lstid = ".$u['lstid'].";";
					$db->executar( $sql );
					$db->commit();
				}			
			}
		...
		$sql = "Select count(lstid) as qtd From temporario.email_pde_interativo_mun Where email_enviado='S';";
		$qtd = $db->pegaUm($sql);
