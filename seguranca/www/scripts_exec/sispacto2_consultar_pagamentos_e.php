...
$sql = "select i.iuscpf, lpad(f.fpbmesreferencia::text,2,'0') as fpbmesreferencia, fpbanoreferencia, p.pboid from sispacto2.pagamentobolsista p 
			...
			foreach($varrer as $an) {
				if($_bolsasanalise[$iuscpf][$an->referencia->ano][$an->referencia->mes]) {
					...
					if($an->situacaoPagamento->codigo=='C' || $an->situacaoPagamento->codigo=='X') {
						...
						$pagamentobolsista = $db->pegaLinha("SELECT d.docid, d.esdid FROM sispacto2.pagamentobolsista p
												  INNER JOIN workflow.documento d ON d.docid = p.docid
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sispacto2_consultar_pagamentos_e.php'";
$db->executar($sql);
$db->commit();
