...
$mensagem->Body = $inicioscript."<br /><br />".$HTML;
...
function processarCargaIndicadores($tipo){
	...
	$inds = $db->carregar("SELECT i.indid, c.causql, c.cautipoproc, c.caudiames, c.caudiasem FROM painel.indicador i
						   INNER JOIN painel.cargaautomatica c ON i.indid = c.indid
				   		   WHERE i.colid='".COLETA_AUTOMATICA."'
				   		   AND i.indstatus='A'
				   		   AND i.indencerrado IS FALSE
				   		   --AND i.indid IN (271,1865)
				   		   ".($tipo==1 ? "AND c.cauindicador IS FALSE" : "AND c.cauindicador IS TRUE" )."
				   		   AND c.causql IS NOT NULL
				   		   AND c.cautipoproc < 9 --INDICADORES DESATIVADOS MANUALMENTE
				   		   ORDER BY i.indid");
