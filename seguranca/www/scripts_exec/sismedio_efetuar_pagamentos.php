...
$sql = "SELECT * FROM sismedio.remessapagamento WHERE remprocessada=FALSE";
...
if($remessapagamento[0]) {
	foreach($remessapagamento as $rem) {
		...
		if($consultarRemessaDePagamentos_obj->remessa->situacao->codigo=='PROCESSADA') {
			...
			$sql = "UPDATE sismedio.remessapagamento SET remprocessada=true, remdtprocessamento='".formata_data_sql($consultarRemessaDePagamentos_obj->remessa->situacao->data)."' WHERE remid='".$consultarRemessaDePagamentos_obj->remessa->id."'";
			$db->executar($sql);
			$db->commit();
			...
			$sql = "UPDATE sismedio.pagamentobolsista SET remid=null WHERE remid='".$rem['remid']."'";
			$db->executar($sql);
			$sql = "UPDATE sismedio.remessapagamento SET remprocessada=true, remdtprocessamento=NOW() WHERE remid='".$rem['remid']."'";
			$db->executar($sql);
			$db->commit();
		}
...
$sql = "SELECT p.pboid FROM sismedio.identificacaousuario i 
		INNER JOIN sismedio.pagamentobolsista p ON p.iusd = i.iusd 
		INNER JOIN workflow.documento d ON d.docid = p.docid 
		INNER JOIN sismedio.remessapagamento r ON r.remid = p.remid 
...
if($reenvios) {
	foreach($reenvios as $pboid) {
		$db->executar("UPDATE sismedio.pagamentobolsista SET remid=NULL WHERE pboid='".$pboid."'");
		$db->commit();
	}
}
...
$sql = "SELECT i.iusd, p.pboid, p.pboparcela, i.iuscpf, i.nacid, i.iusnome, i.iusdatanascimento, i.iusnomemae, i.iussexo, m.muncod as co_municipio_ibge_nascimento, m.estuf as sg_uf_nascimento, 
		...
		INNER JOIN sismedio.folhapagamento ff ON ff.fpbid = p.fpbid
		INNER JOIN sismedio.universidade u ON u.uniid = p.uniid 
		INNER JOIN sismedio.universidadecadastro un ON un.uniid = u.uniid  
		INNER JOIN sismedio.folhapagamentouniversidade fpu ON fpu.uncid = un.uncid AND fpu.fpbid = ff.fpbid AND p.pflcod = fpu.pflcod 
		INNER JOIN workflow.documento d ON d.docid = p.docid 
...
if($_listapagamento) {
	foreach($_listapagamento as $per => $pagamentos) {
		...
		$sql = "INSERT INTO sismedio.remessapagamento(remdata) VALUES (NOW()) RETURNING remid;";
		$remid = $db->pegaUm($sql);
		...
		foreach($pagamentos as $pg) {
			...
			$sql = "UPDATE sismedio.pagamentobolsista SET remid='".$remid."' WHERE iusd='".$pg['iusd']."'";
			$db->executar($sql);
		}
    	...
    	if($logerro_enviarRemessaDePagamentos=='FALSE') {
			...
			$sql = "UPDATE sismedio.remessapagamento SET remano='".$pers[0]."', remmes='".$pers[1]."', remrastreador='".$enviarRemessaDePagamentos_obj->reciboEnvio->rastreador."' WHERE remid='".$remid."'";
			$db->executar($sql);
			$db->commit();
    	} else {
...
$sql = "UPDATE seguranca.agendamentoscripts SET agstempoexecucao='".round((getmicrotime() - $microtime),2)."' WHERE agsfile='sismedio_efetuar_pagamentos.php'";
$db->executar($sql);
$db->commit();
$db->close();
