...
if( $sistema == 'PAR' ){
	$sql = "select
			...
			from(
				SELECT distinct
			    	prpid codigo, prpnumeroprocesso as processo, 'PAR' as sistema, prpstatus as status, coalesce(p.prpdataconsultasigef, '1900-01-01') as prpdataconsultasigef
	...
	$sql = "select
			...
			from(
				SELECT distinct
			    	proid as codigo, pronumeroprocesso as processo, 'OBRAS' as sistema, prostatus as status, coalesce(p.prodataconsultasigef, '1900-01-01') as prodataconsultasigef
			  	FROM 
	...
	$sql = "select
			...
			from(
				SELECT distinct
			    	proid as codigo, pronumeroprocesso as processo, 'PAC' as sistema, prostatus as status, coalesce(p.prodataconsultasigef, '1900-01-01') as prodataconsultasigef
			  	FROM 
			      	par.processoobra p
			      	inner join par.empenho e on e.empnumeroprocesso = p.pronumeroprocesso and e.empstatus = 'A'
			    where prostatus = 'A'
...
foreach($arrProcesso as $dadosProcesso){
	...
	if( !empty($arrRetorno[0]['numero_do_processo']) ) {
		...
		$totAtualizado++;
		if( $sistema == 'PAR' ){
			$sql = "UPDATE par.processopar SET prpdataconsultasigef = now() WHERE prpid = $codigo";
		} elseif( $sistema == 'OBRAS' ){
			$sql = "UPDATE par.processoobraspar SET prodataconsultasigef = now() WHERE proid = $codigo";
		} elseif( $sistema == 'PAC' ){
			$sql = "UPDATE par.processoobra SET prodataconsultasigef = now() WHERE proid = $codigo";
		}
		$db->executar($sql);
		$db->commit();
	}
...
$html = "<span style='color: red;'><b>Detalhes da Execução - Histórico SIGEF:</b><br/><br/>
									<b>Rotina de Carga de Empenhos ".$sistema." do SIGEF ".$dataInicio." a ".$dataFim.",<br>realizada com sucesso em ".$intervalos.", foi atualizado ".$totAtualizado." empenhos.</b></span>";
$assunto  = SIGLA_SISTEMA. " - Carga Empenho ".$sistema."/".$ano." do SIGEF ";
