	...
	public function __construct(Sender $sender) {
 	...
 	public function readUrls() {
	  	...
	  	$sql = "select
				...
				from(
				    ...
				    where
				        pro.prpstatus = 'A'
				        and emp.empstatus = 'A'
	  					and cast(to_char(coalesce(prpdatapagamentosigef, '1900-01-01'), 'YYYY-MM-DD') as date) <> cast(to_char(now(), 'YYYY-MM-DD') as date)
				    order by emp.empid
				) as foo
				group by ano 
				order by ano";
		...
		foreach ($arrEmpenho as $v) {	
			$urls = $_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/seguranca/scripts_exec/carga_ws_pagamento_simec_sigef.php?sistema=PAR&ano='.$v['ano'].'&qtdregistro='.$v['total'];
...
$strAssunto  = SIGLA_SISTEMA. " - Carga de Processo de Empenho SUBAÇÃO PAR SIGEF";
