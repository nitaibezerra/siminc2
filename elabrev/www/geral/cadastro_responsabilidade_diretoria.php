...
function atribuiUgs($usucpf, $pflcod, $dircods)
{
	...
	if($pflcod == PERFIL_SECRETARIA){
		...
		$db->executar("DELETE FROM elabrev.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."'");		
	}else{
		$db->executar("UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND dircod IS NOT NULL");
	}

	if ($dircods[0]){
		foreach($dircods as $dircod) {
			$dadosur = $db->carregar("SELECT * FROM elabrev.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND prsano = '".$_SESSION['exercicio']."' AND dircod = ". $dircod );
			if($dadosur) {
				...
				$ungcod = $db->pegaUm("SELECT ungcod FROM elabrev.diretoria WHERE dircod = ". $dircod);
				...
				$db->executar("INSERT INTO elabrev.usuarioresponsabilidade(
            				   pflcod, usucpf, dircod, rpustatus, rpudata_inc, unicod, ungcod, prsano )
    						   VALUES ('". $pflcod ."', '". $usucpf ."', ".$dircod.", 'A', NOW(), '".$uo."', '".$ungcod."', '".$_SESSION['exercicio']."');");
			}
		}
	}
	$db->commit();
...
function buscaUgsAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
			...
			inner join elabrev.diretoria d ON d.dircod = ur.dircod
