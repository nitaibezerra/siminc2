<?php
	...
	if($mostraPesquisaTemp){
		$registros = $db->carregar( strtoupper($sql) );
	}
...
<html>
	...
	<body leftmargin="0" topmargin="0" bottommargin="0" marginwidth="0" marginheight="0" bgcolor="#ffffff">
			...
			<table width="100%" align="center" border="0" cellspacing="0" cellpadding="2" class="listagem">
				...
				<tr bgcolor="#cdcdcd">
					...
					<td colspan="2" valign="top">
					    Grupo:
                        <?php
                        $sql = "select guoid as codigo, guonome as descricao
                                from elabrev.grupouo
                                order by descricao";

                        echo $db->monta_combo("guoid",$sql,'S',"Selecione...","","","","200","N","guoid","",'');
                        ?>
                        <input type="button" name="btn_marcar" id="btn_marcar" value="Marcar">
					</td>
				</tr>
				<?php if( $intervalo ){ ?>
					<tr bgcolor="#cdcdcd">
					...
					<? foreach ( $registros as $registro ) : ?>
						...
						<tr bgcolor="#<?= $posicao % 2 == 0 ? 'f4f4f4' : 'e0e0e0' ?>">
							<td width="1">
								...
								<?else: ?>
									<input class="grupouo_<?php echo $registro['guoid'] ?>" descricao="<?= $descricao ?>" name="combo_popup_checkbox" type="checkbox" id="combo_popup_checkbox_<?= $registro['codigo'] ?>" value="<?= $registro['codigo'] ?>" onclick="<?= $onclick ?>"/>
								<?endif ?>
							</td>
							<td colspan="2">
								<?= $descricao ?>
		...
		<script type="text/javascript">
		    jQuery(function(){
        		jQuery('#btn_marcar').click(function(){
            		if (jQuery('#guoid').val()) {
            		    jQuery('.grupouo_' + jQuery('#guoid').val()).click().click().attr('checked', 'checked')
            		}
        		});
        	});
        </script>
