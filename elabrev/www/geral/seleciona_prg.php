...
<?
	require_once "config.inc";
	  ...
	  $sql = "select prgid, prgcod, prgdsc from elabrev.ppaprograma_proposta order by prgcod";
	  $RS = $db->carregar($sql);
	  $nlinhas = count($RS)-1;
	  for ($i=0; $i<=$nlinhas;$i++)
		 {
	   ...
	   ?>
		   		...
		   		<tr bgcolor="<?=$cor?>">
				<td align="right"><input type="Checkbox" name="prgid" id="<?=$prgid?>" value="<?=$prgid?>" onclick="retorna(<?=$i?>);">
				<input type="Hidden" name="prgdsc" value="<?=$prgcod.' - '.$prgdsc?>"></td>
