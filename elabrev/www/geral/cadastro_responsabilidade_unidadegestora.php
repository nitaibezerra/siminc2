	...
	if(!$UGSTrue || $acoesConfirmadas == 1){
		if($pflsncumulativo == 'f'){
			foreach ($_POST['ugsresp'] as $ugsCod){
				...
				if ($respUGS[$x] != '') {
					foreach ($respUGS as $respUnd) {
						$db->executar("Update elabrev.usuarioresponsabilidade SET rpustatus = 'I' Where usucpf = '".$respUnd['usucpf']."' and pflcod = '".$respUnd['pflcod']."' and ungcod = '".$respUnd['ungcod']."'");
					}
				}
				$x = $x + 1;
				$db->commit();
		...
		#Exibir a tela de aviso dos itens que já possuem coordenador e confirmar. A substituição pelo usuario que está sendo liberado e/ou alterado
			...
			$msg = 'Existem usuários ativos com o perfil Reitor selecionado para estas UGS:\\n\\n';
...
function buscaUgsAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
				...
				'('||u.unicod||') - ' || ung.ungdsc  AS descricao
			FROM elabrev.usuarioresponsabilidade ur 
			INNER JOIN public.unidadegestora ung ON ur.ungcod = ung.ungcod 
			inner join public.unidade u ON u.unicod = ung.unicod
...
<html>
	...
	<body leftmargin="0" topmargin="5" bottommargin="5" marginwidth="0" marginheight="0" bgcolor="#ffffff">
		...
		<form name="formassocia" action="cadastro_responsabilidade_unidadegestora.php" method="post">
			<input type="hidden" name="usucpf" value="<?=$usucpf?>">
			<input type="hidden" name="pflcod" value="<?=$pflcod?>">
