...
header('Content-Type: text/html; charset=iso-8859-1');
							...
							?>
								<tr onmouseover="this.bgColor='#ffffcc';" onmouseout="this.bgColor='';">
									...
									<td valign="top" style="border-top: 1px solid #cccccc; padding:2px; width:250px;" >
										<a href="<?= $_SESSION['sisdiretorio'] ?>.php?modulo=relatorio/acao/geraproposta&acao=A&acod=<?= $codigoo?>" target="_blank"><?= $descricao ?></a></td>
									</td>
									<td align="right" style="border-top: 1px solid #cccccc; padding:2px; width:90px;">
										<a href="<?= $_SESSION['sisdiretorio'] ?>.php?modulo=relatorio/acao/geraproposta&acao=A&acod=<?= $codigoo?>" target="_blank"><?= $tipo ?></a></td>
									</td>
								</tr>
							<?
						}
	...
	if ($_REQUEST['tipo'])
	{
		...
		if ($tipo == 'A')
		{
			...
			$sql = '
			select 
				p.prgcod ||  \'.\' || a.acacod ||  \'.\' || a.unicod || \' - \' || a.acadsc as descricao, 
				count(prop2.acaid) as qtd, 
				p.prgdsc, 
				a.acaid as Codigo, 
				a.acadsc 
			from 
			...
			inner join 
				elabrev.v_propostas_acoes prop2 
			ON 
				prop2.acaid = prop.acaid 
			inner join 
				elabrev.ppaacao a 
			ON 
				a.acaid = prop.acaid 
			inner join
				elabrev.ppaprograma p 
			ON 
			p.prgid = prop.prgid 
				where 1 = 1 ' .
				$wh . '
			group by 
			p.prgcod ||  \'.\' || a.acacod ||  \'.\' || a.unicod
			, 
			p.prgdsc
			, 
			a.acaid
		...
		elseif ($tipo == 'U')
		{
			...
			select 
			distinct 
				prop.acaid as codigo, 
				a.acadsc as descricao, 
				count(prop.acaid) as qtd 
			from 
				elabrev.v_propostas_acoes prop 
			inner join 
				elabrev.ppaacao a 
		...
		elseif ($tipo == 'I')
		{
			...
			from
				elabrev.v_propostas_indicadores '.$wh1.' 
			order by 2 ';
		}
//	
//		else
...
//		
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="color:#006600;">
			<tr style="color:#000000;">
				<td valign="top" style="width:24px;" nowrap>
					&nbsp;
				</td>
				<td valign="top" style="width:132px;" nowrap>
					<?= $tit11 ?>
				</td>
					<td valign="top">
					<?= $tit22 ?>
				</td>
				<td valign="top" align="right" style="width:95px;">
					&nbsp;
				</td>
			 </tr>
			...
			else
			{
				for ($i = 0; $i <= $nlinhas; $i ++)
				{
					...
					?>
						<tr onmouseover="this.bgColor='#ffffcc';" onmouseout="this.bgColor='';">
							...
							<td valign="top" colspan="<? if ( $tit3 ) print ''; else '4'; ?>" 
								style="border-top: 1px solid #cccccc; padding:2px;" 
								<? 
					
									if ($tipo != 'I')
									{
										?>
											onclick="abreconteudo('geral/listaacaoproposta.php?&subnivel=3&acaid=<?= $codigoo?>&prgid=<?= $prgidd?>&inclusao=<?= $inclusaoo?>&exclusao=<?= $exclusaoo?>&alteracao=<?= $alteracaoo ?>&fusao=<?= $fusaoo ?>','<?= $codigoo ?>')" 
										<? 
									} 
							...
							>
								<?
									...
									{ 
										?>
											<img src="../imagens/mais.gif" name="+" border="0" id="img<?= $codigoo ?>">
											<?= $descricaoo ?>
										<?
									} 
									else 
									{
										?>
											<a
												href="<?= $_SESSION['sisdiretorio'] ?>.php?modulo=relatorio/relindicadornovo&acao=A&indinum=<?= $codigo ?>"
												target="_blank"
											>
												<?= $descricao ?>
											</a>
					...
					{	
						?>
							<tr bgcolor=" <?= $marcado ?>" >
								<td id="td<?= $codigo ?>" 
								colspan="<? if ($listaprg == 'S') { print '3'; } else { print '5'; }?>" 
				...
				if( $tit5 )
				{
					?>
						<tr>
							<td colspan="6" align="right" style="color:000000;border-top: 2px solid #000000;">
								Total de Ações: (<?=$nlinhas+1?>)
							</td>
						</tr>
					<?
				}
