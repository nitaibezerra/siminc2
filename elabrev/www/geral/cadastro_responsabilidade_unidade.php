...
if(is_array($_POST['usuunidresp']) && @count($_POST['usuunidresp'])>0) {
	...
	$sqlUpdRpu = "UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE ungcod IS NULL AND unicod = '%s' AND pflcod = ".$pflcod;

	$sqlUpdRpuUsu = "UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE ungcod IS NULL AND usucpf = '".$usucpf."' AND pflcod = ".$pflcod;
...
$sql = "select distinct u.unicod as codigo, u.unicod||' - '||u.unidsc as descricao 
		from elabrev.usuarioresponsabilidade ur inner join unidade u on ur.unicod=u.unicod 
