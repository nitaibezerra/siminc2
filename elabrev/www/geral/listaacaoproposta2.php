		...
		for ( $i = 0; $i <= $nlinhas; $i++ )
		{
			...
			<tr onmouseover="this.bgColor='#ffffcc';" onmouseout="this.bgColor='';">
				...
				<td valign="top" style="border-top: 1px solid #cccccc; padding:2px; width:250px;" >
					<a href="<?=$_SESSION['sisdiretorio']?>.php?modulo=relatorio/relacaonovo&acao=A&prgid=<?=$prgid?>&acaid=<?=$codigo?>" target="_blank"><?= $descricao ?></a></td>
				</td>
				<td align="right" style="border-top: 1px solid #cccccc; padding:2px; width:90px;">
					<a href="<?=$_SESSION['sisdiretorio']?>.php?modulo=relatorio/relacaonovo&acao=A&prgid=<?=$prgid?>&acaid=<?=$codigo?>" target="_blank"><?= $tipo ?></a></td>
				</td>
			</tr>
			<?
		}
