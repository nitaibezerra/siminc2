...
function atividade_inserir( $atividade, $titulo ){
	...
	$sql = sprintf(
		"insert into pde.atividade (
			atiidpai, atidescricao, entid, atiordem, _atiprojeto, acaid
		) values (
			%d,
			'%s',
			%d,
	...
	$sql = "SELECT sbaid FROM monitora.subacaoatividade WHERE atiid='".$atividade."'";
	$sbaid = $db->pegaUm($sql);
	
	if($sbaid) {
		$sql = "INSERT INTO monitora.subacaoatividade(sbaid, atiid) VALUES ('".$sbaid."', '".$atiid."');";
		$db->executar($sql);
		$db->commit();
	}
...
function atividade_listar( $atividade, $profundidade = 0, $situacao = array(), $usuario = null, $perfil = array() ){
	...
	$sql = sprintf(
		"select
			...
			(SELECT COUNT(*) FROM monitora.planointerno pi LEFT JOIN monitora.planointernoatividade pia ON pia.pliid = pi.pliid WHERE pia.atiid = a.atiid) AS qtdpi,
...
function enviaEmailStatusPi($pi){
		...
		if($obrid) {
			$join = "LEFT JOIN obras.obrainfraestrutura sa ON pi.obrid = sa.obrid
					 LEFT JOIN entidade.entidade e ON e.entid = sa.entidunidade";
		} else {
			$join = "LEFT JOIN monitora.subacao sa ON sa.sbaid = pi.sbaid
					 LEFT JOIN monitora.subacaounidade su ON su.sbaid = sa.sbaid
					 LEFT JOIN entidade.entidade e ON e.entid = su.entid";
		}
						...
						".$join."
						left JOIN
							seguranca.usuario u ON u.usucpf = pi.usucpf
						WHERE
							pi.plistatus = 'A' 
							AND pi.plicod = '" . $pi . "'
	    ...
	    $sql = "SELECT 
					CASE WHEN 
						...
						THEN 
							' Enviado para Revisão ' END as situacao,
						pihobs,
						to_char(pihdata, 'DD/MM/YYYY  hh24:mi:ss') as pihdata
					FROM 
						monitora.planointernohistorico 
					WHERE	
						plicod = '" . $pi . "'
					ORDER BY
						pihdata desc"; 		
...
function redirecionaUnidadePerfil($tipo){
	global $db;
	
	if ( !$db->testa_superuser() ) {
		$sql = "SELECT count(1) FROM monitora.usuarioresponsabilidade WHERE pflcod in ('".PERFIL_GESTORUNIDPLANEJAM."','".PERFIL_EQUIPAPOIOGESTORUP."','".PERFIL_MONITORA_GESTORUNIDORCAMENTO."','".PERFIL_MONITORA_EQAGESTORUNIDORCAMENTO."')
		and usucpf  in('{$_SESSION['usucpf']}') AND rpustatus = 'A' AND ungcod is not null ";
		$ug = $db->pegaUm($sql);
		
		$sql = "SELECT unicod FROM monitora.usuarioresponsabilidade WHERE pflcod in ('".PERFIL_GESTORUNIDPLANEJAM."','".PERFIL_EQUIPAPOIOGESTORUP."','".PERFIL_MONITORA_GESTORUNIDORCAMENTO."','".PERFIL_MONITORA_EQAGESTORUNIDORCAMENTO."')
		and usucpf  in('{$_SESSION['usucpf']}') AND rpustatus = 'A' AND unicod is not null";
		$un = $db->pegaUm($sql);
		switch ($tipo){
			case 'gerenciar':
				if($ug){
					header("location: ?modulo=principal/planotrabalhoUG/listaUG&acao=A");
					exit;
				} elseif($un){
					header("location: ?modulo=principal/planotrabalhoUN/listaUN&acao=A");
					exit;
				}
			break;
			case 'gerenciarPi':
				if($ug){
					header("location: ?modulo=principal/planotrabalhoUG/gerenciarpiUG&acao=A");
					exit;
				} elseif($un){
					header("location: ?modulo=principal/planotrabalhoUN/gerenciarpiUN&acao=A");
					exit;
				}
			break;
			case 'gerenciarSubacao':
				if($ug){
					header("location: ?modulo=principal/planotrabalhoUG/gerenciarsubacaoUG&acao=A");
					exit;
				} elseif($un){
					header("location: ?modulo=principal/planotrabalhoUN/gerenciarsubacaoUN&acao=A");
					exit;
				}
			break;
