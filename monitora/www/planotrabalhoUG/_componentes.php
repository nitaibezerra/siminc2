...
<script>
		...
		var URL_AJAX = 'http://<?= $_SERVER['SERVER_NAME'] ?>/<?= $diretorio ?>/ajaxUG.php'
			+ '?atividade=<?= $atividade ?>'
...
<td style="font-weight:bold; text-align:center;">Comandos</td><td style="font-weight:bold; text-align:center;">Nível</td>
...
<?php if( $permissao && ($status=='A' || $status=='X' || $status=='Z' || $status=='K' || !$status) ): ?><img align="absmiddle" border="0" src="../imagens/gif_inclui.gif" onclick="cadastrar_atividade( <?= $atividade['atiid'] ?> )" title="Cadastrar Subtividade" style="cursor:pointer;"/><?php else: ?><img align="absmiddle" src="../imagens/gif_inclui_d.gif"/><?php endif; ?> 
...
if( $atividade['qtdrestricoes'] > 0 ): ?> <a href="?modulo=principal/planotrabalhoUG/controle&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/restricao.png" border="0" align="absmiddle" title="<?= $atividade['qtdrestricoes'] ?> <?= $atividade['qtdrestricoes'] == 1 ? 'restrição' : 'restrições' ?>"/></a><?php endif; ?><?php
if( $atividade['qtdanexos'] > 0 ): ?> <a href="?modulo=principal/planotrabalhoUG/instrumento&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/clipe.gif" border="0" align="absmiddle" title="<?= $atividade['qtdanexos'] ?> <?= $atividade['qtdanexos'] == 1 ? 'anexo' : 'anexos' ?>"/></a><?php endif;
if( $pliid ): ?><a href="?modulo=principal/planotrabalhoUG/pesquisa_piUG&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/cifrao.jpg" border="0" align="absmiddle" title="<?php echo $db->pegaUm("SELECT count(1) as count FROM monitora.pi_planointerno p INNER JOIN monitora.pi_planointernoatividade pia ON p.pliid = pia.pliid WHERE p.plistatus = 'A' AND pia.atiid = {$atividade['atiid']}"); ?> PI(s)"/></a><?php endif;
...
<span style="margin: 0 5px 0 0; <?= $permissao ? '' : 'color:#909090;' ?>"><?php
	...
	?> <?= $numero ?></span><a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/planotrabalhoUG/subatividadesUG&acao=A&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="<?= $permissao ? '' : 'color:#909090;' ?>"><?= $atividade['atidescricao'] ?></a>
</div>
