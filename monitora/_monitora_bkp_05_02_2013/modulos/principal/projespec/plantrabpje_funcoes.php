...
function projetoaberto()
 {
	...
	*/
	...
	{
		// se o projeto estiver atrasasdo, ou em dia, ou iniciado, ou sem andamento ou paralisado entï¿½o	//
		// pode acompanhar																					//
		return true;
	}
//	else
...
function elimina_antecessor_recurssivo( $ptoid , $ptoid2 , $ptoordem , $antec )
{
	...
	if ($antec)
	{
		...
		if ($ptoid2)
		{
			// entï¿½o aconteceu a recursividade
	...
	if( $resultado )
	{
		foreach( $resultado as $linha )
		{
			...
			if( $mudaDataIni && $linha[ 'ptosndatafechada' ] == 'f' )
			{
				...
				$intervalo = $tsDataFim - $tsDataIni;//duraï¿½ï¿½o da atividade
