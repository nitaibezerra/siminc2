...
function atividade_inserir( $atividade, $titulo, $subacao = false ){
	global $db;
	$sql = sprintf(
		"insert into pde.atividade (
			atiidpai, atidescricao, ungcod, atiordem, _atiprojeto, acaid
		) values (
			%d,
			'%s',
			'%s',
...
function atividade_listar( $atividade, $profundidade = 0, $situacao = array(), $usuario = null, $perfil = array() ){
	...
	$sql = sprintf(
		...
		where
			...
			and a.atiid != a._atiprojeto
			and a.atistatus = 'A'
			and a.ungcod = '{$_SESSION['monitora_var']['ungcod']}'
			%s %s %s %s %s 
		order by _atiordem",
		PERFIL_GERENTE,
		$projeto,
	...
	//ver($_SESSION['monitora_var']['ungcod'],$sql,d);
...
function atividade_pegar_rastro( $numero ){
	...
	$sql = sprintf(
		...
		where
			...
			a.atistatus = 'A' and
			a.ungcod = '{$_SESSION['monitora_var']['ungcod']}' and
			( %s )
	...
	if ( $atividade ) {
		$sql = sprintf(
			...
			where
				...
				a.atistatus = 'A' and
				a.ungcod = '{$_SESSION['monitora_var']['ungcod']}'
				%s
			order
				by la.ordem",
			$atividade,
...
function enviaEmailStatusPi($pi){
		...
		$arEmailGestorUnidadeObrig = $db->carregar("SELECT u.usuemail FROM monitora.usuarioresponsabilidade ur inner join seguranca.usuario u on ur.usucpf = u.usucpf where ur.rpustatus = 'A' and ur.pflcod = '".PERFIL_MONITORA_GESTORUNIOBRIGATORIA."' and ur.ungcod = '{$arCadastradorPI['ungcod']}' ");
		...
		# Verifica se existe ptres do pi e ptres FNDE
		...
		FROM
			...
			LEFT JOIN public.unidadegestora e ON e.ungcod = pi.ungcod
		...
		//echo "$rementente | $emailUsuPi | $assunto | $conteudo | $emailCopia";
		//exit;
...
//function usuario_possui_perfil(){
//	global $db;
//	if ( $db->testa_superuser() ) {
//		return true;
//	}
...
function possui_perfil($perfil){
	global $db;
	$sql = "select count(1) from seguranca.perfilusuario where usucpf = '{$_SESSION['usucpf']}' and pflcod in ('".$perfil."')";
	return (boolean) $db->pegaUm( $sql );
}

function perfil_unidade(){
	global $db;
	return $db->carregar("SELECT ur.ungcod FROM monitora.usuarioresponsabilidade ur where ur.pflcod in ('".PERFIL_MONITORA_GESTORUNIOBRIGATORIA."','".PERFIL_MONITORA_ESQUIPEGESTORUNIOBRIGATORIA."','".PERFIL_MONITORA_GESTORSUBACOES."') and ur.usucpf = '".$_SESSION['usucpf']."' AND ur.ungcod is not null ");
}

function perfil_unidadegestora(){
	global $db;
	$sql = "SELECT ug.unicod FROM monitora.usuarioresponsabilidade ur inner join unidadegestora ug ON ug.ungcod = ur.ungcod where ur.pflcod in ('".PERFIL_MONITORA_GESTORUNIOBRIGATORIA."','".PERFIL_MONITORA_ESQUIPEGESTORUNIOBRIGATORIA."','".PERFIL_MONITORA_GESTORSUBACOES."') and ur.usucpf = '".$_SESSION['usucpf']."' AND ur.ungcod is not null ";
		...
		{
			array_push( $uns, $linha['unicod'] );
		}
	
	return $uns;
}

function verifica_cadastrosiafi($pliid){
	global $db;
	
	$ugr = perfil_unidadegestora();
	$ugr = $ugr ? $ugr : array(AD) ;
	$sql = "select count(1) from monitora.pi_planointernohistorico pih 	
					INNER JOIN seguranca.usuario u on pih.usucpf = u.usucpf
					INNER JOIN unidade un on un.unicod = u.unicod
					INNER JOIN ( SELECT pliid, MAX(pihid) as pihid FROM monitora.pi_planointernohistorico pih INNER JOIN seguranca.usuario u on pih.usucpf = u.usucpf WHERE PLIID = ".$pliid." and pihsituacao = 'C' and unicod in ('".AD."','".implode("','",$ugr)."') GROUP BY PLIID ) pi on pi.pliid = pih.pliid and pi.pihid = pih.pihid
				where un.unicod in ('".implode("','",$ugr)."') and pihsituacao = 'C' and pih.pliid =".$pliid;
	//dbg($sql);
	return (boolean) $db->pegaUm( $sql );
}
