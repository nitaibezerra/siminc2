...
<?
$sql = "select distinct a.prgcod||'.'||a.acacod||'.'||a.unicod as codigo, a.acadsc as descricao from usuarioresponsabilidade u inner join acao a on u.acaid=a.acaid where a.acasnrap='f' and rpustatus='A' and usucpf = '$usucpf' and u.pflcod=$pflcod";
$RS = $db->carregar($sql);
if(is_array($RS)) {
	$nlinhas = count($RS)-1;
	if ($nlinhas>=0) {
	...
	$sql = "select distinct a.prgcod||'.'||a.acacod||'.'||a.unicod as codigo, a.acadsc as descricao from acao a inner join progacaoproposto p on a.acacod=p.acacod and a.prgid=p.prgid and a.unicod=p.unicod and p.usucpf='".$usucpf."' where a.acasnrap='f' and p.acacod is not null";
	$RS = $db->carregar($sql);
	if(is_array($RS)) {
		$nlinhas = count($RS)-1;
		if ($nlinhas>=0) {
