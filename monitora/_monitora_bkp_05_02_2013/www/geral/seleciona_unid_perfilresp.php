...
if(is_array($_POST['usuunidresp']) && @count($_POST['usuunidresp'])>0) {
	...
	$unidadesConfirmadas = $_REQUEST["unidadesConfirmadas"];//(bool)$_REQUEST["unidadesConfirmadas"];
	$sqlSelResp = "SELECT distinct ur.rpuid, ur.usucpf, ur.rpustatus, u.unidsc, u.unicod, u.uniid FROM elabrev.usuarioresponsabilidade ur 
		INNER JOIN unidade u ON u.unitpocod='U' and u.unicod = ur.unicod AND u.uniid = %s			
		WHERE ur.rpustatus = 'A' AND ur.usucpf <> '" . $usucpf . "'";
	

	$sqlSelUnidade	= "SELECT u.unicod, u.uniid FROM unidade u WHERE u.uniid = %s ";
	...
	$sqlUpdRpu = "UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE rpuid = '%s'";

	//
	// verificar quais unidades possuem outro coordenador de planejamento ativo
...
//dbg('wwww'.$linhasRpu.$sql);
	...
	$sql = "delete from elabrev.usuarioresponsabilidade where unicod is not null and usucpf='$usucpf'";
		$db->executar($sql);
		$concluido = 1;
		foreach ($_POST['usuunidresp'] as $respcod) {
		   $sql = "";
	...
	if($unidadesConfirmadas) {
		...
		foreach ($linhasRpu as $rpu) {
			...
			$db->executar($sql);
			$dados = array($rpu["unicod"], $usucpf, 'A', date("Y-m-d H:i:s"), $pflcod);
			$sql = vsprintf($sqlInsRpu, $dados);
			//dbg('222'.$sql);
			$db->executar($sql);
		}
	...
	// a substituição pelo usuario que está sendo liberado e/ou alterado
		...
		<script>
			...
			{
				document.formassocia.unidadesConfirmadas.value=0;
				document.formassocia.submit();
			}
		</script>
...
<?
$sql = "select distinct u.uniid as codigo, u.unicod||' - '||u.unidsc as descricao from elabrev.usuarioresponsabilidade ur inner join unidade u on ur.unicod=u.unicod where ur.rpustatus='A' and ur.usucpf = '$usucpf' and ur.pflcod=$pflcod";
$RS = @$db->carregar($sql);
if(is_array($RS)) {
	$nlinhas = count($RS)-1;
	if ($nlinhas>=0) {
	...
	$sql = "select distinct u.unicod as codigo, u.unidsc as descricao from unidade u inner join unidproposto up on u.unicod=up.unicod up.usucpf='".$usucpf."' where up.unicod is not null";
	$RS = @$db->carregar($sql);
	if(is_array($RS)) {
		$nlinhas = count($RS)-1;
		if ($nlinhas>=0) {
