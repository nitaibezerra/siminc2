...
$sql = "select p.ptoid as ptoid2, p.ptoid_pai,p.ptotipo,p.ptocod, case when p.ptotipo='S' then 'Sub ação' when p.ptotipo='E' then 'Etapa' else 'Fase' end as tipo,p.ptodsc, p.ptoprevistoexercicio as previsto, p.ptosnpercent, p.ptosnsoma, u.unmdsc, case when sum(e.exprealizado) is null then 0 else sum(e.exprealizado) end as totalrealizado from planotrabalho p inner join unidademedida u on p.unmcod=u.unmcod left join execucaopto e on p.ptoid=e.ptoid where p.ptostatus='A' and p.ptoid in (select ptoid from plantrabacao where acaid=".$_SESSION['acaid'].") and p.ptoid_pai=".$_REQUEST['ptoid']."  group by p.ptoid,p.ptoid_pai,p.ptotipo, p.ptocod, p.ptodsc, p.ptoprevistoexercicio, p.ptosnpercent, p.ptosnsoma,p.ptoordem, u.unmdsc order by p.ptoordem,p.ptotipo desc,p.ptoid_pai, p.ptocod";
...
<table width="95%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#003F7E;">
  <?
	 ...
	 if ($nlinhasp<0) {
       $ptoid3=$ptoid;
    include APPRAIZ.$_SESSION['sisdiretorio']."/modulos/principal/acao/etapafase2.inc";
  }
	 else {
