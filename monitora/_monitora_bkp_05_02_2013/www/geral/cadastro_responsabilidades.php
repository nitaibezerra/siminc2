	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {	
			case "A": // AÇÃO
				...
				$sqlRespUsuario = "SELECT a.prgcod || '.' || a.acacod || '.' || a.unicod || '.' || a.loccod AS codigo, a.acatitulo AS descricao, a.prgid, a.acaid, a.acacod, u.rpustatus AS status
			...
			break;		
			case "O": // Objetivo
				$aca_prg = "Objetivos Associados";
				$sqlRespUsuario = " SELECT 
										o.objcod AS codigo, 
										o.objdsc AS descricao
									FROM 
										monitora.usuarioresponsabilidade u 
									INNER JOIN monitora.objetivo o ON o.objid = u.objid
									WHERE 
										u.usucpf = '%s' 
										AND u.pflcod = '%s' 
										AND u.rpustatus='A' 
			...
			break;			
			case "I": // Ininciativa
				$aca_prg = "Iniciativas Associadas";
				$sqlRespUsuario = " SELECT 
										i.ictcod AS codigo, 
										i.ictdsc AS descricao
									FROM 
										monitora.usuarioresponsabilidade u 
									INNER JOIN monitora.iniciativa i ON i.ictid = u.ictid
									WHERE 
										u.usucpf = '%s' 
										AND u.pflcod = '%s' 
										AND u.rpustatus='A' 
			...
			break;				
			case "M": // Meta
				$aca_prg = "Metas Associadas";
				$sqlRespUsuario = " SELECT 
										m.metid AS codigo, 
										m.metdsc AS descricao
									FROM 
										monitora.usuarioresponsabilidade u 
									INNER JOIN monitora.meta m ON m.metid = u.metid
									WHERE 
										u.usucpf = '%s' 
										AND u.pflcod = '%s' 
										AND u.rpustatus='A' 
