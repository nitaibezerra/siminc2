...
if(is_array($_POST['usuprogresp']) && @count($_POST['usuprogresp'])>0) {
	...
	$sqlSelMeta	= "SELECT m.metid, m.metdsc FROM monitora.meta m WHERE metid = %s AND metstatus = 'A' ";
	
	$sqlInsRpu = "INSERT INTO monitora.usuarioresponsabilidade (metid, usucpf, rpustatus, rpudata_inc, pflcod, prsano) VALUES ('%s', '%s', '%s', '%s', '%s','".$_SESSION['exercicio']."')";

	$sqlUpdRpu = "UPDATE monitora.usuarioresponsabilidade SET rpustatus = 'I' WHERE metid = '%s' AND pflcod = ".$pflcod." AND prsano = '{$_SESSION['exercicio']}'";

	$sqlUpdRpuUsu = "UPDATE monitora.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '".$usucpf."' AND metid IS NOT NULL AND pflcod = ".$pflcod." AND prsano = '{$_SESSION['exercicio']}'";
	...
	if(!$confirmarmetas) {
		...
		foreach ($_POST['usuprogresp'] as $respcod) {
		   $sql = "";
		   if ($respcod>0){
		   $sql = vsprintf($sqlSelMeta, $respcod);
           $linha = $db->carregar($sql);
			if(is_array($linha) && count($linha)>=1) {
				foreach ($linha as $objetivo) {
					$metid = $objetivo["metid"]; 		
					$dados = array($metid, $usucpf, 'A', date("Y-m-d H:i:s"), $pflcod); 						
...
<thead>
	<tr>
		<td valign="top" class="title" style="border-right: 1px solid #c0c0c0; border-bottom: 1px solid #c0c0c0; border-left: 1px solid #ffffff;" colspan="3"><strong>Selecione a(s) Meta(s)</strong></td>
	</tr>
   	<tr bgcolor="<?=$cor?>">
...
<?
	  ...
	  $cabecalho = 'Selecione a(s) Meta(s)';
	  $sql = "SELECT 
	  			m.metid, 
	  			m.metdsc, 
	  		  ...
	  		  FROM 
	  		  	monitora.meta m
	  		  JOIN monitora.objetivometa om ON om.metid = m.metid
	  		  JOIN monitora.objetivo o ON o.objid = om.objid AND
	  		  							  o.objstatus = 'A'
	  		  WHERE 
	  		  	metstatus='A' AND 
	  		  	prgano='".$_SESSION['exercicio']."' 
	  		  ORDER BY objcod, metdsc";
	  ...
	  for ($i=0; $i<=$nlinhas;$i++)
	   ...
	   ?>
		   		...
		   		<tr bgcolor="<?=$cor?>">
					<td align="right">
						<input type="Checkbox" name="prgid" id="<?=$metid?>" value="<?=$metid?>" onclick="retorna(<?=$i?>);">
						<!-- 
						<input type="Hidden" name="prgdsc" value="<?=$objcod.' - '.$metdsc?>">
						-->
						<input type="Hidden" name="prgdsc" value="<?=$metdsc?>">
					</td>
					<td align="justify" valign="top"><?=resumo($objdsc, 150)?></td>
					<td align="justify" valign="top"><?=$metdsc?></td>
				</tr>
...
$sql = "SELECT DISTINCT 
		 m.metid as codigo, 
		 m.metdsc AS descricao 
		FROM 
		 monitora.usuarioresponsabilidade ur 
		 INNER JOIN monitora.meta m ON ur.metid=m.metid
		 JOIN monitora.objetivometa om ON om.metid = m.metid
  		 JOIN monitora.objetivo o ON o.objid = om.objid AND
  		  							 o.objstatus = 'A'
		WHERE 
...
?>
	<option value="">Selecione a(s) Meta(s).</option>
<?
}
?>
</select>
...
function retorna(objeto)
{
	...
	if (document.formulario.prgid[objeto].checked == true){
		...
		for(var i=0; i<=campoSelect.length-1; i++){
			...
			if (!campoSelect.options[0]){campoSelect.options[0] = new Option('Selecione a(s) Meta(s).', '', false, false);}
			sortSelect(campoSelect);
	}
}
