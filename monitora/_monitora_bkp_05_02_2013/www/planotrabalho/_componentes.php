...
<table style="width:100%; border:0; padding:5px;">
	<tr>
		...
		&nbsp;&nbsp;|&nbsp;&nbsp; <span
			...
			<?
			if($atividade) {
				$prof = $db->pegaUm("SELECT _atiprofundidade FROM pde.atividade WHERE atiid='".$atividade."'");
				if($prof > 1) {
					echo "&nbsp;&nbsp;|&nbsp;&nbsp; <span title=\"Vincular Subação\" onclick=\"janela = window.open('/monitora/monitora.php?modulo=principal/planotrabalho/listarsubacoesatividade&acao=A&atiid=".$atividade."', 'janela1', 'menubar=no,location=no,resizable=no,scrollbars=yes,status=yes,width=800,height=400' ); janela.focus();\" style=\"cursor: pointer;\"> Vincular Subação </span>";
				}	
			}
			?>
...
<a href="?modulo=principal/planotrabalho/atividade&acao=A&atiid=<?= $atividade['atiid'] ?>"><img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/></a>
...
if( $atividade['qtdrestricoes'] > 0 ): ?> <a href="?modulo=principal/planotrabalho/controle&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/restricao.png" border="0" align="absmiddle" title="<?= $atividade['qtdrestricoes'] ?> <?= $atividade['qtdrestricoes'] == 1 ? 'restrição' : 'restrições' ?>"/></a><?php endif; ?><?php
if( $atividade['qtdanexos'] > 0 ): ?> <a href="?modulo=principal/planotrabalho/instrumento&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/clipe.gif" border="0" align="absmiddle" title="<?= $atividade['qtdanexos'] ?> <?= $atividade['qtdanexos'] == 1 ? 'anexo' : 'anexos' ?>"/></a><?php endif;
if( $atividade['qtdpi'] > 0 ): ?> <a href="<? echo (($atividade['qtdpi']==1)?"?modulo=principal/planotrabalho/cadastro_pi&acao=A&atiid=".$atividade['atiid']:"?modulo=principal/planotrabalho/cadastro_pi&acao=A&atiid=".$atividade['atiid']); ?>"><img src="../imagens/cifrao.jpg" border="0" align="absmiddle" title="<?= $atividade['qtdpi'] ?> <?= $atividade['qtdpu'] == 1 ? 'PI' : 'PIs' ?>"/></a><?php endif;
...
<span style="margin: 0 5px 0 0; <?= $permissao ? '' : 'color:#909090;' ?>"><?php
	...
	?> <?= $numero ?></span><a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/planotrabalho/subatividades&acao=A&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="<?= $permissao ? '' : 'color:#909090;' ?>"><?= $atividade['atidescricao'] ?></a>
</div>
