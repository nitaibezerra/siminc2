...
<table style="width:100%; border:0; padding:5px;">
	<tr>
		...
		<span title="nova atividade"
			onclick="cadastrar_atividade( <?= $atividade ? $atividade : $projeto ?>, 'E' );"
			style="cursor: pointer;"> <img align="absmiddle"
			src="/imagens/gif_inclui.gif" /> Cadastrar Etapa </span>
			&nbsp;&nbsp;|&nbsp;&nbsp;
...
<script>
		...
		var URL_AJAX = 'http://<?= $_SERVER['SERVER_NAME'] ?>/<?= $diretorio ?>/ajax_tablets.php'
			+ '?atividade=<?= $atividade ?>'
...
<script language="javascript" type="text/javascript">
		...
		function cadastrar_atividade( atividade, tipo ){
			...
			if(tipo == 'E')
				var titulo = window.prompt( 'Título da Etapa:', 'Nova Etapa' );
		...
		function atualizaAtividade(atii1, atiid2){
			requisicao_acao = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject( 'Msxml2.XMLHTTP' );
			requisicao_acao.open( 'GET', URL_AJAX + '&evento=atualiza_atividade&atiid1=' + atii1 + '&atiid2=' + atiid2, true );
			requisicao_acao.onreadystatechange = capturar_resposta;
			requisicao_acao.send( null );
		}
		...
		function mudar_ordem_atividade( origem, destino ){
			...
			requisicao_acao.onreadystatechange = capturar_resposta;
			requisicao_acao.send( null );
			atualizaAtividade(origem, destino);
		}
...
<script>
		...
		var requisicao_status_pai = null
		
		function atualizaBarraStatusPai( intBarraStatusId , strStatus , intStatus, intPercentual )
		{
			var idAtividade = intBarraStatusId.replace( 'situacao_atividade_', '' );
			if( intPercentual == 100 ){
				requisicao_status_pai = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject( 'Msxml2.XMLHTTP' );
				requisicao_status_pai.open( 'GET', URL_AJAX
					+ '&evento=arvore_busca_pai'
					+ '&atiid=' + idAtividade
					+ '&valor=' + intPercentual,
					true
				);
				
				requisicao_status_pai.onreadystatechange = function retorno() {
					
					if ( requisicao_status_pai.readyState == 1 ) {
					} else if ( requisicao_status_pai.readyState == 4 ) {
						if( requisicao_status_pai.responseText != '' ) {
							
							var dados = requisicao_status_pai.responseText.split("_");
							var atiidPai = dados[0];
							var valorPai = dados[1];
							var totalPerc = dados[2];
							...
							intPercentual = valorPai
							intBarraStatusId = 'situacao_atividade_'+atiidPai;
							requisicao_status = null;
				...
				requisicao_status_pai.send( null );
			}
		}
		...
		function atualizaBarraStatus( intBarraStatusId , strStatus , intStatus, intPercentual )
		{
			...
			var requisicao = '';
			requisicao_status.onreadystatechange = function retorno() {
				requisicao = requisicao_status.readyState; 
				recebeConfirmacaoStatus( intBarraStatusId, strStatus, intStatus, intPercentual ); 
				if ( requisicao == 4 ) { atualizaBarraStatusPai( intBarraStatusId , strStatus , intStatus, intPercentual ); }
			};
			requisicao_status.send( null );
		}
...
<a href="?modulo=principal/atividade_tablets/atividade&acao=P&atiid=<?= $atividade['atiid'] ?>">
<img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/>
</a>
<?php endif;?>
<?php if( $atividade['atitipoenem'] == 'S' ): ?>
<a href="?modulo=principal/atividade_tablets/atividade&acao=S&atiid=<?= $atividade['atiid'] ?>">
<img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/>
</a>
<?php endif;?>
<?php if( $atividade['atitipoenem'] == 'A' ): ?>
<a href="?modulo=principal/atividade_tablets/dados_atividade&acao=A&atiid=<?= $atividade['atiid'] ?>">
<img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/>
</a>
<?php endif;?>
...
if(usuario_possui_perfil(PERFIL_ADMINISTRADOR)) $permissao = true;
...
$possuiItens = $db->pegaUm("SELECT iclid FROM pde.itemchecklist WHERE atiid='".$atividade['atiid']."'");
...
<!-- VERIFICA CHECKLIST -->
<?php if( $atividade['atitipoenem'] == 'A' ): ?>
	<a href="?modulo=principal/atividade_tablets/listar_checklist&acao=A&atiid=<?= $atividade['atiid'] ?>">
		<? 
		$sql = "select iclid from pde.itemchecklist icl 
				WHERE icl.atiid = ".$atividade['atiid']."
				order by iclid";
		$listaPendente = $db->carregar($sql);

		$existePendencia = 0;
		if($listaPendente){
			foreach($listaPendente as $l){
				$sql = "select count(vldid) as total from pde.validacao	WHERE iclid = ".$l['iclid'];
				$validacao = $db->pegaUm($sql);

				$sql = "select count(etcid) as total from pde.etapascontrole WHERE iclid = ".$l['iclid'];
				$etapascontrole = $db->pegaUm($sql);
				
				if($validacao != $etapascontrole){
					$existePendencia = 1;
					break;			
				}
			}
		...
		$sql = "select count(icl.iclid) as total from pde.itemchecklist icl
					inner join workflow.documento d ON d.docid = icl.docid
					WHERE icl.atiid = ".$atividade['atiid']."
					AND icl.iclprazo < now()::date
					AND esdid <> ".TABLETS_EST_EM_FINALIZADO;	
		$foraDoPrazo = $db->pegaUm($sql);

		
		if($existePendencia>0 && $foraDoPrazo==0){
			echo '<img width="12px" height="15px" title="Existe checklist e estão em dia, mas não totalmente concluído." align="absmiddle" src="../imagens/lista_verde.gif"/>';
		}
		else if($existePendencia>0 && $listaPendente){
			echo '<img width="12px" height="15px" title="Existe checklist e existem pendências." align="absmiddle" src="../imagens/lista_vermelha.gif"/>';	
		}
		else if($existePendencia==0 && $listaPendente){
			echo '<img width="12px" height="15px" title="Existe checklist e está concluído." align="absmiddle" src="../imagens/lista_azul.gif"/>';
		}
		/*
		else if(!$listaPendente){
			echo '<img width="12px" height="15px" title="Não existe checklist." align="absmiddle" src="../imagens/lista_vermelha.gif"/>';	
		}
		*/
...
if( $atividade['qtdrestricoes'] > 0 ): ?> <a href="?modulo=principal/atividade_tablets/controle&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/restricao.png" border="0" align="absmiddle" title="<?= $atividade['qtdrestricoes'] ?> <?= $atividade['qtdrestricoes'] == 1 ? 'restrição' : 'restrições' ?>"/></a><?php endif; ?><?php
if( $atividade['qtdanexos'] > 0 ): ?> <a href="?modulo=principal/atividade_tablets/instrumento&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/clipe.gif" border="0" align="absmiddle" title="<?= $atividade['qtdanexos'] ?> <?= $atividade['qtdanexos'] == 1 ? 'anexo' : 'anexos' ?>"/></a><?php endif;
...
<span style="margin: 0 5px 0 0; <?= $permissao ? '' : 'color:#909090;' ?>"><?php
	...
	<?php if( $atividade['atitipoenem'] == 'E' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_tablets/subatividades&acao=E&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#000099">(E)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php elseif( $atividade['atitipoenem'] == 'P' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_tablets/subatividades&acao=P&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#0033FF">(P)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php elseif( $atividade['atitipoenem'] == 'S' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_tablets/subatividades&acao=S&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#006600">(S)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php elseif( $atividade['atitipoenem'] == 'A' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_tablets/subatividades&acao=A&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#009933">(A)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php endif; ?>
 ...
 * 
$sql = "SELECT x.atiidpai FROM pde.atividade x WHERE x.atiid = {$atividade['atiid']}";
$idpai = $db->pegaUm($sql);
$sql = "SELECT a.esaid FROM pde.atividade a WHERE a.atiid = {$atividade['atiid']}";
$esaid = $db->pegaUm($sql);
$sql = "SELECT to_char(a.atidatainicio, 'DD/MM/YYYY') FROM pde.atividade a WHERE a.atiid = {$atividade['atiid']}";
$dataI = $db->pegaUm($sql);
$sql = "SELECT to_char(a.atidatafim, 'DD/MM/YYYY') FROM pde.atividade a WHERE a.atiid = {$atividade['atiid']}";
$dataF = $db->pegaUm($sql);
$sql = "SELECT to_char(a.atidatainicio, 'DD/MM/YYYY') FROM pde.atividade a WHERE a.atiid = (SELECT x.atiidpai FROM pde.atividade x WHERE x.atiid = {$atividade['atiid']})";
$dataIpai = $db->pegaUm($sql);
$sql = "SELECT to_char(a.atidatafim, 'DD/MM/YYYY') FROM pde.atividade a WHERE a.atiid = (SELECT x.atiidpai FROM pde.atividade x WHERE x.atiid = {$atividade['atiid']})";
...
$sql = "SELECT a.atiidpai, 
			   ...
			   (SELECT to_char(x.atidatainicio, 'DD/MM/YYYY') FROM pde.atividade x WHERE x.atiid = a.atiid) as dataIpai,
			   (SELECT to_char(x.atidatafim, 'DD/MM/YYYY') FROM pde.atividade x WHERE x.atiid = a.atiid) as dataFpai 
			   FROM pde.atividade a WHERE a.atiid = {$atividade['atiid']}";
...
?>
<? //if(!$possuiItens){ echo $onclick; } ?>
<td <? echo $onclick; ?> id="situacao_atividade_<?= $atividade['atiid'] ?>" <?=$metaNum?> percentual="<?= $percentual?>" status="<?= (integer) $atividade['esaid'] ?>" style="text-align:center;">
<?= montar_barra_execucao( $atividade, $permissao ) ?>
</td>

<?php $onclick = $permissao ? 'onclick="montaCalendario( this );"' : ''; ?>
<? //echo (($atividade['atitipoenem'] != 'A' && $atividade['esaid'] == STATUS_CONCLUIDO)?$onclick:''); ?>
<td <? echo $onclick; ?> <?= $permissao ? 'title="Alterar data de início"' : '' ?> id="data_inicio_atividade_<?= $atividade['atiid'] ?>" style="text-align:center; color: <?= $permissao ? '#008000' : '#909090' ?>;">
...
?>
<? //echo (($atividade['atitipoenem'] != 'A')?$onclick:''); ?>
<td <? echo $onclick; ?> id="data_fim_atividade_<?= $atividade['atiid'] ?>" style="text-align:center; <?= $permissao ? ( strtotime( $atividade['atidatafim'] ) < time() ? 'color:#ff2020;font-weight:bold;' : 'color:#008000;' ) : 'color:#909090;' ?>;">
<?=
...
function montar_formulario_pesquisa( $foco = false ){
	...
	$formulario = sprintf(
	'<form action="/%s/%s.php?modulo=principal/atividade_tablets/pesquisa&acao=A" method="post" name="pesquisa">
...
function retornaUltimoAlteradorAtividade( $atiid ){
	...
	$sql = "SELECT
			...
			FROM
				...
				INNER JOIN pde.historicoatividade hat ON hat.usucpf = usu.usucpf
				WHERE
					atiid = ".$atiid."
				ORDER BY
					hatid DESC 
				LIMIT 1) foo";
	return '<strong>Ultimas alterações feitas por: '.$db->pegaUm($sql).'</strong><br>';
}
