...
<script>
		...
		var URL_AJAX = 'http://<?= $_SERVER['SERVER_NAME'] ?>/<?= $diretorio ?>/ajax_estrategico.php'
			+ '?atividade=<?= $atividade ?>'
...
<td style="font-weight:bold; text-align:center;">Título</td><td style="font-weight:bold; text-align:center;">Início</td>
<!--
<td style="font-weight:bold; text-align:center;">Situação</td><td style="font-weight:bold; text-align:center;">Início</td>
-->
<td style="font-weight:bold; text-align:center;">Término</td><td style="font-weight:bold; text-align:center;">Ordem</td>
</tr>
</thead>
<?php
...
foreach( $lista as $indice => $atividade ) {
	...
	//Verifica a existência de checklist
	$sql = "select micid from pde.monitoraitemchecklist where atiid = {$atividade['atiid']} and micstatus = 'A' limit 1";
	$checklist = $db->pegaUM($sql);
...
<a href="?modulo=principal/atividade_estrategico/atividade&acao=A&atiid=<?= $atividade['atiid'] ?>"><img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/></a>
...
<?php if( $permissao && !$tem_filhos && !$checklist ): ?><img align="absmiddle" border="0" src="../imagens/excluir.gif" onclick="excluir_atividade( <?= $atividade['atiid'] ?> )" title="Excluir Atividade" style="cursor:pointer;"/><?php else : ?><img align="absmiddle" src="../imagens/excluir_01.gif"/><?php endif; ?><a href="/<?= $diretorio ?>/gantt.php?atiid=<?= $atividade['atiid'] ?>" target="_blank"><img align="absmiddle" border="0" src="../imagens/lupa_grafico.gif" title="Exibir Gráfico" style="cursor:pointer; margin-left:5px;" /></a>
<?php if( $checklist ): ?><img align="absmiddle" border="0" src="../imagens/check_p.gif" onclick="window.location.href='estrategico.php?modulo=principal/atividade_estrategico/listar_checklist&acao=A&atiid=<?= $atividade['atiid'] ?>'" title="Listar Check List" style="cursor:pointer;"/><?php endif; ?>
<?
$existesubacao = $db->pegaUm("SELECT sbaid FROM monitora.pi_subacaoatividade WHERE atiid='".$atividade['atiid']."'");

if (usuario_possui_perfil( PERFIL_GESTOR ) && $_SESSION['sisid'] != 11) {
if($existesubacao  ) {
?>
<a style="cursor:pointer;" onclick="janela = window.open('/pde/pde.php?modulo=principal/atividade_estrategico/vincularsubacoesatividadePDE&acao=A&atiid=<?= $atividade['atiid'] ?>', 'janela1', 'menubar=no,location=no,resizable=no,scrollbars=yes,status=yes,width=800,height=400' ); janela.focus();"><img src="../imagens/valida4.gif" border="0" align="top" title="Inserir subação"/></a>	
<?
} else {
?>
...
//verifica se ele pode ou não mudar o nível

$permiteNivel = verificaPermissaoNivel($atividade);

if( $permissao_avo && $profundidade > 0 || $permiteNivel && $profundidade > 0 ): ?><img align="absmiddle" onclick="mudar_irma_atividade( <?= $atividade['atiid'] ?>, <?= $atividade['atiidpai'] ?> );" title="Recuar" src="../imagens/recuo_e.gif" style="cursor:pointer;"/><?php else : ?><img align="absmiddle" src="../imagens/recuo_e_d.gif"/><?php endif; ?> 
<?php if( $permissao_pai && !$primeiro || $permiteNivel && !$primeiro ): ?><img align="absmiddle" onclick="mudar_pai_atividade( <?= $atividade['atiid'] ?>, <?= $id_anterior ?> );" title="Avançar" src="../imagens/recuo_d.gif" style="cursor:pointer;" /><?php else : ?><img align="absmiddle" src="../imagens/recuo_d_d.gif"/><?php endif; ?> 
</td>
...
if( $atividade['qtdrestricoes'] > 0 ): ?> <a href="?modulo=principal/atividade_estrategico/controle&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/restricao.png" border="0" align="absmiddle" title="<?= $atividade['qtdrestricoes'] ?> <?= $atividade['qtdrestricoes'] == 1 ? 'restrição' : 'restrições' ?>"/></a><?php endif; ?><?php
if( $atividade['qtdanexos'] > 0 ): ?> <a href="?modulo=principal/atividade_estrategico/instrumento&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/clipe.gif" border="0" align="absmiddle" title="<?= $atividade['qtdanexos'] ?> <?= $atividade['qtdanexos'] == 1 ? 'anexo' : 'anexos' ?>"/></a><?php endif;
...
<span style="margin: 0 5px 0 0; <?= $permissao ? '' : 'color:#909090;' ?>"><?php
	...
	?> <?= $numero ?></span><a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_estrategico/subatividades&acao=A&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="<?= $permissao ? '' : 'color:#909090;' ?>"><?= $atividade['atidescricao'] ?></a>
</div>
<?php if( $_SESSION["sisid"] == 10 && ( $atividade["_atiprofundidade"] == 1 || $atividade["_atiprofundidade"] == 2 ) ): ?>
<td><?php echo $atividade['usunomecadastro']; ?></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<?php else: ?>
...
?>
<!--
<td><?= $atividade['usunomecadastro'] ?></td>
...
<td <?= $onclick ?> <?= $permissao ? 'title="Alterar data de início"' : '' ?> id="data_inicio_atividade_<?= $atividade['atiid'] ?>" style="text-align:center; ">
<?= formata_data( $atividade['atidatainicio'] ) ?>
</td>
<?php if( $atividade['esaid'] == STATUS_CONCLUIDO ): ?>
<td <?= $onclick ?> <?= $permissao ? 'title="Alterar data de término"' : '' ?> id="data_conclusao_atividade_<?= $atividade['atiid'] ?>" style="text-align:center;">
<?= formata_data( $atividade['atidataconclusao'] ) ?>
</td><?php
else:
?><td <?= $onclick ?> id="data_fim_atividade_<?= $atividade['atiid'] ?>" style="text-align:center; ">
<?=
...
function montar_formulario_pesquisa( $foco = false ){
	...
	$formulario = sprintf(
	'<form action="/%s/%s.php?modulo=principal/atividade_estrategico/pesquisa&acao=A" method="post" name="pesquisa">
...
function montar_rastro_atividade( $numero, $destacar = true ){
	...
	if($_GET['micid']){
		$rastro[count($rastro)-1] = '<p style="margin: 0 0 5px '.(($indice*20)).'px;"><a href="estrategico.php?modulo=principal/atividade_estrategico/listar_checklist&acao=A&atiid='.$item['atiid'].'">'.$rastro[count($rastro)-1].'</a></p>';
		$sql = "select micid,indnome from pde.monitoraitemchecklist mic inner join painel.indicador ind ON ind.indid = mic.indid where micid = {$_GET['micid']}";
		$mic = $db->pegaLinha($sql);
		$rastro[] = '<p style="font-weight:bold; margin: 0 0 5px '.(($indice*20)+20).'px;"><img src="../imagens/seta_filho.gif" align="absmiddle" border="0">&nbsp;'.$mic['indnome'].'</p>';
	}
