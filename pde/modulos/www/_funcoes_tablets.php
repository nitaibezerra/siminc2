...
function posAcaoVldsituacaoFALSE($iclid) {
	...
	$conf = array(TABLETS_EST_EM_VALIDACAO 	=> array('tpvid' => '2'),
				  TABLETS_EST_EM_EXECUCAO  	=> array('tpvid' => '1'),
				  TABLETS_EST_EM_CERTIFICACAO  => array('tpvid' => '3'));
...
function wf_gerencimentoFluxoEnem($tpdid, $docids = array(), $cxentrada = 'pendencias', $parametros = array(), $filtro_pendencias = null) {
	...
	if($docids['pendencias']) {
		foreach($docids['pendencias'] as $docid) {
			...
			$arrDocumentos[] = $db->pegaLinha("SELECT to_char(icl.iclprazo,'DD/MM/YYYY') as data_cadastro,
...
function abrirDocumento(docid, obj) {
	...
	$.ajax({
   		...
   		data: "docid="+docid,
   		async: false,
   		success: function(msg){document.getElementById('detalhes_documento_'+docid).innerHTML=msg;}
 		});
 	<? } else { ?>
...
<tr>
	...
	<ul>
	  <li><a style="cursor:pointer;" href="tablets.php?modulo=principal/atividade_tablets/minhasPendencias&acao=A&atiidraiz=<?=$_REQUEST['atiidraiz']?>&cxentrada=pendencias"><? echo (($cxentrada == 'pendencias')?'<b>Pendências</b>':'Pendências'); ?> (<? echo count($docids['pendencias']); ?>)</a></li>
	  <li><a style="cursor:pointer;" href="tablets.php?modulo=principal/atividade_tablets/minhasPendencias&acao=A&atiidraiz=<?=$_REQUEST['atiidraiz']?>&cxentrada=resolvidas"><? echo (($cxentrada == 'resolvidas')?'<b>Resolvidas</b>':'Resolvidas'); ?> (<? echo (($arrHistoricos[0])?count($arrHistoricos):"0"); ?>)</a></li>
	  <li><a style="cursor:pointer;" href="tablets.php?modulo=principal/atividade_tablets/minhasPendencias&acao=A&atiidraiz=<?=$_REQUEST['atiidraiz']?>&cxentrada=futuras"><? echo (($cxentrada == 'futuras')?'<b>Futuras em dia</b>':'Futuras em dia'); ?> (<? echo count($docids['futuras']); ?>)</a></li>
	  <?//php if($docids['atrazados'][0]):?>
	  <li><a style="cursor:pointer;" href="tablets.php?modulo=principal/atividade_tablets/minhasPendencias&acao=A&atiidraiz=<?=$_REQUEST['atiidraiz']?>&cxentrada=atrazados"><? echo (($cxentrada == 'atrazados')?'<b>Futuras em atraso</b>':'Futuras em atraso'); ?> (<? echo count($docids['atrazados']); ?>)</a></li>
	  <?//php endif;?>
	</ul>	
	</td>
	<td valign="top">
...
function concluiAtividade( $atiid ){
	...
	if( !empty($listaChecklist[0]) ){
		...
		foreach ($listaChecklist as $check) {
			...
			if( $atual['esdid'] != TABLETS_EST_EM_EXECUCAO ){
				$boFinalizado = false;
