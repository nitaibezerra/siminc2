        ...
        jQuery(function () {
            ...
            jQuery('#<?=$nomeUnico?>').highcharts(Highcharts.merge(gaugeOptions, {
                yAxis: {
                    ...
                    title: {
                        text: '<span style="color:black;">Meta Brasil: <?php echo $metaFormatada . $complemento; ?></span>'
                    }
                },
...
function montaTabelaExamesMec($nomeindicador, $arrNumeroProjeto, $subtitulo='')
{
    ...
    if(in_array(26, $arrNumeroProjeto) || in_array(18, $arrNumeroProjeto) || in_array(19, $arrNumeroProjeto) || in_array(37, $arrNumeroProjeto) || in_array(38, $arrNumeroProjeto)){
	...
	<table class="tabela_box" cellpadding="2" cellspacing="1" width="100%" >
		<?php if(in_array(26, $arrNumeroProjeto) || in_array(18, $arrNumeroProjeto) || in_array(19, $arrNumeroProjeto) || in_array(37, $arrNumeroProjeto) || in_array(38, $arrNumeroProjeto)){ ?>
			<tr>
...
function permissaoPerfilConsultaCockpit($url_atual){
    ...
    $sql = "
        ...
        WHERE ur.usucpf = '{$_SESSION['usucpf']}' AND pflcod = ".PERFIL_CONSULTA_COCKPIT." AND rpustatus = 'A'
    ";
...
function exibirTabelaFinanceiro($acaid){
    ...
    foreach ($years as $year) {
        $strSQL = "SELECT v.vacid, vaeid, v.acaid, exercicio, v.vaetituloorcamentario, ve.vaedescricao
            ...
            WHERE exercicio = {$year} and v.acaid = {$acaid}";
    ...
    /**
     * Formata a matriz com os valores da tabela
     */
    foreach ($keys as $k) {
    ...
    <tr>
        ...
        <?php foreach ($years as $k => $year) : ?>
            <th width="150" title="<?= $resultSet[$k]['vaedescricao'] ?>"><?= $year; ?></th>
        <?php endforeach; ?>
    </tr>
    <?php $i=0; foreach ($totalH as $columnArray) : ?>
        <tr class='<?php echo (($i%2) ? "link zebrado" : "link"); ?>'>
            <td width="150"><strong><?= $labels[$i]; ?></strong></td>
            <?php $k=0; foreach ($columnArray as $column) : ?>
                <td align="center" title="<?= $resultSet[$k]['vaedescricao'] ?>"><?= number_format($column,2,",","."); ?></td>
            <?php $k++; endforeach; ?>
        </tr>
    <?php $i++; endforeach; ?>
    </table>
...
function montaColunaCategoriaPronatec(){
    $colunaSituacao = "CASE
                            WHEN dsh.tidid1 IN (5214,5219,5218,8110,8114,8112) OR dsh.tidid2 IN (6032,6041,6046,6055,6060,6069,6074,6083,6088,6097,6102,6111,6116,6125,6916,6921,6926,6931,6936,6941,6946,6953,6960,6971,6978,7449,7458,7465,8048,8057,8064,7877,7881,7879,9271,9275,9273,9291,9292,9294,9311,9303,9307,9330,9322,9326) THEN 'Conclusao'
                            WHEN dsh.tidid1 IN (5216,8111) OR dsh.tidid2 IN (6033,6035,6039,6047,6049,6053,6061,6063,6067,6075,6077,6081,6089,6091,6095,6103,6105,6109,6117,6120,6123,6955,6962,6973,6980,7450,7452,7456,8049,8051,8055,7878,9272,9287,9288,9297,9313,9304,9306,9323,9332,9325) THEN 'Em curso'
                            WHEN dsh.tidid1 IN (5217,5215,8109,8108) OR dsh.tidid2 IN (6037,6051,6065,6079,6093,6107,6121,6915,6920,6925,6930,6935,6940,6945,6956,6963,6974,6981,7454,7464,8053,8063,7876,7875,9270,9269,9293,9296,6979,6972,6961,6954,9310,9314,9305,9329,9324,9333) THEN 'Abandono'
                            WHEN dsh.tidid1 IN (5220,9285,8113) OR dsh.tidid2 IN (6034,6043,6044,6048,6057,6058,6062,6071,6072,6076,6085,6086,6090,6099,6100,6104,6113,6114,6119,6127,6128,6957,6964,6975,6982,7451,7460,7461,8050,8059,8060,7880,9274,9289,9290,9295,9312,9309,9308,9331,9327,9328) THEN 'Interrupcao'
                        ELSE 'Outras'
                        END ";
