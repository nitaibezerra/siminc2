...
/**
 * Função que atriui Itens ao usuário.
 **/
function atribuirItensSelecionados($usucpf, $pflcod, $cocresp){
	...
	$db->commit();
        ...
        echo "
            <script>
                alert('Não foi possivél realizar a operação!');
		self.close();
            </script>
        ";
    }
...
/**
 * Função que busca o itens relacionados com o usuário
 **/
function buscaItensAtribuido($usucpf, $pflcod){
    ...
    if(is_array($RS)) {
        ...
        print '<option value="">Clique no Item para selecioná-lo.</option>';
    }
}

/**
 * Função que lista os Cocpits cadasstrados ao PDE
 **/
