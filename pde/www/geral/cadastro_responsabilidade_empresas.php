...
function listaEmpresas(){
	...
	$empresasExistentes = $db->carregar("SELECT distinct
										  ...
										  INNER JOIN pde.checklistentidade chk ON chk.entid = ent.entid
...
function atribuiEmpresas($usucpf, $pflcod, $empresa){
	...
	$db->executar("UPDATE pde.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."'");
	if ($empresa[0]){
		foreach($empresa as $tipo) {
			$dadosur = $db->carregar("SELECT * FROM pde.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND entid = '". $tipo ."'");
			if($dadosur) {
				// Se existir registro atualizar para ativo
				$db->carregar("UPDATE pde.usuarioresponsabilidade
   							   SET rpustatus = 'A', rpudata_inc= NOW()
 							   WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND entid = '". $tipo ."'");
			} else {
				// Se não existir, inserir novo
				$db->executar("INSERT INTO pde.usuarioresponsabilidade(
