...
function listagemPrincipal()
{
	...
	$sql = "select sub.subid, sub.subtitulo,meta.metfontemunicipio,meta.metfonteestado,meta.mettitulo from sase.submeta as sub left join sase.meta meta on (sub.metid = meta.metid) where sub.substatus = 'A' and sub.metid = ".$_REQUEST['metid']." order by sub.subordem ASC";
	$arr = $db->carregar($sql);
	...
	foreach($arr as $s):	
		...
		echo 
			"<tr>
				<td style=\"font-size:16px;padding:5px 0 5px 5px;font-weight:bold;color: #333;\">$subtitulo</td>
			</tr>			
			<tr>
				<td class=\"tabela_painel\">
					...
					<tr>
						...
						if ($_REQUEST['municipios']) {
							$where .= " OR (pne.muncod IN (" . str_replace("\'","",$municipios) . ") and pnetipo = 'M') ";
						}
						if ($_REQUEST['regioes']) {
                        ...
                        SELECT
                            ...
                            WHERE sub.subid = {$s['subid']}
								AND ((pne.estuf is null and pnetipo = 'E')$where)
							ORDER BY sub.subordem, pne.subid, pne.pneano, pnetipo, descricao
						";
						...
						if($dados)
						{
							...
							{
								$dadosAgrupados[str_replace('1Brasil', 'Brasil', tirar_acentos($dado['descricao']))][] = (float) $dado['pnevalor2'];
	...
	<tr>
		<td id="exibir-tabela" class="fundo_td">
			<span class="linkExibirTabela" style="cursor:pointer;" contador="<?php echo $contador; ?>">
				Exibir tabela
			</span>
			-
			<span class="nota" style="cursor:pointer;" onclick="abreNota(<?php echo $s['subid'];?>)"  id="subid<?php echo $s['subid'];?>" >				
				Exibir Nota Técnica
			</span>
		</td>		
	</tr>
		...
		?>
	<tr class="tabelaPne" id="tabelaPne_<?php echo $contador; ?>">
		<?php 
		echo "
		...
		";
				...
				$sql = "
						SELECT 
							'<td>'||round( pnevalor , $valor)||'</td>' AS pnevalor,
							'<td class=bold>'||pneano||'</td>' AS pneano,
							round( pnevalor , 2) AS pnevalorori, 
							pneano AS pneanoori 
						FROM sase.pne 
						WHERE estuf is null  AND subid =". $s['subid'] . " and pnetipo = 'E' 
						ORDER BY pneano
						";
				$arr = $db->carregar($sql);
		echo "
		...
		if ($mesoregioes && ($_REQUEST['metid'] != 14 
							&& $_REQUEST['metid'] != 17 
							&& $_REQUEST['metid'] != 13 
							&& $_REQUEST['metid'] != 7
							&& $_REQUEST['metid'] != 15
		...
		if ($municipios && ($_REQUEST['metid'] != 14
		&& $_REQUEST['metid'] != 17
		&& $_REQUEST['metid'] != 13
		&& $_REQUEST['metid'] != 7
		&& $_REQUEST['metid'] != 15
	...
	echo "
	<tr>
		<td>Fonte: Município e Mesoregião - {$s['metfontemunicipio']}</td>
	</tr>";
		}				
	}
	echo "
						...
						. '<table cellpadding="0" cellspacing="0" align="right" border="0">'
							.'<tr>'
								.'<td class="titulo_box" colspan="40" align="left">'
									.'Contribuição de estados e municípios para a meta nacional<br/><br/>'
								.'</td>'
								.'<td colspan="1">'
	...
	<script type="text/javascript">
		...
		{			
			...
			if(metaaba == 14 || metaaba == 17 || metaaba == 13 || metaaba == 11 || metaaba ==  12){
				alert("Não foi calculada a contribuição de mesorregiões e municípios para esta meta nacional.");				
				...
				if(metaaba == 7 ||  metaaba == 18 || metaaba==15 || metaaba == 19 || metaaba == 20)
				{
					$('#tabelaMesoregioes').css("display","none");
					$('#tabelaMunicipios').css("display","none");
...
<body>
	...
	<div style="width: 70%;margin:0 auto; padding: 10px 0 0 0;">
		<h2 id="h1contribuicao" style="display:none; margin:15px auto 0 auto; text-align: center;">Contribuição de estados e municípios para a meta nacional</h2>
	</div>
	<div class="container">
