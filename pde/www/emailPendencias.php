...
INNER JOIN pde.monitorametaentidade 	mme  ON mme.mnmid = mnm.mnmid
...
WHERE
	...
	AND 
		(
			dmi.dmidataexecucao BETWEEN 
			(
			SELECT
				CASE 
					WHEN to_char(now(),'DY') = 'MON' THEN now()::date
					WHEN to_char(now(),'DY') = 'TUE' THEN (now()::date-1)
					WHEN to_char(now(),'DY') = 'WED' THEN (now()::date-2)
					WHEN to_char(now(),'DY') = 'THU' THEN (now()::date-3)
					WHEN to_char(now(),'DY') = 'FRI' THEN (now()::date-4)
					WHEN to_char(now(),'DY') = 'SAT' THEN (now()::date+2)
					WHEN to_char(now(),'DY') = 'SUN' THEN (now()::date+1)
					ELSE now()::date
				END
			) AND
			(
