 ...
 *
 * @author Renã de Lima Barbosa <renebarbosa@mec.gov.br> 
 * @since 25/05/2007 
 */
...
include '_funcoes_tablets.php';
include '_componentes_tablets.php';
...
if($_REQUEST['dataInicio'] && $_REQUEST['idAtividade'] && $_REQUEST['desc']){
	$sql = "SELECT usucpf FROM pde.usuarioresponsabilidade WHERE atiid = {$_REQUEST['idAtividade']}";
	$usucpf = $db->pegaUm($sql);
	if(!($usucpf)){
		$sql = "SELECT usucpf FROM pde.usuarioresponsabilidade WHERE atiid = (SELECT x.atiidpai FROM pde.atividade x WHERE x.atiid = {$_REQUEST['idAtividade']})";
		$usucpf = $db->pegaUm($sql);
	}
	...
	if($usucpf){
		...
		if($email){
			...
			$conteudo = "
				...
				<br><br>
				<br>	
				<a href=\"http://simec.mec.gov.br/pde/tablets.php?modulo=principal/atividade_tablets/atividade&acao=A&atiid={$_REQUEST['idAtividade']}\">Clique Aqui para acessar a atividade.</a>
				<br>	
				<br>		
				Atenciosamente,
				<br>	
...
switch ( $_REQUEST['evento'] ) {
	...
	case 'arvore_busca_pai':
		$atiid = $_REQUEST['atiid'];
		$valor = $_REQUEST['valor'];
		
		$atiidPai = $db->pegaUm("select atiidpai from pde.atividade where atiid = $atiid");
		$totalFilhos = $db->pegaUm("select count(atiid) from pde.atividade a 
												where atiidpai = $atiidPai and a.atistatus = 'A'");
		
		$totalFilhosPerc = $db->pegaUm("select count(atiid) from pde.atividade a 
												where atiidpai = $atiidPai and (atiporcentoexec <> 100 or atiporcentoexec is null) and a.atistatus = 'A'");
		
		$totalPerc = $db->pegaUm( "select atiporcentoexec from pde.atividade where atiid = $atiidPai" );
		
		$valorPai = (float) $valor / (float) $totalFilhos;
		$valorPai = round($valorPai) + $totalPerc;
		$valorPai = round( $valorPai );
		
		#arredondando os valor inteiro
		$valorPai = $valorPai / 10;
		$valorPai = round( $valorPai ) * 10;
		
		echo $atiidPai.'_'.$valorPai.'_'.$totalFilhosPerc;
		exit();
	case 'arvore_ocultar':
...
where atiid = %d",
	...
	case atualizar_atividade:
		if($_REQUEST['atiid1']&&$_REQUEST['atiid2']){
			$sql = "UPDATE workflow.documento d set 
					...
					INNER JOIN pde.atividade 	 a ON a.atiid = i.atiid AND a.atiid IN (".$_REQUEST['atiid1'].",".$_REQUEST['atiid2'].")
					) a
					WHERE 
						a.docid = d.docid ";
			$db->executar($sql);
