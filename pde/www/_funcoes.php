...
function calcular_porcentagem_atividade_pai( $atividade ){
	global $db;
	...
	// pega idpai da atividade
	$atividade = (integer) $atividade;
	$sql = "select atiidpai from pde.atividade where atiid = " . $atividade;
	$pai = $db->pegaUm( $sql );

	// pega status do pai
	$sql = "select esaid from pde.atividade where atiid = " . $pai . " and atistatus = 'A'";
	$statusPai = $db->pegaUm( $sql );
	
	// pega total dos filhos
	$sql = "select count(atiid) from pde.atividade where atiidpai = " . $pai . " and atistatus = 'A'";
	$totalFilhos = $db->pegaUm( $sql );
	
	// pega soma da porcentagem dos filhos
	$sql = "select sum(atiporcentoexec) from pde.atividade where atiidpai = " . $pai . " and atistatus = 'A'";
	$somaProcentoFilhos = $db->pegaUm( $sql );
	
	
	// pega total dos filhos cancelados
	$sql = "select count(atiid) from pde.atividade where esaid = 4 and atiidpai = " . $pai . " and atistatus = 'A'";
	$totalFilhosCancelados = $db->pegaUm( $sql );
	
	// pega soma da porcentagem dos filhos cancelados
	$sql = "select sum(atiporcentoexec) from pde.atividade where esaid = 4 and atiidpai = " . $pai . " and atistatus = 'A'";
	$somaProcentoFilhosCancelados = $db->pegaUm( $sql );
	
	
	// atualiza porcentagem pai
	if($totalFilhos > 0){
		
		$somaProcentoFilhosGeral = $somaProcentoFilhos - $somaProcentoFilhosCancelados;
		$totalFilhosGeral = $totalFilhos - $totalFilhosCancelados;
		
		if($totalFilhosGeral > 0){
			$calcProcentoPai = ($somaProcentoFilhosGeral / $totalFilhosGeral);
		}
		else{
			$calcProcentoPai = 0;
		}
		
		//atualiza pai para em andamento
		if($calcProcentoPai > 0 && $calcProcentoPai < 100) $andStatus = ", esaid = 2"; 
		
		//atualiza pai para concluido
		if($calcProcentoPai >= 100) {
			$andStatus = ", esaid = 5";
			$calcProcentoPai = 100;
		}

		//atualiza pai para não iniciado
		if($calcProcentoPai <= 0) {
			$andStatus = ", esaid = 1";
			$calcProcentoPai = 0;
		}
		
		//atualiza pai para não iniciado
		//verifica se todos os filhos estao não iniciados
		$sql = "select count(atiid) from pde.atividade where atiidpai = " . $pai . " and esaid = 1 and atistatus = 'A'";
		$totalFilhosNaoIniciados = $db->pegaUm( $sql );
		if($totalFilhosNaoIniciados == $totalFilhos) {
			$andStatus = ", esaid = 1";	
			$calcProcentoPai = 0;	
		}
		
		//atualiza pai para suspenso
		//verifica se todos os filhos estao suspensos
		$sql = "select count(atiid) from pde.atividade where atiidpai = " . $pai . " and esaid = 3 and atistatus = 'A'";
		$totalFilhosSuspensos = $db->pegaUm( $sql );
		if($totalFilhosSuspensos == $totalFilhos) $andStatus = ", esaid = 3";
		
		//atualiza pai para cancelado
		//verifica se todos os filhos estao cancelados
		$sql = "select count(atiid) from pde.atividade where atiidpai = " . $pai . " and esaid = 4 and atistatus = 'A'";
		$totalFilhosCancelados = $db->pegaUm( $sql );
		if($totalFilhosCancelados == $totalFilhos) $andStatus = ", esaid = 4";		
				
		$sql = "update pde.atividade set atiporcentoexec = $calcProcentoPai	
				$andStatus
				where atiid = $pai";
		//echo $sql;
		//die;
		...
		//verifica se existe o pai do pai
		$atividade = (integer) $atividade;
		$sql = "select atiidpai from pde.atividade where atiid = $pai";
		$atiidpai = $db->pegaUm( $sql );
	
		if($atiidpai){
			calcular_porcentagem_atividade_pai( $pai );
		}
		else{
			return true;
		}
