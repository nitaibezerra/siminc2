...
if($_REQUEST['tipoMapasAntigo2014']){

	/* INÍCIO - Recupera os parametros via $_GET e cria variáveis com os nomes e valores */
	$parametros = "tipoMapasAntigo2014=".$_REQUEST['tipoMapasAntigo2014'];
		...
		default:
			$sql = "select
					...
					from (
							select
								mun.muncod,
								".str_replace("_mais_","+",$parametro)."  as parametro
							from
								painel.dadoproinfancia_2014 pro
							inner join
								territoriosgeo.municipio mun ON mun.muncod = pro.muncod
							where
								".str_replace("_mais_","+",$parametro)." >= 0
						  ) as foo
					group by
						parametro
					order by
		...
		foreach($arrSteps as $i => $range){
			if($range['inicio'] >= 0 && $range['fim'] >= 0){
				if($range['inicio'] == $range['fim']){
					$arrInicio[] = $range['inicio'];
					$arrFim[] = $range['fim'];
					$arrLegendas[] = "".(strstr($parametro,"percent") ?number_format($range['inicio'],2,',','.')."%" : number_format($range['inicio'],0,'','.'));
				}else{
					$arrInicio[] = $range['inicio'];
					$arrFim[] = $range['fim'];
					$arrLegendas[] = "".(strstr($parametro,"percent") ?number_format($range['inicio'],2,',','.')."%" : number_format($range['inicio'],0,'','.'))." - ".(strstr($parametro,"percent") ?number_format($range['fim'],2,',','.')."%" : number_format($range['fim'],0,'','.'));
				}
			}
		}
	}
	...
	if($estuf){
		$arrEstados = explode(",",str_replace("estuf","",$estuf));
	...
	if($dados){
	foreach($dados as $d){
	foreach($arrInicio as $i => $ini){
	if($d['parametro'] >= $arrInicio[$i] && $d['parametro'] <= $arrFim[$i]){
			$arrDadosI[$i][] = $d['parametro'];
			break;
				}
	}
	}
	...
	for($i=0;$i<5;$i++){
	$arrValores[] = count($arrDadosI[$i]);
	}

	//$max = getMaxValue($arrValores);
	$novoArrCores = explode(",",$cor);
	unset($novoArrCores[0]);

	foreach($novoArrCores as $corr)
	{
	if(strlen($corr) < 6){
	$arrNCor[] = "#".$corr[0].$corr[0].$corr[1].$corr[1].$corr[2].$corr[2];
	}else{
	$arrNCor[] = "#".$corr;
	}
}
//dbg($novoArrCores);
	...
	$pie->set_tooltip( '#val# Municipios de #total#<br>#percent# de 100%' );
	$pie->set_colours( $arrNCor );
	$pie->set_values( $arrValores );
	...
	$eixo_y = new y_axis();
	//Range do Eixo Y
	$range = count($arrValores) == 0 ? 1 : ($max / 4);
	...
	$bar = new bar_filled( $arrCores[$parametro] );
	$bar->set_values( $arrValores );
	...
	$x_labels->set_labels( $arrLegendas );
	//rotaciona o eixo x
	$x_labels->rotate(30);
...
if($_REQUEST['tipoMapasAntigo']){
	
	/* INÍCIO - Recupera os parametros via $_GET e cria variáveis com os nomes e valores */
	$parametros = "tipoMapasAntigo=".$_REQUEST['tipoMapasAntigo'];
	$arrPar = explode(";",$parametros);
	foreach($arrPar as $dado){
		$d = explode("=",$dado);
		$arrparametros[ $d[0] ] = $d[1]; 
...
if($_REQUEST['tipoMapas']){
	
	/* INÍCIO - Recupera os parametros via $_GET e cria variáveis com os nomes e valores */
	$parametros = "tipoMapas=".$_REQUEST['tipoMapas'];
	$arrPar = explode(";",$parametros);
	foreach($arrPar as $dado){
		$d = explode("=",$dado);
		$arrparametros[ $d[0] ] = $d[1]; 
	...
	if($tpdid == 6){
		...
		foreach($arrResultadoFinal as $valor){
			$arrRepeticoes[$valor][] = $valor;
		}
		foreach($arrRepeticoes as $chave => $valor){
			$arrDados[] = array('parametro' => $chave, 'qtde' => count($valor));
			$total_mun+=count($valor);
		}
		
		//Pega o total de municípios
		$sql = "select count(distinct muncod) from territorios.municipio";
		$mun = $db->pegaUm($sql);
		array_unshift($arrDados,array('parametro' => 0, 'qtde' => $mun-$total_mun));
		
		if($arrResultadoFinal){
			foreach($arrResultadoFinal as $muncod => $valor){
				$arrDados[] = array("muncod" => $muncod,"parametro" => $valor, "itemcor" => $tmacor, "tipoparametro" => $parametro);
				$arrMuncod[] = $muncod;
			}
		}
		...
		if($_GET['arrInicio']){
			unset($arrSteps);
			$arrGetInicio = explode(",",$_GET['arrInicio']);
			foreach($arrGetInicio as $inicio){
				$arrSteps[]['inicio'] = $inicio;
			}
		}
		
		if($_GET['arrFim']){
			$arrGetFim = explode(",",$_GET['arrFim']);
			foreach($arrGetFim as $chave => $fim){
				$arrSteps[$chave]['fim'] = $fim;
			}
		}
		...
		if($arrDados){
			...
			foreach($arrDados as $linha => $dado){
				if(count($arrSteps) < ($valorChave/$numLegenda)){//apenas 5 legendas
					if($dado['qtde'] >= $valorChave){//se a qtde for maior que a chave
						$arrSteps[] = array("inicio" => $dado['parametro'], "fim" => $dado['parametro']);
						(float)$valorChave = ($mun+$total_mun)-$dado['qtde']; //total sem o número
					}else{ //se não for maior que a chave
						$arrLeg[] = $dado['parametro']; //grava o parametro
						$num+=(float)$dado['qtde']; //soma o numero de qtde
						if($numLegenda-count($arrSteps)> 0 && $num >= ($valorChave/($numLegenda-count($arrSteps))) ){ //se o numero for maior que a chave
							$arrSteps[] = array("inicio" => $arrLeg[0], "fim" => $arrLeg[count($arrLeg)-1]); //atribui o inicio e o fim da legenda
							unset($arrLeg); //reinicia o array de legendas
						}	
					}
			...
			foreach($arrSteps as $i => $range){
				if($range['inicio'] >= 0 && $range['fim'] >= 0){
					if($range['inicio'] == $range['fim']){
						$arrInicio[] = $range['inicio'];
						$arrFim[] = $range['fim'];
						$arrLegendas[] = "".($percent ?number_format($range['inicio'],2,',','.')."%" : number_format($range['inicio'],0,'','.'));
					}else{
						$arrInicio[] = $range['inicio'];
						$arrFim[] = $range['fim'];
						$arrLegendas[] = "".($percent ?number_format($range['inicio'],2,',','.')."%" : number_format($range['inicio'],0,'','.'))." - ".($percent ?number_format($range['fim'],2,',','.')."%" : number_format($range['fim'],0,'','.'));	
					}
				}
			}
		}
		...
		foreach($arrM as $m){
			$arrDados[] = array("muncod" => $m,"parametro" => 0, "itemcor" => $tmacor, "tipoparametro" => $parametro);
		}
		$dados = $arrDados;
		...
		if($camada != "municipio" && strstr($camada,"tipmunici_")){ //Tipo de Município
			...
			$campo = "distinct tpm.tpmid as muncod";
			...
			$campo = "distinct mun.muncod as muncod";
			$campoQtde = "$tpdcampotema";
		}
		
		//Verifica se existe legenda do tema
		$sql = "select * from mapa.legendatema where mapid = $mapid and tmaid = $parametro order by lgtposicao";
		$arrLegTema = $db->carregar($sql);
		if(!$arrLegTema){
			$sql = "select * from mapa.legendatema where tmaid = $parametro and mapid is null order by lgtposicao";
			$arrLegTema = $db->carregar($sql);
		}
		...
		if($arrLegTema){
			...
			$sql = "select 
					...
					from (
							...
							left join
								mapa.temadado pro ON mun.muncod = pro.muncod $whereCampo and pro.tmaid = $parametro
		...
		$arrD = $db->pegaLinha("select tpdid,tmacor from mapa.tema where tmaid = $parametro");
		...
		$sql = "select 
					$campo,
					COALESCE($campoQtde,$coalesce) as parametro,
					COALESCE(tmacor,(SELECT tmacor from mapa.tema where tmaid = $parametro) ) as itemcor,
					'$parametro' as tipoparametro 
				from 
					territorios.municipio mun
				".($arrInnerJoin ? " inner join ".implode(" inner join ",$arrInnerJoin) : "")." 
				left join
					mapa.temadado  tem ON tem.muncod = mun.muncod and tmaid = $parametro
				left join
	...
	if($dados){
		...
		if($tpdcampotema == "tmdboleano"){
			$arrInicio = array(0 => "t", 1 => "f");
			$arrFim = array(0 => "t", 1 => "f");
		}
	...
	$ToLeg = count($novoArrCores) == 0 ? 1 : count($novoArrCores);
	
	unset($arrValores);
	unset($arrCoresFinal);
	
	for($i=0;$i<$ToLeg;$i++){
		if(count($arrDadosI[$i]) > 0){
			$arrValores[] = count($arrDadosI[$i]);
			$arrCoresFinal[] = $arrNCor[$i];
		}
	}
...
//			dbg($arrMetasvalorIndicador);
			...
			if($finac_valor){
				...
				$line_meta_valor2->set_colour($projetar == "nao" ? "#FFFFFF" : "#A52A2A"); //indica a cor
			...
			$line_meta_valor->set_colour($projetar == "nao" ? "#FFFFFF" : "#FF0000"); //indica a cor
