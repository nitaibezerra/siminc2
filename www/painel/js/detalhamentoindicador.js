...
function exibeGraficoPorIndicador(indid,acaid,reg,detalhe,valorDetalhe,tipografico,dpeid_inicio,dpeid_fim,estuf,muncod,indqtdevalor){
	if(tipografico){
		...
		if(tipografico == "barra"){
			...
			geraGrafico = "/painel/geraGrafico.php?tipo="  			 + tipografico     + 
										 ";indid=" 			 + indid 		   + 
										 ";dpeid=" 			 + dpeid_inicio    +
										 ";dpeid2=" 		 + dpeid_fim 	   +
		...
		if(tipografico != "linha" && tipografico != "barra"){
			...
			if(tpGrafico == "barra"){
				...
				geraGrafico = "/painel/geraGrafico.php?tipo=barra_fatia"	           		  + 
										 ";indid=" 			 + indid 		   		  + 
										 ";sehid=" 			 + sehid 		   		  +
			...
			if(tpGrafico == "pizza"){
				...
				geraGrafico = "/painel/geraGrafico.php?tipo="        + tpGrafico			  +
										 ";indid=" 			 + indid 		   		  + 
										 ";sehid=" 			 + sehid 		   		  +
										 ";tidid=tdiid"		 + fatia + "_" +  idFatia +
										 ";dpeid=" 			 + dpeid_inicio    		  +
			...
			if(tpGrafico == "barracomp"){
				...
				geraGrafico = "/painel/geraGrafico.php?tipo=barra_comp"		           		  + 
										 ";indid=" 			 + indid 		   		  + 
										 ";sehid=" 			 + sehid 		   		  +
										 ";tidid=tdiid"		 + fatia + "_" +  idFatia +
										 ";dpeid=" 			 + dpeid_inicio    		  +
		...
		if(tipografico != "linha" && tipografico != "barra" && jQuery("#hdn_tipo_grafico").val() != "highcharts"){
			var myAjax = new Ajax.Request(
						'/painel/painel.php?modulo=principal/painel_controle&acao=A',
						{
							method: 'post',
		...
		if(jQuery("#hdn_tipo_grafico").val() == "highcharts"){
			geraGraficoHighChartsPorDiv("grafico_indicador_"+indid,indid,geraGrafico)
		}else{
...
function geraGraficoHighChartsPorDiv(div,indid,dados)
{
	dados = dados.replace("/painel/geraGrafico.php?","");
	document.getElementById(div).innerHTML = "<center>Carregando...</center>";
	
	jQuery.ajax({
	  type: "POST",
	  url: 'painel.php?modulo=principal/painel_controle&acao=A',
	  cache: false,
	  data: "&geraGraficoJpGraph=true&dados=" + dados + "&div=" + div + indid,
	  success: function(msg){
	   		 jQuery("#" + div ).html(msg);
	   		 jQuery("#" + div ).find("script").each(function(i) {
