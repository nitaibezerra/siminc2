...
$mensagem->FromName		= SIGLA_SISTEMA. " Painel - Atualização dos dados";
				...
				if($resp[0]) {
					...
					foreach($resp as $r) {
						...
						if(strlen($r['telefone']) == 10) {
							$envio = $client->call('sendMessage', array('user' => 'inep', 'password' => 'tmmjee', 'testMode' => false, 'sender' => SIGLA_SISTEMA, 'target' => '55'.$r['telefone'], 'body' => 'Existem '.count($at).' indicadores da secretaria '.$r['secdsc'].' sob sua responsabilidade desatualizados, acesse simec.mec.gov.br e proceda a atualização. MEC', 'ID' => substr($r['respid'],0,6).date("Ymdhis")));
						}
					}
