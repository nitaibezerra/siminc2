...
function geraXML2($indid,$sehid,$muncod = null){
	...
	$sql = "select
			...
			switch($unmdesc){
				case "Número inteiro":
					$SQLsehqtde = "coalesce(dshqtde,0)::bigint as dshqtde ";
					break;
				case "Percentual":
...
function buscaMunicipioAjax2($dsc){
	...
	if($_POST['camada'] == "municipio" || !$_POST['camada']){
		...
		if($mun){
				$sqlMUN = "where
	        				removeacento(mun.mundescricao) ilike removeacento('%".utf8_decode($mun)."%')";
		}
		...
		if(!$uf){
				...
				$sqlUF = "and
	        				removeacento(mun.estuf) like removeacento('%".strtoupper(utf8_decode($uf))."%')";
		}
        ...
        echo "</fieldset></div>";
			...
			$gtmid = str_replace("tipmunici_","",$_POST['camada']);
			$sql = "select gtmdsc from territoriosgeo.grupotipomunicipio where gtmid = $gtmid";
			$gtmdsc = $db->pegaUm($sql);
			$sql = "select 
						tpmid,
						tpmdsc,
						( 
							select 
					            ...
					            --############### FIM LONGITUDE ###################### -- 
							from
								territoriosgeo.muntipomunicipio mtm 
							inner join
								territoriosgeo.municipio mun ON mun.muncod = mtm.muncod
							where
								mtm.tpmid = tpm1.tpmid
							limit 1 
						) as longitude,
						( 
							select 
					...
					from
						territoriosgeo.tipomunicipio tpm1
					where
						gtmid = $gtmid
					and
						removeacento(upper(tpmdsc)) like removeacento(upper('%$dsc%'))";
			$tpmdsc = $db->carregar($sql);
			...
			echo "<fieldset><legend>".$gtmdsc."</legend>";
	        	foreach($tpmdsc as $tpm){
	        		echo "<span style=\"cursor:pointer\" onclick=\"localizaTipoMunicipio('{$tpm['tpmid']}',{$tpm['longitude']},{$tpm['latitude']})\" >".$tpm['tpmdsc']."</span><br>";
	        	}
	        echo "</fieldset>";
		}
	exit;
