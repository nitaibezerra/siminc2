	...
	switch($tabela) {
		...
		case 'entidade.campusprofissional':
			$sql = "select 
					...
					where fen.funid=17 and fen2.funid=11 AND ent.entstatus='A'
					order by 
		...
		case 'entidade.campussuperior':
			$sql = "select 
					...
					inner join entidade.funcaoentidade fen2 on fen2.entid = ent2.entid   
					inner join entidade.endereco ende on ende.entid = ent.entid 
					where fen.funid=18 and fen2.funid=12 AND ent.entstatus='A'
					order by 
						ent2.entsig, descricao";
			break;
		case 'entidade.hospitais':
			$sql = "SELECT 
					...
					INNER JOIN entidade.entidade ent2 ON ent2.entid = fea.entid 
					INNER JOIN entidade.funcaoentidade fen2 ON fen2.entid = ent2.entid 
					INNER JOIN rehuf.estruturaunidade esu ON esu.entid = ent.entid
					WHERE fen.funid='16' AND (fen2.funid='12' OR fen2.funid='16') AND ent.entstatus='A'
		...
		case 'entidade.instituto':
			$sql = "select distinct unicod as codigo, unidsc as descricao, ende.estuf as uf from public.unidade uni
					inner join entidade.entidade ent on ent.entunicod=uni.unicod 
					inner join entidade.endereco ende on ende.entid=ent.entid   
					where gunid=2 and unistatus='A' AND ent.entstatus='A'
					order by codigo, descricao";
			break;
		case 'painel.ies':
			$sql = "select 
    			...
    			switch($value['funcaovalidacao']) {
    				...
    				case 'validarcodunidade':
    					...
    					$uni = $db->pegaLinha("SELECT muncod, estuf FROM entidade.entidade ent 
    										   ...
    										   WHERE entunicod='".trim($dadoscsv[$key])."' AND ent.entstatus='A'");
    			...
    			switch($value['funcaovalidacao']) {
    				...
    				case 'validarcodentidade':
    					...
    					$ent = $db->pegaLinha("SELECT ende.muncod, ende.estuf, ent.entunicod FROM entidade.endereco ende
    										   ...
    										   WHERE ende.entid='".trim($dadoscsv[$key])."' AND fen.funid IN(17,18) AND ent.entstatus='A'");
...
function inserircargaautomatica($dados) {
	...
	$sql = "INSERT INTO painel.cargaautomatica(
            indid, causql, cautipoproc, caudiasem, caudiames, cauindicador)
    		VALUES ('".$dados['indid']."', '".addslashes($dados['causql'])."', '".$dados['cautipoproc']."', ".(($dados['caudiasem'])?"'".implode(";", $dados['caudiasem'])."'":"NULL").", ".(($dados['caudiames'])?"'".$dados['caudiames']."'":"NULL").", ".$dados['cauindicador'].");";
...
function atualizarcargaautomatica($dados) {
	...
	$sql = "UPDATE painel.cargaautomatica SET
            causql = '".addslashes($dados['causql'])."',
            ...
            cauindicador = ".$dados['cauindicador']."
