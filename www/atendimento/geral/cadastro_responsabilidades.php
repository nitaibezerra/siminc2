...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM atendimento.tprperfil p
							   INNER JOIN atendimento.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "A": // Unidades 
				...
				$sqlRespUsuario = "SELECT 
									DISTINCT 
									a.agdid AS codigo, 
									a.agddescricao AS descricao, 
									ur.rpustatus AS status
								   FROM
								    atendimento.usuarioresponsabilidade ur 
								    INNER JOIN atendimento.agenda a ON a.agdid = ur.agdid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM atendimento.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
