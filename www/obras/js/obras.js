...
function obrFiltraLista(){
	document.getElementById( 'requisicao' ).value = "filtrar";	
...
function obrVerXls(){
	document.getElementById( 'requisicao' ).value = "xls";
	document.getElementById( 'obrFormPesquisa' ).submit();
}
...
function verPagamento( preid ){
	window.open('obras.php?modulo=principal/popup/pagamento&acao=A&preid=' + preid,'pagamento','width=200,height=150')
}
...
function visualizaResponsavel(entid){
	var index = window.document.getElementById('linha_'+entid).rowIndex;
	return windowOpen( caminho_atual + '?modulo=principal/inserir_responsavel&acao=A&busca=entnumcpfcnpj&entid=' + entid + '&tr=' + index + '&habilita=N','blank','height=700,width=600,status=yes,toolbar=no,menubar=no,scrollbars=yes,location=no,resizable=yes' )	;
}
...
function adicionarFases( objeto ){
	return windowOpen( caminho_atual + '?modulo=principal/fases_licitacao&acao=A&objeto='+objeto,'blank','height=450,width=650,status=yes,toolbar=no,menubar=no,scrollbars=yes,location=no,resizable=yes');
}

function atualizaFase(id,flcid){
	return windowOpen( caminho_atual + '?modulo=principal/fases_licitacao&acao=A&tflid=' +id+'&flcid='+flcid,'blank','height=450,width=650,status=yes,toolbar=no,menubar=no,scrollbars=yes,location=no,resizable=yes');
}
...
function Validacao( superuser ){
	...
	if(orgao == 3){
		var moeid = document.formulario.moeid.value;
		if (moeid == '' && !superuser ){
...
// }
	...
	var requisicao = document.formulario.requisicao.value; 
	
	var supvdtmax = document.formulario.supvdtmax.value;
	
	/*
	if ( supvdtmax && supvdt && requisicao == 'cadastra' ){
		maxano = supvdtmax.substr(0,4);
		maxmes = supvdtmax.substr(5,2);
		maxdia = supvdtmax.substr(8,2);
		supvdtmax2 = maxano + maxmes + maxdia;
		supvdt2 = supvdt.substr(6,4) + supvdt.substr(3,2) + supvdt.substr(0,2);
		
		if( parseFloat(supvdtmax2) > parseFloat(supvdt2) ){
			mensagem += 'Data da Vistoria deve ser maior que a data da última vistoria ('+maxdia+'/'+maxmes+'/'+maxano+').\n';
			validacao = false;
		}
	}
	*/
...
function ajaxTipologia( tpoid )
{
	ajaxPrograma( tpoid );
	ajaxModalidade( tpoid );
	ajaxTipoObra( tpoid );
	ajaxFonte( tpoid );
	ajaxDescricao( tpoid );
}

function ajaxPrograma( tpoid )
{
	var url = caminho_atual + '?modulo=principal/cadastro&acao=A&ajaxCarregaPrograma=1&tpoid='+tpoid+'&orgid='+$('orgid').value;
...
function ajaxModalidade( tpoid )
{
	var url = caminho_atual + '?modulo=principal/cadastro&acao=A&ajaxCarregaModalidade=1&tpoid='+tpoid;
...
function ajaxTipoObra( tpoid )
{
	var url = caminho_atual + '?modulo=principal/cadastro&acao=A&ajaxCarregaTipoObra=1&tpoid='+tpoid;
...
function ajaxFonte( tpoid )
{
	var url = caminho_atual + '?modulo=principal/cadastro&acao=A&ajaxCarregaFonte=1&tpoid='+tpoid;
...
function ajaxDescricao( tpoid )
{
	var url = caminho_atual + '?modulo=principal/cadastro&acao=A&ajaxCarregaDescricao=1&tpoid='+tpoid;
...
function mostraDescricaoTipologia(){
	if ( $('tpoid') ) {
		if( $('tpoid').value ) {
			...
			var i = document.formulario.tpoid.selectedIndex;  
     		$('obrcomposicao').value = document.formulario.tpoid[i].text;
...
function criaObraNova( obrid ){
	...
	if ( confirm('Será criada uma cópia da Obra: ( '+obrid+' ), deseja realmente continuar?') ){
...
function obrPesquisaListaObras(){
	...
	var fntid   		   = document.getElementById( 'fntid' );
	...
	*/
	...
	if ( supervisao != '' || vlrmenor.value != '' || vlrmaior.value != '' || entidunidade.value != '' || estado.value != '' || repdtlimiteinicial.value != '' || repdtlimitefinal.value != '' || tobaid.value != '' || stoid.value != '' || cloid.value != '' || prfid.value != '' /*|| entidunidade2.value != ''*/ || obrtextobusca.value != '' || foto/*.value*/ != '' || vistoria/*.value*/ != '' || restricao/*.value*/ != '' || planointerno/*.value*/ != '' || aditivo/*.value*/ != '' || percentualinicial.value != '' || percentualfinal.value != '' ){
		...
		$.ajax({
				...
				async: true,
				cache: false,
				data: '&requisicao=lista&entidunidade=' + entidunidade.value + '&estuf=' + estado.value + '&repdtlimiteinicial=' + repdtlimiteinicial.value + '&repdtlimitefinal=' + repdtlimitefinal.value + '&tobaid=' + tobaid.value + '&stoid=' + stoid.value + '&cloid=' + cloid.value + '&prfid=' + prfid.value /*+ '&entidunidade2=' + entidunidade2.value*/ + '&obrtextobusca=' + obrtextobusca.value + '&foto=' + foto/*.value*/ + '&vistoria=' + vistoria/*.value*/ + '&restricao=' + restricao/*.value*/ + '&planointerno=' + planointerno/*.value*/ + '&aditivo=' + aditivo/*.value*/ + '&percentualinicial=' + percentualinicial.value + '&percentualfinal=' + percentualfinal.value + '&supervisao=' + supervisao + '&vlrmenor=' + vlrmenor + '&vlrmaior=' + vlrmaior + '&fntid=' + fntid.value,
				dataType: "html",
				success: function(msg){
...
function obrValidaGrupo(){
	...
	if( document.getElementById( 'estuf_h' ).value == '' ){
...
function carregaEmpresaGrupo(estuf){	
	$.getJSON("?modulo=principal/supervisao/criarLote&acao=A&requisicao2=carregarempresa&estuf=" + estuf, function(json) {
	   $('#empresalote').html(json.entnome);
	   $('#estuflote').html(json.estuf);
	 });
}
	...
	$.ajax({
		...
		$('#listaObrasRepositorioLote').html(msg);
			 ...
			 /*if( estuf && obrItensLote.length == 0 ){
			 	document.getElementById("estuf_h").value = estuf;
			 }*/
...
function obrIncluiObraNoLote( obrid, repid, nomeObra, municipio, area, unidademedida, unidade, situacao, uf, tipoensino, convenio, percentual ){
	...
	if( document.getElementById( "repid_" + repid ).checked == true ){
		...
		document.getElementById("estuf_h").value = uf;
		...
		var colPrior     = linha.insertCell(2);
		var colArea      = linha.insertCell(3);
		var colMunicipio = linha.insertCell(4);
		var colUnidade   = linha.insertCell(5);
		var colTpEnsino  = linha.insertCell(6);
		var colSituacao  = linha.insertCell(7);
		var colPercen    = linha.insertCell(8);
		
		// var colDtInicio = linha.insertCell(5);
		// var colDtFinal = linha.insertCell(6);
		
		colProce.style.textAlign   = "center";
		colPrior.style.textAlign   = "center";
		...
		if( obrItensLote.length == 0 ){
			document.getElementById("estuf_h").value = null;
		}
...
function excluirGrupoDistribuicao( repid, itgid ){
	if(confirm('Tem certeza que deseja excluir este registro?')){
		janela('?modulo=principal/supervisao/verDados&acao=A&id=' + repid + '&itgid='+itgid+'&tipo=excluir', 600, 550, 'verDados');
	}
}
...
function verificaTipoEnsino(){
	...
	var boEduBasica = false;
	
	for(i=0; i<form.length; i++){		
		if(form.elements[i].type == "checkbox" && form.elements[i].id == 'orgid' ){
			if(form.elements[i].checked && form.elements[i].value == 3){
				boEduBasica = true;
			}
		}
	}
	if( boEduBasica ){
		//document.getElementById('percentualinicial').value = '90';
		document.getElementById('vistoriainseridaunidade').style.display = '';			
	} else {
		//document.getElementById('percentualinicial').value = '0';
		document.getElementById('vistoriainseridaunidade').style.display = 'none';
	}
}

function verificaVistoriaInserida(formulario){
	var form = document.getElementById(formulario);
	var boEduBasica = false;
	
	for(i=0; i<form.length; i++){		
		if(form.elements[i].type == "radio" ){
			if(form.elements[i].checked && (form.elements[i].id == 'vistoriainseridasim' || form.elements[i].id == 'vistoriainseridanao') ){
				boEduBasica = true;
			}
		}
	}
