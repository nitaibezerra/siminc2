...
function lista(){
	...
	if( $db->testa_superuser() || in_array(665,$pflcods) ){
		...
		$sql = "SELECT 		curid AS codigo, 
							curdesc AS descricao
				FROM 		catalogocurso2014.curso
				WHERE		curstatus = 'A' AND curano = {$_SESSION['exercicio']}";
		
		$reg = $db->carregar($sql);
	} else {
		$sql = "SELECT 		DISTINCT coordid 
				FROM 		catalogocurso2014.usuarioresponsabilidade
				WHERE		usucpf = '{$_SESSION['usucpf']}' AND pflcod = 651 AND coordid IS NOT NULL AND rpustatus = 'A'";
...
function atribui($usucpf, $pflcod, $resp){
	...
	$db->executar("UPDATE catalogocurso2014.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '{$usucpf}' AND pflcod = '{$pflcod}' AND curid IS NOT NULL");
	if ($resp[0]){
		foreach($resp as $tipo) {
			$dadosur = $db->carregar("SELECT * FROM catalogocurso2014.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = '{$pflcod}' AND curid = '{$tipo}'");
			if($dadosur) {
				...
				$db->executar("INSERT INTO catalogocurso2014.usuarioresponsabilidade(pflcod, usucpf, curid, rpustatus, rpudata_inc)
...
function buscaAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT 		c.curid AS codigo, 
						curdesc AS descricao
			FROM 		catalogocurso2014.curso c
			INNER JOIN 	catalogocurso2014.usuarioresponsabilidade u ON u.curid = c.curid
			WHERE		curstatus = 'A'	AND u.rpustatus = 'A' AND u.usucpf = '{$usucpf}' AND u.pflcod = {$pflcod} AND c.coorano = {$_SESSION['exercicio']}";
