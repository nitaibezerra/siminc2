...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM escolasexterior.tprperfil p
	INNER JOIN escolasexterior.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "E": // Escola
				...
				$sqlRespUsuario = "SELECT 
										ee.eexid as codigo,
										eexnomeestabelecimento as descricao
									FROM
										escolasexterior.escolasexterior ee
									INNER JOIN
										escolasexterior.usuarioresponsabilidade eu ON eu.eexid = ee.eexid
									 WHERE 
								    eu.usucpf = '%s' AND 
								    eu.pflcod = '%s' AND 
								    eu.rpustatus = 'A'";
			...
			case "P": // País
				...
				$sqlRespUsuario = "SELECT 
									...
									FROM
										territorios.pais pa
									INNER JOIN
										escolasexterior.usuarioresponsabilidade eu ON eu.paiid = pa.paiid
									 WHERE 
								    eu.usucpf = '%s' AND 
								    eu.pflcod = '%s' AND 
								    eu.rpustatus = 'A'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM escolasexterior.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
