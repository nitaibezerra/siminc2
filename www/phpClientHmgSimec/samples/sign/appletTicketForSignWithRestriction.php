<?php
	if (isset($_POST['submit'])) {
		...
		try {
			msgOutput("TESTE DE ASSINATURA - TIQUETE DE APPLET PARA ASSINATURA DE DOCUMENTO COM RESTRICAO GERADO POR USUARIO");
			...
			msgOutput("requisitando tiquete...");
			$cpf = $_POST['cpf'];
			$marker = $_POST['mark'];
			$resposta = $SSDWs->getAppletTicketInfoForUserDocSigningWithRestriction($cpf , $marker);
...
?>
	<h3>TESTE DE ASSINATURA - TIQUETE DE APPLET PARA ASSINATURA DE DOCUMENTO COM RESTRICAO GERADO POR USUARIO</h3>
	<form method="POST">
		<label>Flag:</label> <input type="text" name="mark" /><br />
