<?php
	if (isset($_POST['submit'])) {
		...
		try {
			msgOutput("TESTE DE USUARIO - ALTERAR PERMISSAO PARA USUARIO");
			...
			$resposta = $SSDWs->changeUserPermission($userId, $oldPermissionId, $newPermissionId, $justificativa);
...
?>
	<h3>TESTE DE USUARIO - ALTERAR PERMISSAO PARA USUARIO</h3>
	<form method="POST">
		<label>ID do Usu&aacute;rio:</label> <input type="text" name="userId" value="1404"/><br />
		<label>ID da Permiss&atilde;o Antiga:</label> <input type="text" name="oldPermissionId" value="14" /><br />
		<label>ID da Permiss&atilde;o Nova:</label> <input type="text" name="newPermissionId" value="19" /><br />
		<label>Justificativa:</label> <input type="text" name="justificativa" value="justificativa" /><br />
