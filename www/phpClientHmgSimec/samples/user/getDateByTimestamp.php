<?php
	if (isset($_REQUEST['timestampCreation'])) {
		...
		try {
			...
			$timestampCreation = (Integer) ($_POST["timestampCreation"]);
			$timestampExpiration = (Integer) $_POST["timestampExpiration"];			

			//var_dump($timestampCreation);
			
			//var_dump($timestampExpiration);

			//debug($timestampCreation);

			msgOutput("Requisitando informacoes");
			$resposta = $SSDWs->getDateByTimestamp($timestampCreation, $timestampExpiration);
...
?>
	...
	<form method="POST">
		<label>Creation:</label> <input type="text" name="timestampCreation" />
		<label>Expiration:</label> <input type="text" name="timestampExpiration" />
