<?php
	if (isset($_POST['userId'])) {
		...
		try {
			...
			$SSDWs = new SSDWsAdmin($tmpDir, $clientCert, $privateKey, $privateKeyPassword, $trustedCaChain);
			...
			$systemId = (Integer) $_POST["systemId"];
			$userId = (Integer) $_POST["userId"];
			$flag = $_POST["flag"];
			$serviceId = (Integer) $_POST["serviceId"];									
			
			msgOutput("Requisitando informacoes.");
			$resposta = $SSDWs->getUserMaintenanceTicketInfo($systemId, $userId, $flag, $serviceId);
...
?>
	...
	<form method="POST">
		...
		<label>SystemId:</label> <input type="text" name="systemId" /><br />
		<label>Flag:</label> <input type="text" name="flag" /><br />
		<label>ServiceId:</label> <input type="text" name="serviceId" /><br />
