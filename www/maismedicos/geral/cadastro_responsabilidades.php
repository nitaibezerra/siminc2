...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM maismedicos.tprperfil p
							   INNER JOIN maismedicos.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "I": // Indicadores 
				...
				$sqlRespUsuario = "SELECT 
										DISTINCT 
										u.uniid AS codigo, 
										u.unisigla || ' - ' || u.uninome AS descricao, 
										ur.rpustatus AS status
								   FROM
								    	maismedicos.usuarioresponsabilidade ur 
								   INNER JOIN 
							   			maismedicos.universidade u ON u.uniid = ur.uniid
								   WHERE
									   	ur.usucpf = '%s' AND 
									    ur.pflcod = '%s' AND 
									    ur.rpustatus='A' AND
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM maismedicos.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
