			...
			for ($i = 0; $i < $count; $i++){
				$municipio = $muncod[$i];
						$sql_insere = $db->executar("
												INSERT INTO
													proinfantil.usuarioresponsabilidade
													(muncod, usucpf, rpustatus, rpudata_inc, pflcod, prsano)
												VALUES
													('{$municipio}', '{$usucpf}', 'A', '{$data}', '{$pflcod}', '{$_SESSION["exercicio"]}');");
			}
	}
...
function buscaMunicipiosAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
			...
			INNER JOIN 
				territorios.municipio m ON ur.muncod::integer = m.muncod::integer
	 		WHERE
