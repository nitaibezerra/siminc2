...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM sca.tprperfil p
	INNER JOIN sca.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "U": // UORG
				$aca_prg = "UORG";
				$sqlRespUsuario = "SELECT e.co_interno_uorg AS codigo, e.sg_unidade_org || ' - ' || e.no_unidade_org AS descricao, ur.rpustatus AS status
					FROM sca.usuarioresponsabilidade ur 
					INNER JOIN sca.vwunidadeorganizacional e ON e.co_interno_uorg = ur.co_uorg
