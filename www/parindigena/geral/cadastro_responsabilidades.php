...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM parindigena.tprperfil p
	INNER JOIN parindigena.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "U": // Unidades 
				...
				$sqlRespUsuario = "SELECT u.unicod AS codigo, u.unidsc AS descricao, ur.rpustatus AS status
					FROM parindigena.usuarioresponsabilidade ur 
			...
			case "E": // Estados
				...
				$sqlRespUsuario = "SELECT
									 e.estuf AS codigo, e.estdescricao AS descricao, ur.rpustatus AS status
								   FROM
								   	 parindigena.usuarioresponsabilidade ur 
