...
var Mapas = {
	...
	buscaEstadoForm: function( estuf, origemRequisicao ){
		...
		if( selecionados == null ){
			...
			if( origemRequisicao == 'organizacoesterritoriais' ){
				requisicao.orgid = jQuery('#orgao').val();
			}

            if( origemRequisicao == 'acompanhamento-pne-municipio' ){
                requisicao.subid  = jQuery('#subid').val();
                requisicao.pneano = jQuery('#pneano').val();
            }

            if(origemRequisicao == 'planocarreira-municipio'
                || origemRequisicao == 'planocarreira-estado'
                || origemRequisicao == 'planocarreira-estado-externo'){
                requisicao.tpdid = jQuery('#tpdid').val();
            }
	...
	desenhaPoligonos: function(){
		...
		if( 
            ...
            || this.origemRequisicao == 'municipio-externo'
            || this.origemRequisicao == 'planocarreira-estado-externo'
            || this.origemRequisicao == 'planocarreira-estado-legenda-externo'){
			...
			jQuery.each( poli, function( index, value ){
				...
				for (im in value.poli.coordinates) { // LOOP MULTIPOLIGONO
					...
					google.maps.event.addListener(that.poligonosDesenhados[ that.casa ], 'click', function(event){
                        switch (that.origemRequisicao){
                            case 'pais-externo':
                            case 'pais-legenda-externo':
                                gEstuf = this.estuf;
                                //acaoMapa = jQuery("#hidAcaoMapa").val();
                                //if(acaoMapa == 'est') {
                                //    baixarLei(gEstuf);
                                //} else {
                                //    abrirMapa(gEstuf);
