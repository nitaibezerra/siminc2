...
if(is_array($_POST['unidadeSelecionadas'])) {
	$sql = "update
			 tarefa.usuarioresponsabilidade 
			set
			 rpustatus = 'I' 
			where
			 usucpf = '$usucpf'  
	...
	if($_POST['unidadeSelecionadas'][0]){
		foreach($_POST['unidadeSelecionadas'] as $unaid){
			$sql = "insert into tarefa.usuarioresponsabilidade (pflcod, usucpf,  rpustatus, rpudata_inc, unaid)
...
<?
	  ...
	  $sql = "SELECT unaid,unasigla,unadescricao FROM tarefa.unidade order by unadescricao";
...
<?
$sql = "select distinct u.unaid, u.unasigla as codigo, u.unadescricao as descricao from tarefa.usuarioresponsabilidade ur
