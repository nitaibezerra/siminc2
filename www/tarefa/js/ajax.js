			...
			if(r.responseText != ""){
				...
				if(arDados.length >= 1){
			       	for (var j = 0; j < arDados.length; j++) {
		                ...
		                td1.innerHTML = "<img src=\"../imagens/gif_inclui.gif\" onClick=\"window.location.href='tarefa.php?modulo=principal/cadAtividade&acao=A&tarpai="+tarid+"&tartarefa="+tartarefa+"'\" style=\"border:0; cursor:pointer;\" title=\"Incluir uma atividade a Atividade\">"
						...
						var imgRestricao = "";
						if(boRestricao){
							imgRestricao = "<img src=\"../imagens/restricao.png\" onClick=\"window.location.href='tarefa.php?modulo=principal/cadAcompanhamento&acao=A&tarid="+tarid+"&tartarefa="+tartarefa+"&tarpai="+tarpai+"&boPadraoRetricao=1';\" style=\"border:0; cursor:pointer;\" title=\"Restrição\">";
						}
						var imgDepexterna = "";
						if(tardepexterna){
                		...
                		*/
                		if(boCarregaLinkAjax){
                			...
                			var onclick = "onClick=\"window.location.href='tarefa.php?modulo=principal/cadAcompanhamento&acao=A&tarid="+tarid+"&tartarefa="+tartarefa+"&tarpai="+tarpai+"';\"";
                		}
						td2 = tr.insertCell(2);
				        if( boFilho ) {
...
function mostraDataPrazoFormatada(td5, dataPrazo, tarid){
	...
	/**
	* FEITO ASSIM POR CAUSA DA PRESSA
	*/
