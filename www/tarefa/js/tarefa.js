...
function montaPai(_tartarefa, arFiltros, boCarregaLinkAjax) {
	...
	/*if((arFiltros == "" || arFiltros == ",,,") && $('cadAcompanhamento') == null){
		...
		td1.innerHTML = "É necessário escolher um filtro.";
		return false;
	}*/
				...
				if(arDados.length >= 1){
					for (var j = 0; j < arDados.length; j++) {
						...
						*/
						...
						var imgRestricao = "";
						if(boRestricao){
							imgRestricao = "<img src=\"../imagens/restricao.png\" onClick=\"window.location.href='tarefa.php?modulo=principal/cadAcompanhamento&acao=A&tarid="+tarid+"&boPadraoRetricao=1';\" style=\"border:0; cursor:pointer;\" title=\"Restrição\">";
						}
						var imgDepexterna = "";
						if(tardepexterna){
...
function montaCalendario(td) {
	...
	/**
	* VALIDAÇÃO DE DATA PRAZO ATENDIMENTO 
	*/
...
function verificaDataPaiEDataFilha(objData, tarid, boCadastro){
	...
	*/
	...
	if(!tarid) {
		//alert comentado por, ainda, não ter achado a lógica dele... 
		//alert('Tarefa não enviada.');
		return false;
	}
