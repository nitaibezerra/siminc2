...
function carregarCoordenadorLocal($dados) {
	...
	$infprof = $db->pegaLinha("SELECT i.iusd, i.iusnome, i.iuscpf 
							   FROM sisindigena2.identificacaousuario i 
							   INNER JOIN sisindigena2.tipoperfil t ON t.iusd=i.iusd 
							   WHERE i.iusd='".$dados['iusd']."' AND t.pflcod='".PFL_COORDENADORLOCAL."'");
	
	$_SESSION['sisindigena2']['coordenadorlocal'] = array("descricao" => $arr['descricao']."( ".$infprof['iusnome']." )",
	...
	if($dados['direcionar']) {
		$al = array("location"=>"sisindigena2.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=principal");
		alertlocation($al);
	}
