...
function removerTipoPerfil($dados) {
	...
	$sql = "SELECT p.pboid FROM sisindigena2.tipoperfil t 
			INNER JOIN sisindigena2.pagamentobolsista p ON p.tpeid = t.tpeid  
	...
	if($pboid) {
		if(!$dados['naoredirecionar']) {
			if($dados['picid']) $al = array("alert"=>"Coordenador Local ja possui pagamento e não pode ser removido, somente substituido","location"=>"sisindigena2.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
			if($dados['uncid']) $al = array("alert"=>"Coordenador IES ja possui pagamento e não pode ser removido, somente substituido","location"=>"sisindigena2.php?modulo=principal/universidade/gerenciarcoordenadories&acao=A&uncid=".$dados['uncid']);
	...
	$existe_equipe = $db->pegaUm("SELECT count(*) as nu FROM sisindigena2.orientadorturma WHERE turid IN(SELECT turid FROM sisindigena2.turmas WHERE iusd='".$dados['iusd']."')");
	
	if($existe_equipe) {
		
		$al = array("alert"=>"Perfil selecionado possui equipe cadastrada e não pode ser removido, somente substituido","location"=>"sisindigena2.php?modulo=".$dados['modulo']."&acao=A&aba=".$dados['aba']);
		alertlocation($al);
		
	} else {
		
		$sql = "DELETE FROM sisindigena2.turmas WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
		
	}
	
	$sql = "DELETE FROM sisindigena2.tipoperfil WHERE iusd='".$dados['iusd']."' AND pflcod='".$dados['pflcod']."'";
	$db->executar($sql);
	
	$usucpf = $db->pegaUm("SELECT iuscpf FROM sisindigena2.identificacaousuario WHERE iusd='".$dados['iusd']."'");
	
	if($usucpf) {
		$sql = "DELETE FROM seguranca.perfilusuario WHERE usucpf='".$usucpf."' AND pflcod='".$dados['pflcod']."'";
		$db->executar($sql);
		$sql = "DELETE FROM sisindigena2.usuarioresponsabilidade WHERE usucpf='".$usucpf."' AND pflcod='".$dados['pflcod']."'";
		$db->executar($sql);
	}
	
	$sql = "DELETE FROM sisindigena2.orientadorturma WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);
	
	
	$sql = "INSERT INTO sisindigena2.historicoidentificaousuario(
	...
	$db->executar("UPDATE sisindigena2.identificacaousuario SET iusstatus='I' WHERE iusd='".$dados['iusd']."'");
	
	$db->commit();
	
	if(!$dados['naoredirecionar']) {
		$al = array("alert"=>"Membro removido com sucesso","location"=>"sisindigena2.php?modulo=".$dados['modulo']."&acao=A".(($dados['picid'])?"&picid=".$dados['picid']:"").(($dados['aba'])?"&aba=".$dados['aba']:""));
		alertlocation($al);
	}
...
function verificaPermissao() {
	...
	$sql = "SELECT * FROM sisindigena2.usuarioresponsabilidade WHERE usucpf='".$_SESSION['usucpf']."' AND rpustatus='A'";
	$ur = $db->carregar($sql);
	...
	if(in_array(PFL_COORDENADORLOCAL,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_COORDENADORLOCAL && $urr['muncod']==$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['muncod']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_EQUIPEMUNICIPALAP,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_EQUIPEMUNICIPALAP && $urr['muncod']==$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['muncod']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_CONSULTAMUNICIPAL,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_CONSULTAMUNICIPAL && $urr['muncod']==$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['muncod']) {
					return true;
				}
			}
		}
	...
	if(in_array(PFL_EQUIPEESTADUALAP,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_EQUIPEESTADUALAP && $urr['estuf']==$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['estuf']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_CONSULTAESTADUAL,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_CONSULTAESTADUAL && $urr['estuf']==$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['estuf']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_COORDENADORIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_COORDENADORIES && $urr['uncid']==$_SESSION['sisindigena2']['universidade']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_COORDENADORADJUNTOIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_COORDENADORADJUNTOIES && $urr['uncid']==$_SESSION['sisindigena2']['coordenadoradjuntoies']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_PROFESSORALFABETIZADOR,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_PROFESSORALFABETIZADOR && $urr['uncid']==$_SESSION['sisindigena2']['professoralfabetizador']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_ORIENTADORESTUDO,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_ORIENTADORESTUDO && $urr['uncid']==$_SESSION['sisindigena2']['orientadorestudo']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_CONTEUDISTA,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_CONTEUDISTA && $urr['uncid']==$_SESSION['sisindigena2']['conteudista']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_PESQUISADOR,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_PESQUISADOR && $urr['uncid']==$_SESSION['sisindigena2']['pesquisador']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_FORMADORIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_FORMADORIES && $urr['uncid']==$_SESSION['sisindigena2']['formadories']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_SUPERVISORIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_SUPERVISORIES && $urr['picid']==$_SESSION['sisindigena2']['supervisories']['picid']) {
					return false;
				}
			}
		}
...
function montaAbasSisIndigena($abapai, $abaativa) {
	...
	$sql = "SELECT abaordem, abadsc, abaendereco, abafuncaomostrar, abapai FROM sisindigena2.abas WHERE abapai='".$abapai."' ORDER BY abaordem";
...
function carregarOrientacaoPorFiltro($dados) {
	...
	$sql = "SELECT oabdesc FROM sisindigena2.orientacaoaba WHERE abaid='".$dados['abaid']."'";
...
function salvarOrientacaoAdm($dados) {
	...
	$oabid = $db->pegaUm("SELECT oabid FROM sisindigena2.orientacaoaba WHERE abaid='".$dados['abaid']."'");

	if($oabid) {

		$sql = "UPDATE sisindigena2.orientacaoaba SET oabdesc='".$dados['oabdesc']."' WHERE oabid='".$oabid."'";
		$db->executar($sql);

	} else {

		$sql = "INSERT INTO sisindigena2.orientacaoaba(
...
function carregarDadosIdentificacaoUsuario($dados) {
	...
	if(!$dados['pflcod']) {
		$al = array("alert"=>"Problemas para carregar os dados usuário","location"=>"sisindigena2.php?modulo=inicio&acao=C");
		alertlocation($al);
	}
	...
	$sql = "SELECT i.cadastradosgb, i.uncid, i.iusd, i.iuscpf, i.iusnome, i.iusdatanascimento, i.iusnomemae, i.iustipoprofessor, i.iusnaodesejosubstituirbolsa,
			...
			LEFT  JOIN sisindigena2.identiusucursoformacao f ON f.iusd = i.iusd 
			LEFT  JOIN sisindigena2.identusutipodocumento d ON d.iusd = i.iusd 
			LEFT  JOIN sisindigena2.identificaoendereco e ON e.iusd = i.iusd 
			LEFT  JOIN territorios.municipio mm ON mm.muncod = e.muncod 
			LEFT  JOIN territorios.municipio ma ON ma.muncod = i.muncodatuacao
			LEFT  JOIN sisindigena2.cursoformacao cf ON cf.cufid = f.cufid 
			LEFT  JOIN sisindigena2.orientadorturma ot ON ot.iusd = i.iusd 
	...
	if($identificacaousuario[0]) {
		...
		foreach($identificacaousuario as $key => $iu) {
			...
			$idusuarios[$key] = $iu;
			unset($telefones);
			$sql = "SELECT itetipo, itedddtel, itenumtel FROM sisindigena2.identificacaotelefone WHERE iusd='".$iu['iusd']."'";
...
function reiniciarSenha($dados) {
	...
	$al = array("alert"=>"Senha reiniciada com sucesso","location"=>"sisindigena2.php?modulo=".$dados['modulo']."&acao=A&aba=".$dados['aba']);
	alertlocation($al);
...
function atualizarDadosIdentificacaoUsuario($dados) {
	...
	$iusagenciasugerida_atual = $db->pegaUm("SELECT iusagenciasugerida FROM sisindigena2.identificacaousuario WHERE iusd='".$dados['iusd']."'");
	if($iusagenciasugerida_atual != substr($dados['iusagenciasugerida'],0,4)) {
		$sqlsgb = "cadastradosgb=FALSE,"; 
	}

	$sql = "UPDATE sisindigena2.identificacaousuario SET
	...
	$iufid = $db->pegaUm("SELECT iufid FROM sisindigena2.identiusucursoformacao WHERE iusd='".$dados['iusd']."'");
	
	// controlando formação
	if($iufid) {
		
		$sql = "UPDATE sisindigena2.identiusucursoformacao SET
		...
		$sql = "INSERT INTO sisindigena2.identiusucursoformacao(
	...
	$itdid = $db->pegaUm("SELECT itdid FROM sisindigena2.identusutipodocumento WHERE iusd='".$dados['iusd']."'");
	
	// controlando documento
	if($itdid) {
		
		$sql = "UPDATE sisindigena2.identusutipodocumento SET
		...
		$sql = "INSERT INTO sisindigena2.identusutipodocumento(
	...
	$ienid = $db->pegaUm("SELECT ienid FROM sisindigena2.identificaoendereco WHERE iusd='".$dados['iusd']."'");
	
	// controlando endereço
	if($ienid) {
		
		$sql = "UPDATE sisindigena2.identificaoendereco SET
		...
		$sql = "INSERT INTO sisindigena2.identificaoendereco(
	...
	$db->executar("DELETE FROM sisindigena2.identificacaotelefone WHERE iusd='".$dados['iusd']."'");
	...
	$sql = "UPDATE sisindigena2.tipoperfil SET tpeatuacaoinicio=".(($dados['tpeatuacaoinicio_mes'] && $dados['tpeatuacaoinicio_ano'])?"'".$dados['tpeatuacaoinicio_ano']."-".$dados['tpeatuacaoinicio_mes']."-01'":"NULL").", 
...
function carregarOrientacao($endereco) {
	...
	$sql = "SELECT a.abaid, o.oabdesc FROM sisindigena2.abas a
			LEFT JOIN sisindigena2.orientacaoaba o ON o.abaid = a.abaid
...
function efetuarTrocaUsuarioPerfil($dados) {
	...
	$sql = "SELECT * FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd WHERE i.iusd='".$dados['iusdantigo']."'";
	$identificacaousuario_antigo = $db->pegaLinha($sql);
	
	if($identificacaousuario_antigo['pflcod']==PFL_PROFESSORALFABETIZADOR) {
		
		$pboid = $db->pegaUm("SELECT pboid FROM sisindigena2.pagamentobolsista WHERE tpeid='".$identificacaousuario_antigo['tpeid']."'");
	...
	$sql = "SELECT COUNT(*) as t FROM sisindigena2.mensario m 
	...
	$sql = "SELECT i.iusd, t.tpeid, i.iusnome FROM sisindigena2.identificacaousuario i LEFT JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd WHERE i.iuscpf='".str_replace(array(".","-"),array("",""),$dados['iuscpf_'])."'";
	...
	if(!$identificacaousuario_novo['iusd']) {
		...
		$sql = "UPDATE sisindigena2.identificacaousuario SET iusstatus='A', picid=".(($identificacaousuario_antigo['picid'])?"'".$identificacaousuario_antigo['picid']."'":"NULL").", uncid=".(($identificacaousuario_antigo['uncid'])?"'".$identificacaousuario_antigo['uncid']."'":"NULL").", 
	...
	$sql = "DELETE FROM sisindigena2.usuarioresponsabilidade WHERE usucpf='".str_replace(array(".","-"),array(""),$dados['iuscpf_'])."'";
	$db->executar($sql);
	
	$sql = "UPDATE sisindigena2.usuarioresponsabilidade SET usucpf='".str_replace(array(".","-"),array(""),$dados['iuscpf_'])."', uncid=".(($identificacaousuario_antigo['uncid'])?"'".$identificacaousuario_antigo['uncid']."'":"NULL")." WHERE rpustatus='A' AND usucpf='".$identificacaousuario_antigo['usucpf']."' AND pflcod='".$identificacaousuario_antigo['pflcod']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sisindigena2.tipoperfil SET iusd='".$identificacaousuario_novo['iusd']."' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sisindigena2.turmas SET iusd='".$identificacaousuario_novo['iusd']."' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sisindigena2.orientadorturma SET iusd='".$identificacaousuario_novo['iusd']."' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sisindigena2.identificacaousuario SET iusstatus='I' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	...
	$sql = "SELECT m.menid FROM sisindigena2.mensario m 
	...
	if($menids) {
		...
		$sql = "SELECT mavid FROM sisindigena2.mensarioavaliacoes WHERE menid IN('".implode("','",$menids)."')";
		$mavids = $db->carregarColuna($sql);
		
		if($mavids) {
			$db->executar("DELETE FROM sisindigena2.historicoreaberturanota WHERE mavid IN('".implode("','",$mavids)."')");
			$db->executar("DELETE FROM sisindigena2.mensarioavaliacoes WHERE mavid IN('".implode("','",$mavids)."')");
		}
	}
	
	$sql = "INSERT INTO sisindigena2.historicotrocausuario(iusdnovo, iusdantigo, pflcod, hstdata, usucpf, uncid)
	...
	if($dados['chk']) {
		...
		foreach($dados['chk'] as $pflcod => $cpfs) {
			...
			foreach($cpfs as $cpf) {
				...
				$sql = "SELECT * FROM sisindigena2.identificacaousuario WHERE iuscpf='".str_replace(array(".","-"),array(""),$cpf)."'";
			    ...
			    $rpuid = $db->pegaUm("select rpuid from sisindigena2.usuarioresponsabilidade where usucpf='".str_replace(array(".","-"),array(""),$identificacaousuario['iuscpf'])."' and pflcod='".$pflcod."' AND rpustatus='A'");
			    ...
			    if($dados['uncid']) {
				    ...
				    if(!$rpuid) {
				    	...
				    	$sql = "UPDATE sisindigena2.usuarioresponsabilidade SET uncid='".$dados['uncid']."' WHERE rpuid='".$rpuid."'";
				    	$db->executar($sql);
				    }
			    ...
			    if($identificacaousuario['picid']) {
			    	...
			    	$sql = "SELECT * FROM sisindigena2.nucleouniversidade WHERE picid='".$identificacaousuario['picid']."'";
				    ...
				    if(!$rpuid) {
				    	...
				    	$sql = "UPDATE sisindigena2.usuarioresponsabilidade SET {$cl} WHERE rpuid='".$rpuid."'";
				    	$db->executar($sql);
				    }
...
function verificarFormacaoCompleta($dados) {
	global $db;
	$sql = "SELECT foecompleto FROM sisindigena2.formacaoescolaridade WHERE foeid='".$dados['foeid']."'";
...
function listarCursosFormacao($dados) {
	global $db;
	$sql = "SELECT cufid as codigo, cufcursodesc as descricao FROM sisindigena2.cursoformacao WHERE cufstatus='A' AND cufcodareageral='".$dados['cufcodareageral']."' ORDER BY cufcursodesc";
...
function removerDocumentoDesignacao($dados) {
	global $db;
	$sql = "DELETE FROM sisindigena2.identificacaousuarioanexo WHERE iuaid='".$dados['iuaid']."'";
	$db->executar($sql);
	$db->commit();
...
function removerAnexoPortaria($dados) {
	global $db;
	$sql = "DELETE FROM sisindigena2.portarianomeacao WHERE ponid='".$dados['ponid']."'";
	$db->executar($sql);
	$db->commit();
...
function carregarDadosTurma($dados) {
	global $db;
	$sql = "SELECT * FROM sisindigena2.turmas t
			LEFT JOIN sisindigena2.identificacaousuario i ON i.iusd = t.iusd 
...
function carregarAlunosTurma($dados) {
	global $db;
	if($dados['turid']) {
		$sql = "SELECT '<center>".((!$dados['consulta'])?"<img src=../imagens/excluir.gif style=\"cursor:pointer;\" onclick=\"excluirAlunoTurma('||i.iusd||');\">":"")." ".(($dados['formacaoinicial'])?"'|| CASE WHEN SUBSTR(i.iuscpf,1,3)!='SIS' THEN '<input type=radio name=\"iusd['||i.iusd||']\" value=\"TRUE\" '||CASE WHEN i.iusformacaoinicialorientador=true THEN 'checked' ELSE '' END||'> Presente <input type=radio name=\"iusd['||i.iusd||']\" value=\"FALSE\" '||CASE WHEN i.iusformacaoinicialorientador=false THEN 'checked' ELSE '' END||'> Ausente' ELSE '' END ||'":"")."</center>' as acao, i.iuscpf, i.iusnome, i.iusemailprincipal, m.estuf || ' / ' || m.mundescricao as municipio, CASE WHEN pp.muncod IS NULL THEN 'Estadual' ELSE 'Municipal' END as esfera, tu.turdesc FROM sisindigena2.orientadorturma ot 
				INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = ot.iusd 
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
				INNER JOIN sisindigena2.turmas tu ON tu.turid = ot.turid 
				LEFT JOIN sisindigena2.pactoidadecerta pp ON pp.picid = i.picid 
...
function criarMensario($dados) {
	global $db;
	$sql = "SELECT m.menid, d.docid, d.esdid FROM sisindigena2.mensario m INNER JOIN workflow.documento d ON d.docid = m.docid WHERE m.iusd='".$dados['iusd']."' AND fpbid='".$dados['fpbid']."'";
	...
	if(!$menid) {
		...
		$arrUs    = $db->pegaLinha("SELECT i.iusnome, p.pfldsc FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod WHERE i.iusd='".$dados['iusd']."'");
		$iusnome  = $arrUs['iusnome'];
		$pfldsc   = $arrUs['pfldsc'];
		
		$referencia = $db->pegaUm("SELECT fpbmesreferencia || ' / ' || fpbanoreferencia as descricao FROM sisindigena2.folhapagamento WHERE fpbid='".$dados['fpbid']."'");
		
		$docid = wf_cadastrarDocumento( TPD_FLUXOMENSARIO, 'Mensário : '.$iusnome.' - '.$pfldsc.' Ref.'.$referencia );
		$esdid = ESD_EM_ABERTO_MENSARIO;
		
		$sql = "INSERT INTO sisindigena2.mensario(
            	iusd, fpbid, docid)
	...
	if($dados['iusd_avaliados']) {
		foreach($dados['iusd_avaliados'] as $iusd) {
			...
			if($dadosmensario['esdid']!=ESD_APROVADO_MENSARIO) {
				...
				$sql = "SELECT mavid FROM sisindigena2.mensarioavaliacoes WHERE menid='".$dadosmensario['memid']."' AND iusdavaliador='".$dados['iusdavaliador']."'";
				$mavid = $db->pegaUm($sql);
				...
				if($mavid) {
					...
					if(is_numeric($dados['frequencia'][$iusd]) || is_numeric($dados['atividadesrealizadas'][$iusd])) {
						...
						$sql = "UPDATE sisindigena2.mensarioavaliacoes SET mavfrequencia=".((is_numeric($dados['frequencia'][$iusd]))?"'".$dados['frequencia'][$iusd]."'":"NULL").", 
					...
					if(is_numeric($dados['frequencia'][$iusd]) || is_numeric($dados['atividadesrealizadas'][$iusd])) {
						...
						$sql = "INSERT INTO sisindigena2.mensarioavaliacoes(
				...
				$sql = "UPDATE sisindigena2.mensarioavaliacoes ma SET mavtotal=foo.total FROM (
						SELECT * FROM (
						SELECT 
						m.menid,
						mavid,
						...
						FROM sisindigena2.mensarioavaliacoes ma 
						INNER JOIN sisindigena2.mensario m ON m.menid = ma.menid 
						INNER JOIN sisindigena2.identificacaousuario u ON u.iusd = m.iusd 
						INNER JOIN sisindigena2.tipoperfil t ON t.iusd = u.iusd 
						INNER JOIN sisindigena2.fatoresdeavaliacao f ON f.fatpflcodavaliado = t.pflcod 
						) fee
				...
				if($mavid && $dados['cpfresponsavel'][$iusd] && $dados['mavdsc'][$iusd]) {
					$sql = "INSERT INTO sisindigena2.historicoreaberturanota(
...
function condicaoEnviarMensario($fpbid,$pflcod=null) {
	...
	if($pflcod == PFL_SUPERVISORIES) {
		...
		$sql_tot = sqlAvaliacaoSupervisor(array('picid'=>$_SESSION['sisindigena2']['supervisories']['picid'],'iusd'=>$_SESSION['sisindigena2']['supervisories']['iusd'],'fpbid'=>$fpbid));
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sisindigena2.mensario me 
							INNER JOIN sisindigena2.mensarioavaliacoes ma ON ma.menid = me.menid
							WHERE me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sisindigena2']['supervisories']['iusd']."' AND ma.mavtotal IS NOT NULL");
	...
	if($pflcod == PFL_COORDENADORADJUNTOIES) {
		...
		$sql_tot = sqlAvaliacaoCoordenadorAdjuntoIES(array('picid'=>$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid'],'iusd'=>$_SESSION['sisindigena2']['coordenadoradjuntoies']['iusd'],'fpbid'=>$fpbid));
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sisindigena2.mensario me 
							INNER JOIN sisindigena2.mensarioavaliacoes ma ON ma.menid = me.menid
							WHERE me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['iusd']."' AND ma.mavtotal IS NOT NULL");
	...
	if($pflcod == PFL_COORDENADORIES) {
		...
		$sql_tot = sqlAvaliacaoCoordenadorIES(array('uncid'=>$_SESSION['sisindigena2']['universidade']['uncid'],'iusd'=>$_SESSION['sisindigena2']['universidade']['iusd'],'fpbid'=>$fpbid));
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sisindigena2.mensario me 
							INNER JOIN sisindigena2.mensarioavaliacoes ma ON ma.menid = me.menid
							INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = me.iusd AND i.iusstatus='A'
							INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd
							WHERE me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sisindigena2']['universidade']['iusd']."' AND ma.mavtotal IS NOT NULL");
...
function posEnviarMensario($fpbid, $pflcod=null) {
	...
	if($pflcod == PFL_ORIENTADORESTUDO) {
		...
		$sql = "SELECT i.iusnome, me.docid, ma.mavtotal FROM sisindigena2.mensario me 
				INNER JOIN workflow.documento dc ON dc.docid = me.docid AND dc.tpdid=".TPD_FLUXOMENSARIO." 
				INNER JOIN sisindigena2.mensarioavaliacoes ma ON ma.menid = me.menid 
				INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = me.iusd 
				INNER JOIN sisindigena2.orientadorturma ot ON ot.iusd = me.iusd 
				INNER JOIN sisindigena2.turmas tt ON tt.turid = ot.turid 		
				WHERE tt.iusd='".$_SESSION['sisindigena2']['orientadorestudo']['iusd']."' AND dc.esdid='".ESD_EM_ABERTO_MENSARIO."' AND ma.iusdavaliador='".$_SESSION['sisindigena2']['orientadorestudo']['iusd']."' AND me.fpbid='".$fpbid."'";
		...
		$sql = "UPDATE sisindigena2.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				SELECT ma.mavid, fa.fatmonitoramento FROM sisindigena2.mensario me 
				INNER JOIN sisindigena2.mensarioavaliacoes ma ON ma.menid = me.menid 
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sisindigena2.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sisindigena2']['orientadorestudo']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
	...
	if($pflcod == PFL_FORMADORIES) {
		...
		$sql = "UPDATE sisindigena2.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sisindigena2.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sisindigena2']['formadories']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_SUPERVISORIES) {
		...
		$sql = "UPDATE sisindigena2.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sisindigena2.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sisindigena2']['supervisories']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_COORDENADORADJUNTOIES) {
		...
		$sql = "UPDATE sisindigena2.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sisindigena2.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_COORDENADORIES) {
		...
		$sql = "UPDATE sisindigena2.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sisindigena2.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sisindigena2']['universidade']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_COORDENADORLOCAL) {
		...
		$sql = "UPDATE sisindigena2.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sisindigena2.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sisindigena2.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
...
function carregarAjudaAvaliacao($dados) {
	...
	$sql = "SELECT * FROM sisindigena2.fatoresdeavaliacao WHERE fatid='".$dados['fatid']."'";
...
function carregarAvaliacaoEquipe($dados) {
	...
	$sql = "SELECT DISTINCT CASE WHEN foo.mais = '' THEN '' ELSE '<img align=\"absmiddle\" style=\"cursor:pointer\" src=\"../imagens/mais.gif\" title=\"mais\" onclick=\"exibirAvaliacaoSub(\''||foo.mais||'\', this)\"> ' END, '<span style=\"white-space: nowrap\"><img align=\"absmiddle\" src=\"../imagens/'|| CASE WHEN (SELECT esdid FROM workflow.documento WHERE docid=foo.docid) IN(".ESD_APROVADO_MENSARIO.") THEN 'money.gif' ELSE CASE WHEN me.mavtotal IS NULL THEN 'valida5.gif' WHEN me.mavtotal < 7 THEN 'valida6.gif' ELSE 'valida4.gif' END END ||'\" id=\"img_'||foo.iusd||'\"> <img align=\"absmiddle\" src=\"../imagens/ajuda.png\" width=\"16\" height=\"16\" style=\"cursor:pointer;\" onclick=\"verAjuda(\''||fat.fatid||'\');\"> 
			...
			INNER JOIN sisindigena2.fatoresdeavaliacao fat ON fat.fatpflcodavaliado = foo.pflcod 
			LEFT JOIN (SELECT iusd, mavfrequencia, mavatividadesrealizadas, mavmonitoramento, mavtotal, ma.mavid FROM sisindigena2.mensario m INNER JOIN sisindigena2.mensarioavaliacoes ma ON ma.menid = m.menid WHERE m.fpbid='".$dados['fpbid']."' AND iusdavaliador='".$dados['iusd']."') me ON me.iusd = foo.iusd 
...
function inserirDadosLog($dados) {
	...
	$sql = "INSERT INTO log_historico.logsgb_sisindigena2(
...
function sincronizarDadosUsuarioSGB($dados) {
	...
	$sql = "SELECT i.iuscpf, i.nacid, i.iusnome, i.iusdatanascimento, i.iusnomemae, i.iussexo, m.muncod as co_municipio_ibge_nascimento, m.estuf as sg_uf_nascimento, 
		...
		FROM sisindigena2.identificacaousuario i 
		LEFT JOIN territorios.municipio m ON m.muncod = i.muncod 
		LEFT JOIN sisindigena2.identificaoendereco ie ON ie.iusd = i.iusd 
		LEFT JOIN territorios.municipio m2 ON m2.muncod = ie.muncod 
		LEFT JOIN sisindigena2.identusutipodocumento it ON it.iusd = i.iusd 
	...
	if($dadosusuario) {
    	...
    	$sql = "UPDATE sisindigena2.identificacaousuario SET cadastradosgb=".(($logerro_gravarDadosBolsista=='TRUE')?'FALSE':'TRUE')." WHERE iusd='".$dados['iusd']."'";
    	$db->executar($sql);
    	$db->commit();
...
function sincronizarDadosEntidadeSGB($dados) {
	...
	$sql = "SELECT un.unicnpj, un.uninome, un.muncod, un.uniuf
			...
			INNER JOIN sisindigena2.universidade un ON un.uniid = u.uniid  
    ...
    if($existecnpj) $logerro_gravaDadosEntidade = 'FALSE';
   	...
   	$sql = "UPDATE sisindigena2.universidadecadastro SET cadastrosgb=".(($logerro_gravaDadosEntidade=='TRUE')?'FALSE':'TRUE')." WHERE uncid='".$dados['uncid']."'";
   	$db->executar($sql);
   	$db->commit();
...
function verificarTermoCompromisso($dados) {
	...
	if($termo['iustermocompromisso']!="t") {
		$al = array("alert"=>"Por favor preencha todos os campos obrigatórios da tela “Dados”.","location"=>"sisindigena2.php?modulo=principal/{$dados['sis']}/{$dados['sis']}&acao=A&aba=dados");
		alertlocation($al);
	}
}
...
function gerarVersaoProjetoUniversidade($dados) {
	...
	$sql = "INSERT INTO sisindigena2.versoesprojetouniversidade(
...
function carregarMudancasTroca($dados) {
	...
	$sql = "SELECT CASE WHEN h.hstacao='T' THEN 'Troca'
			...
			FROM sisindigena2.historicotrocausuario h 
			LEFT JOIN sisindigena2.identificacaousuario i ON i.iusd = h.iusdnovo 
			LEFT JOIN sisindigena2.identificacaousuario i2 ON i2.iusd = h.iusdantigo 
			LEFT JOIN seguranca.perfil p ON p.pflcod = h.pflcod 
			LEFT JOIN seguranca.usuario u ON u.usucpf = h.usucpf 
			LEFT JOIN sisindigena2.turmas t1 ON t1.turid = h.turidantigo 
			LEFT JOIN sisindigena2.identificacaousuario i3 ON i3.iusd = t1.iusd 
			LEFT JOIN sisindigena2.turmas t2 ON t2.turid = h.turidnovo 
			LEFT JOIN sisindigena2.identificacaousuario i4 ON i4.iusd = t2.iusd
...
function verificarValidacaoVisualizacaoProjeto($dados) {
	...
	if($resp!==true) {
		$al = array("alert"=>"Há ".$resp." orientadores que não foram vinculados a nenhuma turma.","location"=>"sisindigena2.php?modulo=principal/universidade/universidade&acao=A&aba=turmas");
		alertlocation($al);
	}
	
	$msgs = validarFormadoresTurmas($dados);
	if($msgs) {
		$al = array("alert"=>"Há ".count($msgs)." formador(es) que não foi/ foram vinculado(s) a nenhuma turma.","location"=>"sisindigena2.php?modulo=principal/universidade/universidade&acao=A&aba=turmas");
		alertlocation($al);
	}
	
	$tso = validarTurmasSemOrientadores($dados);
	if($tso) {
		$al = array("alert"=>"Há ".count($tso)." turma(s) sem Orientadores de Estudo vinculados.","location"=>"sisindigena2.php?modulo=principal/universidade/universidade&acao=A&aba=turmas");
		alertlocation($al);
	}
	...
	if($numCoordenadorAjunto>$maxCoordenadorAjunto) {
		$al = array("alert"=>"Há mais Coordenadores Adjuntos do que o limite permitido pelo MEC. Reveja a composição da Equipe da IES.","location"=>"sisindigena2.php?modulo=principal/universidade/universidade&acao=A&aba=turmas");
		alertlocation($al);
	}
	
	$nturmas =  numeroMaximoTurmas($dados);
	if(!$nturmas) {
		$al = array("alert"=>"Não existem turmas cadastradas. Reveja a Estrutura da Formação.","location"=>"sisindigena2.php?modulo=principal/universidade/universidade&acao=A&aba=turmas");
		alertlocation($al);
	}
	
	$numFormadorIes = numeroFormadorIES($dados);
	if($numFormadorIes>$nturmas) {
		$al = array("alert"=>"Há mais formadores do que o número de turmas estimado e justificado. Reveja a composição da Equipe da IES.","location"=>"sisindigena2.php?modulo=principal/universidade/universidade&acao=A&aba=turmas");
		alertlocation($al);
	}
...
function processarPagamentoBolsistaSGB($dados) {
	...
	$sql = "SELECT * FROM sisindigena2.pagamentobolsista WHERE pboid='".$dados->id."'";
		...
		if($dados->situacao->codigo=='10001' || 
		   ...
		   $dados->situacao->codigo=='00025') {
			...
			if($pagamentobolsista['pboparcela']) {
				...
				$novaparcela = $db->pegaUm("SELECT (rfuparcela+1) as novaparcela FROM sisindigena2.folhapagamentouniversidade f 
							 				INNER JOIN sisindigena2.universidadecadastro u ON u.uncid = f.uncid 
							 				WHERE u.uniid='".$pagamentobolsista['uniid']."' AND f.fpbid='".$pagamentobolsista['fpbid']."'");
			}
			
			$sql = "UPDATE sisindigena2.pagamentobolsista SET remid=null, pboparcela='".$novaparcela."' WHERE pboid='".$pagamentobolsista['pboid']."'";
			$db->executar($sql);
			$db->commit();
			...
			$sql = "UPDATE sisindigena2.pagamentobolsista SET remid=null WHERE pboid='".$pagamentobolsista['pboid']."'";
			$db->executar($sql);
			$db->commit();
		}
	}
...
function consultarDetalhesAvaliacoes($dados) {
	...
	$sql = "SELECT	uni.unisigla || ' - ' || uni.uninome as universidade, 
			...
			LEFT JOIN sisindigena2.universidadecadastro unc ON unc.uncid = i.uncid 
			LEFT JOIN sisindigena2.universidade uni ON uni.uniid = unc.uniid 
			LEFT JOIN sisindigena2.nucleouniversidade ptc ON ptc.picid = i.picid 
			INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil per ON per.pflcod = t.pflcod
			INNER JOIN sisindigena2.fatoresdeavaliacao f ON f.fatpflcodavaliado = t.pflcod
			INNER JOIN sisindigena2.folhapagamento fa ON fa.fpbid = m.fpbid 
	...
	$sql = "SELECT foo.iusnome,
		    ...
		    FROM sisindigena2.mensarioavaliacoes m
			INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = m.iusdavaliador 
			LEFT JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
...
function aprovarEquipe($dados) {
	...
	if($dados['menid']) {
		foreach($dados['menid'] as $menid) {
			...
			$sql = "SELECT * FROM sisindigena2.mensario m 
	...
	$al = array("alert"=>"Equipe aprovada com sucesso","location"=>"sisindigena2.php?modulo=".$dados['modulo']."&acao=A&aba=aprovarusuario&fpbid=".$dados['fpbid']."&pflcodaprovar=".$dados['pflcodaprovar']);
	alertlocation($al);
...
function posAprovarMensario($menid) {
	...
	$sql = "SELECT	t.tpeid, m.iusd, m.fpbid, p.pflcod, p.pfldsc, i.iuscpf, i.iusnaodesejosubstituirbolsa, i.iusnome, f.fpbmesreferencia, f.fpbanoreferencia, pp.plpvalor, un.uniid FROM sisindigena2.mensario m 
			INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = m.iusd
			INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
	...
	if($arrInfo['iusnaodesejosubstituirbolsa']!='t') {
		...
		$sql = "SELECT 'Não foi possível criar o registro de bolsa para ".str_replace(array("'"),array(" "),$arrInfo['iusnome']).", pois a bolsa ja foi paga para ' || i.iusnome || ' => ' || 'Ref. ' || m.mesdsc || ' / ' || fpbanoreferencia ||' )' as descricao FROM sisindigena2.pagamentobolsista p 
...
function calculaPorcentagemUsuarioAtivos($dados) {
	...
	if($_REQUEST['modulo']=='principal/universidade/universidadeexecucao') {
		$sql_equipe = sqlEquipeCoordenadorIES(array("uncid"=>$_SESSION['sisindigena2']['universidade']['uncid']));
		$sis = 'universidade';
	}
	
	if($_REQUEST['modulo']=='principal/coordenadoradjuntoies/coordenadoradjuntoies') {
		$sql_equipe = sqlEquipeCoordenadorAdjunto(array("uncid"=>$_SESSION['sisindigena2']['coordenadoradjuntoies']['uncid']));
		$sis = 'coordenadoradjuntoies';
	}
	
	if($_REQUEST['modulo']=='principal/supervisories/supervisories') {
		$sql_equipe = sqlEquipeSupervisor(array("uncid"=>$_SESSION['sisindigena2']['supervisories']['uncid']));
		$sis = 'supervisories';
	}
	
	if($_REQUEST['modulo']=='principal/formadories/formadories') {
		$sql_equipe = sqlEquipeFormador(array("iusd"=>$_SESSION['sisindigena2']['formadories']['iusd'],"uncid"=>$_SESSION['sisindigena2']['formadories']['uncid']));
		$sis = 'formadories';
	}
	
	if($_REQUEST['modulo']=='principal/orientadorestudo/orientadorestudo') {
		$sql_equipe = sqlEquipeOrientador(array("iusd"=>$_SESSION['sisindigena2']['orientadorestudo']['iusd'],"uncid"=>$_SESSION['sisindigena2']['orientadorestudo']['uncid']));
		$sis = 'orientadorestudo';
	}
	
	if($_REQUEST['modulo']=='principal/coordenadorlocal/coordenadorlocalexecucao') {
		$sql_equipe_p = sqlEquipeCoordenadorLocal(array("picid"=>$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['picid']));
		$sis = 'coordenadorlocal';
	}
	
	if($sql_equipe_p) {
		$sql_total = "SELECT COUNT(*) as tot FROM ({$sql_equipe_p}) foo WHERE foo.status='A' AND CASE WHEN foo.pflcod=".PFL_ORIENTADORESTUDO." OR foo.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN foo.iusd in( SELECT i.iusd FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.pactoidadecerta p ON p.picid = i.picid INNER JOIN workflow.documento d ON d.docid = p.docidturma WHERE d.esdid='".ESD_FECHADO_TURMA."') ELSE true END";
		...
		gerenciarAtividadePacto(array('iusd'=>$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['iusd'],'apadatainicio'=>$apadatainicio,'apadatafim'=>$apadatafim,'apassituacao'=>$apassituacao,'suaid'=>$dados['suaid'],'picid'=>$_SESSION['sisindigena2']['coordenadorlocal'][$_SESSION['sisindigena2']['esfera']]['picid']));
	}
	...
	if($sql_equipe) {
		...
		$ecuid = pegarEstruturaCurso(array("uncid" => $_SESSION['sisindigena2'][$sis]['uncid']));
		
		if($total) $aunsituacao = round(($total_a/$total)*100);
		gerenciarAtividadeUniversidade(array('iusd'=>$_SESSION['sisindigena2'][$sis]['iusd'],'aundatafim'=>$aundatafim,'aundatainicio'=>$aundatainicio,'aunsituacao'=>$aunsituacao,'ecuid'=>$ecuid,'suaid'=>$dados['suaid']));
	}
...
function gerenciarAtividadeUniversidade($dados) {
	...
	$sql = "SELECT aunid FROM sisindigena2.atividadeuniversidade a 
	...
	if($aunid) {
		...
		$sql = "UPDATE sisindigena2.atividadeuniversidade SET 
...
function gerenciarAtividadePacto($dados) {
	...
	$sql = "SELECT apaid FROM sisindigena2.atividadepacto WHERE suaid='".$dados['suaid']."' AND picid='".$dados['picid']."'".(($dados['iusd'])?" AND iusd='".$dados['iusd']."'":"");
	$apaid = $db->pegaUm($sql);
	
	if($apaid) {
		
		$sql = "UPDATE sisindigena2.atividadepacto SET 
		...
		$sql = "INSERT INTO sisindigena2.atividadepacto(
...
function carregarExecucaoAtividadeUniversidade($dados) {
	...
	$execucao_atividade = $db->pegaLinha("SELECT ROUND(AVG(aunsituacao)) as apassituacao, 
											  ...
											  FROM sisindigena2.subatividades su
										  	  INNER JOIN sisindigena2.atividadeuniversidade ap ON su.suaid = ap.suaid 
										  	  INNER JOIN sisindigena2.estruturacurso es ON es.ecuid = ap.ecuid 
...
function carregarExecucaoSubAcaoUniversidade($dados) {
	...
	$atividadeuni = $db->pegaLinha("SELECT aunid,
									  ...
									  FROM sisindigena2.atividadeuniversidade au 
									  INNER JOIN sisindigena2.estruturacurso es ON es.ecuid = au.ecuid
...
function pegarEstruturaCurso($dados) {
	global $db;
	$sql = "SELECT ecuid FROM sisindigena2.estruturacurso WHERE uncid='".$dados['uncid']."'";
	$ecuid = $db->pegaUm($sql);
	
	if(!$ecuid) {
		
		$sql = "INSERT INTO sisindigena2.estruturacurso(
...
function carregarPeriodoReferencia($dados) {
	global $db;
	$sql = "SELECT DISTINCT f.fpbid as codigo, m.mesdsc || ' / ' || fpbanoreferencia as descricao 
			FROM sisindigena2.folhapagamento f 
			INNER JOIN sisindigena2.folhapagamentouniversidade rf ON rf.fpbid = f.fpbid 
			INNER JOIN public.meses m ON m.mescod::integer = f.fpbmesreferencia
			WHERE f.fpbstatus='A' AND rf.uncid='".$dados['uncid']."' ".(($dados['picid'])?"AND rf.picid='".$dados['picid']."'":"")." AND to_char(NOW(),'YYYYmmdd')>=to_char((fpbanoreferencia::text||lpad(fpbmesreferencia::text, 2, '0')||'15')::date,'YYYYmmdd') ORDER BY f.fpbid";
...
function exibirAcessoUsuarioSimec($dados) {
	...
	$sql = "SELECT 
			...
			FROM sisindigena2.identificacaousuario u 
			INNER JOIN sisindigena2.tipoperfil t on t.iusd=u.iusd 
			INNER JOIN seguranca.perfil p on p.pflcod = t.pflcod 
...
function exibirSituacaoPagamento($dados) {
	...
	$sql = "SELECT '{$acao}' as acao,
			...
			INNER JOIN sisindigena2.pagamentoperfil pp ON pp.pflcod = foo.pflcod";
...
function cadastrarPeriodoReferencia($dados) {
		...
		foreach($uncids as $uncid) {
			...
			if($picids) {
				...
				foreach($picids as $picid) {
					...
					$sql = "DELETE FROM sisindigena2.folhapagamentouniversidade WHERE uncid='".$uncid."' AND picid='".$picid."'";
					$db->executar($sql);
					
					$sql = "select foo.fpbid from (
							select fpbid, fpbanoreferencia||'-'||lpad(fpbmesreferencia::text,2,'0') as dt from sisindigena2.folhapagamento ) foo
					...
					if($fpbids) {
						foreach($fpbids as $key => $fpbid) {
							$sql = "INSERT INTO sisindigena2.folhapagamentouniversidade(
	...
	$al = array("alert"=>"Período de referência aprovado com sucesso","location"=>"sisindigena2.php?modulo=principal/mec/mec&acao=A&aba=configuracoes");
	alertlocation($al);
...
function carregarLogCadastroSGB($dados) {
	...
	$iusd = $db->pegaUm("SELECT iusd FROM sisindigena2.identificacaousuario WHERE iuscpf='".$dados['usucpf']."'");
	
	if($iusd) echo "<input type=hidden name=iusd id=iusd_log value=\"".$iusd."\">";
	
	$sql = "SELECT u.iuscpf, u.iusnome, to_char(logdata,'dd/mm/YYYY HH24:MI') as logdata, logresponse FROM log_historico.logsgb_sisindigena2 l 
			INNER JOIN sisindigena2.identificacaousuario u ON u.iuscpf = l.logcpf 
...
function verificarEmailUnico($dados) {
	...
	$sql = "SELECT '- '||i.iusnome||' ( '||p.pfldsc||' );' as descricao FROM sisindigena2.identificacaousuario i 
			INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod
...
function consultarDetalhesPagamento($dados) {
	global $db;
	$sql = "SELECT i.iusnome, me.mesdsc||'/'||fa.fpbanoreferencia as periodo, e.esddsc, p.pbovlrpagamento, pp.pfldsc, uni.uninome, uni.unicnpj, p.docid FROM sisindigena2.pagamentobolsista p 
			INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = p.iusd 
			INNER JOIN sisindigena2.folhapagamento fa ON fa.fpbid = p.fpbid 
			INNER JOIN public.meses me ON me.mescod::integer = fa.fpbmesreferencia 
			...
			INNER JOIN sisindigena2.universidade uni ON uni.uniid = p.uniid  
...
function carregarMateriais($dados) {
	...
	if($dados['uncid']) {
		...
		$sql = "SELECT * FROM (
				(
				SELECT count(p.picid) as tot, ".$dados['group']." as grouper FROM sisindigena2.materiais m 
				INNER JOIN sisindigena2.pactoidadecerta p ON p.picid = m.picid 
				INNER JOIN sisindigena2.abrangencia a ON a.muncod = p.muncod 
				...
				INNER JOIN sisindigena2.abrangencia a ON a.muncod = mu.muncod 
				INNER JOIN sisindigena2.estruturacurso e ON e.ecuid = a.ecuid 
		...
		$materiais = $db->carregar("SELECT count(picid) as tot, ".$dados['group']." as grouper FROM sisindigena2.materiais GROUP BY ".$dados['group']);
...
function carregarMateriaisProfessores($dados) {
	global $db;
	$materiais = $db->carregar("SELECT count(iusd) as tot, ".$dados['group']." as grouper FROM sisindigena2.materiaisprofessores GROUP BY ".$dados['group']);
...
function excluirUsuarioPerfil($dados) {
	...
	$npagamentos = $db->pegaUm("SELECT COUNT(*) FROM sisindigena2.pagamentobolsista WHERE iusd='".$dados['iusd']."'");
	
	if($npagamentos > 0) {
		
		$identificacaousuario = $db->pegaLinha("SELECT i.iuscpf, t.pflcod, p.pfldsc FROM sisindigena2.identificacaousuario i 
												INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
												INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod  
												WHERE i.iusd='".$dados['iusd']."'");
		...
		$sql = "INSERT INTO sisindigena2.identificacaousuario(
				...
				SELECT picid, muncod, eciid, nacid, fk_cod_docente, 'REM'||SUBSTR(iuscpf,4,8) as iuscpf, iusnome || ' - {$identificacaousuario['pfldsc']} - REMOVIDO' as iusnome, 
				       ...
				       iusformacaoinicialorientador, cadastradosgb, iustipoprofessor, 
				       iusdocumento, iusnaodesejosubstituirbolsa
				  FROM sisindigena2.identificacaousuario where iusd='".$dados['iusd']."'
				RETURNING iusd;";
		...
		if($identificacaousuario) {
			$sql = "DELETE FROM sisindigena2.usuarioresponsabilidade  WHERE rpustatus='A' AND usucpf='".$identificacaousuario['iuscpf']."' AND pflcod='".$identificacaousuario['pflcod']."'";
			$db->executar($sql);
		}
	
		$sql = "UPDATE sisindigena2.tipoperfil SET iusd='".$iusd_novo."' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	
		$sql = "UPDATE sisindigena2.turmas SET iusd='".$iusd_novo."' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	
		$sql = "UPDATE sisindigena2.orientadorturma SET iusd='".$iusd_novo."' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	
		$sql = "UPDATE sisindigena2.identificacaousuario SET iusstatus='I' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	...
	if(!$dados['uncid']) $dados['uncid'] = $db->pegaUm("SELECT uncid FROM sisindigena2.identificacaousuario WHERE iusd='".$dados['iusd']."'");

	$sql = "INSERT INTO sisindigena2.historicotrocausuario(
            iusdantigo, pflcod, hstdata, usucpf, uncid, 
            hstacao)
    		VALUES ('".$dados['iusd']."', '".$dados['pflcod']."', NOW(), '".$_SESSION['usucpf']."', '".(($dados['uncid'])?$dados['uncid']:"NULL")."', 'R');";
	...
	$al = array("alert"=>"Exclusão ocorrida com sucesso","location"=>"sisindigena2.php?modulo=".$dados['modulo']."&acao=".$dados['acao']."&aba=".$dados['aba'].(($dados['uncid'])?"&uncid=".$dados['uncid']:""));
	alertlocation($al);
...
<tr>
	...
	$_SESSION['imgparams']['tabela'] = "sisindigena2.materiaisfotos";
	$_SESSION['imgparams']['filtro'] = "1=1";
	
	if($dados['uncid']) {
		$sql = "SELECT m.arqid, m.matid, m.mafdsc FROM sisindigena2.materiaisfotos m
				INNER JOIN sisindigena2.materiais ma ON ma.matid = m.matid 
				INNER JOIN sisindigena2.pactoidadecerta p ON p.picid = ma.picid 
				INNER JOIN sisindigena2.abrangencia a ON a.muncod = p.muncod 
...
<tr>
	...
	$_SESSION['imgparams']['tabela'] = "sisindigena2.materiaisprofessoresfotos";
	$_SESSION['imgparams']['filtro'] = "1=1";
	
	$sql = "SELECT m.arqid, m.mapid, m.mpfdsc FROM sisindigena2.materiaisprofessoresfotos m 
			INNER JOIN sisindigena2.materiaisprofessores ma ON ma.mapid = m.mapid 
			ORDER BY random() LIMIT 6";
...
function verMunicipioMateriais($dados) {
	...
	if($dados['uncid']) {
		...
		$sql = "SELECT * FROM (
				...
				COALESCE(array_to_string(array(SELECT i.iusnome FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ', '),'Coordenador Local não cadastrado') as coordenadorlocal,
				COALESCE(array_to_string(array(SELECT i.iusemailprincipal FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ', '),'Coordenador Local não cadastrado') as emailcoordenadorlocal,
				COALESCE(array_to_string(array(SELECT '('||itedddtel||') '||itenumtel as tel FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd INNER JOIN sisindigena2.identificacaotelefone it ON it.iusd = i.iusd AND it.itetipo='C' WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ', '),'-') as telefonecoordenadorlocal
				FROM sisindigena2.materiais m 
				INNER JOIN sisindigena2.pactoidadecerta p ON p.picid = m.picid 
				INNER JOIN territorios.municipio mu ON mu.estuf = p.estuf 
		...
		$sql = "SELECT 
				...
				COALESCE((SELECT iusnome FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."),'Coordenador Local não cadastrado') as coordenadorlocal,
				COALESCE((SELECT iusemailprincipal FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."),'Coordenador Local não cadastrado') as emailcoordenadorlocal,
				COALESCE((SELECT '('||itedddtel||') '||itenumtel as tel FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd INNER JOIN sisindigena2.identificacaotelefone it ON it.iusd = i.iusd AND it.itetipo='C' WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."),'-') as telefonecoordenadorlocal 
				FROM sisindigena2.materiais m 
				INNER JOIN sisindigena2.pactoidadecerta p ON p.picid = m.picid 
...
function trocarTurmas($dados) {
	...
	if($dados['troca']) {
		foreach($dados['troca'] as $iusd => $turid) {
			if($turid) {
				$db->executar("UPDATE sisindigena2.orientadorturma SET turid='".$turid."' WHERE iusd='".$iusd."'");
				$db->executar("INSERT INTO sisindigena2.historicotrocausuario(
            				   iusdantigo, pflcod, hstdata, usucpf, uncid, hstacao, turidnovo, turidantigo)
    						   VALUES ('".$iusd."', 
    						   		   (SELECT pflcod FROM sisindigena2.tipoperfil WHERE iusd='".$iusd."'), 
    						   		    NOW(), 
    						   		    '".$_SESSION['usucpf']."', 
    						   		    '".$dados['uncid']."', 
    						   		    'F', 
...
function atualizarEmail($dados) {
	...
	$sql = "UPDATE sisindigena2.identificacaousuario SET iusemailprincipal='".$dados['iusemailprincipal']."' WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);
	$db->commit();
...
function exibirPorcentagemPagamentoPerfil($dados) {
	...
	$sql = "SELECT p.pflcod, p.pfldsc FROM sisindigena2.pagamentoperfil pp 
	...
	if($perfil) {
		...
		foreach($perfil as $p) {
			...
			$sql = "SELECT count(*) as tot FROM sisindigena2.identificacaousuario i 
					INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
					INNER JOIN sisindigena2.pagamentoperfil p ON p.pflcod = t.pflcod 
					LEFT JOIN sisindigena2.nucleouniversidade pa ON pa.picid = i.picid 
					WHERE t.pflcod='".$p['pflcod']."' AND CASE WHEN t.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN i.iustipoprofessor='censo' ELSE true END AND i.uncid IN( SELECT rf.uncid FROM sisindigena2.folhapagamentouniversidade rf WHERE rf.fpbid='".$dados['fpbid']."' ) ".(($wh)?" AND ".implode(" AND ", $wh):"");
			
			$totalus = $db->pegaUm($sql);
			
			$sql = "SELECT count(*) as tot FROM sisindigena2.pagamentobolsista p 
					INNER JOIN sisindigena2.universidadecadastro i ON i.uniid = p.uniid 
					WHERE p.pflcod='".$p['pflcod']."' AND p.fpbid='".$dados['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sisindigena2.folhapagamentouniversidade rf WHERE rf.fpbid='".$dados['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
			...
			$sql = "SELECT count(*) as tot FROM sisindigena2.pagamentobolsista p 
					...
					WHERE d.esdid='".ESD_PAGAMENTO_EFETIVADO."' AND p.pflcod='".$p['pflcod']."' AND p.fpbid='".$dados['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sisindigena2.folhapagamentouniversidade rf WHERE rf.fpbid='".$dados['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
...
function exibirPorcentagemPagamento($dados) {
	...
	$sql = "SELECT f.fpbid, 'Ref.'||m.mesdsc||'/'||f.fpbanoreferencia as periodo FROM sisindigena2.folhapagamento f
			INNER JOIN public.meses m ON m.mescod::numeric = f.fpbmesreferencia 
			".(($dados['uncid'])?"INNER JOIN sisindigena2.folhapagamentouniversidade fp ON fp.fpbid = f.fpbid AND fp.uncid='".$dados['uncid']."'":"")."
			WHERE (fpbanoreferencia||'-'||fpbmesreferencia||'-'||'01')::date <= '".date("Y-m")."-01'";
	...
	if($folhapagamento[0]) {
		...
		foreach($folhapagamento as $fp) {
			...
			$sql = "SELECT count(*) as tot FROM sisindigena2.identificacaousuario i 
					...
					INNER JOIN sisindigena2.pagamentoperfil p ON p.pflcod = t.pflcod 
					LEFT JOIN sisindigena2.nucleouniversidade pa ON pa.picid = i.picid 
					WHERE CASE WHEN t.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN i.iustipoprofessor='censo' ELSE true END AND i.uncid IN( SELECT rf.uncid FROM sisindigena2.folhapagamentouniversidade rf WHERE rf.fpbid='".$fp['fpbid']."' ) ".(($wh)?" AND ".implode(" AND ", $wh):"");
			
			$totalus = $db->pegaUm($sql);
			
			$sql = "SELECT count(*) as tot FROM sisindigena2.pagamentobolsista p 
					INNER JOIN sisindigena2.universidadecadastro i ON i.uniid = p.uniid 
					WHERE p.fpbid='".$fp['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sisindigena2.folhapagamentouniversidade rf WHERE rf.fpbid='".$fp['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
			...
			$sql = "SELECT count(*) as tot FROM sisindigena2.pagamentobolsista p 
					INNER JOIN sisindigena2.universidadecadastro i ON i.uniid = p.uniid 
					INNER JOIN workflow.documento d ON d.docid = p.docid 
					WHERE d.esdid='".ESD_PAGAMENTO_EFETIVADO."' AND p.fpbid='".$fp['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sisindigena2.folhapagamentouniversidade rf WHERE rf.fpbid='".$fp['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
...
function carregarDetalhesPerfil($dados) {
	...
	if($dados['pflcod_']==PFL_ORIENTADORESTUDO) {
		...
		$sql = "(
				...
				FROM sisindigena2.pactoidadecerta p 
				INNER JOIN sisindigena2.abrangencia a ON a.muncod = p.muncod
				INNER JOIN sisindigena2.estruturacurso es ON es.ecuid = a.ecuid 
				...
				INNER JOIN sisindigena2.abrangencia a ON a.muncod = m.muncod
				INNER JOIN sisindigena2.estruturacurso es ON es.ecuid = a.ecuid 
	...
	if($dados['pflcod_']==PFL_PROFESSORALFABETIZADOR) {
		...
		echo $db->pegaUm("SELECT u.unisigla||' - '||u.uninome AS descricao FROM sisindigena2.nucleouniversidade p INNER JOIN sisindigena2.universidade u ON u.uniid = p.uniid WHERE p.picid='".$dados['picid']."'");
		echo '</td>';
...
function exibirMunicipiosNaoFechados($dados) {
	...
	$sql = "SELECT  m.estuf,
					m.mundescricao,
			    COALESCE(array_to_string(array(SELECT iusnome FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=pic.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ','),'Coordenador Local não cadastrado') as coordenadorlocal, 
			    COALESCE(array_to_string(array(SELECT iusemailprincipal FROM sisindigena2.identificacaousuario i INNER JOIN sisindigena2.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=pic.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ','),'Coordenador Local não cadastrado') as emailcoordenador
			FROM sisindigena2.abrangencia a 
			INNER JOIN sisindigena2.estruturacurso e ON e.ecuid = a.ecuid 
			INNER JOIN territorios.municipio m ON m.muncod = a.muncod 
			INNER JOIN sisindigena2.pactoidadecerta pic ON pic.muncod = a.muncod 
...
function efetuarInsercaoUsuarioPerfil($dados) {
	...
	$iusd = $db->pegaUm("SELECT iusd FROM sisindigena2.identificacaousuario WHERE iuscpf='".str_replace(array(".","-"),array("",""),$dados['iuscpf__'])."'");
	...
	if($iusd) {
		...
		$sql = "SELECT
				...
				LEFT  JOIN sisindigena2.nucleouniversidade n ON n.picid = i.picid
				LEFT JOIN sisindigena2.universidade u ON u.uniid = n.uniid
				WHERE t.iusd='".$iusd."'";
	...
	$tpeid = $db->pegaUm("SELECT tpeid FROM sisindigena2.tipoperfil WHERE iusd='".$iusd."'");
	...
	if($dados['iusdsup']) {
		...
		$dados['turid__'] = $db->pegaUm("SELECT turid FROM sisindigena2.turmas WHERE iusd='".$dados['iusdsup']."'");
		if(!$dados['turid__']) {
	...
	if($dados['turid__']) {
		$otuid = $db->pegaUm("SELECT otuid FROM sisindigena2.orientadorturma WHERE iusd='".$iusd."'");
		...
		if(!$otuid) {
			...
			$sql = "UPDATE sisindigena2.orientadorturma SET turid='".$dados['turid__']."' WHERE iusd='".$iusd."';";
			$db->executar($sql);
...
function sincronizarUsuariosSIMEC($dados) {
	...
	$sql = "UPDATE seguranca.usuario u SET 
			...
			SELECT 
			i.iuscpf,
			(SELECT itedddtel FROM sisindigena2.identificacaotelefone WHERE iusd=i.iusd AND itetipo='T') as dddtel,
			u.usufoneddd,
			(SELECT itenumtel FROM sisindigena2.identificacaotelefone WHERE iusd=i.iusd AND itetipo='T') as tel,
			u.usufonenum,
...
function carregarOrientadoresSISPorMunicipio($dados) {
	...
	if($p) {
		...
		$db->executar("UPDATE sisindigena2.pactoidadecerta SET picselecaopublica=true, picincluirprofessorrede=false WHERE picid='".$p['picid']."'");
		$db->commit();
		...
		if($totalalfabetizadores['total_orientadores_a_serem_cadastrados'] > count($orientadoresestudo)) {
			...
			for($i = 0;$i < $restantes;$i++) {
				...
				$num_ius = $db->pegaUm("SELECT substr(iuscpf, 8) as num FROM sisindigena2.identificacaousuario WHERE picid='".$p['picid']."' AND iuscpf ilike 'SIS%' ORDER BY iusd DESC");
				if($num_ius) $num_ius++;
				else $num_ius=1;
				...
				if($p['muncod']) {
					$uncid = $db->pegaUm("SELECT e.uncid FROM sisindigena2.abrangencia a 
										  INNER JOIN sisindigena2.estruturacurso e ON e.ecuid = a.ecuid 
				...
				$sql = "INSERT INTO sisindigena2.identificacaousuario(picid, 
																  muncod, 
																  iuscpf, 
																  iusnome, 
            													  iusemailprincipal, 
				...
				$sql = "INSERT INTO sisindigena2.tipoperfil( iusd, pflcod, tpestatus)
    					VALUES ( '".$iusd."', '".PFL_ORIENTADORESTUDO."', 'A');";
				
				$db->executar($sql);
				
				if($uncid) {
					$turid = $db->pegaUm("SELECT t.turid FROM sisindigena2.turmas t 
										  INNER JOIN sisindigena2.identificacaousuario i ON i.iusd = t.iusd 
										  INNER JOIN sisindigena2.tipoperfil tt ON tt.iusd = i.iusd 
										  WHERE tt.pflcod='".PFL_FORMADORIES."' AND i.uncid='".$uncid."' LIMIT 1");
					
					if($turid) {
						$db->executar("INSERT INTO sisindigena2.orientadorturma(
			...
			$al = array("alert"=>"O município selecionado não possui vagas para Orientadores de Estudo.","location"=>"sisindigena2.php?modulo=principal/mec/mec&acao=A");
			alertlocation($al);
		}
	
	}
	
	$al = array("alert"=>"Foram inseridos {$restantes} Orientadores de Estudo SIS.","location"=>"sisindigena2.php?modulo=principal/mec/mec&acao=A");
	alertlocation($al);
...
function invalidarMensario($dados) {
	global $db;
	$sql = "SELECT d.esdid FROM sisindigena2.mensario m 
			INNER JOIN workflow.documento d ON d.docid = m.docid 
	...
	$al = array("alert"=>"Mensário invalidado com sucesso","location"=>"sisindigena2.php?modulo={$dados['modulo']}&acao=A&aba=aprovarusuario&fpbid=".$dados['fpbid']."&pflcodaprovar=".$dados['pflcodaprovar']);
	alertlocation($al);
...
function corrigirAcessoUniversidade($dados) {
	global $db;
	$sql = "SELECT i.uncid, i.iuscpf, i.picid, t.pflcod, i.muncodatuacao, i.iusd FROM sisindigena2.identificacaousuario i 
			INNER JOIN sisindigena2.tipoperfil t ON t.iusd = t.iusd
	...
	if($identificacaousuario['uncid']) {
		...
		$sql = "UPDATE sisindigena2.usuarioresponsabilidade SET uncid='".$identificacaousuario['uncid']."' WHERE usucpf='".$_SESSION['usucpf']."' AND pflcod='".$identificacaousuario['pflcod']."' AND rpustatus='A'";
		$db->executar($sql);
		$db->commit();

		if($dados['sis']) $_SESSION['sisindigena2'][$dados['sis']]['uncid'] = $identificacaousuario['uncid'];
		
	} elseif($identificacaousuario['picid']) {
		
		$sql = "SELECT * FROM sisindigena2.pactoidadecerta WHERE picid=".$identificacaousuario['picid'];
		$pactoidadecerta = $db->pegaLinha($sql);
		...
		$sql = "SELECT uncid FROM sisindigena2.abrangencia a 
				INNER JOIN sisindigena2.estruturacurso e ON e.ecuid = a.ecuid 
		...
		if($uncid) {
			$db->executar("UPDATE sisindigena2.identificacaousuario SET uncid='".$uncid."' WHERE iusd='".$identificacaousuario['iusd']."'");
			$db->executar("UPDATE sisindigena2.usuarioresponsabilidade SET uncid='".$uncid."' WHERE usucpf='".$_SESSION['usucpf']."' AND pflcod='".$identificacaousuario['pflcod']."' AND rpustatus='A'");
			$db->commit();
		}
		
		if($dados['sis']) $_SESSION['sisindigena2'][$dados['sis']]['uncid'] = $uncid;
...
function criarDocumentosPagamentos($dados) {
	...
	$pagamentos = $db->carregar("SELECT p.pboid, pf.pfldsc, i.iuscpf, i.iusnome, f.fpbmesreferencia, f.fpbanoreferencia FROM sisindigena2.pagamentobolsista p 
	...
	if($pagamentos[0]) {
		foreach($pagamentos as $arrInfo) {
			...
			$db->executar("UPDATE sisindigena2.pagamentobolsista SET docid='".$docid."' WHERE pboid='".$arrInfo['pboid']."'");
		}
...
function atualizarNomeUsuario($dados) {
	...
	if($obj['PESSOA']->no_pessoa_rf) {
		$db->executar("UPDATE sisindigena2.identificacaousuario SET iusnome='".$obj['PESSOA']->no_pessoa_rf."' WHERE iuscpf='".$dados['iuscpf']."'");
		$db->executar("UPDATE seguranca.usuario SET usunome='".$obj['PESSOA']->no_pessoa_rf."' WHERE usucpf='".$dados['iuscpf']."'");
		$db->commit();
	}
	
	$al = array("alert"=>"Nome atualizado com sucesso","location"=>"sisindigena2.php?modulo=".$_REQUEST['modulo']."&acao=A&aba=dados");
	alertlocation($al);
...
function aprovarTrocaNomesSGB($dados) {
	...
	if($dados['cpf']) {
		foreach($dados['cpf'] as $cpf) {
			if($dados['nome_receita'][$cpf]) {
				$sql = "UPDATE sisindigena2.identificacaousuario SET iusnome='".$dados['nome_receita'][$cpf]."' WHERE iuscpf='".str_replace(array(".","-"),array("",""),$cpf)."'";
				$db->executar($sql);
			}	
		}
		$db->commit();
	...
	$al = array("alert"=>"Trocas realizadas com sucesso","location"=>"sisindigena2.php?modulo=".$_REQUEST['modulo']."&acao=A&aba=aprovarnomes");
	alertlocation($al);
...
function reenviarPagamentos($dados) {
	...
	if($dados['doc']) {
		foreach($dados['doc'] as $docid) {
			...
			if($aedid) {
				...
				$db->executar("UPDATE sisindigena2.pagamentobolsista SEt remid=null WHERE docid='".$docid."'");
				$db->commit();
			}
	...
	$al = array("alert"=>"Reenvio agendado com sucesso","location"=>"sisindigena2.php?modulo=principal/mec/mec&acao=A&aba=reenviarpagamentos");
	alertlocation($al);
...
function excluirAvaliacoesMensario($dados) {
	...
	$db->executar("DELETE FROM sisindigena2.historicoreaberturanota WHERE mavid='".$dados['mavid']."'");
	$db->executar("DELETE FROM sisindigena2.mensarioavaliacoes WHERE mavid='".$dados['mavid']."'");
	
	$db->commit();
	
	$al = array("alert"=>"Avaliação apagada","location"=>"sisindigena2.php?modulo=".$dados['modulo']."&acao=A&aba=".$dados['aba']."&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function carregarInformes($dados) {
	...
	$informes = $db->carregar("SELECT inpdescricao, to_char(inpdatainserida,'dd/mm/YYYY HH24:MI') as inpdatainserida FROM sisindigena2.informespacto WHERE pflcoddestino='".$dados['pflcoddestino']."' AND inpstatus='A'");
...
function posDevolverCoordenadorIES() {
	...
	if($dadosVerificacao['picid'] && $_SESSION['sisindigena2']['coordenadoradjuntoies']) {
		$iusd   = $_SESSION['sisindigena2']['coordenadoradjuntoies']['iusd'];
		$pflcod = PFL_COORDENADORADJUNTOIES; 
		$docid  = $_SESSION['sisindigena2']['coordenadoradjuntoies']['docid'];
	}
	
	if($dadosVerificacao['picid'] && $_SESSION['sisindigena2']['supervisories']) {
		$iusd   = $_SESSION['sisindigena2']['supervisories']['iusd'];
		$pflcod = PFL_SUPERVISORIES;
		$docid  = $_SESSION['sisindigena2']['supervisories']['docid'];
	}
	
	if($dadosVerificacao['uncid']) {
		$iusd   = $_SESSION['sisindigena2']['universidade']['iusd'];
		$pflcod = PFL_COORDENADORIES;
		$docid  = $_SESSION['sisindigena2']['universidade']['docid'];
	}
	...
	if(!strstr($_SERVER['HTTP_HOST'],"simec-local")) {
		...
		$sql = "SELECT i.iusnome, i.iusemailprincipal FROM sisindigena2.identificacaousuario i 
				INNER JOIN sisindigena2.tipoperfil t  ON t.iusd = i.iusd AND t.pflcod='".$pflcod."'
				WHERE i.iusd='".$iusd."'";
...
function exibirCadastramentoCoordenadorAdjunto($dados) {
	...
	<table class="tabela" bgcolor="#f5f5f5" cellSpacing="1" cellPadding="3" align="center">
		...
		<tr>
			...
			<td>
			<? 
	 		$sql = "SELECT * FROM sisindigena2.eixospedagogicos WHERE expstatus='A'";
			...
			<?
			$eixospedagogicosuniversidade = $db->pegaLinha("SELECT epuid, eprobs FROM sisindigena2.eixospedagogicosnucleo WHERE expid='".$ep['expid']."' AND picid='".$dados['picid']."'");
		...
		$nucleouniversidade = $db->pegaLinha("SELECT picmetodologiaaplicada, 
											  ...
											  FROM sisindigena2.nucleouniversidade 
	...
	$sql = "SELECT CASE WHEN i.iuscpf ~ '^[0-9]*.?[0-9]*$' THEN replace(to_char(i.iuscpf::numeric, '000:000:000-00'), ':', '.') ELSE i.iuscpf END as iuscpf, 
			...
			FROM sisindigena2.identificacaousuario i 
			INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN sisindigena2.redeterritorios r ON r.retid = i.retid 
...
function verificarValidacao($dados) {
	...
	if(!$dados['docid']) {
		$al = array("alert"=>"Fluxo não foi identificado. Tente novamente!","location"=>"sisindigena2.php?modulo=inicio&acao=C");
		alertlocation($al);
	}
...
function condicaoRetornarMEC() {
	...
	if($_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']) {
		$sql = "SELECT count(*) as numero FROM sisindigena2.folhapagamentouniversidade WHERE picid='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']."'";
	...
	if($_SESSION['sisindigena2']['supervisories']['picid']) {
		$sql = "SELECT count(*) as numero FROM sisindigena2.folhapagamentouniversidade WHERE picid='".$_SESSION['sisindigena2']['supervisories']['picid']."'";
		$numero = $db->pegaUm($sql);
	...
	if($_SESSION['sisindigena2']['universidade']['uncid']) {
		$sql = "SELECT count(*) as numero FROM sisindigena2.folhapagamentouniversidade WHERE uncid='".$_SESSION['sisindigena2']['universidade']['uncid']."'";
		$numero = $db->pegaUm($sql);
...
function inserirDocumento($dados) {
	...
	$campos = array("picid"   => (($_SESSION['sisindigena2']['coordenadoradjuntoies']['picid'])?"'".$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']."'":"NULL"),
					"uncid"   => (($_SESSION['sisindigena2']['universidade']['uncid'])?"'".$_SESSION['sisindigena2']['universidade']['uncid']."'":"NULL"),
	...
	$al = array("alert" => "Documento salvo com sucesso","location" => "sisindigena2.php?modulo=".$dados['modulo']."&acao=A&aba=documentos");
	alertlocation($al);
...
function excluirDocumento($dados) {
	...
	$db->executar("DELETE FROM sisindigena2.documentos WHERE domid='".$dados['domid']."'");
	$db->commit();
	
	$al = array("alert" => "Documento excluído com sucesso","location" => "sisindigena2.php?modulo=principal/coordenadoradjuntoies/coordenadoradjuntoiesexecucao&acao=A&aba=documentos");
	alertlocation($al);
	
	
}

function importarOrcamentoSisindigena2014($dados) {
	global $db;
	...
	$sql = "INSERT INTO sisindigena2.orcamento(
            ...
            gdeid, orcdescricao FROM sisindigena.orcamento WHERE orcstatus='A' AND uncid='".$_SESSION['sisindigena2']['universidade']['uncid']."'";
	
	$db->executar($sql);
	$db->commit();
}

function importarProjetoPedagogicoSisindigena2014($dados) {
	global $db;
	
	$sql = "INSERT INTO sisindigena2.listaabrangenciaacaonucleo(
            picid, laaid, lanaldeiadsc, arqid, lanescoladsc)
		    SELECT distinct l1.picid, l1.laaid, l1.lanaldeiadsc, l1.arqid, l1.lanescoladsc FROM sisindigena.listaabrangenciaacaonucleo l1 
			LEFT JOIN sisindigena2.listaabrangenciaacaonucleo l2 ON l1.picid = l2.picid AND (l1.laaid = l2.laaid OR l1.lanescoladsc = l2.lanescoladsc) 
			WHERE l1.picid='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']."' AND l2.lanid IS NULL";
	
	$db->executar($sql);
	
	$sql = "INSERT INTO sisindigena2.eixospedagogicosnucleo(
            expid, picid, eprobs)
    SELECT distinct e1.expid, e1.picid, e1.eprobs FROM sisindigena.eixospedagogicosnucleo e1 
	LEFT JOIN sisindigena2.eixospedagogicosnucleo e2 ON e2.picid = e1.picid AND e2.expid = e1.expid 
	WHERE e1.picid='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']."' AND e2.epuid IS NULL";
	
	$db->executar($sql);
	
	$sql = "UPDATE sisindigena2.nucleouniversidade x SET picmetodologiaaplicada=foo.picmetodologiaaplicada,
													   picmetodologiaavaliacao=foo.picmetodologiaavaliacao,
													   picmetodologiaacompanhamento=foo.picmetodologiaacompanhamento,
													   picsituacaosociolinguistica=foo.picsituacaosociolinguistica
			FROM (
			SELECT picid, picmetodologiaaplicada, picmetodologiaavaliacao, picmetodologiaacompanhamento, picsituacaosociolinguistica FROM sisindigena.nucleouniversidade WHERE picid='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']."'
			) foo WHERE x.picid=foo.picid";
	...
	$al = array("alert" => "Projeto Pedagógico foi importado com sucesso","location" => "sisindigena2.php?modulo=principal/coordenadoradjuntoies/coordenadoradjuntoies&acao=A&aba=projetopedagogico");
	alertlocation($al);
	
}

function importarEquipeCASisindigena2014($dados) {
	global $db;
	
	$sql = "SELECT i.iuscpf, i.iusnome, i.iusemailprincipal, i.uncid, i.picid, i.retid, (SELECT pflcod FROM seguranca.perfil WHERE sisid=".SIS_INDIGENA." AND pfldsc=p.pfldsc) as pflcod FROM sisindigena.identificacaousuario i 
			INNER JOIN sisindigena.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod 
			WHERE i.iusstatus='A' AND i.picid='".$_SESSION['sisindigena2']['coordenadoradjuntoies']['picid']."' AND t.pflcod IN(".PFL_CONTEUDISTA2014.",".PFL_SUPERVISORIES2014.",".PFL_FORMADORIES2014.",".PFL_PESQUISADOR2014.",".PFL_COORDENADORLOCAL2014.")";
	
	$arr2014 = $db->carregar($sql);
	
	if($arr2014[0]) {
		foreach($arr2014 as $ar) {

			$existe = $db->pegaUm("SELECT iusd FROM sisindigena2.identificacaousuario WHERE iuscpf='".$ar['iuscpf']."'");
			
			if(!$existe) {

				$sql = "INSERT INTO sisindigena2.identificacaousuario(
	            		picid, iuscpf, iusnome, iusemailprincipal, iusdatainclusao, iusstatus, uncid, retid)
	    				VALUES ('".$ar['picid']."', '".$ar['iuscpf']."', '".$ar['iusnome']."', '".$ar['iusemailprincipal']."', NOW(), 'A', '".$ar['uncid']."', ".(($ar['retid'])?"'".$ar['retid']."'":"NULL").") RETURNING iusd";
				
				$iusd = $db->pegaUm($sql);
				
				$sql = "INSERT INTO sisindigena2.tipoperfil(
			            iusd, pflcod, tpestatus)
					    VALUES ('".$iusd."', '".$ar['pflcod']."', 'A');";
	...
	$al = array("alert" => "Equipe foi importada com sucesso","location" => "sisindigena2.php?modulo=principal/coordenadoradjuntoies/coordenadoradjuntoies&acao=A&aba=definirequipe");
	alertlocation($al);
	
}

function importarEquipeSUSisindigena2014($dados) {
	global $db;
	
	
	$sql = "(
						
			SELECT i2.iuscpf as sup, i.iuscpf, i.iusnome, i.iusemailprincipal, i.uncid, i.picid, i.retid, (SELECT pflcod FROM seguranca.perfil WHERE sisid=".SIS_INDIGENA." AND pfldsc=p.pfldsc) as pflcod
			...
			WHERE t.pflcod='".PFL_ORIENTADORESTUDO2014."' AND i.picid='".$_SESSION['sisindigena2']['supervisories']['picid']."' AND i2.iuscpf='".$_SESSION['sisindigena2']['supervisories']['iuscpf']."'
			) UNION ALL (
			...
			WHERE t.pflcod='".PFL_PROFESSORALFABETIZADOR2014."' AND i.picid='".$_SESSION['sisindigena2']['supervisories']['picid']."' AND i3.iuscpf='".$_SESSION['sisindigena2']['supervisories']['iuscpf']."' ORDER BY i2.iusnome, i.iusnome
	...
	if($arr2014[0]) {
		foreach($arr2014 as $ar) {
			...
			$iusd = $db->pegaUm("SELECT iusd FROM sisindigena2.identificacaousuario WHERE iuscpf='".$ar['iuscpf']."'");
			...
			if(!$iusd) {
				...
				if($ar['sup']) {
					...
					$iusdsup = $db->pegaUm("SELECT iusd FROM sisindigena2.identificacaousuario WHERE iuscpf='".$ar['sup']."'");
						
					if($iusdsup) {
				
						$turid = $db->pegaUm("SELECT turid FROM sisindigena2.turmas WHERE iusd IN(SELECT iusd FROM sisindigena2.identificacaousuario WHERE iuscpf='".$ar['sup']."')");
				
						if(!$turid) {
							$docid = wf_cadastrarDocumento(TPD_COORDENADORIES,"SIS Indigena Supervisor IES ".$ar['picid']);
							$sql = "INSERT INTO sisindigena2.turmas(
							            		iusd, turdesc, turstatus, picid, docid)
							    				VALUES ('".$iusdsup."', 'Turma ".$iusdsup."', 'A', '".$ar['picid']."', '".$docid."') RETURNING turid;";
							$turid = $db->pegaUm($sql);
				
						}
				
						$exis = $db->pegaUm("SELECT otuid FROM sisindigena2.orientadorturma WHERE iusd='{$iusd}' AND turid='{$turid}'");
				...
				if(!$tpeid) {
					...
					$sql = "UPDATE sisindigena2.identificacaousuario SET iusstatus='A' WHERE iusd='{$iusd}'";
	...
	$al = array("alert" => "Equipe foi importada com sucesso","location" => "sisindigena2.php?modulo=principal/supervisories/supervisories&acao=A&aba=definirequipe");
	alertlocation($al);
