...
function sqlAvaliacaoCoordenadorIES($dados) {
	...
	$sql = "SELECT i.iusd, 
					...
					p.pfldsc, 
					i.picid,
					(SELECT m.docid FROM sisindigena2.mensario m WHERE iusd=i.iusd AND fpbid='".$dados['fpbid']."') as docid,
					CASE WHEN (SELECT esdid FROM sisindigena2.mensario m INNER JOIN workflow.documento d ON d.docid = m.docid AND d.tpdid=".TPD_FLUXOMENSARIO." WHERE iusd=i.iusd AND fpbid='".$dados['fpbid']."') IN('".ESD_ENVIADO_MENSARIO."','".ESD_APROVADO_MENSARIO."') THEN 'TRUE' ELSE 'FALSE' END as mon,
			...
			INNER JOIN sisindigena2.folhapagamentouniversidade fu ON fu.picid = n.picid AND fu.fpbid='".$dados['fpbid']."' AND fu.uncid='".$dados['uncid']."'
...
function sqlAvaliacaoSupervisor($dados) {
	...
	$sql = "(
			...
			WHERE t.pflcod IN('".PFL_ORIENTADORESTUDO."','".PFL_PROFESSORALFABETIZADOR."') AND i.picid='".$dados['picid']."' AND 
				  (i.iusd IN(SELECT ot.iusd FROM sisindigena2.orientadorturma ot INNER JOIN sisindigena2.turmas t ON t.turid = ot.turid WHERE t.iusd='".$dados['iusd']."') OR
				   i.iusd IN(SELECT ot.iusd FROM sisindigena2.orientadorturma ot INNER JOIN sisindigena2.turmas t ON t.turid = ot.turid WHERE t.iusd IN(SELECT ot.iusd FROM sisindigena2.orientadorturma ot INNER JOIN sisindigena2.turmas t ON t.turid = ot.turid WHERE t.iusd='".$dados['iusd']."'))
...
function sqlAvaliacaoCoordenadorLocal($dados) {
	...
	$sql = "(
			...
			SELECT  faa.iusd, 
					...
					faa.mon,
					faa.mais,
					(SELECT m.docid FROM sisindigena2.mensario m WHERE iusd=faa.iusd AND fpbid='".$dados['fpbid']."') as docid 
			FROM (
			...
			LEFT JOIN sisindigena2.mensario m ON m.iusd = faa.iusd  AND m.fpbid='".$dados['fpbid']."'
			LEFT JOIN sisindigena2.mensarioavaliacoes ma 
				INNER JOIN sisindigena2.tipoperfil tp ON tp.iusd = ma.iusdavaliador AND tp.pflcod=".PFL_COORDENADORLOCAL."
			ON ma.menid = m.menid
...
function sqlAvaliacaoCoordenadorAdjuntoIES($dados) {
	...
	$sql = "SELECT i.iusd, 
			...
			INNER JOIN sisindigena2.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod 
			INNER JOIN sisindigena2.folhapagamentouniversidade fu ON fu.picid = i.picid AND fu.fpbid='".$dados['fpbid']."' AND fu.picid='".$dados['picid']."'
