...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM ies.tprperfil p
	INNER JOIN ies.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "I": // Instituições 
				$aca_prg = "Instituições Associadas";
				$sqlRespUsuario = "SELECT DISTINCT 
									ie.iescodigo as codigo,
									ie.iesnome as descricao								
								   FROM
								   	ies.ies ie
								   INNER JOIN
								   	ies.usuarioresponsabilidade ur ON ur.iesid = ie.iesid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
			...
			case "E": // Estados
				...
				$sqlRespUsuario = "SELECT DISTINCT 
									e.estuf AS codigo, 
									e.estdescricao||' - '||orgdesc AS descricao, 
									ur.rpustatus AS status
								   FROM 
								    ies.usuarioresponsabilidade ur 
									INNER JOIN territorios.estado e ON e.estuf = ur.estuf
									INNER JOIN ies.orgao o ON o.orgid = ur.orgid
								   WHERE 
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
			...
			case "M": // Municípios
				...
				$sqlRespUsuario = "
					select DISTINCT
						...
						ur.rpustatus aS status
					from ies.usuarioresponsabilidade ur
			...
			case "O": // Órgão
				...
				$sqlRespUsuario = "
					...
					FROM 
						ies.orgao AS o 
					INNER JOIN 
						ies.usuarioresponsabilidade AS ur 
					ON 
						o.orgid = ur.orgid
					WHERE 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM ies.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
