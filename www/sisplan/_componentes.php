...
<script>
		...
		var URL_AJAX = '/<?= $diretorio ?>/ajax.php'
			+ '?atividade=<?= $atividade ?>'
		...
		function capturar_resposta() {
			try {
				if ( requisicao_acao.readyState == 1 ) {
					...
					if( trim( requisicao_acao.responseText ) != '' ) {
...
foreach( $lista as $indice => $atividade ) {
	...
	$obra = atividade_obra( $atividade['atiid'] );
...
<?php if( $permissao && !$tem_filhos ): ?><img align="absmiddle" border="0" src="../imagens/excluir.gif" onclick="excluir_atividade( <?= $atividade['atiid'] ?> )" title="Excluir Atividade" style="cursor:pointer;"/><?php else : ?><img align="absmiddle" src="../imagens/excluir_01.gif"/><?php endif; ?>
<?php if( $obra ): ?><img align="absmiddle" onclick="window.location.href = '/obras/obras.php?modulo=principal/cadastro&acao=A&atiid=<?= $atividade['atiid'] ?>'" title="Avançar" src="../imagens/icone_capacete_laranja.png" style="cursor:pointer;" /><?php endif; ?><a href="/<?= $diretorio ?>/gantt.php?atiid=<?= $atividade['atiid'] ?>" target="_blank"><img align="absmiddle" border="0" src="../imagens/lupa_grafico.gif" title="Exibir Gráfico" style="cursor:pointer; margin-left:5px;" /></a>
</td>
...
function montar_titulo_projeto_sisplan( $atividade = '' ){
	global $titulo_modulo;
	global $db;
	// pega o projeto
	$sql = sprintf( "select atiid, atidescricao, atipropameta, atiorcamentocusteio+atiorcamentocapital as valorglobal, atidatainicio, atidatafim, usunome, atiobra  from pde.atividade a inner join seguranca.usuario u ON u.usucpf = a.usucpfcoordenacao where atiid = %d", $_SESSION['projeto'] );
	$projeto = $db->pegaLinha( $sql );
