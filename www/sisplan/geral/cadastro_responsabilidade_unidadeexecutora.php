...
if(is_array($_POST['ususecresp']) && @count($_POST['ususecresp'])>0) {
	...
	$sqlSelResp = "SELECT ur.rpuid, ur.usucpf, ur.rpustatus, s.uexcod || ' - ' || s.uexdsc || ' - ' || s.uexsigla as descricao, s.uexid, u.usunome FROM sisplan.usuarioresponsabilidade ur 
		INNER JOIN planointerno.unidadeexecutora_uo s ON s.uexid = ur.uexi
		INNER JOIN usuario u on ur.usucpf=u.usucpf 
		WHERE ur.rpustatus = 'A' AND ur.usucpf <> '".$usucpf."' AND ur.pflcod = ".$pflcod;

	$sqlSelItem	= "SELECT s.uexid FROM planointerno.unidadeexecutora_uo s 
		WHERE s.uexid = '%s'";

	$sqlInsRpu = "INSERT INTO sisplan.usuarioresponsabilidade (uexid, usucpf, rpustatus, rpudata_inc, pflcod) VALUES ('%s', '%s', '%s', '%s', '%s')";

	$sqlUpdRpu = "UPDATE sisplan.usuarioresponsabilidade SET rpustatus = 'I' WHERE uexid = '%s' AND pflcod = ".$pflcod;
...
<title>Atribuir Unidades Executoras</title>
...
<thead><tr>
<td valign="top" class="title" style="border-right: 1px solid #c0c0c0; border-bottom: 1px solid #c0c0c0; border-left: 1px solid #ffffff;" colspan="2"><strong>Selecione a(s) Unidade(s) Executora(s)</strong></td>
</tr>
<?php
$sql = "select s.uexid as codigo, s.uexcod|| ' - ' || s.uexdsc || ' - ' || s.uexsigla as descricao from planointerno.unidadeexecutora_uo s order by 2";
...
<?
$sql = "select distinct s.uexid as codigo, s.uexcod|| ' - ' || s.uexdsc || ' - ' || s.uexsigla as descricao from sisplan.usuarioresponsabilidade u inner join planointerno.unidadeexecutora_uo s on s.uexid=u.uexid where rpustatus='A' and usucpf = '$usucpf' and u.pflcod=$pflcod";
...
{
	$sql = "select distinct s.uexid as codigo, s.uexcod|| ' - ' || s.uexdsc || ' - ' || s.uexsigla as descricao from planointerno.unidadeexecutora_uo s inner join sisplan.unidadeexecutoraproposta p on p.uexid=s.uexid and p.usucpf='".$usucpf."' where p.uexid is not null";
	$dados = $db->carregar($sql);
	if ( is_array($dados) && count( $dados ) )
	{
		foreach( $dados as $linha ):
	...
	{
		?>
		<option value="">Selecione a(s) Secretaria(s).</option>
		<?php
	}
}
?>
...
function retorna(objeto)
{
	...
	if (document.getElementById( 'cod_'+objeto ).checked == true){
		...
		for(var i=0; i<=campoSelect.length-1; i++){
			...
			if (!campoSelect.options[0]){campoSelect.options[0] = new Option('Selecione a(s) Unidades(s) Executora(s).', '', false, false);}
			sortSelect(campoSelect);
	}
}
</script>