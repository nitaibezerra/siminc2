...
$sqlResponsabilidadesPerfil = "
	SELECT
		tr.*
	FROM sisplan.tprperfil p
		INNER JOIN sisplan.tiporesponsabilidade tr ON
			p.tprcod = tr.tprcod
	WHERE
		p.pflcod = '%s'
	ORDER BY
...
";
			break;
			case "Unidade Executora": // Unidade Executora
				$sqlRespUsuario = "
					SELECT
						a.uexid,
						a.uexcod || ' - ' || a.uexdsc || ' - ' || a.uexsigla AS descricao
					FROM sisplan.usuarioresponsabilidade u
						INNER JOIN planointerno.unidadeexecutora a ON
							a.uexid = u.uexid
					WHERE
						u.usucpf    = '%s' AND
						u.pflcod    = '%s' AND
					...
					ORDER BY 2";
			break;	
			case "Ação PPA": // Ação PPa
				$sqlRespUsuario = "
					SELECT
						a.acacod as codigo, 
						a.acacod || ' - ' || a.acadsc as descricao
					FROM sisplan.usuarioresponsabilidade u 
						INNER JOIN ( SELECT acacod, max(acadsc) as acadsc FROM monitora.acao a 
									WHERE a.acasnrap = false AND a.unicod = '" . UNIDADE_IPHAN . "' 
									GROUP BY acacod ) a ON a.acacod  = u.acacod
					WHERE
						u.usucpf    = '%s' AND
						u.pflcod    = '%s' AND
						u.rpustatus = 'A'
			...
			case "Departamento": // Departamento
				$sqlRespUsuario = "
					SELECT
						d.depid,
						d.depsigla || ' - ' || d.depdsc as descricao
					FROM sisplan.usuarioresponsabilidade u
						INNER JOIN sisplan.departamento d ON
							d.depid = u.depid
					WHERE
						u.usucpf    = '%s' AND
						u.pflcod    = '%s' AND
		...
		{
			print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem itens associados a este Perfil.</font>";
		}
		else
		{
			?>
