...
function listaCoordenacoes(){
	...
	$sql = "SELECT d.cooid, ungabrev || ' - ' || coodsc as coodsc
			...
			INNER JOIN ted.coordenacao d ON d.ungcodconcedente = ug.ungcod
...
function atribuiCoordenacoes($usucpf, $pflcod, $cooids){
	...
	$db->executar("UPDATE ted.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND cooid IS NOT NULL");

	if ($cooids[0]){
		foreach($cooids as $cooid) {
			$dadosur = $db->carregar("SELECT * FROM ted.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND prsano = '".$_SESSION['exercicio']."' AND cooid = ". $cooid );
			if($dadosur) {
				...
				$ungcod = $db->pegaUm("SELECT ungcodconcedente FROM ted.coordenacao WHERE cooid = ". $cooid);
				...
				$db->executar("INSERT INTO ted.usuarioresponsabilidade(
            				   rpuid, pflcod, usucpf, cooid, rpustatus, rpudata_inc, unicod, ungcod, prsano )
    						   VALUES ((select max(rpuid) from ted.usuarioresponsabilidade), '". $pflcod ."', '". $usucpf ."', ".$cooid.", 'A', NOW(), '".$uo."', '".$ungcod."', '".$_SESSION['exercicio']."');");
			}
		}
	}
	$db->commit();
...
function buscaUgsAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
			...
			inner join ted.coordenacao d ON d.cooid = ur.cooid
