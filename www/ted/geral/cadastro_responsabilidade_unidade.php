...
if(is_array($_POST['usuunidresp']) && @count($_POST['usuunidresp'])>0) {
	...
	$sqlSelResp = "SELECT distinct ur.rpuid, ur.usucpf, ur.rpustatus, u.unidsc, u.unicod, u.unicod 
				   ...
				   WHERE ur.rpustatus = 'A' AND ur.usucpf <> '" . $usucpf . "' AND ur.ungcod IS NULL";
	
	$sqlSelUnidade	= "SELECT u.unicod, u.unicod FROM unidade u WHERE u.unicod = '%s' ";
	
	$sqlInsRpu = "INSERT INTO ted.usuarioresponsabilidade (rpuid, unicod, usucpf, rpustatus, rpudata_inc, pflcod,prsano) VALUES ((select max(rpuid) from ted.usuarioresponsabilidade),'%s', '%s', '%s', '%s', '%s','".$_SESSION['exercicio']."')";

	$sqlUpdRpu = "UPDATE ted.usuarioresponsabilidade SET rpustatus = 'I' WHERE ungcod IS NULL AND unicod = '%s' AND pflcod = ".$pflcod;

	$sqlUpdRpuUsu = "UPDATE ted.usuarioresponsabilidade SET rpustatus = 'I' WHERE ungcod IS NULL AND usucpf = '".$usucpf."' AND pflcod = ".$pflcod;
...
<?
	  ...
	  for ($i=0; $i<=$nlinhas;$i++)
		 {
		 	if($RS[0]) foreach($RS[$i] as $k=>$v) ${$k}=$v;
...
<?
$sql = "select distinct u.unicod as codigo, u.unicod||' - '||u.unidsc as descricao 
		from ted.usuarioresponsabilidade ur inner join unidade u on ur.unicod=u.unicod 
