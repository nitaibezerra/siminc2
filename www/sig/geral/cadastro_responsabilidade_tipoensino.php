...
function listaTipoEnsino(){
	...
	for ($i = 0; $i < $count; $i++){
		$codigo    = $tipoensinoExistentes[$i]["orgid"];
		$descricao = $tipoensinoExistentes[$i]["orgdesc"];
		if (fmod($i,2) == 0){ 
			$cor = '#f4f4f4';
		} else {
			$cor='#e0e0e0';
...
function atribuiTipoEnsino($usucpf, $pflcod, $tipoensino){
	...
	if ($tipoensino[0]){
		foreach($tipoensino as $tipo) {
			$dadosur = $db->carregar("SELECT * FROM sig.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND orgid = '". $tipo ."'");
			if($dadosur) {
				// Se existir registro atualizar para ativo
				$db->carregar("UPDATE sig.usuarioresponsabilidade
   							   SET rpustatus = 'A', rpudata_inc= NOW()
 							   WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND orgid = '". $tipo ."'");
			} else {
				// Se não existir, inserir novo
				$db->executar("INSERT INTO sig.usuarioresponsabilidade(
            				   pflcod, usucpf, orgid, rpustatus, rpudata_inc)
...
function buscaTipoEnsinoAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
				tpe.orgid AS codigo, 
				tpe.orgdesc AS descricao 
			FROM 
				sig.usuarioresponsabilidade ur 
			INNER JOIN 
				academico.orgao tpe ON ur.orgid = tpe.orgid 
			WHERE 
