...
if ($_REQUEST['unidaderesp']){
	$uoresp = $_REQUEST['unidaderesp'];
	atribuiUnidade($usucpf, $pflcod, $uoresp);
}
...
function listaUnidade(){
	global $_tipoentidade;
	$db = new cls_banco();
	foreach($_tipoentidade as $t) {
		if(is_array($t)) {
			$sqlf[] = "funid IN('".implode("','",$t)."')";
		} else {
			$sqlf[] = "funid IN('".$t."')";
		}
	}
	$sql = "SELECT e.entid, e.entnome
			FROM entidade.entidade e
			INNER JOIN entidade.funcaoentidade ef ON ef.entid = e.entid
			WHERE ".implode(" OR ",$sqlf)."
			ORDER BY entnome";
	
	// SQL para buscar estados existentes
	$uoExistentes = $db->carregar($sql);
	$count = count($uoExistentes);

	// Monta as TR e TD com as unidades
	for ($i = 0; $i < $count; $i++){
		$codigo    = $uoExistentes[$i]["entid"];
		$descricao = $uoExistentes[$i]["entnome"];
		if (fmod($i,2) == 0){ 
			$cor = '#f4f4f4';
		} else {
			$cor='#e0e0e0';
...
function atribuiUnidade($usucpf, $pflcod, $unidade){
	$db = new cls_banco();
...
function buscaUnidadeAtribuido($usucpf, $pflcod){
	
	$db = new cls_banco();
	$sql = "SELECT DISTINCT 
				ent.entid AS codigo, 
				ent.entnome AS descricao 
			FROM sig.usuarioresponsabilidade ur 
...
<html>
	...
	<body leftmargin="0" topmargin="5" bottommargin="5" marginwidth="0" marginheight="0" bgcolor="#ffffff">
		...
		<div style="overflow:auto; width:496px; height:350px; border:2px solid #ececec; background-color: #ffffff;">
			<form name="formulario">
				<table width="100%" align="center" border="0" cellspacing="0" cellpadding="2" class="listagem" id="tabela">
					...
					<thead>
						<tr>
							<td valign="top" class="title" style="border-right: 1px solid #c0c0c0; border-bottom: 1px solid #c0c0c0; border-left: 1px solid #ffffff;" colspan="3"><strong>Selecione a unidade</strong></td>		
						</tr>
					</thead>
					<?php listaUnidade(); ?>
				</table>
		...
		<form name="formassocia" action="" method="post">
			...
			<select multiple size="8" name="unidaderesp[]" id="estresp" style="width:500px;" class="CampoEstilo">
				<?php 
					buscaUnidadeAtribuido($usucpf, $pflcod);
				?>
			</select>
		</form>
