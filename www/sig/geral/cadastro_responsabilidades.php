...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM sig.tprperfil p
							   INNER JOIN sig.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "U": // Unidades 
				...
				break;
			case "T": // Estados
				$aca_prg = "Tipo(s) de Ensino(s)";
				$sqlRespUsuario = "SELECT DISTINCT 
									t.orgid AS codigo, 
									t.orgdesc AS descricao, 
									ur.rpustatus AS status
								   FROM 
								    sig.usuarioresponsabilidade ur 
									INNER JOIN academico.orgao t ON t.orgid = ur.orgid
								   WHERE 
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM sig.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associados a este Perfil.</font>";
	}
}
$db->close();
exit();
