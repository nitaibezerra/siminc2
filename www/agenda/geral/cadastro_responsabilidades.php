...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM agenda.tprperfil p
							   INNER JOIN agenda.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {	
			case "A": // Agenda
				$aca_prg = "Agendas Associadas";
				$sqlRespUsuario = "SELECT 
									a.agdid AS codigo, 
									a.agddsc AS descricao
								   FROM agenda.usuarioresponsabilidade u 
								   INNER JOIN agenda.agenda a ON a.agdid = u.agdid
								   WHERE 
								   	u.usucpf = '%s' AND 
								   	u.pflcod = '%s' AND 
								   	u.rpustatus='A' 
			...
			case "V": // Agenda
				...
				$sqlRespUsuario = "SELECT 
								   ...
								   FROM agenda.usuarioresponsabilidade u 
								   INNER JOIN agenda.areaenvolvida a ON a.aevid = u.aevid
								   WHERE 
								   	u.usucpf = '%s' AND 
								   	u.pflcod = '%s' AND 
								   	u.rpustatus='A' 
