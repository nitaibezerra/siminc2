...
if(is_array($_POST['usuprogresp']) && @count($_POST['usuprogresp'])>0) {
	...
	foreach ($_POST['usuprogresp'] as $respcod) {
		...
		if ($respcod>0){
			$agdid = $respcod; 		
			$dados = array($agdid, $usucpf, $pflcod); 						
...
<thead>
	<tr>
		<td valign="top" class="title" style="border-right: 1px solid #c0c0c0; border-bottom: 1px solid #c0c0c0; border-left: 1px solid #ffffff;" colspan="3"><strong>Selecione a(s) Agenda(s)</strong></td>
	</tr>
   	<tr bgcolor="<?=$cor?>">
...
<?
	  ...
	  $cabecalho = 'Selecione a(s) Agenda(s)';
	  $sql = "SELECT 
	  			agdid, 
	  			agddsc 
	  		  FROM 
	   ...
	   ?>
		   		...
		   		<tr bgcolor="<?=$cor?>">
					<td align="center" width="10">
						<input type="Checkbox" name="prgid" id="<?=$agdid?>" value="<?=$agdid?>" onclick="retorna(<?=$i?>);">
						<input type="Hidden" name="prgdsc" value="<?=$agddsc?>">
					</td>
					<td align="justify" ><?=$agddsc?></td>
				</tr>
...
$sql = "SELECT DISTINCT 
		 a.agdid as codigo, 
		 a.agddsc AS descricao 
		FROM 
		 agenda.usuarioresponsabilidade ur 
		 INNER JOIN agenda.agenda a ON ur.agdid=a.agdid 
		WHERE 
		 ur.rpustatus='A' AND 
...
?>
	<option value="">Selecione a(s) Agenda(s).</option>
<?
}
?>
</select>
...
function retorna(objeto)
{
	...
	if (document.formulario.prgid[objeto].checked == true){
		...
		for(var i=0; i<=campoSelect.length-1; i++){
			...
			if (!campoSelect.options[0]){campoSelect.options[0] = new Option('Selecione a(s) Agenda(s).', '', false, false);}
			sortSelect(campoSelect);
	}
}
