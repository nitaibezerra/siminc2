...
function listaCoordenacoes(){
	...
	$coordenacoesExistentes = $db->carregar("SELECT	
												c.coonid, c.coodsc
								 			 FROM 
								 			 	conjur.coordenacao c
								 			 INNER JOIN conjur.perfilunidade p ON p.coonid = c.coonid AND p.pflcod = $pflcod
											 ORDER BY coonid");
...
function atribuiCoordenacoes($usucpf, $pflcod, $coonid){
	...
	$db->executar("UPDATE conjur.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."'");
	if ($coonid[0]){
		foreach($coonid as $coid) {
			$dadosur = $db->carregar("SELECT * FROM conjur.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND coonid = '". $coid ."'");
			if($dadosur) {
				// Se existir registro atualizar para ativo
				$db->carregar("UPDATE conjur.usuarioresponsabilidade
   							   SET rpustatus = 'A', rpudata_inc= NOW()
 							   WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND coonid = '". $coid ."'");
			} else {
				// Se não existir, inserir novo
				$db->executar("INSERT INTO conjur.usuarioresponsabilidade(
...
function buscaCoordenacoesAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT DISTINCT 
			...
			FROM conjur.usuarioresponsabilidade ur 
			INNER JOIN conjur.coordenacao cor ON ur.coonid = cor.coonid 
