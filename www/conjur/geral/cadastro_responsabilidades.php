...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM conjur.tprperfil p
	INNER JOIN conjur.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "C": // Unidades 
				$aca_prg = "Coordenações Associadas";
				$sqlRespUsuario = "SELECT c.coonid AS codigo, c.coodsc AS descricao, ur.rpustatus AS status
					FROM conjur.usuarioresponsabilidade ur 
					INNER JOIN conjur.coordenacao c ON c.coonid = ur.coonid
