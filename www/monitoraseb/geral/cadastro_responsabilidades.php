...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM monitoraseb.tprperfil p
	INNER JOIN monitoraseb.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "C": // Unidades 
				...
				$sqlRespUsuario = "SELECT c.coonid AS codigo, c.coosigla || ' - ' || c.coodsc AS descricao, ur.rpustatus AS status
					FROM monitoraseb.usuarioresponsabilidade ur 
					INNER JOIN monitoraseb.coordenacao c ON c.coonid = ur.coonid
