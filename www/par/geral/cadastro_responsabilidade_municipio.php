	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $muncod){
			...
			$at_entid = "SELECT MAX(ent.entid) FROM entidade.entidade ent 
						 ...
						 WHERE fen.funid=1 AND entnumcpfcnpj IN(SELECT iuecnpj FROM par.instrumentounidadeentidade ie INNER JOIN par.instrumentounidade iu ON iu.inuid = ie.inuid WHERE iu.muncod='$muncod' AND iu.itrid=2)";
			
			$sql = "INSERT INTO par.usuarioresponsabilidade (muncod, usucpf, rpustatus, rpudata_inc, pflcod, entid) 
																   VALUES ('$muncod', '$usucpf', 'A',  now(), '$pflcod', ($at_entid))";
			$db->executar($sql);
		}		
	}
...
<form name="formassocia" style="margin: 0px;" method="POST"><input
	...
	$sql = "
			...
			from 
				par.usuarioresponsabilidade ur inner join territorios.municipio m on ur.muncod = m.muncod
