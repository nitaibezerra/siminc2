...
function listaUnidades(){
	global $db, $funid, $estuf, $muncod, $orgid, $iueid;
	...
	$sql = "SELECT DISTINCT 
			...
			FROM
				par.instrumentounidadeentidade iue
			INNER JOIN 
				par.usuarioresponsabilidade ur ON iue.iueid = ur.iueid AND ur.rpustatus = 'A'
	...
	for ($i = 0; $i < $count; $i++){
		$codigo    = $unidadesExistentes[$i]["iueid"];
		$descricao = $unidadesExistentes[$i]["nome"];
		$check = "";
		$orgid = 3;
 ...
 * @param string $unicod
 */
function atribuiUnidade($usucpf, $pflcod, $iueid, $orgid = 'null'){

	global $db;
	
	$data = date("Y-m-d H:i:s");
	
	$sql_zera = $db->executar("UPDATE par.usuarioresponsabilidade SET rpustatus = 'I' 
							   WHERE 
									usucpf = '{$usucpf}' AND 
									pflcod = '{$pflcod}' AND  
									iueid IS NOT NULL");
	
	if (is_array($iueid) && !empty($iueid[0])){
		$count = count($iueid);
		
		// Insere a nova unidade
		$sql_insert = "INSERT INTO par.usuarioresponsabilidade(iueid, usucpf, rpustatus, rpudata_inc, pflcod ) VALUES ";
		
		for ($i = 0; $i < $count; $i++){
			
			list($orgid,$entidade) = explode("|", $iueid[$i]);
...
function buscaUnidadesCadastradas($usucpf, $pflcod){
	...
	if (!$_POST['gravar'] && $_REQUEST["uniresp"][0]){
		foreach ($_REQUEST["uniresp"] as $v){
			list(,$iueid[]) = explode('|', $v );
		}
		$where = " iue.iueid IN (".implode(',',$iueid).") ";
	}else{
	...
	if ( !$perfilSuperUser ){
		...
		$sql = "select iueid from par.usuarioresponsabilidade where usucpf = '{$_SESSION["usucpf"]}' AND pflcod = {$pflcod};";
		$iueid = $db->pegaUm($sql);
		if($iueid){
			$where = " iue.iueid = $iueid ";
		}
	}
	...
	$sql = "SELECT DISTINCT 
			...
			FROM
				par.instrumentounidadeentidade iue
		    INNER JOIN 
		    	par.usuarioresponsabilidade ur ON iue.iueid = ur.iueid AND
													ur.rpustatus = 'A'
			WHERE 
			 ".$where;
