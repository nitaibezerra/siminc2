	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $estuf){
			...
			$at_entid = "SELECT MAX(ent.entid) FROM entidade.entidade ent 
						 ...
						 WHERE fen.funid=6 AND entnumcpfcnpj IN(SELECT iuecnpj FROM par.instrumentounidadeentidade ie INNER JOIN par.instrumentounidade iu ON iu.inuid = ie.inuid WHERE estuf='$estuf' AND iu.itrid=1)";
						 
			$sql = "INSERT INTO par.usuarioresponsabilidade (estuf, usucpf, rpustatus, rpudata_inc, pflcod, entid) 
