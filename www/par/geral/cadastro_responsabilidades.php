...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM par.tprperfil p
	INNER JOIN par.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			...
			case "U": // Unidades
				...
				$sqlRespUsuario = 	"SELECT iue.entid as codigo, iue.iuenome as descricao, ur.rpustatus AS status
									...
									INNER JOIN par.usuarioresponsabilidade ur ON iue.entid = ur.entid
								   	WHERE ur.usucpf = '%s' AND ur.pflcod = '%s' AND ur.rpustatus = 'A'				
			...
			case "X": // Executora
				...
				$sqlRespUsuario = 	"SELECT iue.iuecnpj as codigo, iue.iuenome as descricao, ur.rpustatus AS status
								  	FROM par.instrumentounidadeentidade iue
									INNER JOIN par.usuarioresponsabilidade ur ON iue.iueid = ur.iueid
								   	WHERE ur.usucpf = '%s' AND ur.pflcod = '%s' AND ur.rpustatus = 'A'				
								   	ORDER BY iue.iuenome";
				break;
			case "P": //Programas
				...
				$sqlRespUsuario = "SELECT DISTINCT pr.prgid AS codigo, pr.prgdsc AS descricao, ur.rpustatus AS status
				   		   		   FROM par.usuarioresponsabilidade ur
				   		   		   INNER JOIN par.programa pr ON pr.prgid = ur.prgid
				   		   		   INNER JOIN par.pfadesao pf ON ur.prgid = pf.prgid			
								   WHERE ur.usucpf = '%s' AND ur.pflcod = '%s' AND ur.rpustatus='A'";			
				break;
		}
