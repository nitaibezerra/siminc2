<?php
    ...
    $sqlSelecionados = "
		SELECT mun.muncod FROM par.propostasubacaomunicipio pps
						INNER JOIN territorios.municipio mun ON mun.muncod = pps.muncod
						WHERE ppsid = {$ppsid} order by mun.estuf, mundescricao";
    
    $result = $db->carregar($sqlSelecionados);
    
    $comFiltro = ($_REQUEST['button']['filtrar']) ? true : false;
    ...
    if(count($result) > 0)
    {
	    foreach($result as $k => $v )
	    {
	    	...
	    	$arrMuncodCheck[] 	= $v['muncod'];
	    }
    }    
    
    $arrMuncodCheck = 	($arrMuncodCheck) ? $arrMuncodCheck : Array() ;
    $arrMuncod 		= 	($arrMuncod) ? $arrMuncod : Array();
    ...
    {
	    $whereMuncod = implode($arrMuncod, ', ');
	    $where[] =  "muncod in( {$whereMuncod})";
    }
    elseif( ! $comFiltro )
    {
    	$where[] = "0=1";
        ...
        <tr>
            <td style="background-color:<?=$Cor?>;">
                ...
                <?php 
                	...
                	if(in_array($municipios['codigo'], $arrMuncodCheck))
                	{
