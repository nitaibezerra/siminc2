...
function buscarFotos($tabelacontrole, $filtro, $pagina = 0,$get = null) {
	...
	if($get){
		foreach($_GET as $chave => $valor){
			if($chave != "pagina" && $chave != "arqid" && $chave != "getFiltro" && $chave != "_sisarquivo"){
...
<?
	...
	$total = buscarTotalRegistros($tabela = $_SESSION['imgparams']['tabela'], $filtro = $_SESSION['imgparams']['filtro'], $get);
	$param = "";
	if($get){
		foreach($_GET as $chave => $valor){
			if($chave != "pagina"){
...
?>
			...
			?></p>
			<br />
			<form method="post" action="../../geral/downloadfilesPar.php" target="popup" onsubmit="window.open('', 'popup', 'width=5,height=5');" id="download">
...
//					onClick: false
	...
	<div id="rodape">
	<img src="../_common/img/bplay.jpg" id="bplay" title="Play" onclick="show.paused = false; this.style.display = 'none'; document.getElementById('pause').style.display = 'inline'; show.play(); return false;" border="0"> 
            <img src="../_common/img/bpause.jpg"  onclick="this.style.display = 'none'; document.getElementById('bplay').style.display = 'inline'; show.stop(); show.paused = false; return false;" id="pause" style="display:none;" border="0"> <img src="../_common/img/bprevious.jpg" title="Voltar foto" onclick="show.previous(); document.getElementById('bplay').style.display = 'inline'; document.getElementById('pause').style.display = 'none'; show.paused = true; return false;" border="0"> <img src="../_common/img/bnext.jpg" onclick="show.play(); document.getElementById('bplay').style.display = 'inline'; document.getElementById('pause').style.display = 'none'; show.paused = true; return false;" border="0"> 
            <input type="image" src="../_common/img/bdownload.jpg" title="Baixar todas" onclick="window.open('../../geral/downloadfilesPar.php?enderecoabsolutoarquivo='+show.images[show.image], 'popup', 'width=5,height=5');">
	</div>
