...
function condicaoRetornarIES() {
	...
	$sql = "SELECT jtoid FROM sispacto3.justificativatrocaorientador WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND jtostatus='I'";
...
function removerProfessorAlfabetizador($dados) {
	...
	$sql = "SELECT pboid FROM sispacto3.pagamentobolsista WHERE iusd='".$dados['iusd']."'";
	$pboid = $db->pegaUm($sql);
	...
	$sql = "DELETE FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."' AND pflcod='".PFL_PROFESSORALFABETIZADOR."'";
	$db->executar($sql);
	$sql = "DELETE FROM sispacto3.professoralfabetizadorturma WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);
...
function inserirProfessoresAlfabetizadores($dados) {
	...
	if(!$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid'])
		die("<script>alert('As informações não foram gravadas. Houve perdas de informações internas. Você será direcionado para a tela principal.');window.location='sispacto3.php?modulo=inicio&acao=C';</script>");
		
	$uncid = $db->pegaUm("SELECT i.uncid FROM sispacto3.turmas t INNER JOIN sispacto3.identificacaousuario i ON i.iusd=t.iusd WHERE t.turid='".$dados['turid']."'");
	...
	if($dados['cpf']) {
		foreach($dados['cpf'] as $cpf) {
			...
			$palid = $db->pegaUm("SELECT palid FROM sispacto3.professoresalfabetizadores WHERE cpf='".str_replace(array(".","-"),array(""),$cpf)."'");
			if($palid) $dados['tipo'][$cpf] = 'censo';
			
			$iusd = $db->pegaUm("SELECT iusd FROM sispacto3.identificacaousuario WHERE iuscpf='".str_replace(array(".","-"),array(""),$cpf)."'");
			
			if($iusd) {
				
				$sql = "UPDATE sispacto3.identificacaousuario SET 
						picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."',
						".(($uncid)?"uncid='".$uncid."',":"")."
				...
				$sql = "INSERT INTO sispacto3.identificacaousuario(
					    ...
					    VALUES ('".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."', 
			...
			$sql = "SELECT p.pfldsc, CASE WHEN pa.muncod IS NOT NULL THEN mu.estuf||' / '||mu.mundescricao
										  WHEN pa.estuf IS NOT NULL THEN es.estuf||' / '||es.estdescricao END  as descricao FROM sispacto3.tipoperfil t
					INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod 
					INNER JOIN sispacto3.identificacaousuario i ON i.iusd = t.iusd 
					INNER JOIN sispacto3.pactoidadecerta pa ON pa.picid = i.picid 
			...
			if($pfldsc) {
				...
				$al = array("alert"=>"Caso queira indicar este CPF(".$dados['nome'][$cpf].") como Professor Alfabetizador, é necessário antes remove-lo do perfil(".$pfldsc.",".$descricao.").","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
				alertlocation($al);
			}
			...
			$existe_turma = $db->pegaUm("SELECT otuid FROM sispacto3.professoralfabetizadorturma WHERE iusd='{$iusd}'");
			
			if($existe_turma) {
				
				$sql = "UPDATE sispacto3.professoralfabetizadorturma SET turid='".$dados['turid']."' WHERE iusd='{$iusd}'";
				$db->executar($sql);
				
			} else {
			
				$sql = "INSERT INTO sispacto3.professoralfabetizadorturma(
	...
	if($_FILES['anexoportaria']) {
		foreach($_FILES['anexoportaria']['error'] as $cpf => $erro) {
			...
			if($erro == 0) {
		        ...
		        $file = new FilesSimec( "portarianomeacao", $campos, "sispacto3" );
...
function inserirOrientadoresEstudo($dados) {
	...
	if($dados['picincluirprofessorrede']=="TRUE") {
		...
		$aliid = $db->pegaUm("SELECT aliid FROM sispacto3.alfabetizadoresindicados WHERE alimuncodorigem='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']."'");
		
		if($aliid) {
			
			$sql = "UPDATE sispacto3.alfabetizadoresindicados SET aliestufdestino='".$dados['aliestufdestino']."' WHERE aliid='".$aliid."'";
			$db->executar($sql);
			...
			$sql = "INSERT INTO sispacto3.alfabetizadoresindicados(
	            	alimuncodorigem, aliestufdestino, aliquantidade, alistatus)
	    			VALUES ('".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']."', 
		...
		$db->executar("DELETE FROM sispacto3.alfabetizadoresindicados WHERE alimuncodorigem='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']."'");
	}
	...
	if($dados['cpf']) {
		foreach($dados['cpf'] as $cpf) {
			...
			if($pfldsc) {
				...
				$al = array("alert"=>"Caso queira indicar este CPF(".$dados['nome'][$cpf].") como Orientador de Estudo, é necessário antes remove-lo do perfil(".$pfldsc.",".$descricao.").","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
				alertlocation($al);
			}
...
function carregarCoordenadorLocal($dados) {
	...
	if($dados['muncod']) { // se tiver muncod, simular coordenador municipal
		unset($_SESSION['sispacto3']['coordenadorlocal']['estadual']);
		$arr = $db->pegaLinha("SELECT p.picid, p.docid, m.estuf || ' - ' || m.mundescricao as descricao, p.docidturma FROM sispacto3.pactoidadecerta p INNER JOIN territorios.municipio m ON m.muncod=p.muncod WHERE p.muncod='".$dados['muncod']."'");
		$picid 	    = $arr['picid'];
		$docid 	    = $arr['docid'];
		...
		if($picid) {
			...
			if(in_array(PFL_COORDENADORLOCAL,$perfis)) {
				...
				if(!$dados['iusd']) $dados['iusd'] = $_SESSION['sispacto3']['coordenadorlocal']['iusd'];
			...
			if(!$docid) {
				...
				$db->executar("UPDATE sispacto3.pactoidadecerta SET docid='".$docid."' WHERE picid='".$picid."'");
				$db->commit();
			}
			
			if(!$docidturma) {
				$docidturma = wf_cadastrarDocumento(TPD_FLUXOTURMA,"Sispacto_CoordenadorLocal_turma_estuf_".$dados['estuf']."_muncod_".$dados['muncod']);
				$db->executar("UPDATE sispacto3.pactoidadecerta SET docidturma='".$docidturma."' WHERE picid='".$picid."'");
				$db->commit();
			}
			
			unset($_SESSION['sispacto3']['coordenadorlocal']['municipal']);
			$_SESSION['sispacto3']['esfera'] = 'municipal';
			$_SESSION['sispacto3']['coordenadorlocal']['iusd'] = $infprof['iusd'];
			$_SESSION['sispacto3']['coordenadorlocal']['uncid'] = $infprof['uncid'];
			$_SESSION['sispacto3']['coordenadorlocal']['iuscpf'] = $infprof['iuscpf'];
			$_SESSION['sispacto3']['coordenadorlocal']['municipal'] = array("estuf" => $dados['estuf'],"muncod" => $dados['muncod'],"picid" => $picid,"docid" => $docid,"descricao" => $descricao,"iusd" => $infprof['iusd'],"docidturma" => $docidturma);
		}	
	}elseif($dados['estuf']) { // senão se tiver estuf, simular coordenador estadual
		unset($_SESSION['sispacto3']['coordenadorlocal']['municipal']);
		$arr = $db->pegaLinha("SELECT p.picid, p.docid, e.estuf || ' - ' || e.estdescricao as descricao, p.docidturma FROM sispacto3.pactoidadecerta p INNER JOIN territorios.estado e ON e.estuf=p.estuf WHERE p.estuf='".$dados['estuf']."'");
		$picid 	    = $arr['picid'];
		$docid 	    = $arr['docid'];
		$docidturma = $arr['docidturma'];
		$descricao  = $arr['descricao'];
		...
		if($picid) {
			...
			unset($_SESSION['sispacto3']['coordenadorlocal']['estadual']);
			$_SESSION['sispacto3']['esfera'] = 'estadual';
			$_SESSION['sispacto3']['coordenadorlocal']['iusd']  = $infprof['iusd'];
			$_SESSION['sispacto3']['coordenadorlocal']['uncid'] = $infprof['uncid'];
			$_SESSION['sispacto3']['coordenadorlocal']['iuscpf'] = $infprof['iuscpf'];
			$_SESSION['sispacto3']['coordenadorlocal']['estadual'] = array("estuf" => $dados['estuf'],"picid" => $picid,"docid" => $docid,"descricao" => $descricao,"iusd" => $infprof['iusd'],"docidturma" => $docidturma);
		}	
	}
	
	if($dados['direcionar']) {
		$al = array("location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=principal");
		alertlocation($al);
	}
...
function removerOrientadorEstudo($dados) {
	...
	$sql = "SELECT tpeid FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."'";
	$tpeid = $db->pegaUm($sql);
	...
	if($tpeid) {
		...
		$sql = "SELECT i.iusnome || '( R$ ' || p.pbovlrpagamento || ' )' as pagamentovaga FROM sispacto3.pagamentobolsista p 
	...
	$iusds = $db->carregarColuna("SELECT i2.iusd FROM sispacto3.identificacaousuario i 
								 ...
								 INNER JOIN sispacto3.professoralfabetizadorturma ot ON ot.turid = tu.turid 
	...
	if($iusds) {
		$sql = "UPDATE sispacto3.identificacaousuario SET iusstatus='I' WHERE iusd IN('".implode("','",$iusds)."')";
		$db->executar($sql);
		
		$sql = "DELETE FROM sispacto3.tipoperfil WHERE iusd IN('".implode("','",$iusds)."')";
		$db->executar($sql);
		
		$sql = "DELETE FROM sispacto3.professoralfabetizadorturma WHERE iusd IN('".implode("','",$iusds)."')";
		$db->executar($sql);
	}
	
	// removendo professores
	
	$sql = "DELETE FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."' AND pflcod='".PFL_ORIENTADORESTUDO."'";
	$db->executar($sql);
	...
	$sql = "DELETE FROM sispacto3.orientadorestudoturma WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);
...
function calculaPorcentagemCadastroOrientadores($dados) {
	...
	$orientadores = carregarDadosIdentificacaoUsuario(array("picid"=>$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid'],"pflcod"=>PFL_ORIENTADORESTUDO));
	...
	gerenciarAtividadePacto(array('apadatafim'=>$apadatafim,'apadatainicio'=>$apadatainicio,'apassituacao'=>$apassituacao,'suaid'=>$dados['suaid'],'picid'=>$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']));
...
function calculaPorcentagemCadastroProfessores($dados) {
	...
	$professores = carregarDadosIdentificacaoUsuario(array("picid"=>$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid'],"pflcod"=>PFL_PROFESSORALFABETIZADOR));
	...
	$sql = "SELECT apaid FROM sispacto3.atividadepacto WHERE suaid='".$dados['suaid']."' AND picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."'";
	$apaid = $db->pegaUm($sql);
	...
	if($apaid) {
		...
		$sql = "INSERT INTO sispacto3.atividadepacto(
	            ...
	            apastatus)
			    VALUES ('".$dados['suaid']."', '".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."', 
...
function calculaPorcentagemAutorizarSubstitutos($dados) {
	...
	if($_SESSION['sispacto3']['esfera']=='municipal') {
		$f = "fioesfera='M' AND muncod='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']."'";
	} elseif($_SESSION['sispacto3']['esfera']=='estadual') {
		$f = "fioesfera='E' AND estuf='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['estuf']."'";
	}
	
	$sql = "SELECT COUNT(*) as tot, MAX(fiodata) as apadatafim, MIN(fiodata) as apadatainicio FROM sispacto3.formacaoinicialouvintes WHERE ".$f;
	...
	$sql = "SELECT COUNT(*) as tot FROM sispacto3.formacaoinicialouvintes WHERE fiostatus='I' AND ".$f;
...
function carregarTotalAlfabetizadores($dados) {
	...
	if($dados['estuf']) {
		...
		if($dados['dependencia']=='estadual') {
			...
			$total_indicados = $db->pegaUm("SELECT SUM(aliquantidade) as o 
											FROM sispacto3.alfabetizadoresindicados 
	...
	if($f) {
		$sql = "SELECT total, total_orientadores_a_serem_cadastrados FROM sispacto3.totalalfabetizadores 
...
function inserirJustificativas($dados) {
	...
	if($dados['joecomentario']) {
		foreach($dados['joecomentario'] as $tipo => $comentario) {
			...
			$sql = "SELECT joeid FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND joestatus='A' AND joetipo='".$tipo."'";
			$joeid = $db->pegaUm($sql);
			
			if($joeid) {
				
				$sql = "UPDATE sispacto3.justificativaorientadorestudo
				...
				$sql = "INSERT INTO sispacto3.justificativaorientadorestudo(
			...
			$db->executar("DELETE FROM sispacto3.justificativaorientadortipo WHERE joeid='".$joeid."'");
			
			if($dados['tjuid'][$tipo]) {
				foreach($dados['tjuid'][$tipo] as $tjuid) {
					$sql = "INSERT INTO sispacto3.justificativaorientadortipo(
...
function carregarJustificativasRespostas($dados) {
	...
	$sql = "SELECT joeid, joetipo, joecomentario FROM sispacto3.justificativaorientadorestudo WHERE picid='".$dados['picid']."' AND joestatus='A'";
	...
	if($justificativaorientadorestudo[0]) {
		foreach($justificativaorientadorestudo as $jo) {
			$sql = "SELECT tjuid FROM sispacto3.justificativaorientadortipo WHERE joeid='".$jo['joeid']."'";
...
function inserirAnexoPrincipalCoordenadorLocal($dados) {
    ...
    if ( $_FILES['arquivo']['error'] == 0 ) {
        ...
        $file = new FilesSimec( "documentoatividade", $campos, "sispacto3" );
...
function excluirDocumento($dados) {
	...
	$sql = "DELETE FROM sispacto3.documentoatividade WHERE doaid='".$dados['doaid']."'";
	$db->executar($sql);
	$db->commit();
	
	$al = array("alert"=>"Documento excluído com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/inseriranexos&acao=A&apaid=".$dados['apaid']);
	alertlocation($al);
...
function atualizarSelecaoPublica($dados) {
	...
	$sql = "UPDATE sispacto3.pactoidadecerta SET picselecaopublica=".$dados['picselecaopublica']." WHERE picid='".$dados['picid']."'";
	$db->executar($sql);
	
	if($dados['picselecaopublica']=="FALSE") {
		$iusds = $db->carregarColuna("SELECT i.iusd FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON i.iusd=t.iusd WHERE i.picid='".$dados['picid']."' AND t.pflcod='".PFL_ORIENTADORESTUDO."'");
		if($iusds) {
			$sql = "DELETE FROM sispacto3.tipoperfil WHERE iusd in('".implode("','",$iusds)."')";
			$db->executar($sql);
			$sql = "UPDATE sispacto3.identificacaousuario SET iusstatus='I' WHERE iusd in('".implode("','",$iusds)."')";
		...
		$sql = "DELETE FROM sispacto3.justificativaorientadortipo WHERE joeid IN(SELECT joeid FROM sispacto3.justificativaorientadorestudo WHERE picid='".$dados['picid']."')";
		$db->executar($sql);
		$sql = "DELETE FROM sispacto3.justificativaorientadorestudo WHERE picid='".$dados['picid']."'";
		$db->executar($sql);
...
function atualizarInclusaoProfessorRede($dados) {
	...
	$sql = "UPDATE sispacto3.pactoidadecerta SET picincluirprofessorrede=".$dados['picincluirprofessorrede']." WHERE picid='".$dados['picid']."'";
	$db->executar($sql);
...
function verificarPerfilOutrosMunicipios($dados) {
	global $db;
	$sql = "SELECT pp.pfldsc||' cadastrado em : '||m.estuf||' - '||m.mundescricao as texto FROM sispacto3.identificacaousuario i 
...
function validarEnvioAnaliseIES() {
	...
	if(!$_SESSION['sispacto3']['coordenadorlocal']['naoValidarEnvioAnaliseIES']) {
		...
		if($orientadoresestudo) {
			foreach($orientadoresestudo as $oe) {
				if($oe['iustipoorientador']=='profissionaismagisterio' && substr($oe['iuscpf'],0,3)!='SIS') {
					$possuidoc = $db->pegaUm("SELECT COUNT(*) FROM sispacto3.portarianomeacao WHERE iusd='".$oe['iusd']."'");
					if($possuidoc==0) {
		...
		if($totalalfabetizadores['total_orientadores_a_serem_cadastrados'] != $total['orientadorsispacto2014']) {
			$arr = $db->pegaLinha("SELECT joeid, joecomentario FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND joetipo='orientadorsispacto2014' AND joestatus='A'");
			if($arr) $arr2 = $db->carregarColuna("SELECT jotid FROM sispacto3.justificativaorientadortipo WHERE joeid='".$arr['joeid']."' AND juostatus='A'");
			if(count($arr2)==0) {
				$erro .= ' - É necessário marcar o Tipo Justificativa (Orientadores de Estudo do Pacto 2014 recomendados para certificaçãoo);<br>\n';
			}
			if(!$arr['joecomentario']) {
				$erro .= ' - É necessário preencher o comentários adicionais (Orientadores de Estudo do Pacto 2014 recomendados para certificação);<br>\n';
			}
		}
		
		if($totalalfabetizadores['total_orientadores_a_serem_cadastrados'] != ($total['orientadorsispacto2014']+$total['professorsispacto2014'])) {
			$arr = $db->pegaLinha("SELECT joeid, joecomentario FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND joetipo='professorsispacto2014' AND joestatus='A'");
			if($arr) $arr2 = $db->carregarColuna("SELECT jotid FROM sispacto3.justificativaorientadortipo WHERE joeid='".$arr['joeid']."' AND juostatus='A'");
			if(count($arr2)==0) {
				$erro .= ' - É necessário marcar o Tipo Justificativa (Professores Alfabetizadores do Pacto 2014 recomendados para certificação);<br>\n';
			}
			if(!$arr['joecomentario']) {
				$erro .= ' - É necessário preencher o comentários adicionais (Professores Alfabetizadores do Pacto 2014 recomendados para certificação);<br>\n';
			}
		}
		
		if($totalalfabetizadores['total_orientadores_a_serem_cadastrados'] != ($total['tutoresproletramento']+$total['orientadorsispacto2014']+$total['professorsispacto2014'])) {
			$arr = $db->pegaLinha("SELECT joeid, joecomentario FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND joetipo='tutoresproletramento' AND joestatus='A'");
			if($arr) $arr2 = $db->carregarColuna("SELECT jotid FROM sispacto3.justificativaorientadortipo WHERE joeid='".$arr['joeid']."' AND juostatus='A'");
			if(count($arr2)==0) {
		...
		if($totalalfabetizadores['total_orientadores_a_serem_cadastrados'] != ($total['tutoresproletramento']+$total['tutoresredesemproletramento']+$total['orientadorsispacto2014']+$total['professorsispacto2014'])) {
			$arr = $db->pegaLinha("SELECT joeid, joecomentario FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND joetipo='tutoresredesemproletramento' AND joestatus='A'");
			if($arr) $arr2 = $db->carregarColuna("SELECT jotid FROM sispacto3.justificativaorientadortipo WHERE joeid='".$arr['joeid']."' AND juostatus='A'");
			if(count($arr2)==0) {
		...
		if($_SESSION['sispacto3']['esfera']=='estadual') {
			$sql = "SELECT COUNT(*) FROM sispacto3.alfabetizadoresindicados WHERE alistatus='P' AND aliestufdestino='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['estuf']."'";
...
function verificarCoordenadorLocalTermoCompromisso($dados) {
	...
	if($coordlocal['iustermocompromisso']!="t") {
		$al = array("alert"=>"Antes de cadastrar os Orientadores de Estudo, preencha todos os campos obrigatórios da tela “Dados do Coordenador”.","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=dados");
		alertlocation($al);
	}
}
...
function atualizarBotoesOrientadoresEstudo($dados) {
	global $db;
	$sql = "UPDATE sispacto3.pactoidadecerta SET picmostrarbotao".$dados['tipo']."=TRUE WHERE picid='".$dados['picid']."'";
	$db->executar($sql);
	$db->commit();
}
...
function inserirCoordenadorLocalGerenciamento($dados) {
	...
	$sql = "SELECT					  i.iusd,
									  i.picid,
	     							  ...
	     							  END as descricao 
	     						FROM sispacto3.identificacaousuario i 
	     						LEFT JOIN sispacto3.tipoperfil t ON i.iusd=t.iusd 
	     						LEFT JOIN seguranca.perfil p ON p.pflcod=t.pflcod 
	     						LEFT JOIN sispacto3.pactoidadecerta c ON c.picid=i.picid 
	...
	if($dados['uncid']) {
		$at_unc = "uncid='".$dados['uncid']."', ";
		$in_unc[0] = ", uncid";
		$in_unc[1] = ", '".$dados['uncid']."'";
	}
	...
	if($iusd) {
		...
		if($pflcod) {
			if($picid) {
				if($picid!=$dados['picid']) {
		 			$al = array("alert"=>"Este CPF ja possui um perfil (".$identificacaousuario['pfldsc'].",".$identificacaousuario['descricao'].") no sistema e não pode ser cadastrado","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
		 			alertlocation($al);
				}
			} else {
		...
		$sql = "UPDATE sispacto3.identificacaousuario SET {$at_unc}{$at_pic} iusstatus='A', iusemailprincipal='".$dados['iusemailprincipal']."' WHERE iusd='".$iusd."'";
		$db->executar($sql);
	} else {
    	$sql = "INSERT INTO sispacto3.identificacaousuario(
	            picid, iuscpf, iusnome, iusemailprincipal,  
	            iusdatainclusao, iusstatus {$in_unc[0]})
			    VALUES ('".$dados['picid']."', '".str_replace(array(".","-"),array(""),$dados['iuscpf'])."', '".$dados['iusnome']."', '".$dados['iusemailprincipal']."',  
			            NOW(), 'A' {$in_unc[1]}) returning iusd;";
    	$iusd = $db->pegaUm($sql);
	}
	...
	if($dados['reenviarsenha']=="S") {
		$remetente = array("nome" => "SIMEC - MÓDULO SISPACTO 2014","email" => $dados['iusemailprincipal']);
   	...
   	$adesao = $db->pegaLinha("SELECT * FROM sispacto3.pactoidadecerta WHERE picid='".$dados['picid']."'");
   	
    $existe_usr = $db->pegaUm("select usucpf from sispacto3.usuarioresponsabilidade where usucpf='".str_replace(array(".","-"),array(""),$dados['iuscpf'])."' and pflcod='".PFL_COORDENADORLOCAL."' AND rpustatus='A' ".(($adesao['muncod'])?"AND muncod='".$adesao['muncod']."'":"").(($adesao['estuf'])?"AND estuf='".$adesao['estuf']."'":""));
    ...
    if(!$dados['naoredirecionar']) {
		$al = array("alert"=>"Coordenador Local inserido com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
		alertlocation($al);
    }
...
function verificarSituacaoAdesao($dados) {
	global $db;
	$sql = "SELECT esdid FROM sispacto3.pactoidadecerta p 
...
function alterarStatusIndicacao($dados) {
	...
	$sql = "UPDATE sispacto3.alfabetizadoresindicados SET alistatus='".$dados['status']."' WHERE aliid='".$dados['aliid']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sispacto3.totalalfabetizadores totl SET total_orientadores_a_serem_cadastrados = foo2.total_orientadores 
			FROM (
			       SELECT 
			       CASE 
			       WHEN total = 0 THEN 0
				   ...
				   FROM (
					       SELECT 
					       floor((total+coalesce((SELECT SUM(aliquantidade) FROM sispacto3.alfabetizadoresindicados WHERE alistatus='A' AND aliestufdestino=sigla::character(2)),0))/25) as num,
					       total+coalesce((SELECT SUM(aliquantidade) FROM sispacto3.alfabetizadoresindicados WHERE alistatus='A' AND aliestufdestino=sigla::character(2)),0) as total,
					       talid 
					       FROM sispacto3.totalalfabetizadores
					) foo
			) foo2 WHERE foo2.talid = totl.talid AND totl.sigla::character(2) IN(SELECT aliestufdestino FROM sispacto3.alfabetizadoresindicados WHERE aliid='".$dados['aliid']."') AND totl.dependencia ilike 'ESTADUAL'";
	
	$db->executar($sql);
	
	if($dados['status']=='I') {
		$sql = "DELETE FROM sispacto3.alfabetizadoresindicados WHERE aliid='".$dados['aliid']."'";
		$db->executar($sql);
	}
	
	$db->commit();
	
	$al = array("alert"=>"Situação alterada com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	alertlocation($al);
...
function carregarExecucaoAtividade($dados) {
	...
	$execucao_atividade = $db->pegaLinha("SELECT ROUND(AVG(apassituacao)) as apassituacao, 
											  ...
											  FROM sispacto3.atividadepacto ap 
										  	  INNER JOIN sispacto3.subatividades su ON su.suaid = ap.suaid 
...
function mostrarAbaTurma($dados) {
	...
	$sql = "SELECT esdid FROM workflow.documento WHERE docid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['docid']."'";
	$esdid = $db->pegaUm($sql);
...
function inserirTurmaCoordenadorLocal($dados) {
	global $db;
	$sql = "SELECT turid FROM sispacto3.turmas WHERE iusd='".$dados['iusd']."' AND picid='".$dados['picid']."'";
	$turid = $db->pegaUm($sql);
...
function carregarTurmasOrientadores($dados) {
	...
	$sql = "SELECT 
				...
				(SELECT COUNT(*) FROM sispacto3.professoralfabetizadorturma WHERE turid=t.turid) as nalunos
			FROM sispacto3.turmas t 
			INNER JOIN sispacto3.identificacaousuario i ON i.iusd = t.iusd 
			INNER JOIN sispacto3.tipoperfil tt ON tt.iusd = i.iusd AND tt.pflcod=".PFL_ORIENTADORESTUDO."
...
function gravarJustificativasTroca($dados) {
	...
	if($dados['resp1']) {
		foreach($dados['resp1'] as $iusd => $resp) {
			...
			$sql = "SELECT jtoid FROM sispacto3.justificativatrocaorientador WHERE iusd='".$iusd."' AND picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."'";
			$jtoid = $db->pegaUm($sql);
			
			if(!$jtoid) {
				
				$sql = "INSERT INTO sispacto3.justificativatrocaorientador(picid, iusd) 
						VALUES ('".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."', 
			...
			$sql = "UPDATE sispacto3.justificativatrocaorientador SET jtooutrosdsc=".(($dados['jtooutrosdsc'.$iusd])?"'".$dados['jtooutrosdsc'.$iusd]."'":"NULL").", resp1=".(($up_resp1)?"'".implode(";",$up_resp1)."'":"NULL").", resp2=".(($up_resp2)?"'".implode(";",$up_resp2)."'":"NULL")." WHERE jtoid='".$jtoid."'";
			$db->executar($sql);
	...
	$db->commit();
	$al = array("alert"=>"Justificativas para trocar os Orientadores de Estudo foram salvas com sucesso. Para submeter ao MEC, acesse a aba Resumo Orientadores de Estudo e clique em Enviar para análise da substituição do(s) Orientador(es) pelo MEC.","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	alertlocation($al);
...
function removerJustificativaTroca($dados) {
	global $db;
	$sql = "DELETE FROM sispacto3.justificativatrocaorientador WHERE picid='".$dados['picid']."' AND iusd='".$dados['iusd']."'";
	$db->executar($sql);
	$db->commit();
	
	$al = array("alert"=>"Justificativa para substituição do(s) Orientadores de Estudo removida com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	alertlocation($al);
...
function autorizarTrocaOrientadores($picid) {
	...
	$sql = "SELECT iusd FROM sispacto3.justificativatrocaorientador WHERE picid='".$picid."'";
	$iusds = $db->carregarColuna($sql);
	
	if($iusds) {
		foreach($iusds as $iusd) {
			removerOrientadorEstudo(array("iusd"=>$iusd));
			$sql = "UPDATE sispacto3.justificativatrocaorientador SET jtostatus='A' WHERE picid='".$picid."' AND iusd='".$iusd."'";
			$db->executar($sql);
		}
	}
	
	$sql = "DELETE FROM sispacto3.justificativaorientadortipo WHERE joeid IN(SELECT joeid FROM sispacto3.justificativaorientadorestudo WHERE picid='".$picid."')";
	$db->executar($sql);
	
	$sql = "DELETE FROM sispacto3.justificativaorientadorestudo WHERE picid='".$picid."'";
	$db->executar($sql);
...
function naoAutorizarTrocaOrientadores() {
	...
	$sql = "DELETE FROM sispacto3.justificativatrocaorientador WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND jtostatus='I'";
	$db->executar($sql);
...
function condicaoAnaliseTrocaMEC() {
	...
	$sql = "SELECT jtoid FROM sispacto3.justificativatrocaorientador WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."'";
	$jtoids = $db->carregarColuna($sql);
...
function posEfetuarTrocaOrientadores() {
	...
	$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['redirecionartroca'] = true;
...
function trocarOrientadorEstudoMunicipio($dados) {
	...
	$sql = "SELECT * FROM sispacto3.formacaoinicialouvintes WHERE fioid='".$dados['fioid']."'";
	$fioarr = $db->pegaLinha($sql);
	
	if(!$_SESSION['sispacto3']['coordenadorlocal']['uncid']) {
	 	$al = array("alert"=>"Coordenador Local não vinculado a nenhuma Universidade","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	 	alertlocation($al);
	}
	
	carregarCoordenadorIES(array('uncid'=>$_SESSION['sispacto3']['coordenadorlocal']['uncid']));
	...
	unset($_SESSION['sispacto3']['universidade']);
	
	if($retorno) {
		$sql = "UPDATE sispacto3.identificacaousuario SET iusformacaoinicialorientador=true WHERE iuscpf='".$fioarr['fiocpf']."'";
		$db->executar($sql);
		
		$sql = "UPDATE sispacto3.formacaoinicialouvintes SET fiostatus='I', fiodata=NOW() WHERE fioid='".$dados['fioid']."'";
		$db->executar($sql);
		
		$sql = "DELETE FROM sispacto3.justificativaorientadortipo WHERE joeid IN(SELECT joeid FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."')";
		$db->executar($sql);
		
		$sql = "DELETE FROM sispacto3.justificativaorientadorestudo WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."'";
		$db->executar($sql);
		
		$db->commit();
		
	 	$al = array("alert"=>"Troca efetuada com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	 	alertlocation($al);
	} else {
	 	$al = array("alert"=>"Novo Usuário ja possui atribuções no SISPACTO, por isso não pode ser inserido","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	 	alertlocation($al);
	}
...
function sqlEquipeCoordenadorLocal($dados) {
	...
	$sql = "(
			...
			FROM sispacto3.identificacaousuario i 
			INNER JOIN sispacto3.pactoidadecerta pi ON pi.picid = i.picid  
			INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil pp ON pp.pflcod = t.pflcod 
...
function verificarCenso2014($dados) {
	global $db;
	$sql = "SELECT m.estuf||'/'||m.mundescricao FROM sispacto3.professoresalfabetizadores p INNER JOIN territorios.municipio m ON m.muncod = p.muncod WHERE cpf='".$dados['cpf']."'";
...
function gerenciarMateriais($dados) {
	...
	$sql = "SELECT matid FROM sispacto3.materiais WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."'";
	$matid = $db->pegaUm($sql);
	
	if($matid) {
		
		$sql = "UPDATE sispacto3.materiais
		...
		$sql = "INSERT INTO sispacto3.materiais(
	...
	if($_FILES['arquivo']['error']=='0') {
		...
		$file = new FilesSimec("materiaisfotos", $campos ,"sispacto3");
				
		$arquivoSalvo = $file->setUpload($dados['mafdsc']);
	}
	
	
	$al = array("alert"=>"Informações sobre materiais salvas com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=materiais");
	alertlocation($al);
...
function salvarAtividadeCoordenadorLocal($dados) {
	...
	if($erros) {
		$al = array("alert"=>"Não foi possível concluir o cadastro. Foram identificados ausência de informações no formulario, tente novamente mais tarde, e caso o erro persista, entre em contato com o responsável. As informações que faltam :".'\n\n'.implode('\n',$erros),"location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&aba=gestaomobilizacao");
		alertlocation($al);
	}
	...
	if($dados['gmcid']) {
		...
		$sql = "UPDATE sispacto3.gestaomobilizacaocoordenadorlocal SET 
	...
	$db->executar("UPDATE sispacto3.gestaomobilizacaoperguntas SET gmpnaoatividades=FALSE WHERE iusd='".$_SESSION['sispacto3']['coordenadorlocal']['iusd']."'");
	
	if($_FILES['arquivo1']['error']=='0') {
				
		include_once APPRAIZ . "includes/classes/fileSimec.class.inc";
				
		$file = new FilesSimec("gestaomobilizacaocoordenadorlocal", array() ,"sispacto3");
				
		$arquivoSalvo = $file->setUpload(null,'arquivo1',false);
		
		$db->executar("UPDATE sispacto3.gestaomobilizacaocoordenadorlocal SET arqid1='".$file->getIdArquivo()."' WHERE gmcid='".$gmcid."'");
	...
	if($_FILES['arquivo2']['error']=='0') {
		...
		$file2 = new FilesSimec("gestaomobilizacaocoordenadorlocal", array() ,"sispacto3");
				
		$arquivoSalvo = $file2->setUpload(null,'arquivo2',false);
		
		$db->executar("UPDATE sispacto3.gestaomobilizacaocoordenadorlocal SET arqid2='".$file2->getIdArquivo()."' WHERE gmcid='".$gmcid."'");
		
	}
	
	$db->commit();

	$al = array("alert"=>"Atividade salva com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&aba=gestaomobilizacao");
	alertlocation($al);
...
function excluirAtividadeCoordenadorLocal($dados) {
	...
	$sql = "UPDATE sispacto3.gestaomobilizacaocoordenadorlocal SET gmcstatus='I' WHERE gmcid='".$dados['gmcid']."'";
	$db->executar($sql);
	$db->commit();
	
	$al = array("alert"=>"Atividade excluída com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&aba=gestaomobilizacao");
	alertlocation($al);
...
function pegarAtividadeCoordenadorLocal($dados) {
	...
	$sql = "SELECT gmcid,
				   gmcatividade,
  				   ...
  				   '<img src=../imagens/anexo.gif style=cursor:pointer; onclick=\"window.location=\'sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&requisicao=downloadDocumento&arqid='||p.arqid||'\'\"> '||p.arqnome||'.'||p.arqextensao as anexo1,
  				   '<img src=../imagens/anexo.gif style=cursor:pointer; onclick=\"window.location=\'sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&requisicao=downloadDocumento&arqid='||p2.arqid||'\'\"> '||p2.arqnome||'.'||p2.arqextensao as anexo2
  				     
  			FROM sispacto3.gestaomobilizacaocoordenadorlocal g 
...
function gerenciarPerguntasCoordenadorLocal($dados) {
	...
	$sql = "SELECT gmpid FROM sispacto3.gestaomobilizacaoperguntas WHERE iusd='".$_SESSION['sispacto3']['coordenadorlocal']['iusd']."'";
	$gmpid = $db->pegaUm($sql);
	
	if($gmpid) {
		
		$sql = "UPDATE sispacto3.gestaomobilizacaoperguntas SET
		...
		$sql = "INSERT INTO sispacto3.gestaomobilizacaoperguntas(
	            iusd, 
	            gmppergunta1, 
	            ...
	            gmpnaoatividades)
	    		VALUES (
	    		'".$_SESSION['sispacto3']['coordenadorlocal']['iusd']."', 
	    		'".$dados['gmppergunta1']."', 
	...
	$al = array("alert"=>"Perguntas salvadas com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&aba=gestaomobilizacao");
	alertlocation($al);
...
function verificarNumeroAtividades($dados) {
	...
	$sql = "SELECT COUNT(*) as natividades 
			FROM sispacto3.gestaomobilizacaocoordenadorlocal 
			WHERE iusd='".$_SESSION['sispacto3']['coordenadorlocal']['iusd']."' AND gmcstatus='A'";
	
	echo $db->pegaUm($sql);
	
}

function carregarOrientadoresestudoSispacto2014($dados) {
	global $db;
	...
	echo "<input type=\"hidden\" name=\"requisicao\" value=\"efetuarImportacaoSispacto2014\">";
	echo "<input type=\"hidden\" name=\"pflcod\" value=\"".PFL_ORIENTADORESTUDO."\">";
	
	echo "<p align=center><b>Orientadores de Estudo SISPACTO 2014</b></p>";
	echo "<br>";
	
	$sql = "SELECT '<center>'||CASE WHEN cerfrequencia >= 70 THEN '<input type=\"checkbox\" name=\"iuscpf[]\" value=\"'||i.iuscpf||'\">' ELSE '' END||'</center>' as chk, replace(to_char(i.iuscpf::numeric, '000:000:000-00'), ':', '.') as iuscpf, i.iusnome, CASE WHEN cerfrequencia>= 70 THEN '<font color=blue;>'||cerfrequencia||'</font>' ELSE '<font color=red;>'||cerfrequencia||'</font>' END as cerfrequencia 
			FROM sispacto2.identificacaousuario i 
			INNER JOIN sispacto2.tipoperfil t ON t.iusd = i.iusd AND t.pflcod=".PFL_ORIENTADORESTUDO2014." 
			LEFT JOIN sispacto3.identificacaousuario i2 ON i2.iuscpf = i.iuscpf 
			LEFT JOIN sispacto3.tipoperfil t2 ON t2.iusd = i2.iusd 
			LEFT JOIN sispacto2.certificacao cc ON cc.iusd=i.iusd
			WHERE i.picid=".$dados['picid']." AND i.iusstatus='A' AND i.iuscpf NOT ILIKE 'SIS%' AND (t2.tpeid IS NULL OR i2.iusstatus='I')
			ORDER BY i.iusnome";
	
	echo "<p><input type=\"checkbox\" id=\"marcartodos\" onclick=\"marcarTodos(this);\"> <b>Marcar todos</b></p>";
	$cabecalho = array("&nbsp;","CPF","Nome","Frequência");
	$db->monta_lista_simples($sql,$cabecalho,1000,5,'N','100%',$par2);
...
function carregarProfessoresalfabetizadoresSispacto2014($dados) {
	global $db;
	...
	echo "<p align=center><b>Professores Alfabetizadores SISPACTO 2014</b></p>";
	echo "<br>";

	$sql = "SELECT '<center>'||CASE WHEN c.cerfrequencia >= 75 THEN '<input type=\"checkbox\" name=\"iuscpf[]\" value=\"'||i.iuscpf||'\">' ELSE '' END||'</center>' as chk, replace(to_char(i.iuscpf::numeric, '000:000:000-00'), ':', '.') as iuscpf, i.iusnome, CASE WHEN c.cerfrequencia >= 75 THEN '<font color=blue;>'||(c.cerfrequencia)||'%</font>' ELSE '<font color=red;>'||(c.cerfrequencia)||'%</font>' END as cerfrequencia FROM sispacto2.identificacaousuario i
			...
			INNER JOIN sispacto2.identificacaousuario i3 ON i3.iusd = tu.iusd 
			INNER JOIN sispacto2.certificacao c ON c.iusd = i.iusd  
			LEFT JOIN sispacto3.identificacaousuario i2 ON i2.iuscpf = i.iuscpf
			LEFT JOIN sispacto3.tipoperfil t2 ON t2.iusd = i2.iusd
			WHERE i3.iuscpf='".$dados['iuscpf']."' AND i.iusstatus='A' AND i.iuscpf NOT ILIKE 'SIS%' AND (t2.tpeid IS NULL OR i2.iusstatus='I')
			ORDER BY i.iusnome";

	echo "<p><input type=\"checkbox\" id=\"marcartodos\" onclick=\"marcarTodos(this);\"> <b>Marcar todos</b></p>";
	$cabecalho = array("&nbsp;","CPF","Nome","Nota Final");
	$db->monta_lista_simples($sql,$cabecalho,1000,5,'N','100%',$par2);
...
function efetuarImportacaoSispacto2014($dados) {
	global $db;
	
	if($dados['iuscpf']) {
		
		if($_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']) {
			$db->executar("UPDATE sispacto3.pactoidadecerta SET picselecaopublica=true WHERE picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."'");
		}
		...
		foreach($dados['iuscpf'] as $cpf) {
			...
			$sql = "INSERT INTO sispacto3.identificacaousuario(
		            ...
		            i.iussituacao, 'A' as iusstatus, i.funid, i.iusagenciaend, 'orientadorsispacto2014' as iustipoorientador,
			...
			if(!$iusd) {
				...
				if($dados['pflcod']==PFL_ORIENTADORESTUDO) 
					$db->executar("UPDATE sispacto3.identificacaousuario SET iusstatus='A', iustipoorientador='orientadorsispacto2014' WHERE iusd='".$iusd."'");
				
				if($dados['pflcod']==PFL_PROFESSORALFABETIZADOR) {
					$sql = "SELECT cpf FROM sispacto3.professoresalfabetizadores WHERE cpf='".$cpf."'";
					$cpf_existe = $db->pegaUm($sql);
					$db->executar("UPDATE sispacto3.identificacaousuario SET iusstatus='A', iustipoprofessor='".(($cpf_existe)?"censo":"cpflivre")."' WHERE iusd='".$iusd."'");
				}
			...
			$db->executar("DELETE FROM sispacto3.tipoperfil WHERE iusd='".$iusd."'");
			...
			$sql = "INSERT INTO sispacto3.portarianomeacao(
    					...
    					SELECT arqid, '".$iusd."', ponstatus FROM sispacto2.portarianomeacao WHERE iusd IN( SELECT iusd FROM sispacto2.identificacaousuario WHERE iuscpf='".$cpf."' )";
			...
			$sql = "INSERT INTO sispacto3.identificacaotelefone(
    				...
    				SELECT '".$iusd."', itedddtel, itenumtel, itetipo, itestatus FROM sispacto2.identificacaotelefone WHERE iusd IN( SELECT iusd FROM sispacto2.identificacaousuario WHERE iuscpf='".$cpf."' )";
			...
			$sql = "INSERT INTO sispacto3.identificaoendereco(
		            ...
		            iensatatus, ienlogradouro, ienbairro FROM sispacto2.identificaoendereco WHERE iusd IN( SELECT iusd FROM sispacto2.identificacaousuario WHERE iuscpf='".$cpf."' )";
			...
			if($dados['turid']) {
				...
				$db->executar("DELETE FROM sispacto3.".$plpabreviacao."turma WHERE iusd='".$iusd."'");
	...
	$al = array("alert"=>"Importação efetuada com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=".$dados['aba']);
	alertlocation($al);
...
function atualizarSerieProfessorAlfabetizador($dados) {
	...
	$db->executar("UPDATE sispacto3.identificacaousuario SET iusserieprofessor=".(($iusserieprofessor)?"'".$iusserieprofessor."'":"NULL")." WHERE iusd='".$dados['iusd']."'");
	$db->commit();
	
	$al = array("alert"=>"Série atualizada com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=turmas&turid=".$dados['turid']);
	alertlocation($al);
