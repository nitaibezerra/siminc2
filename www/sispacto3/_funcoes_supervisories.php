...
function sqlEquipeSupervisor($dados) {
	...
	$sql = "SELECT  i.iusd, 
			...
			WHERE (t.pflcod IN('".PFL_FORMADORIES."') OR (t.pflcod='".PFL_ORIENTADORESTUDO."' AND i.iusformacaoinicialorientador=true)) AND i.uncid='".$dados['uncid']."' AND i.iusstatus='A' AND CASE WHEN pic.picid IS NOT NULL THEN d.esdid=".ESD_VALIDADO_COORDENADOR_LOCAL." ELSE true END ORDER BY p.pflcod, i.iusnome";
...
function carregarSupervisorIES($dados) {
	...
	$_SESSION['sispacto3']['supervisories'] = array("descricao" => $arr['descricao']."( ".$infprof['iusnome']." )",
												   "curid" => $arr['curid'], 
												   "uncid" => $arr['uncid'], 
												   "reiid" => $arr['reiid'], 
												   "estuf" => $arr['uniuf'], 
	...
	if($dados['direcionar']) {
		$al = array("location"=>"sispacto3.php?modulo=principal/supervisories/supervisories&acao=A&aba=principal");
		alertlocation($al);
	}
