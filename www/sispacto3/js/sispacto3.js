...
function importarInformacoesSispacto(tela) {
	var conf = confirm('Deseja importar as informações desta tela inseridas no SISPACTO 2014? As informações inseridas serão sobrescritas');

	if(conf) {
		window.location='sispacto3.php?modulo=principal/universidade/universidade&acao=A&requisicao=importarInformacoesSispacto&tela='+tela;
	}
...
function abrirTurma(turid, plpabreviacaosub, obj) {
	var params='';
	...
	if(obj.title=="mais") {
		...
		ncol.colSpan = 8;
		ncol.id      = 'tur_coluna_'+turid+'_'+nlinha.rowIndex;
		ajaxatualizar('requisicao=carregarAlunosTurma&plpabreviacaosub='+plpabreviacaosub+'&consulta=true&turid='+turid+params,ncol.id);
	} else {
		obj.title    = "mais";
		obj.src      = "../imagens/mais.gif";
