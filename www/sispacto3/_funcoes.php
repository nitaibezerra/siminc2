...
function removerTipoPerfil($dados) {
	...
	$sql = "SELECT p.pboid FROM sispacto3.tipoperfil t 
			INNER JOIN sispacto3.pagamentobolsista p ON p.tpeid = t.tpeid  
	...
	if($pboid) {
		if(!$dados['naoredirecionar']) {
			if($dados['picid']) $al = array("alert"=>"Coordenador Local ja possui pagamento e não pode ser removido, somente substituido","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
			if($dados['uncid']) $al = array("alert"=>"Coordenador IES ja possui pagamento e não pode ser removido, somente substituido","location"=>"sispacto3.php?modulo=principal/universidade/gerenciarcoordenadories&acao=A&uncid=".$dados['uncid']);
			alertlocation($al);
		} else {
			return false;
		}
	...
	$sql = "DELETE FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."' AND pflcod='".$dados['pflcod']."'";
	$db->executar($sql);
	
	$usucpf = $db->pegaUm("SELECT iuscpf FROM sispacto3.identificacaousuario WHERE iusd='".$dados['iusd']."'");
	
	if($usucpf) {
		$sql = "DELETE FROM seguranca.perfilusuario WHERE usucpf='".$usucpf."' AND pflcod='".$dados['pflcod']."'";
		$db->executar($sql);
		$sql = "DELETE FROM sispacto3.usuarioresponsabilidade WHERE usucpf='".$usucpf."' AND pflcod='".$dados['pflcod']."'";
		$db->executar($sql);
	}
	
	$plpabreviacao = $db->pegaUm("SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod='".$dados['pflcod']."'");
	$sql = "DELETE FROM sispacto3.".$plpabreviacao."turma WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);
	
	$db->commit();
	
	if(!$dados['naoredirecionar']) {
		if($dados['picid']) $al = array("alert"=>"Coordenador Local removido com sucesso","location"=>"sispacto3.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
		if($dados['uncid']) $al = array("alert"=>"Coordenador IES removido com sucesso","location"=>"sispacto3.php?modulo=principal/universidade/gerenciarcoordenadories&acao=A&uncid=".$dados['uncid']);
		alertlocation($al);
	}
...
function verificaPermissao() {
	...
	$sql = "SELECT * FROM sispacto3.usuarioresponsabilidade WHERE usucpf='".$_SESSION['usucpf']."' AND rpustatus='A'";
	$ur = $db->carregar($sql);
	...
	if(in_array(PFL_COORDENADORLOCAL,$perfis)) {
		if($_REQUEST['modulo']=='principal/coordenadorlocal/coordenadorlocal' || $_REQUEST['modulo']=='principal/coordenadorlocal/coordenadorlocalexecucao') {
			if($ur[0]) {
				foreach($ur as $urr) {
					if($urr['pflcod']==PFL_COORDENADORLOCAL && $urr['muncod']==$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']) {
						return false;
					}
				}
			}
	...
	if(in_array(PFL_EQUIPEMUNICIPALAP,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_EQUIPEMUNICIPALAP && $urr['muncod']==$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_CONSULTAMUNICIPAL,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_CONSULTAMUNICIPAL && $urr['muncod']==$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['muncod']) {
					return true;
				}
			}
		}
	...
	if(in_array(PFL_EQUIPEESTADUALAP,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_EQUIPEESTADUALAP && $urr['estuf']==$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['estuf']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_CONSULTAESTADUAL,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_CONSULTAESTADUAL && $urr['estuf']==$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['estuf']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_COORDENADORIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_COORDENADORIES && $urr['uncid']==$_SESSION['sispacto3']['universidade']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_COORDENADORADJUNTOIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_COORDENADORADJUNTOIES && $urr['uncid']==$_SESSION['sispacto3']['coordenadoradjuntoies']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_PROFESSORALFABETIZADOR,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_PROFESSORALFABETIZADOR && $urr['uncid']==$_SESSION['sispacto3']['professoralfabetizador']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_ORIENTADORESTUDO,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_ORIENTADORESTUDO && $urr['uncid']==$_SESSION['sispacto3']['orientadorestudo']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_FORMADORIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_FORMADORIES && $urr['uncid']==$_SESSION['sispacto3']['formadories']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_FORMADORIESP,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_FORMADORIESP && $urr['uncid']==$_SESSION['sispacto3']['formadoriesp']['uncid']) {
					return false;
				}
			}
		}
	...
	if(in_array(PFL_SUPERVISORIES,$perfis)) {
		if($ur[0]) {
			foreach($ur as $urr) {
				if($urr['pflcod']==PFL_SUPERVISORIES && $urr['uncid']==$_SESSION['sispacto3']['supervisories']['uncid']) {
					return false;
				}
			}
		}
...
function montaAbasSispacto($abapai, $abaativa) {
	...
	$sql = "SELECT abaordem, abadsc, abaendereco, abafuncaomostrar, abapai FROM sispacto3.abas WHERE abapai='".$abapai."' ORDER BY abaordem";
...
function carregarOrientacaoPorFiltro($dados) {
	...
	$sql = "SELECT oabdesc FROM sispacto3.orientacaoaba WHERE abaid='".$dados['abaid']."'";
...
function carregarDadosIdentificacaoUsuario($dados) {
	...
	if(!$dados['pflcod']) {
		$al = array("alert"=>"Problemas para carregar os dados usuário","location"=>"sispacto3.php?modulo=inicio&acao=C");
		alertlocation($al);
	}
	
	$abreviacao = $db->pegaUm("SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod='".$dados['pflcod']."'");
	...
	$sql = "SELECT i.cadastradosgb, i.uncid, i.iusd, i.iuscpf, i.iusnome, i.iusdatanascimento, i.iusnomemae, i.iustipoprofessor, i.iusnaodesejosubstituirbolsa,
			...
			LEFT  JOIN sispacto3.identiusucursoformacao f ON f.iusd = i.iusd 
			...
			LEFT  JOIN sispacto3.cursoformacao cf ON cf.cufid = f.cufid 
			LEFT  JOIN sispacto3.".$abreviacao."turma ot ON ot.iusd = i.iusd 
			WHERE t.pflcod='".$dados['pflcod']."' ".(($dados['uncid'])?" AND i.uncid='".$dados['uncid']."'":"")." 
												  ".(($dados['picid'])?" AND i.picid='".$dados['picid']."'":"")." 
												  ...
												  ".(($dados['iusd'])?" AND i.iusd='".$dados['iusd']."'":"")." AND iusstatus='A' 
			ORDER BY i.iusd";
	...
	if($identificacaousuario[0]) {
		...
		foreach($identificacaousuario as $key => $iu) {
			...
			$idusuarios[$key] = $iu;
			unset($telefones);
			$sql = "SELECT itetipo, itedddtel, itenumtel FROM sispacto3.identificacaotelefone WHERE iusd='".$iu['iusd']."'";
...
function reiniciarSenha($dados) {
	...
	$al = array("alert"=>"Senha reiniciada com sucesso","location"=>"sispacto3.php?modulo=".$dados['modulo']."&acao=A&aba=".$dados['aba']);
	alertlocation($al);
...
function atualizarDadosIdentificacaoUsuario($dados) {
	...
	$iusagenciasugerida_atual = $db->pegaUm("SELECT iusagenciasugerida FROM sispacto3.identificacaousuario WHERE iusd='".$dados['iusd']."'");
	if($iusagenciasugerida_atual != substr($dados['iusagenciasugerida'],0,4)) {
		$sqlsgb = "cadastradosgb=FALSE,";
	}
	

	$sql = "UPDATE sispacto3.identificacaousuario SET
	...
	$iufid = $db->pegaUm("SELECT iufid FROM sispacto3.identiusucursoformacao WHERE iusd='".$dados['iusd']."'");
	
	// controlando formação
	if($iufid) {
		
		$sql = "UPDATE sispacto3.identiusucursoformacao SET
	...
	$itdid = $db->pegaUm("SELECT itdid FROM sispacto3.identusutipodocumento WHERE iusd='".$dados['iusd']."'");
	
	// controlando documento
	if($itdid) {
		
		$sql = "UPDATE sispacto3.identusutipodocumento SET
	...
	$ienid = $db->pegaUm("SELECT ienid FROM sispacto3.identificaoendereco WHERE iusd='".$dados['iusd']."'");
	
	// controlando endereço
	if($ienid) {
		
		$sql = "UPDATE sispacto3.identificaoendereco SET
	...
	$db->executar("DELETE FROM sispacto3.identificacaotelefone WHERE iusd='".$dados['iusd']."'");
	...
	$sql = "UPDATE sispacto3.tipoperfil SET tpeatuacaoinicio=".(($dados['tpeatuacaoinicio_mes'] && $dados['tpeatuacaoinicio_ano'])?"'".$dados['tpeatuacaoinicio_ano']."-".$dados['tpeatuacaoinicio_mes']."-01'":"NULL").", 
...
function carregarOrientacao($endereco) {
	...
	$sql = "SELECT a.abaid, o.oabdesc FROM sispacto3.abas a
			LEFT JOIN sispacto3.orientacaoaba o ON o.abaid = a.abaid
...
function efetuarTrocaUsuarioPerfil($dados) {
	...
	$sql = "SELECT * FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd WHERE i.iusd='".$dados['iusdantigo']."'";
	$identificacaousuario_antigo = $db->pegaLinha($sql);
	
	if($identificacaousuario_antigo['pflcod']==PFL_PROFESSORALFABETIZADOR) {
		
		$docids = $db->carregarColuna("SELECT docid FROM sispacto3.pagamentobolsista WHERE tpeid='".$identificacaousuario_antigo['tpeid']."'");
		...
		if($docids) {
			...
			if($possuipagamento) {
				...
				$db->executar("DELETE FROM sispacto3.pagamentobolsista WHERE docid IN('".implode("','",$docids)."')");				
			}
		}
	...
	$sql = "SELECT COUNT(*) as t FROM sispacto3.mensario m 
			INNER JOIN sispacto3.identificacaousuario i ON i.iusd = m.iusd 
			INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = m.menid
	...
	$sql = "SELECT i.iusd, t.tpeid, i.iusnome FROM sispacto3.identificacaousuario i LEFT JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd WHERE i.iuscpf='".str_replace(array(".","-"),array("",""),$dados['iuscpf_'])."'";
	...
	if(!$identificacaousuario_novo['iusd']) {
		...
		$sql = "UPDATE sispacto3.identificacaousuario SET iusstatus='A', picid=".(($identificacaousuario_antigo['picid'])?"'".$identificacaousuario_antigo['picid']."'":"NULL").", uncid=".(($identificacaousuario_antigo['uncid'])?"'".$identificacaousuario_antigo['uncid']."'":"NULL").", 
	...
	$sql = "DELETE FROM sispacto3.usuarioresponsabilidade WHERE usucpf='".str_replace(array(".","-"),array(""),$dados['iuscpf_'])."'";
	$db->executar($sql);
	
	$sql = "UPDATE sispacto3.usuarioresponsabilidade SET usucpf='".str_replace(array(".","-"),array(""),$dados['iuscpf_'])."', uncid=".(($identificacaousuario_antigo['uncid'])?"'".$identificacaousuario_antigo['uncid']."'":"NULL")." WHERE rpustatus='A' AND usucpf='".$identificacaousuario_antigo['usucpf']."' AND pflcod='".$identificacaousuario_antigo['pflcod']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sispacto3.tipoperfil SET iusd='".$identificacaousuario_novo['iusd']."' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sispacto3.turmas SET iusd='".$identificacaousuario_novo['iusd']."' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	
	$plpabreviacao = $db->pegaUm("SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod='".$identificacaousuario_antigo['pflcod']."'");
	
	$sql = "UPDATE sispacto3.".$plpabreviacao."turma SET iusd='".$identificacaousuario_novo['iusd']."' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	
	$sql = "UPDATE sispacto3.identificacaousuario SET iusstatus='I' WHERE iusd='".$identificacaousuario_antigo['iusd']."'";
	$db->executar($sql);
	...
	if($dados['pflcod_']==PFL_ORIENTADORESTUDO) {
		$existe_proletramento 	 = $db->pegaUm("SELECT cpf FROM sispacto3.tutoresproletramento WHERE cpf='".str_replace(array(".","-"),array(""),$dados['iuscpf_'])."'");
		$existe_semproletramento = $db->pegaUm("SELECT cpf FROM sispacto3.tutoressemproletramento WHERE cpf='".str_replace(array(".","-"),array(""),$dados['iuscpf_'])."'");
		...
		$sql = "UPDATE sispacto3.identificacaousuario SET iustipoorientador='{$iustipoorientador}' WHERE iusd='".$identificacaousuario_novo['iusd']."'";
		$db->executar($sql);
	}
	
	// removendo avaliações não concluidas
	$sql = "SELECT m.menid FROM sispacto3.mensario m 
	...
	if($menids) {
		...
		$sql = "SELECT mavid FROM sispacto3.mensarioavaliacoes WHERE menid IN('".implode("','",$menids)."')";
		$mavids = $db->carregarColuna($sql);
		
		if($mavids) {
			$db->executar("DELETE FROM sispacto3.historicoreaberturanota WHERE mavid IN('".implode("','",$mavids)."')");
			$db->executar("DELETE FROM sispacto3.mensarioavaliacoes WHERE mavid IN('".implode("','",$mavids)."')");
		}
	}
	
	$sql = "INSERT INTO sispacto3.historicotrocausuario(iusdnovo, iusdantigo, pflcod, hstdata, usucpf, uncid)
	...
	if($dados['chk']) {
		...
		foreach($dados['chk'] as $pflcod => $cpfs) {
			...
			foreach($cpfs as $cpf) {
				...
				$sql = "SELECT * FROM sispacto3.identificacaousuario WHERE iuscpf='".str_replace(array(".","-"),array(""),$cpf)."'";
			    ...
			    $rpuid = $db->pegaUm("select rpuid from sispacto3.usuarioresponsabilidade where usucpf='".str_replace(array(".","-"),array(""),$identificacaousuario['iuscpf'])."' and pflcod='".$pflcod."' AND rpustatus='A'");
			    ...
			    if($dados['uncid']) {
				    ...
				    if(!$rpuid) {
				    	...
				    	$sql = "UPDATE sispacto3.usuarioresponsabilidade SET uncid='".$dados['uncid']."' WHERE rpuid='".$rpuid."'";
				    	$db->executar($sql);
				    }
			    ...
			    if($identificacaousuario['picid']) {
			    	...
			    	$sql = "SELECT * FROM sispacto3.pactoidadecerta WHERE picid='".$identificacaousuario['picid']."'";
				    ...
				    if(!$rpuid) {
				    	...
				    	$sql = "UPDATE sispacto3.usuarioresponsabilidade SET {$cl} WHERE rpuid='".$rpuid."'";
				    	$db->executar($sql);
				    }
...
function verificarFormacaoCompleta($dados) {
	global $db;
	$sql = "SELECT foecompleto FROM sispacto3.formacaoescolaridade WHERE foeid='".$dados['foeid']."'";
...
function listarCursosFormacao($dados) {
	global $db;
	$sql = "SELECT cufid as codigo, cufcursodesc as descricao FROM sispacto3.cursoformacao WHERE cufstatus='A' AND cufcodareageral='".$dados['cufcodareageral']."' ORDER BY cufcursodesc";
...
function anexarDocumentoDesignacao($dados) {
    ...
    $file = new FilesSimec( "identificacaousuarioanexo", $campos, "sispacto3" );
...
function downloadDocumentoDesignacao($dados) {
    include_once APPRAIZ . "includes/classes/fileSimec.class.inc";
    $file = new FilesSimec( "identificacaousuarioanexo", NULL, "sispacto3" );
    $file->getDownloadArquivo( $dados['arqid'] );
}

function removerDocumentoDesignacao($dados) {
	global $db;
	$sql = "DELETE FROM sispacto3.identificacaousuarioanexo WHERE iuaid='".$dados['iuaid']."'";
	$db->executar($sql);
	$db->commit();
...
function downloadDocumento($dados) {
    include_once APPRAIZ . "includes/classes/fileSimec.class.inc";
    $file = new FilesSimec( "documentoatividade", NULL, "sispacto3" );
    $file->getDownloadArquivo( $dados['arqid'] );
}

function removerAnexoPortaria($dados) {
	global $db;
	$sql = "DELETE FROM sispacto3.portarianomeacao WHERE ponid='".$dados['ponid']."'";
	$db->executar($sql);
	$db->commit();
...
function carregarDadosTurma($dados) {
	...
	$sql = "SELECT t.turdesc, t.turid, i.iusnome, p.pfldsc, p2.pfldsc as pfldscsub, p2.pflcod as pflcodsub, pp.plpabreviacao as plpabreviacaosub, t.uncid FROM sispacto3.turmas t 
		 	LEFT JOIN sispacto3.pagamentoperfil pp ON pp.pflcod = t.pflcod 
		 	LEFT JOIN seguranca.perfil p2 ON p2.pflcod = pp.pflcod
			LEFT JOIN sispacto3.identificacaousuario i ON i.iusd = t.iusd 
		 	LEFT JOIN sispacto3.tipoperfil tt ON tt.iusd = i.iusd 
		 	LEFT JOIN seguranca.perfil p ON p.pflcod = tt.pflcod 
...
function carregarAlunosTurma($dados) {
	...
	if($dados['turid']) {
		...
		$sql = "SELECT '<center>".((!$dados['consulta'])?"<img src=../imagens/excluir.gif style=\"cursor:pointer;\" onclick=\"excluirAlunoTurma('||i.iusd||');\">":"")." ".(($dados['formacaoinicial'])?"'|| CASE WHEN SUBSTR(i.iuscpf,1,3)!='SIS' THEN '<input type=radio name=\"iusd['||i.iusd||']\" value=\"TRUE\" '||CASE WHEN i.iusformacaoinicialorientador=true THEN 'checked' ELSE '' END||'> Presente <input type=radio name=\"iusd['||i.iusd||']\" value=\"FALSE\" '||CASE WHEN i.iusformacaoinicialorientador=false THEN 'checked' ELSE '' END||'> Ausente' ELSE '' END ||'":"")."</center>' as acao, replace(to_char(i.iuscpf::numeric, '000:000:000-00'), ':', '.') as iuscpf, i.iusnome, i.iusemailprincipal, m.estuf || ' / ' || m.mundescricao as municipio, CASE WHEN pp.muncod IS NULL THEN 'Estadual' ELSE 'Municipal' END as esfera, tu.turdesc FROM sispacto3.".$dados['plpabreviacaosub']."turma ot 
				...
				WHERE ot.turid='".$dados['turid']."' AND i.iusstatus='A' ORDER BY i.iusnome";
...
function carregarAlunosTurmaOutros($dados) {
	global $db;
	if($dados['turid']) {
		$sql = "SELECT '<center>".((!$dados['consulta'])?"<img src=../imagens/excluir.gif style=\"cursor:pointer;\" onclick=\"excluirAlunoTurmaOutros('||i.iusd||');\">":"")." ".(($dados['formacaoinicial'])?"'|| CASE WHEN SUBSTR(i.iuscpf,1,3)!='SIS' THEN '<input type=radio name=\"iusd['||i.iusd||']\" value=\"TRUE\" '||CASE WHEN i.iusformacaoinicialorientador=true THEN 'checked' ELSE '' END||'> Presente <input type=radio name=\"iusd['||i.iusd||']\" value=\"FALSE\" '||CASE WHEN i.iusformacaoinicialorientador=false THEN 'checked' ELSE '' END||'> Ausente' ELSE '' END ||'":"")."</center>' as acao, i.iuscpf, i.iusnome, i.iusemailprincipal, m.estuf || ' / ' || m.mundescricao as municipio, CASE WHEN pp.muncod IS NOT NULL THEN 'Municipal' WHEN pp.estuf IS NOT NULL THEN 'Estadual' ELSE 'Equipe IES' END ||'( '||p.pfldsc||' )' as esfera, tu.turdesc FROM sispacto3.orientadorestudoturmacl ot
				INNER JOIN sispacto3.identificacaousuario i ON i.iusd = ot.iusd
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd
...
function excluirAlunoTurmaOutros($dados) {
	...
	$sql = "DELETE FROM sispacto3.orientadorestudoturmacl WHERE iusd='".$dados['iusd']."' AND turid='".$dados['turid']."'";
	$db->executar($sql);
	$db->commit();
...
function criarMensario($dados) {
	global $db;
	$sql = "SELECT m.menid, d.docid, d.esdid FROM sispacto3.mensario m INNER JOIN workflow.documento d ON d.docid = m.docid WHERE m.iusd='".$dados['iusd']."' AND fpbid='".$dados['fpbid']."'";
	...
	if(!$menid) {
		...
		$arrUs    = $db->pegaLinha("SELECT i.iusnome, p.pfldsc FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod WHERE i.iusd='".$dados['iusd']."'");
		$iusnome  = $arrUs['iusnome'];
		$pfldsc   = $arrUs['pfldsc'];
		
		$referencia = $db->pegaUm("SELECT fpbmesreferencia || ' / ' || fpbanoreferencia as descricao FROM sispacto3.folhapagamento WHERE fpbid='".$dados['fpbid']."'");
		
		$docid = wf_cadastrarDocumento( TPD_FLUXOMENSARIO, 'Mensário : '.$iusnome.' - '.$pfldsc.' Ref.'.$referencia );
		$esdid = ESD_EM_ABERTO_MENSARIO;
		
		$sql = "INSERT INTO sispacto3.mensario(
            	iusd, fpbid, docid, pflcod)
    			VALUES ('".$dados['iusd']."', '".$dados['fpbid']."', '".$docid."', (SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."')) RETURNING menid;";
	...
	if($dados['iusd_avaliados']) {
		foreach($dados['iusd_avaliados'] as $iusd) {
			...
			if($dadosmensario['esdid']!=ESD_APROVADO_MENSARIO) {
				...
				$sql = "SELECT mavid, mavfrequencia, mavatividadesrealizadas FROM sispacto3.mensarioavaliacoes WHERE menid='".$dadosmensario['memid']."' AND (iusdavaliador='".$dados['iusdavaliador']."' OR pflcodavaliador IN(SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusdavaliador']."'))";
				$ma = $db->pegaLinha($sql);
				$mavid = $ma['mavid'];
				...
				if($mavid) {
					...
					if(is_numeric($dados['frequencia'][$iusd]) || is_numeric($dados['atividadesrealizadas'][$iusd])) {
						...
						$sql = "UPDATE sispacto3.mensarioavaliacoes SET mavfrequencia=".((is_numeric($dados['frequencia'][$iusd]))?"'".$dados['frequencia'][$iusd]."'":"NULL").", 
						...
						$sql = "INSERT INTO sispacto3.mensarioavaliacoes(
		            			...
		            			mavmonitoramento, mavtotal, menid, pflcodavaliador)
			    					SELECT '".$dados['iusdavaliador']."', 
										   ...
										   '".$dadosmensario['memid']."',
											(SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusdavaliador']."')
								 	WHERE (SELECT mavid FROM sispacto3.mensarioavaliacoes WHERE iusdavaliador='".$dados['iusdavaliador']."' AND menid='".$dadosmensario['memid']."') IS NULL
				...
				if($mavid && $dados['cpfresponsavel'][$iusd] && $dados['mavdsc'][$iusd]) {
					$sql = "INSERT INTO sispacto3.historicoreaberturanota(
...
function condicaoEnviarMensario($fpbid,$pflcod=null) {
	...
	if($pflcod == PFL_ORIENTADORESTUDO) {
		...
		if(!$_SESSION['sispacto3']['orientadorestudo']['uncid']) return 'Este usuário não esta vinculado a nenhuma IES';
		
		$funcaoavaliacao = $db->pegaUm("SELECT tpatipoavaliacao FROM sispacto3.tipoavaliacaoperfil WHERE pflcod='".$pflcod."' AND uncid='".$_SESSION['sispacto3']['orientadorestudo']['uncid']."' AND fpbid='".$fpbid."'");
		
		// tratando condição de tipos de monitoramento
		if($funcaoavaliacao=='monitoramentoTextual') return true;
		
		
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sispacto3.mensario me 
							INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid
							INNER JOIN sispacto3.identificacaousuario i ON i.iusd = me.iusd 
							INNER JOIN sispacto3.professoralfabetizadorturma ot ON ot.iusd = me.iusd 
							INNER JOIN sispacto3.turmas tt ON tt.turid = ot.turid 		
							WHERE tt.iusd='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."' AND me.fpbid='".$fpbid."'");
		if(!$tot) {
			return 'Nenhuma avaliação foi salva';
		} else {
			
			$sql = "SELECT i.iusnome FROM sispacto3.mensario me 
					INNER JOIN workflow.documento d ON d.docid = me.docid
					INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid 
					INNER JOIN sispacto3.identificacaousuario i ON i.iusd = me.iusd 
					INNER JOIN sispacto3.professoralfabetizadorturma ot ON ot.iusd = me.iusd 
					INNER JOIN sispacto3.turmas tt ON tt.turid = ot.turid 		
					WHERE tt.iusd='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."' AND ma.iusdavaliador='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."' AND me.fpbid='".$fpbid."' AND ma.mavtotal IS NULL AND d.esdid != ".ESD_APROVADO_MENSARIO." 
					ORDER BY i.iusnome";
		...
		$sql = "SELECT count(*) as tot FROM sispacto3.respostasavaliacaocomplementar WHERE iusdavaliador='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."'";
	...
	if($pflcod == PFL_FORMADORIES) {
		...
		if(!$_SESSION['sispacto3']['formadories']['iusd']) return 'Formador IES não foi IDENTIFICADO, faça o LOGOUT, e acesse novamente o SISPACTO.';
		
		$funcaoavaliacao = $db->pegaUm("SELECT tpatipoavaliacao FROM sispacto3.tipoavaliacaoperfil WHERE pflcod='".$pflcod."' AND uncid='".$_SESSION['sispacto3']['formadories']['uncid']."' AND fpbid='".$fpbid."'");
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sispacto3.mensario me 
							INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid
							INNER JOIN sispacto3.identificacaousuario i ON i.iusd = me.iusd 
							INNER JOIN sispacto3.orientadorestudoturma ot ON ot.iusd = me.iusd 
							INNER JOIN sispacto3.turmas tt ON tt.turid = ot.turid 		
							WHERE tt.iusd='".$_SESSION['sispacto3']['formadories']['iusd']."' AND me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sispacto3']['formadories']['iusd']."' AND ma.mavtotal IS NOT NULL");
		...
		if(!$tot) {
			...
			$sql = "SELECT i.iusnome FROM sispacto3.mensario me 
					INNER JOIN workflow.documento d ON d.docid = me.docid 
					LEFT JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid AND ma.iusdavaliador='".$_SESSION['sispacto3']['formadories']['iusd']."'
					INNER JOIN sispacto3.identificacaousuario i ON i.iusd = me.iusd AND i.iusformacaoinicialorientador=TRUE 
					INNER JOIN sispacto3.orientadorestudoturma ot ON ot.iusd = me.iusd 
					INNER JOIN sispacto3.turmas tt ON tt.turid = ot.turid 		
					WHERE tt.iusd='".$_SESSION['sispacto3']['formadories']['iusd']."' AND me.fpbid='".$fpbid."' AND ma.mavtotal IS NULL AND d.esdid != ".ESD_APROVADO_MENSARIO." 
					ORDER BY i.iusnome";
	...
	if($pflcod == PFL_SUPERVISORIES) {
		...
		$sql_tot = sqlAvaliacaoSupervisor(array('uncid'=>$_SESSION['sispacto3']['supervisories']['uncid'],'iusd'=>$_SESSION['sispacto3']['supervisories']['iusd'],'fpbid'=>$fpbid));
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sispacto3.mensario me 
							INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid
							WHERE me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sispacto3']['supervisories']['iusd']."' AND ma.mavtotal IS NOT NULL");
	...
	if($pflcod == PFL_COORDENADORADJUNTOIES) {
		...
		$sql_tot = sqlAvaliacaoCoordenadorAdjuntoIES(array('uncid'=>$_SESSION['sispacto3']['coordenadoradjuntoies']['uncid'],'iusd'=>$_SESSION['sispacto3']['coordenadoradjuntoies']['iusd'],'fpbid'=>$fpbid));
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sispacto3.mensario me 
							INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid
							WHERE me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sispacto3']['coordenadoradjuntoies']['iusd']."' AND ma.mavtotal IS NOT NULL");
	...
	if($pflcod == PFL_COORDENADORIES) {
		...
		$sql_tot = sqlAvaliacaoCoordenadorIES(array('uncid'=>$_SESSION['sispacto3']['universidade']['uncid'],'iusd'=>$_SESSION['sispacto3']['universidade']['iusd'],'fpbid'=>$fpbid));
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sispacto3.mensario me 
							INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid
							WHERE me.fpbid='".$fpbid."' AND ma.iusdavaliador='".$_SESSION['sispacto3']['universidade']['iusd']."' AND ma.mavtotal IS NOT NULL");
	...
	if($pflcod == PFL_COORDENADORLOCAL) {
		...
		$funcaoavaliacao = $db->pegaUm("SELECT tpatipoavaliacao FROM sispacto3.tipoavaliacaoperfil WHERE pflcod='".$pflcod."' AND uncid='".$_SESSION['sispacto3']['coordenadorlocal']['uncid']."' AND fpbid='".$fpbid."'");
		...
		$tot = $db->pegaUm("SELECT COUNT(*) as tot FROM sispacto3.mensario me
							INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid
							INNER JOIN sispacto3.identificacaousuario i ON i.iusd = me.iusd
							INNER JOIN sispacto3.orientadorestudoturmacl ot ON ot.iusd = me.iusd
							INNER JOIN sispacto3.turmas tt ON tt.turid = ot.turid
							WHERE tt.iusd='".$_SESSION['sispacto3']['coordenadorlocal']['iusd']."' AND me.fpbid='".$fpbid."' AND ma.mavtotal IS NOT NULL");
...
function posEnviarMensario($fpbid, $pflcod=null) {
	...
	if($pflcod == PFL_ORIENTADORESTUDO) {
		...
		$sql = "SELECT i.iusnome, me.docid, ma.mavtotal FROM sispacto3.mensario me 
				INNER JOIN workflow.documento dc ON dc.docid = me.docid AND dc.tpdid=".TPD_FLUXOMENSARIO." 
				INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid 
				INNER JOIN sispacto3.identificacaousuario i ON i.iusd = me.iusd 
				INNER JOIN sispacto3.professoralfabetizadorturma ot ON ot.iusd = me.iusd 
				INNER JOIN sispacto3.turmas tt ON tt.turid = ot.turid 		
				WHERE tt.iusd='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."' AND dc.esdid='".ESD_EM_ABERTO_MENSARIO."' AND ma.iusdavaliador='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."' AND me.fpbid='".$fpbid."'";
		...
		$sql = "UPDATE sispacto3.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				SELECT ma.mavid, fa.fatmonitoramento FROM sispacto3.mensario me 
				INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = me.menid 
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sispacto3.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sispacto3']['orientadorestudo']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
	...
	if($pflcod == PFL_FORMADORIES) {
		...
		$sql = "UPDATE sispacto3.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sispacto3.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sispacto3']['formadories']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_SUPERVISORIES) {
		...
		$sql = "UPDATE sispacto3.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sispacto3.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sispacto3']['supervisories']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_COORDENADORADJUNTOIES) {
		...
		$sql = "UPDATE sispacto3.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sispacto3.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sispacto3']['coordenadoradjuntoies']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_COORDENADORIES) {
		...
		$sql = "UPDATE sispacto3.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sispacto3.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sispacto3']['universidade']['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
	...
	if($pflcod == PFL_COORDENADORLOCAL) {
		...
		$sql = "UPDATE sispacto3.mensarioavaliacoes co SET mavtotal=(mavtotal+foo.fatmonitoramento), mavmonitoramento=foo.fatmonitoramento FROM (
				...
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = me.iusd 
				INNER JOIN sispacto3.fatoresdeavaliacao fa ON fa.fatpflcodavaliado = t.pflcod	
				WHERE me.iusd='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['iusd']."' AND me.fpbid='".$fpbid."'
				) foo 
				WHERE co.mavid=foo.mavid";
...
function carregarAjudaAvaliacao($dados) {
	...
	$sql = "SELECT * FROM sispacto3.fatoresdeavaliacao WHERE fatid='".$dados['fatid']."'";
...
function carregarAvaliacaoEquipe($dados) {
	...
	$sql = "WITH tmp_avaliacao AS (
				SELECT m.iusd, ma.mavfrequencia, ma.mavatividadesrealizadas, ma.mavmonitoramento, ma.mavtotal, ma.mavid, CASE WHEN ma.iusdavaliador!='".$dados['iusd']."' THEN 'Avaliado pelo '||i.iusnome ELSE '' END as obs 
									   ...
									   INNER JOIN sispacto3.identificacaousuario ius ON ius.iusd = m.iusd 
									   INNER JOIN sispacto3.mensarioavaliacoes ma ON ma.menid = m.menid 
									   INNER JOIN sispacto3.identificacaousuario i ON i.iusd = ma.iusdavaliador 
									   WHERE (ius.uncid='".$dados['uncid']."' OR i.uncid='".$dados['uncid']."') AND m.fpbid='".$dados['fpbid']."' AND (iusdavaliador='".$dados['iusd']."' OR pflcodavaliador IN(SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."'))
			)
			...
			INNER JOIN sispacto3.fatoresdeavaliacao fat ON fat.fatpflcodavaliado = foo.pflcod
	...
	echo '<tr>';
	echo '<td class="SubTituloCentro">Selecionar Todos:</td>';
	echo '<td class="SubTituloDireita">Frequência :</td>';
	echo '<td>';
	$db->monta_combo('todosfrequencia', $OPT_AV['frequencia'], 'S', 'Selecione', 'marcarTodosFrequencia', '', '', '', 'N', 'todosfrequencia','');
	echo '</td>';
	echo '<td class="SubTituloDireita">Atividades Realizadas :</td>';
	echo '<td>';
	$db->monta_combo('todosatividadesrealizadas', $OPT_AV['atividadesrealizadas'], 'S', 'Selecione', 'marcarTodosAtividadesRealizadas', '', '', '', 'N', 'todosatividadesrealizadas','');
	echo '</td>';
	echo '</tr>';
	echo '</table>';
...
function inserirDadosLog($dados) {
	...
	$sql = "INSERT INTO log_historico.logsgb_sispacto3(
...
function sincronizarDadosUsuarioSGB($dados) {
	...
	if($dadosusuario) {
    	...
    	if(!$dados['sincronizacao']) $lnscpf = $db->carregarColuna("SELECT lnscpf FROM sispacto3.listanegrasgb");
    	else $lnscpf = array();
    	...
    	$sql = "UPDATE sispacto3.identificacaousuario SET cadastradosgb=".(($logerro_gravarDadosBolsista=='TRUE')?'FALSE':'TRUE')." WHERE iusd='".$dados['iusd']."'";
    	$db->executar($sql);
    	$db->commit();
...
function sincronizarDadosEntidadeSGB($dados) {
    ...
    if($existecnpj) $logerro_gravaDadosEntidade = 'FALSE';
   	...
   	$sql = "UPDATE sispacto3.universidadecadastro SET cadastrosgb=".(($logerro_gravaDadosEntidade=='TRUE')?'FALSE':'TRUE')." WHERE uncid='".$dados['uncid']."'";
   	$db->executar($sql);
   	$db->commit();
...
function avaliarComplementarEquipe($dados) {
	...
	$sql = "DELETE FROM sispacto3.respostasavaliacaocomplementar WHERE iusdavaliador='".$dados['iusdavaliador']."' AND fpbid='".$dados['fpbid']."'";
	$db->executar($sql);
	
	if($dados['icc']) {
		foreach($dados['icc'] as $iacid => $iccid) {
			if($iccid) {
				$racid = $db->pegaUm("SELECT racid FROM sispacto3.respostasavaliacaocomplementar WHERE iusdavaliador='".$dados['iusdavaliador']."' AND iacid='".$iacid."' AND fpbid='".$dados['fpbid']."'");
				
				if($racid) {
					
					$sql = "UPDATE sispacto3.respostasavaliacaocomplementar SET 
					...
					$sql = "INSERT INTO sispacto3.respostasavaliacaocomplementar(
...
function verificarTermoCompromisso($dados) {
	...
	if($dados['iusd']) 
		$iustermocompromisso = $db->pegaUm("SELECT iustermocompromisso FROM sispacto3.identificacaousuario WHERE iusd='".$dados['iusd']."'");
	
	if($iustermocompromisso!="t") {
		
		$al = array("alert"    => "Por favor preencha todos os campos obrigatórios da tela “Dados”.",
					"location" => "sispacto3.php?modulo=".$_REQUEST['modulo']."&acao=A");
...
function gerarVersaoProjetoUniversidade($dados) {
	...
	include APPRAIZ.'sispacto3/modulos/principal/universidade/visualizacao_projeto.inc';
	$html = ob_get_contents();
	ob_clean();
		
	$sql = "INSERT INTO sispacto3.versoesprojetouniversidade(
...
function carregarMudancasTroca($dados) {
	...
	$sql = "SELECT CASE WHEN h.hstacao='T' THEN '<span style=font-size:xx-small>Troca</span>'
			...
			LEFT JOIN sispacto3.turmas t1 ON t1.turid = h.turidantigo 
			LEFT JOIN sispacto3.identificacaousuario i3 ON i3.iusd = t1.iusd 
			LEFT JOIN sispacto3.turmas t2 ON t2.turid = h.turidnovo 
			LEFT JOIN sispacto3.identificacaousuario i4 ON i4.iusd = t2.iusd
...
function processarPagamentoBolsistaSGB($dados) {
	...
	$sql = "SELECT * FROM sispacto3.pagamentobolsista WHERE pboid='".$dados->id."'";
		...
		if($dados->situacao->codigo=='10001' || 
		   ...
		   $dados->situacao->codigo=='00025') {
			...
			if($pagamentobolsista['pboparcela']) {
				...
				$novaparcela = $db->pegaUm("SELECT (rfuparcela+1) as novaparcela FROM sispacto3.folhapagamentouniversidade f 
							 				INNER JOIN sispacto3.universidadecadastro u ON u.uncid = f.uncid 
							 				WHERE u.uniid='".$pagamentobolsista['uniid']."' AND f.fpbid='".$pagamentobolsista['fpbid']."'");
			}
			
			$sql = "UPDATE sispacto3.pagamentobolsista SET remid=null, pboparcela='".$novaparcela."' WHERE pboid='".$pagamentobolsista['pboid']."'";
			$db->executar($sql);
			$db->commit();
			...
			$sql = "UPDATE sispacto3.pagamentobolsista SET remid=null WHERE pboid='".$pagamentobolsista['pboid']."'";
			$db->executar($sql);
			$db->commit();
		}
	}
...
function consultarDetalhesAvaliacoes($dados) {
	...
	$sql = "SELECT CASE WHEN ptc.muncod IS NOT NULL THEN 'Municipal ('|| mun.estuf ||' / '|| mun.mundescricao ||')'
			...
			LEFT JOIN sispacto3.pactoidadecerta ptc ON ptc.picid = i.picid 
			...
			INNER JOIN sispacto3.folhapagamento fa ON fa.fpbid = m.fpbid 
	...
	$sql = "SELECT '<span style=font-size:x-small;>'||foo.iusnome||'</span>' as iusnome,
		    ...
		    FROM sispacto3.mensarioavaliacoes m
			INNER JOIN sispacto3.identificacaousuario i ON i.iusd = m.iusdavaliador 
	...
	if($mensario['iusd'] && $mensario['fpbid']) {
		...
		$sql_atv_com = "SELECT i.iusnome, p.pfldsc, ia.iacdsc, ic.iccdsc, ic.iccvalor FROM sispacto3.respostasavaliacaocomplementar r 
				INNER JOIN sispacto3.identificacaousuario i ON i.iusd = r.iusdavaliador 
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
				INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod 
				INNER JOIN sispacto3.itensavaliacaocomplementar ia ON ia.iacid = r.iacid 
				INNER JOIN sispacto3.itensavaliacaocomplementarcriterio ic ON ic.iccid = r.iccid 
		...
		$sql = "(
				...
				SELECT '', '', '', CASE WHEN AVG(ic.iccvalor) > 0 THEN 'Média' ELSE '<span style=color:red;>Não existem avaliações complementares</span>' END as l, AVG(ic.iccvalor) as media FROM sispacto3.respostasavaliacaocomplementar r 
				INNER JOIN sispacto3.identificacaousuario i ON i.iusd = r.iusdavaliador 
				INNER JOIN sispacto3.itensavaliacaocomplementar ia ON ia.iacid = r.iacid 
				INNER JOIN sispacto3.itensavaliacaocomplementarcriterio ic ON ic.iccid = r.iccid 
	...
	<table class="listagem" cellspacing="0" cellpadding="3" align="center" style="width: 95%;">
		<thead>
			<?php if ( count( $historico ) ) : ?>
				<tr>
					<td style="width: 20px;text-align:center;font-size:x-small;">Seq.</td>
					<td style="width: 200px;text-align:center;font-size:x-small;">Estado do pagamento</td>
					<td style="width: 90px;text-align:center;font-size:x-small;">Quem fez</td>
					<td style="width: 120px;text-align:center;font-size:x-small;">Quando fez</td>
					<td style="width: 17px;text-align:center;font-size:x-small;">&nbsp;</td>
				</tr>
			<?php endif; ?>
		</thead>
		<?php $i = 1; ?>
		...
		<?php $marcado = $i++ % 2 == 0 ? "" : "#f7f7f7";?>
		<tr bgcolor="<?=$marcado?>" onmouseover="this.bgColor='#ffffcc';" onmouseout="this.bgColor='<?=$marcado?>';">
			<td style="text-align:right;font-size:x-small;" colspan="5">
				Estado atual: <span style="font-size:x-small;"><b><?php echo $atual['esddsc']; ?></b></span>
			</td>
		</tr>
	</table>
	<?
...
function aprovarEquipe($dados) {
	...
	if($dados['menid']) {
		foreach($dados['menid'] as $menid) {
			...
			$sql = "SELECT * FROM sispacto3.mensario m 
					INNER JOIN workflow.documento d ON d.docid = m.docid 
					WHERE menid='".$menid."'";
	...
	$al = array("alert"=>"Equipe aprovada com sucesso","location"=>"sispacto3.php?modulo=".$dados['modulo']."&acao=A&aba=aprovarusuario&fpbid=".$dados['fpbid']."&pflcodaprovar=".$dados['pflcodaprovar']);
	alertlocation($al);
...
function posAprovarMensario($menid) {
	...
	$sql = "SELECT	t.tpeid, m.iusd, m.fpbid, p.pflcod, p.pfldsc, i.iuscpf, i.iusnaodesejosubstituirbolsa, i.iusnome, f.fpbmesreferencia, f.fpbanoreferencia, pp.plpvalor, un.uniid FROM sispacto3.mensario m 
			INNER JOIN sispacto3.identificacaousuario i ON i.iusd = m.iusd
			INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod 
			INNER JOIN sispacto3.folhapagamento f ON f.fpbid = m.fpbid 
			INNER JOIN sispacto3.pagamentoperfil pp ON pp.pflcod = t.pflcod 
	...
	if($arrInfo['iusnaodesejosubstituirbolsa']!='t') {
		...
		$sql = "SELECT 'Não foi possível criar o registro de bolsa para ".str_replace(array("'"),array(" "),$arrInfo['iusnome']).", pois a bolsa ja foi paga para ' || i.iusnome || ' => ' || 'Ref. ' || m.mesdsc || ' / ' || fpbanoreferencia ||' )' as descricao FROM sispacto3.pagamentobolsista p 
				INNER JOIN sispacto3.identificacaousuario i ON i.iusd = p.iusd 
				INNER JOIN sispacto3.folhapagamento f ON f.fpbid = p.fpbid 
...
function calculaPorcentagemUsuarioAtivos($dados) {
	...
	if($_REQUEST['modulo']=='principal/universidade/universidadeexecucao') {
		$sql_equipe = sqlEquipeCoordenadorIES(array("uncid"=>$_SESSION['sispacto3']['universidade']['uncid']));
		$sis = 'universidade';
	}
	
	if($_REQUEST['modulo']=='principal/coordenadoradjuntoies/coordenadoradjuntoies') {
		$sql_equipe = sqlEquipeCoordenadorAdjunto(array("uncid"=>$_SESSION['sispacto3']['coordenadoradjuntoies']['uncid']));
		$sis = 'coordenadoradjuntoies';
	}
	
	if($_REQUEST['modulo']=='principal/supervisories/supervisories') {
		$sql_equipe = sqlEquipeSupervisor(array("uncid"=>$_SESSION['sispacto3']['supervisories']['uncid']));
		$sis = 'supervisories';
	}
	
	if($_REQUEST['modulo']=='principal/formadories/formadories') {
		$sql_equipe = sqlEquipeFormador(array("iusd"=>$_SESSION['sispacto3']['formadories']['iusd'],"uncid"=>$_SESSION['sispacto3']['formadories']['uncid']));
		$sis = 'formadories';
	}
	
	if($_REQUEST['modulo']=='principal/orientadorestudo/orientadorestudo') {
		$sql_equipe = sqlEquipeOrientador(array("iusd"=>$_SESSION['sispacto3']['orientadorestudo']['iusd'],"uncid"=>$_SESSION['sispacto3']['orientadorestudo']['uncid']));
		$sis = 'orientadorestudo';
	}
	
	if($_REQUEST['modulo']=='principal/coordenadorlocal/coordenadorlocalexecucao') {
		$sql_equipe_p = sqlEquipeCoordenadorLocal(array("picid"=>$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']));
		$sis = 'coordenadorlocal';
	}
	
	if($sql_equipe_p) {
		$sql_total = "SELECT COUNT(*) as tot FROM ({$sql_equipe_p}) foo WHERE foo.status='A' AND CASE WHEN foo.pflcod=".PFL_ORIENTADORESTUDO." OR foo.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN foo.iusd in( SELECT i.iusd FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.pactoidadecerta p ON p.picid = i.picid INNER JOIN workflow.documento d ON d.docid = p.docidturma WHERE d.esdid='".ESD_FECHADO_TURMA."') ELSE true END";
		...
		gerenciarAtividadePacto(array('iusd'=>$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['iusd'],'apadatainicio'=>$apadatainicio,'apadatafim'=>$apadatafim,'apassituacao'=>$apassituacao,'suaid'=>$dados['suaid'],'picid'=>$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']));
	}
	...
	if($sql_equipe) {
		...
		$ecuid = pegarEstruturaCurso(array("uncid" => $_SESSION['sispacto3'][$sis]['uncid']));
		
		if($total) $aunsituacao = round(($total_a/$total)*100);
		gerenciarAtividadeUniversidade(array('iusd'=>$_SESSION['sispacto3'][$sis]['iusd'],'aundatafim'=>$aundatafim,'aundatainicio'=>$aundatainicio,'aunsituacao'=>$aunsituacao,'ecuid'=>$ecuid,'suaid'=>$dados['suaid']));
	}
...
function gerenciarAtividadeUniversidade($dados) {
	...
	$sql = "SELECT aunid FROM sispacto3.atividadeuniversidade a 
	...
	if($aunid) {
		...
		$sql = "UPDATE sispacto3.atividadeuniversidade SET 
...
function gerenciarAtividadePacto($dados) {
	...
	$sql = "SELECT apaid FROM sispacto3.atividadepacto WHERE suaid='".$dados['suaid']."' AND picid='".$dados['picid']."'".(($dados['iusd'])?" AND iusd='".$dados['iusd']."'":"");
	$apaid = $db->pegaUm($sql);
	
	if($apaid) {
		
		$sql = "UPDATE sispacto3.atividadepacto SET 
		...
		$sql = "INSERT INTO sispacto3.atividadepacto(
...
function carregarExecucaoAtividadeUniversidade($dados) {
	...
	$execucao_atividade = $db->pegaLinha("SELECT ROUND(AVG(aunsituacao)) as apassituacao, 
											  ...
											  FROM sispacto3.subatividades su
										  	  INNER JOIN sispacto3.atividadeuniversidade ap ON su.suaid = ap.suaid 
										  	  INNER JOIN sispacto3.estruturacurso es ON es.ecuid = ap.ecuid 
...
function carregarExecucaoSubAcaoUniversidade($dados) {
	...
	$atividadeuni = $db->pegaLinha("SELECT aunid,
									  ...
									  FROM sispacto3.atividadeuniversidade au 
									  INNER JOIN sispacto3.estruturacurso es ON es.ecuid = au.ecuid
...
function pegarEstruturaCurso($dados) {
	global $db;
	$sql = "SELECT ecuid FROM sispacto3.estruturacurso WHERE uncid='".$dados['uncid']."'";
	$ecuid = $db->pegaUm($sql);
	
	if(!$ecuid) {
		
		$sql = "INSERT INTO sispacto3.estruturacurso(
...
function carregarPeriodoReferencia($dados) {
	...
	if($db->testa_superuser()) {
		$dados['todosmeses'] = true;
	}
	
	if($dados['fpbid'] && !is_numeric($dados['fpbid'])) {
		$al = array("alert"=>"Período de Referência não identificado. Tente novamente","location"=>"sispacto3.php?modulo=".$_REQUEST['modulo']."&acao=".$_REQUEST['acao']."&aba=".$_REQUEST['aba']);
		alertlocation($al);
	}
	
	if($dados['pflcod_avaliador']) {
		$plpmaximobolsas = $db->pegaUm("SELECT plpmaximobolsas FROM sispacto3.pagamentoperfil WHERE pflcod='".$dados['pflcod_avaliador']."'");
		if($plpmaximobolsas) $limit = "LIMIT {$plpmaximobolsas}";
	}
	
	if($dados['fpbid']) {
		$existe = $db->pegaUm("SELECT fpbid FROM sispacto3.folhapagamento WHERE ".((!$dados['todosmeses'])?"fpbstatus='A' AND":"")." fpbid='".$dados['fpbid']."'");
		if(!$existe) {
			$al = array("alert"=>"Período de Referência não identificado. Tente novamente","location"=>"sispacto3.php?modulo=".$_REQUEST['modulo']."&acao=".$_REQUEST['acao']."&aba=".$_REQUEST['aba']);
			alertlocation($al);
		} 
	...
	$sql = "SELECT f.fpbid as codigo, rf.rfuparcela ||'º Parcela ( Ref. ' || m.mesdsc || ' / ' || fpbanoreferencia ||' )' as descricao 
			...
			INNER JOIN sispacto3.folhapagamentouniversidade rf ON rf.fpbid = f.fpbid 
			INNER JOIN public.meses m ON m.mescod::integer = f.fpbmesreferencia
			WHERE ".((!$dados['todosmeses'])?"f.fpbstatus='A' AND":"")." rf.uncid='".$dados['uncid']."' ".(($dados['pflcod_avaliador'])?"AND rf.pflcod='".$dados['pflcod_avaliador']."'":"AND rf.pflcod IS NULL")." AND to_char(NOW(),'YYYYmmdd')>=to_char((fpbanoreferencia::text||lpad(fpbmesreferencia::text, 2, '0')||'15')::date,'YYYYmmdd') 
			ORDER BY (fpbanoreferencia::text||'-'||lpad(fpbmesreferencia::text, 2, '0')||'-15')::date {$limit}";
...
function exibirSituacaoMensario($dados) {
	...
	INNER JOIN sispacto3.identificacaousuario i ON i.iusd = m.iusd 
 	INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd  
	INNER JOIN sispacto3.folhapagamentouniversidade fpu ON fpu.uncid = i.uncid AND fpu.pflcod = m.pflcod AND fpu.fpbid = m.fpbid
	LEFT JOIN sispacto3.pagamentoperfil pp ON pp.pflcod = m.pflcod 
...
function exibirAcessoUsuarioSimec($dados) {
	...
	$sql = "SELECT 
			...
			FROM sispacto3.identificacaousuario u 
			INNER JOIN sispacto3.tipoperfil t on t.iusd=u.iusd 
			INNER JOIN seguranca.perfil p on p.pflcod = t.pflcod 
...
function exibirSituacaoPagamento($dados) {
	...
	$sql = "SELECT '{$acao}' as acao,
			...
			INNER JOIN sispacto3.pagamentobolsista pb ON pb.pflcod = p.pflcod 
			INNER JOIN sispacto3.universidadecadastro un ON un.uniid = pb.uniid 
			...
			INNER JOIN sispacto3.pagamentoperfil pp ON pp.pflcod = foo.pflcod";
...
function cadastrarPeriodoReferencia($dados) {
	...
	if($uncids) {
		foreach($uncids as $uncid) {
			...
			$sql = "DELETE FROM sispacto3.folhapagamentouniversidade WHERE uncid='".$uncid."'";
			$db->executar($sql);
			
			$sql = "select foo.fpbid from (
					select fpbid, fpbanoreferencia||'-'||lpad(fpbmesreferencia::text,2,'0') as dt from sispacto3.folhapagamento ) foo
			...
			if($fpbids) {
				foreach($fpbids as $key => $fpbid) {
					$sql = "INSERT INTO sispacto3.folhapagamentouniversidade(
	...
	$al = array("alert"=>"Período de referência aprovado com sucesso","location"=>"sispacto3.php?modulo=principal/mec/mec&acao=A&aba=configuracoes");
	alertlocation($al);
...
function carregarLogCadastroSGB($dados) {
	...
	$iusd = $db->pegaUm("SELECT iusd FROM sispacto3.identificacaousuario WHERE iuscpf='".$dados['usucpf']."'");
	
	if($iusd) echo "<input type=hidden name=iusd id=iusd_log value=\"".$iusd."\">";
	
	$sql = "SELECT u.iuscpf, u.iusnome, to_char(logdata,'dd/mm/YYYY HH24:MI') as data, logresponse FROM log_historico.logsgb_sispacto3 l 
			INNER JOIN sispacto3.identificacaousuario u ON u.iuscpf = l.logcpf 
...
function criteriosAprovacao($cla) {
	...
	$cl['restricao4'] = "CASE 
						 ...
						 WHEN foo.reuid IS NOT NULL THEN '<div style=\"color:red;font-size:x-small;width:300px;height:40px;overflow:auto;\">'||(SELECT COALESCE(reurestricao,'') FROM sispacto3.restricaousuario WHERE reuid=foo.reuid)||'</div>'
						 WHEN foo.pflcod=".PFL_ORIENTADORESTUDO." THEN
				   		 ...
				   		 WHEN foo.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN
					   														CASE WHEN (foo.esdorien != '".ESD_VALIDADO_COORDENADOR_LOCAL."' OR foo.esdturpr != '".ESD_FECHADO_TURMA."') THEN '<center><span style=color:red;font-size:x-small;>Cadastro dos professores e/ou Orientadores de Estudo não foi finalizado.<br>Contate o Coordenador Local('||foo.rede||')</span></center>'
																				 ...
																				 WHEN foo.aprendizagemPor != ".APRENDIZAGEM_PORTUGUES." AND foo.rfuparcela > 1 THEN '<span style=color:red;font-size:x-small;>Preencher a aba Atividades Obrigatórias (2º Parcela)</span>'
																				 WHEN foo.aprendizagemMat != ".APRENDIZAGEM_MATEMATICA." AND foo.rfuparcela > 2 THEN '<span style=color:red;font-size:x-small;>Preencher a aba Atividades Obrigatórias (3º Parcela)</span>'
																				 ...
																				 --WHEN foo.contribuicaopacto = 0 AND foo.rfuparcela > 9 THEN '<span style=color:red;font-size:x-small;>Preencher a aba Atividades Obrigatórias (10º Parcela)</span>'
					   															 ...
					   															 ELSE '<span style=color:red;font-size:x-small;>Professor Alfabetizador não cadastrado no censo 2014</span>' END
					   														...
					   														CASE WHEN (foo.esdorien != '".ESD_VALIDADO_COORDENADOR_LOCAL."' OR foo.esdturpr != '".ESD_FECHADO_TURMA."') THEN '<center><span style=color:red;font-size:x-small;>Cadastro dos professores e/ou Orientadores de Estudo não foi finalizado.<br>Contate o Coordenador Local - '||foo.rede||'</span></center>'
																				 ...
																				 --WHEN foo.aprendizagemPor2 != ".APRENDIZAGEM_PORTUGUES." AND foo.rfuparcela > 8 THEN '<span style=color:red;font-size:x-small;>Preencher a aba Atividades Obrigatórias (9º Parcela) - Português</span>' 
					 															 --WHEN foo.contribuicaopacto = 0 AND foo.rfuparcela > 9 THEN '<span style=color:red;font-size:x-small;>Preencher a aba Atividades Obrigatórias (10º Parcela) - Português</span>'
					   															 WHEN foo.iustipoprofessor = 'censo' THEN '<span style=color:blue;font-size:x-small;>Nenhuma restrição</span>' 
					   															 ELSE '<span style=color:red;>Professor Alfabetizador não cadastrado no censo 2014</span>' END 
				   		 WHEN foo.pflcod=".PFL_COORDENADORLOCAL." THEN 
					   	 													CASE WHEN (foo.esdorien != '".ESD_VALIDADO_COORDENADOR_LOCAL."' OR foo.esdturpr != '".ESD_FECHADO_TURMA."') THEN '<center><span style=color:red;font-size:x-small;>Cadastro dos professores e/ou Orientadores de Estudo não foi finalizado.</span></center>'
						 --														 WHEN foo.rcoordenadorlocal = 0 THEN '<span style=color:red;>Falta preencher as informações sobre \"Gestão e Mobilização\" no SisPacto</span>' 
					   	 													ELSE '<span style=color:blue;>Nenhuma restrição</span>' END
						...
						(CASE WHEN foo.pflcod=".PFL_ORIENTADORESTUDO." THEN 
							...
							WHEN foo.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN 
																				CASE WHEN (foo.esdorien != '".ESD_VALIDADO_COORDENADOR_LOCAL."' OR foo.esdturpr != '".ESD_FECHADO_TURMA."') THEN false
			    																	 ...
			    																	 WHEN foo.aprendizagemPor != ".APRENDIZAGEM_PORTUGUES." AND foo.rfuparcela > 1 THEN false
																					 WHEN foo.aprendizagemMat != ".APRENDIZAGEM_MATEMATICA." AND foo.rfuparcela > 2 THEN false 
...
function verificarEmailUnico($dados) {
	...
	$sql = "SELECT '- '||i.iusnome||' ( '||p.pfldsc||' );' as descricao FROM sispacto3.identificacaousuario i 
			INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
			INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod
...
function consultarDetalhesPagamento($dados) {
	global $db;
	$sql = "SELECT i.iusnome, me.mesdsc||'/'||fa.fpbanoreferencia as periodo, e.esddsc, p.pbovlrpagamento, pp.pfldsc, uni.uninome, uni.unicnpj, p.docid FROM sispacto3.pagamentobolsista p 
			INNER JOIN sispacto3.identificacaousuario i ON i.iusd = p.iusd 
			INNER JOIN sispacto3.folhapagamento fa ON fa.fpbid = p.fpbid 
			INNER JOIN public.meses me ON me.mescod::integer = fa.fpbmesreferencia 
			...
			INNER JOIN sispacto3.universidade uni ON uni.uniid = p.uniid  
...
function carregarMateriais($dados) {
	...
	if($dados['uncid']) {
		...
		$sql = "SELECT * FROM (
				(
				SELECT count(p.picid) as tot, ".$dados['group']." as grouper FROM sispacto3.materiais m 
				INNER JOIN sispacto3.pactoidadecerta p ON p.picid = m.picid 
				INNER JOIN sispacto3.abrangencia a ON a.muncod = p.muncod 
				...
				INNER JOIN sispacto3.abrangencia a ON a.muncod = mu.muncod 
				INNER JOIN sispacto3.estruturacurso e ON e.ecuid = a.ecuid 
		...
		$materiais = $db->carregar("SELECT count(picid) as tot, ".$dados['group']." as grouper FROM sispacto3.materiais GROUP BY ".$dados['group']);
...
function carregarMateriaisProfessores($dados) {
	global $db;
	$materiais = $db->carregar("SELECT count(iusd) as tot, ".$dados['group']." as grouper FROM sispacto3.materiaisprofessores GROUP BY ".$dados['group']);
...
function excluirUsuarioPerfil($dados) {
	...
	$npagamentos = $db->pegaUm("SELECT COUNT(*) FROM sispacto3.pagamentobolsista WHERE iusd='".$dados['iusd']."'");
	
	if($npagamentos > 0) {


		$sql = "SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod IN(SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."')";
		$plpabreviacao = $db->pegaUm($sql);
		
		$identificacaousuario = $db->pegaLinha("SELECT i.iuscpf, t.pflcod, p.pfldsc FROM sispacto3.identificacaousuario i 
												INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
												INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod  
												WHERE i.iusd='".$dados['iusd']."'");
		...
		$sql = "INSERT INTO sispacto3.identificacaousuario(
				...
				SELECT picid, muncod, eciid, nacid, fk_cod_docente, 'REM'||SUBSTR(iuscpf,4,8) as iuscpf, iusnome || ' - {$identificacaousuario['pfldsc']} - REMOVIDO' as iusnome, 
				       ...
				       iusformacaoinicialorientador, cadastradosgb, iustipoprofessor, 
				       iusdocumento, iusnaodesejosubstituirbolsa
				  FROM sispacto3.identificacaousuario where iusd='".$dados['iusd']."'
				RETURNING iusd;";
		
		$iusd_novo = $db->pegaUm($sql);
		
		
		$sql = "DELETE FROM sispacto3.usuarioresponsabilidade  WHERE rpustatus='A' AND usucpf='".$identificacaousuario['iuscpf']."' AND pflcod='".$identificacaousuario['pflcod']."'";
		$db->executar($sql);
	
		$sql = "UPDATE sispacto3.tipoperfil SET iusd='".$iusd_novo."' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	
		$sql = "UPDATE sispacto3.turmas SET iusd='".$iusd_novo."' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	
		$sql = "UPDATE sispacto3.{$plpabreviacao}turma SET iusd='".$iusd_novo."' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
		
		$sql = "UPDATE sispacto3.identificacaousuario SET iusstatus='I' WHERE iusd='".$dados['iusd']."'";
		$db->executar($sql);
	...
	if(!$dados['uncid']) $dados['uncid'] = $db->pegaUm("SELECT uncid FROM sispacto3.identificacaousuario WHERE iusd='".$dados['iusd']."'");

	$sql = "INSERT INTO sispacto3.historicotrocausuario(
	...
	$al = array("alert"=>"Exclusão ocorrida com sucesso","location"=>"sispacto3.php?modulo=".$dados['modulo']."&acao=".$dados['acao']."&aba=gerenciarusuario&uncid=".$dados['uncid']);
	alertlocation($al);
...
<tr>
	...
	if($dados['uncid']) {
		$sql = "SELECT m.arqid, m.matid, m.mafdsc FROM sispacto3.materiaisfotos m
				INNER JOIN sispacto3.materiais ma ON ma.matid = m.matid 
				INNER JOIN sispacto3.pactoidadecerta p ON p.picid = ma.picid 
				INNER JOIN sispacto3.abrangencia a ON a.muncod = p.muncod 
...
<tr>
	...
	$_SESSION['imgparams']['tabela'] = "sispacto3.materiaisprofessoresfotos";
	$_SESSION['imgparams']['filtro'] = "1=1";
	
	$sql = "SELECT m.arqid, m.mapid, m.mpfdsc FROM sispacto3.materiaisprofessoresfotos m 
			INNER JOIN sispacto3.materiaisprofessores ma ON ma.mapid = m.mapid 
			ORDER BY random() LIMIT 6";
...
function verMunicipioMateriais($dados) {
	...
	if($dados['uncid']) {
		...
		$sql = "SELECT * FROM (
				...
				COALESCE(array_to_string(array(SELECT i.iusnome FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ', '),'Coordenador Local não cadastrado') as coordenadorlocal,
				COALESCE(array_to_string(array(SELECT i.iusemailprincipal FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=p.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ', '),'Coordenador Local não cadastrado') as emailcoordenadorlocal,
...
function carregarInformes($dados) {
	...
	$informes = $db->carregar("SELECT inpid, inpdescricao, to_char(inpdatainserida,'dd/mm/YYYY HH24:MI') as inpdatainserida FROM sispacto3.informespacto WHERE pflcoddestino='".$dados['pflcoddestino']."' AND inpstatus='A' ORDER BY inpdatainserida DESC");
...
function inserirInforme($dados) {
	...
	$sql = "INSERT INTO sispacto3.informespacto(
...
function excluirInforme($dados) {
	...
	$sql = "DELETE FROM sispacto3.informespacto WHERE inpid='".$dados['inpid']."'";
	$db->executar($sql);
	$db->commit();
		...
		foreach($dados['troca'] as $iusd => $turid) {
			...
			if($turid) {
				...
				if($existe) {
					...
					$db->executar("INSERT INTO sispacto3.historicotrocausuario(
	            				   iusdantigo, pflcod, hstdata, usucpf, uncid, hstacao, turidnovo, turidantigo)
	    						   VALUES ('".$iusd."', 
	    						   		   (SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$iusd."'), 
	    						   		    NOW(), 
	    						   		    '".$_SESSION['usucpf']."', 
	    						   		    '".$dados['uncid']."', 
	    						   		    'F', 
...
function atualizarEmail($dados) {
	...
	$sql = "UPDATE sispacto3.identificacaousuario SET iusemailprincipal='".$dados['iusemailprincipal']."' WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);
	$db->commit();
...
function exibirPorcentagemPagamentoPerfil($dados) {
	...
	$sql = "SELECT p.pflcod, p.pfldsc FROM sispacto3.pagamentoperfil pp 
	...
	if($perfil) {
		...
		foreach($perfil as $p) {
			...
			$uncids = $db->carregarColuna("SELECT DISTINCT uncid FROM sispacto3.tipoavaliacaoperfil WHERE fpbid <='".$dados['fpbid']."'");
			...
			$sql = "SELECT count(*) as tot FROM (
					...
					SELECT i.iusd, i.uncid,  t.pflcod, p.plpmaximobolsas, (SELECT count(*) FROM sispacto3.mensario WHERE iusd=i.iusd) as numerobolsas
					FROM sispacto3.identificacaousuario i 
					INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
					INNER JOIN sispacto3.pagamentoperfil p ON p.pflcod = t.pflcod 
					INNER JOIN sispacto3.folhapagamentouniversidade rf ON rf.uncid = i.uncid AND rf.pflcod = t.pflcod AND rf.fpbid='".$dados['fpbid']."'
					LEFT JOIN sispacto3.pactoidadecerta pa ON pa.picid = i.picid 
					LEFT JOIN workflow.documento dc ON dc.docid = pa.docidturma
					WHERE i.iusstatus='A' AND t.pflcod='".$p['pflcod']."' AND CASE WHEN t.pflcod=".PFL_PROFESSORALFABETIZADOR." THEN i.iustipoprofessor='censo' AND dc.esdid='".ESD_FECHADO_TURMA."' ELSE true END AND CASE WHEN t.pflcod=".PFL_ORIENTADORESTUDO." THEN i.iusformacaoinicialorientador=true ELSE true END AND i.uncid IN( SELECT rf.uncid FROM sispacto3.folhapagamentouniversidade rf WHERE rf.fpbid='".$dados['fpbid']."' ) ".(($wh)?" AND ".implode(" AND ", $wh):"")."
			...
			$sql = "SELECT count(*) as tot FROM sispacto3.pagamentobolsista p 
					INNER JOIN sispacto3.universidadecadastro i ON i.uniid = p.uniid 
					WHERE p.pflcod='".$p['pflcod']."' AND p.fpbid='".$dados['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sispacto3.folhapagamentouniversidade rf WHERE rf.fpbid='".$dados['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
			...
			$sql = "SELECT count(*) as tot FROM sispacto3.pagamentobolsista p 
					...
					WHERE d.esdid IN('".ESD_PAGAMENTO_EFETIVADO."','".ESD_PAGAMENTO_NAO_AUTORIZADO."') AND p.pflcod='".$p['pflcod']."' AND p.fpbid='".$dados['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sispacto3.folhapagamentouniversidade rf WHERE rf.fpbid='".$dados['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
...
function exibirPorcentagemPagamento($dados) {
	...
	$sql = "SELECT f.fpbid, 'Ref.'||m.mesdsc||'/'||f.fpbanoreferencia as periodo FROM sispacto3.folhapagamento f
			INNER JOIN public.meses m ON m.mescod::numeric = f.fpbmesreferencia 
			".(($dados['uncid'])?"INNER JOIN sispacto3.folhapagamentouniversidade fp ON fp.fpbid = f.fpbid AND fp.uncid='".$dados['uncid']."' AND fp.pflcod IS NULL":"")."
			".(($wh1)?" WHERE ".implode(" AND ", $wh1):"")." ORDER BY (fpbanoreferencia::text||'-'||lpad(fpbmesreferencia::text, 2, '0')||'-15')::date";
	...
	if($folhapagamento[0]) {
		...
		foreach($folhapagamento as $fp) {
			...
			$sql = "SELECT count(*) as tot, sum(foo.plpvalor) as vlr FROM (
 					...
 					SELECT i.iusd, i.uncid,  t.pflcod, p.plpmaximobolsas, (SELECT count(*) FROM sispacto3.mensario WHERE iusd=i.iusd) as numerobolsas, p.plpvalor 
 					FROM sispacto3.identificacaousuario i 
					INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
					INNER JOIN sispacto3.pagamentoperfil p ON p.pflcod = t.pflcod 
					INNER JOIN sispacto3.folhapagamentouniversidade rf ON rf.uncid = i.uncid AND rf.pflcod = t.pflcod AND rf.fpbid='".$fp['fpbid']."'
					LEFT JOIN sispacto3.pactoidadecerta pa ON pa.picid = i.picid 
					LEFT JOIN workflow.documento dc ON dc.docid = pa.docidturma
			...
			$sql = "SELECT count(*) as tot FROM sispacto3.pagamentobolsista p 
					INNER JOIN sispacto3.universidadecadastro i ON i.uniid = p.uniid 
					WHERE p.fpbid='".$fp['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sispacto3.folhapagamentouniversidade rf WHERE rf.fpbid='".$fp['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
			
			$totalpag = $db->pegaUm($sql);
			
			$sql = "SELECT count(*) as tot, sum(p.pbovlrpagamento) as vlr FROM sispacto3.pagamentobolsista p 
					INNER JOIN sispacto3.universidadecadastro i ON i.uniid = p.uniid 
					INNER JOIN workflow.documento d ON d.docid = p.docid 
					WHERE d.esdid IN('".ESD_PAGAMENTO_EFETIVADO."','".ESD_PAGAMENTO_NAO_AUTORIZADO."') AND p.fpbid='".$fp['fpbid']."' AND i.uncid IN( SELECT rf.uncid FROM sispacto3.folhapagamentouniversidade rf WHERE rf.fpbid='".$fp['fpbid']."' )".(($wh)?" AND ".implode(" AND ", $wh):"");
...
function carregarDetalhesPerfil($dados) {
	...
	if($dados['pflcod_']==PFL_ORIENTADORESTUDO) {
		...
		$sql = "(
				...
				INNER JOIN sispacto3.abrangencia a ON a.muncod = p.muncod
				INNER JOIN sispacto3.estruturacurso es ON es.ecuid = a.ecuid 
	...
	if($dados['pflcod_']==PFL_COORDENADORLOCAL) {
		...
		$sql = "(
				...
				FROM sispacto3.pactoidadecerta p 
				INNER JOIN territorios.municipio m ON m.estuf = p.estuf
				INNER JOIN sispacto3.abrangencia a ON a.muncod = m.muncod AND a.esfera='E'
				INNER JOIN sispacto3.estruturacurso es ON es.ecuid = a.ecuid 
				INNER JOIN territorios.estado e ON e.estuf = p.estuf 
				WHERE es.uncid='".$dados['uncid']."'
				ORDER BY 2
...
function exibirMunicipiosNaoFechados($dados) {
	...
	$sql = "SELECT  m.estuf,
					m.mundescricao,
			    COALESCE(array_to_string(array(SELECT iusnome FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=pic.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ','),'Coordenador Local não cadastrado') as coordenadorlocal, 
			    COALESCE(array_to_string(array(SELECT iusemailprincipal FROM sispacto3.identificacaousuario i INNER JOIN sispacto3.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=pic.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ','),'Coordenador Local não cadastrado') as emailcoordenador
			FROM sispacto3.abrangencia a 
			INNER JOIN sispacto3.estruturacurso e ON e.ecuid = a.ecuid 
			INNER JOIN territorios.municipio m ON m.muncod = a.muncod 
			INNER JOIN sispacto3.pactoidadecerta pic ON pic.muncod = a.muncod 
...
function efetuarInsercaoUsuarioPerfil($dados) {
	global $db;
	if($dados['picid__']) $dados['muncodatuacao__'] = $db->pegaUm("SELECT muncod FROM sispacto3.pactoidadecerta WHERE picid='".$dados['picid__']."'");
	
	
	$iusd = $db->pegaUm("SELECT iusd FROM sispacto3.identificacaousuario WHERE iuscpf='".str_replace(array(".","-"),array("",""),$dados['iuscpf__'])."'");
	...
	$sql = "SELECT p.pfldsc, p.pflcod FROM sispacto3.tipoperfil t INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod WHERE t.iusd='".$iusd."'";
	...
	$tpeid = $db->pegaUm("SELECT tpeid FROM sispacto3.tipoperfil WHERE iusd='".$iusd."'");
	...
	if($dados['turid__']) {
		...
		$plpabreviacao = $db->pegaUm("SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod='".$dados['pflcod__']."'");

		$otuid = $db->pegaUm("SELECT otuid FROM sispacto3.{$plpabreviacao}turma WHERE iusd='".$iusd."'");
		
		if(!$otuid) {
			$sql = "INSERT INTO sispacto3.{$plpabreviacao}turma(
	            	turid, iusd)
			...
			$sql = "UPDATE sispacto3.{$plpabreviacao}turma SET turid='".$dados['turid__']."' WHERE iusd='".$iusd."';";
			$db->executar($sql);
...
function sincronizarUsuariosSIMEC($dados) {
	...
	$sql = "UPDATE seguranca.usuario u SET 
			...
			SELECT 
			i.iuscpf,
			(SELECT itedddtel FROM sispacto3.identificacaotelefone WHERE iusd=i.iusd AND itetipo='T') as dddtel,
			u.usufoneddd,
			(SELECT itenumtel FROM sispacto3.identificacaotelefone WHERE iusd=i.iusd AND itetipo='T') as tel,
			u.usufonenum,
...
function carregarOrientadoresSISPorMunicipio($dados) {
	...
	if($p) {
		...
		$db->executar("UPDATE sispacto3.pactoidadecerta SET picselecaopublica=true, picincluirprofessorrede=false WHERE picid='".$p['picid']."'");
		$db->commit();
		...
		if($totalalfabetizadores['total_orientadores_a_serem_cadastrados'] > count($orientadoresestudo)) {
			...
			for($i = 0;$i < $restantes;$i++) {
				...
				$num_ius = $db->pegaUm("SELECT substr(iuscpf, 8) as num FROM sispacto3.identificacaousuario WHERE picid='".$p['picid']."' AND iuscpf ilike 'SIS%' ORDER BY iusd DESC");
				if($num_ius) $num_ius++;
				else $num_ius=1;
				...
				if($p['muncod']) {
					$uncid = $db->pegaUm("SELECT e.uncid FROM sispacto3.abrangencia a 
										  INNER JOIN sispacto3.estruturacurso e ON e.ecuid = a.ecuid 
				...
				$sql = "INSERT INTO sispacto3.identificacaousuario(picid, 
																  muncod, 
																  iuscpf, 
																  iusnome, 
            													  iusemailprincipal, 
				...
				$sql = "INSERT INTO sispacto3.tipoperfil( iusd, pflcod, tpestatus)
    					VALUES ( '".$iusd."', '".PFL_ORIENTADORESTUDO."', 'A');";
				
				$db->executar($sql);
				
				if($uncid) {
					$turid = $db->pegaUm("SELECT t.turid FROM sispacto3.turmas t 
										  INNER JOIN sispacto3.identificacaousuario i ON i.iusd = t.iusd 
										  INNER JOIN sispacto3.tipoperfil tt ON tt.iusd = i.iusd 
										  WHERE tt.pflcod='".PFL_FORMADORIES."' AND i.uncid='".$uncid."' LIMIT 1");
					
					if($turid) {
						$db->executar("INSERT INTO sispacto3.orientadorturma(
			...
			$al = array("alert"=>"O município selecionado não possui vagas para Orientadores de Estudo.","location"=>"sispacto3.php?modulo=principal/mec/mec&acao=A");
			alertlocation($al);
		}
	
	}
	
	$al = array("alert"=>"Foram inseridos {$restantes} Orientadores de Estudo SIS.","location"=>"sispacto3.php?modulo=principal/mec/mec&acao=A");
	alertlocation($al);
...
function invalidarMensario($dados) {
	global $db;
	$sql = "SELECT d.esdid FROM sispacto3.mensario m 
			INNER JOIN workflow.documento d ON d.docid = m.docid 
	...
	$al = array("alert"=>"Mensário invalidado com sucesso","location"=>"sispacto3.php?modulo={$dados['modulo']}&acao=A&aba=aprovarusuario&fpbid=".$dados['fpbid']."&pflcodaprovar=".$dados['pflcodaprovar']);
	alertlocation($al);
...
function corrigirAcessoUniversidade($dados) {
	global $db;
	$sql = "SELECT i.uncid, i.iuscpf, i.picid, t.pflcod, i.muncodatuacao, i.iusd FROM sispacto3.identificacaousuario i 
			INNER JOIN sispacto3.tipoperfil t ON t.iusd = t.iusd
	...
	if($identificacaousuario['uncid']) {
		...
		$sql = "UPDATE sispacto3.usuarioresponsabilidade SET uncid='".$identificacaousuario['uncid']."' WHERE usucpf='".$_SESSION['usucpf']."' AND pflcod='".$identificacaousuario['pflcod']."' AND rpustatus='A'";
		$db->executar($sql);
		$db->commit();

		if($dados['sis']) $_SESSION['sispacto3'][$dados['sis']]['uncid'] = $identificacaousuario['uncid'];
		...
		$sql = "SELECT uncid FROM sispacto3.abrangencia a 
				INNER JOIN sispacto3.estruturacurso e ON e.ecuid = a.ecuid 
		...
		if($uncid) {
			$db->executar("UPDATE sispacto3.identificacaousuario SET uncid='".$uncid."' WHERE iusd='".$identificacaousuario['iusd']."'");
			$db->executar("UPDATE sispacto3.usuarioresponsabilidade SET uncid='".$uncid."' WHERE usucpf='".$_SESSION['usucpf']."' AND pflcod='".$identificacaousuario['pflcod']."' AND rpustatus='A'");
			$db->commit();
		}
		
		if($dados['sis']) $_SESSION['sispacto3'][$dados['sis']]['uncid'] = $uncid;
...
function criarDocumentosPagamentos($dados) {
	...
	$pagamentos = $db->carregar("SELECT p.pboid, pf.pfldsc, i.iuscpf, i.iusnome, f.fpbmesreferencia, f.fpbanoreferencia FROM sispacto3.pagamentobolsista p 
	...
	if($pagamentos[0]) {
		foreach($pagamentos as $arrInfo) {
			$docid = wf_cadastrarDocumento(TPD_PAGAMENTOBOLSA, "Pagamento - ".$arrInfo['pfldsc']." - (".$arrInfo['iuscpf'].")".$arrInfo['iusnome']." - ".$arrInfo['fpbmesreferencia']."/".$arrInfo['fpbanoreferencia']);
			$db->executar("UPDATE sispacto3.pagamentobolsista SET docid='".$docid."' WHERE pboid='".$arrInfo['pboid']."'");
		}
...
function atualizarNomeUsuario($dados) {
	...
	if($obj['PESSOA']->no_pessoa_rf) {
		$db->executar("UPDATE sispacto3.identificacaousuario SET iusnome='".$obj['PESSOA']->no_pessoa_rf."' WHERE iuscpf='".$dados['iuscpf']."'");
		$db->executar("UPDATE seguranca.usuario SET usunome='".$obj['PESSOA']->no_pessoa_rf."' WHERE usucpf='".$dados['iuscpf']."'");
		$db->commit();
	}
	
	$al = array("alert"=>"Nome atualizado com sucesso","location"=>"sispacto3.php?modulo=".$_REQUEST['modulo']."&acao=A&aba=dados");
	alertlocation($al);
...
function aprovarTrocaNomesSGB($dados) {
	...
	if($dados['cpf']) {
		foreach($dados['cpf'] as $cpf) {
			if($dados['nome_receita'][$cpf]) {
				$sql = "UPDATE sispacto3.identificacaousuario SET iusnome='".$dados['nome_receita'][$cpf]."' WHERE iuscpf='".str_replace(array(".","-"),array("",""),$cpf)."'";
				$db->executar($sql);
			}	
		}
		$db->commit();
	...
	$al = array("alert"=>"Trocas realizadas com sucesso","location"=>"sispacto3.php?modulo=".$_REQUEST['modulo']."&acao=A&aba=aprovarnomes");
	alertlocation($al);
...
function reenviarPagamentos($dados) {
	...
	if($dados['doc']) {
		foreach($dados['doc'] as $docid) {
			...
			if($aedid) {
				...
				$db->executar("UPDATE sispacto3.pagamentobolsista SET remid=null WHERE docid='".$docid."'");
				$db->commit();
			}
	...
	$al = array("alert"=>"Reenvio agendado com sucesso","location"=>"sispacto3.php?modulo=principal/mec/mec&acao=A&aba=reenviarpagamentos");
	alertlocation($al);
...
function excluirAvaliacoesMensario($dados) {
	...
	$db->executar("DELETE FROM sispacto3.historicoreaberturanota WHERE mavid='".$dados['mavid']."'");
	$db->executar("DELETE FROM sispacto3.mensarioavaliacoes WHERE mavid='".$dados['mavid']."'");
	
	$db->commit();
	
	$al = array("alert"=>"Avaliação apagada","location"=>"sispacto3.php?modulo=".$dados['modulo']."&acao=A&aba=".$dados['aba']."&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function atualizarMunicipioAtuacao($dados) {
	...
	$db->executar("UPDATE sispacto3.identificacaousuario SET muncodatuacao='".$dados['muncod']."' WHERE iusd='".$dados['iusd']."'");
	$db->commit();
...
function carregarTurmasUniversidade($dados) {
	...
	if($dados['pflcod']) {
	    ...
	    $sql = "SELECT turid as codigo, i.iusnome || ' ( '||tu.turdesc||' )' as descricao 
	    		...
	    		INNER JOIN sispacto3.universidade uni ON uni.uniid = unc.uniid 
	    ...
	    if($dados['pflcod']==PFL_FORMADORIES) {
			...
			$sql = "SELECT turid as codigo, i.iusnome || ' ( '||tu.turdesc||' )' as descricao
				    		...
				    		INNER JOIN sispacto3.universidadecadastro unc ON unc.uncid = i.uncid
				    		INNER JOIN sispacto3.universidade uni ON uni.uniid = unc.uniid
				    		WHERE t.pflcod='".PFL_COORDENADORLOCAL."' AND unc.uncid='".$dados['uncid']."' ORDER BY descricao";
			 
			$db->monta_combo('turidcl_destino', $sql, 'S', 'Selecione', '', '', '', '', 'S', 'turidcl_destino','', $_REQUEST['turid']);
...
function trocarUniversidade($dados) {
	...
	$db->executar($sql);
	$sql = "UPDATE sispacto3.usuarioresponsabilidade SET uncid='".$dados['uncid']."' WHERE usucpf=(SELECT iuscpf FROM sispacto3.identificacaousuario WHERE iusd='".$dados['iusd']."') AND rpustatus='A'";
	$db->executar($sql);
	$sql = "UPDATE sispacto3.turmas SET uncid='".$dados['uncid']."' WHERE iusd='".$dados['iusd']."'";
	$db->executar($sql);

	$pflcod = $db->pegaUm("SELECT pflcod FROM sispacto3.tipoperfil WHERE iusd='".$dados['iusd']."'");
	$plpabreviacao = $db->pegaUm("SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod IN( {$pflcod} )");
	if($_HIERARQUIA_PFL[$pflcod]) $plpabreviacaosub = $db->pegaUm("SELECT plpabreviacao FROM sispacto3.pagamentoperfil WHERE pflcod IN( ".implode(",",$_HIERARQUIA_PFL[$pflcod])." )");
	
	if($dados['turid_destino']) {

		$otuid = $db->pegaUm("SELECT otuid FROM sispacto3.{$plpabreviacao}turma WHERE iusd='".$dados['iusd']."'");
		
		if($otuid) {

			$sql = "UPDATE sispacto3.{$plpabreviacao}turma SET turid='".$dados['turid_destino']."' WHERE iusd='".$dados['iusd']."'";
			$db->executar($sql);
	...
	if($dados['turidcl_destino']) {
	
		$otuid = $db->pegaUm("SELECT otuid FROM sispacto3.{$plpabreviacao}turmacl WHERE iusd='".$dados['iusd']."'");
	
		if($otuid) {
	
			$sql = "UPDATE sispacto3.{$plpabreviacao}turmacl SET turid='".$dados['turidcl_destino']."' WHERE iusd='".$dados['iusd']."'";
			$db->executar($sql);
				
		} else {
	
			$sql = "INSERT INTO sispacto3.{$plpabreviacao}turmacl(
			turid, iusd, otustatus, otudata)
			VALUES ('".$dados['turidcl_destino']."', '".$dados['iusd']."', 'A', NOW());";
	...
	if($turid) {
		...
		$sql = "SELECT * FROM sispacto3.{$plpabreviacaosub}turma WHERE turid='".$turid."'";
		$mt = $db->carregar($sql);
	}
	...
	$iu = $db->pegaLinha("SELECT iusnome, pfldsc FROM sispacto3.identificacaousuario i 
					INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
					INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod  
					WHERE i.iusd='".$dados['iusd']."'");
	...
	if($dados['return']) {
		...
		$al = array("alert"=>str_replace(";",'\n',$msg),"location"=>"sispacto3.php?modulo=principal/mec/mec&acao=A&aba=trocaruniversidade");
		alertlocation($al);
...
function carregarCertificacaoEquipe($dados) {
	...
	$limit = $db->pegaUm("SELECT plpmaximobolsas FROM sispacto3.pagamentoperfil WHERE pflcod='".$pflcod."'");
	
	$sql = "SELECT f.fpbid,  m.mesdsc || '/' || fpbanoreferencia as referencia FROM sispacto3.folhapagamentouniversidade u 
			INNER JOIN sispacto3.folhapagamento f ON f.fpbid = u.fpbid 
			INNER JOIN public.meses m ON m.mescod::integer = f.fpbmesreferencia
	...
	if($folhapagamento) {
		foreach($folhapagamento as $key => $fpb) {
			...
			$c[] = "COALESCE((SELECT mavtotal FROM sispacto3.mensarioavaliacoes ma 
		    		 INNER JOIN sispacto3.mensario me ON me.menid = ma.menid 
					 WHERE me.iusd = i.iusd AND me.fpbid='{$fpb['fpbid']}'),0.00) as total_{$key}";
			
			$c[] = "COALESCE((SELECT mavfrequencia FROM sispacto3.mensarioavaliacoes ma
					 INNER JOIN sispacto3.mensario me ON me.menid = ma.menid
	...
	$sql = "SELECT '<img align=\"absmiddle\" id=\"img_'||foo.iuscpf||'\" src=\"../imagens/'||CASE WHEN foo.mavrecomendadocertificacao='1' THEN 'valida4.gif'
			...
			SELECT fee.iuscpf,
				   ...
				   (SELECT mavrecomendadocertificacao FROM sispacto3.mensarioavaliacoes ma INNER JOIN sispacto3.mensario m ON m.menid = ma.menid INNER JOIN sispacto3.identificacaousuario i ON i.iusd = m.iusd WHERE i.iuscpf = fee.iuscpf AND ma.iusdavaliador='".$dados['iusd']."' AND m.fpbid='".$dados['fpbid']."' AND mavrecomendadocertificacao is not null) as mavrecomendadocertificacao,
				   (SELECT mavrecomendadocertificacaojustificativa FROM sispacto3.mensarioavaliacoes ma INNER JOIN sispacto3.mensario m ON m.menid = ma.menid INNER JOIN sispacto3.identificacaousuario i ON i.iusd = m.iusd WHERE i.iuscpf = fee.iuscpf AND ma.iusdavaliador='".$dados['iusd']."' AND m.fpbid='".$dados['fpbid']."' AND mavrecomendadocertificacao is not null) as mavrecomendadocertificacaojustificativa
...
function certificarEquipe($dados) {
	...
	if($dados['certificacao']) {
		foreach($dados['certificacao'] as $iuscpf => $recomendacao) {
			...
			$iusd = $db->pegaUm("SELECT iusd FROM sispacto3.identificacaousuario WHERE iuscpf='".$iuscpf."'");

			$dadosmensario = criarMensario(array("iusd"=>$iusd,"fpbid"=>$dados['fpbid']));
				
			if($dadosmensario['esdid']!=ESD_APROVADO_MENSARIO) {
	
				$sql = "SELECT mavid FROM sispacto3.mensarioavaliacoes WHERE menid='".$dadosmensario['memid']."' AND iusdavaliador='".$dados['iusdavaliador']."'";
				$mavid = $db->pegaUm($sql);
	
				if($mavid) {
							
					$sql = "UPDATE sispacto3.mensarioavaliacoes SET mavrecomendadocertificacao='".$recomendacao."', mavtotal='0', mavrecomendadocertificacaojustificativa=".(($dados['mavrecomendadocertificacaojustificativa'][$iuscpf])?"'".$dados['mavrecomendadocertificacaojustificativa'][$iuscpf]."'":"NULL")." WHERE mavid='".$mavid."'";
					$db->executar($sql);
...
function salvarOrientacaoAdm($dados) {
	...
	$oabid = $db->pegaUm("SELECT oabid FROM sispacto3.orientacaoaba WHERE abaid='".$dados['abaid']."'");

	if($oabid) {

		$sql = "UPDATE sispacto3.orientacaoaba SET oabdesc='".$dados['oabdesc']."' WHERE oabid='".$oabid."'";
		$db->executar($sql);

	} else {

		$sql = "INSERT INTO sispacto3.orientacaoaba(
...
function criarBotoesNavegacao($dados) {
	...
	if($dados['url']) {
		...
		$aba = $db->pegaLinha("SELECT abaendereco, abaordem, abapai, abafuncaomostrar FROM sispacto3.abas WHERE abaendereco='".$dados['url']."'");
		
		$abaanterior = $db->pegaLinha("SELECT abaendereco, abafuncaomostrar FROM sispacto3.abas WHERE abaordem='".($aba['abaordem']-1)."' AND abapai='".$aba['abapai']."'");
		$abaproxima = $db->pegaLinha("SELECT abaendereco, abafuncaomostrar FROM sispacto3.abas WHERE abaordem='".($aba['abaordem']+1)."' AND abapai='".$aba['abapai']."'");
...
function carregarTurmasCoordenadorLocal($dados) {
	...
	$sql = "INSERT INTO sispacto3.orientadorturmaoutros(
            turid, iusd, otustatus, otudata)
			...
			INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd AND t.pflcod=".PFL_ORIENTADORESTUDO." 
			LEFT JOIN sispacto3.orientadorturmaoutros ot ON ot.iusd = i.iusd  
	...
	$sql = "INSERT INTO sispacto3.orientadorturmaoutros(
			...
			INNER JOIN workflow.documento d2 ON d2.docid = u.docidformacaoinicial
			INNER JOIN sispacto3.estruturacurso e ON e.uncid = u.uncid
			INNER JOIN sispacto3.abrangencia a ON a.ecuid = e.ecuid AND a.esfera='E' 
			INNER JOIN territorios.municipio m ON m.muncod = a.muncod 
			INNER JOIN sispacto3.pactoidadecerta p ON p.estuf = m.estuf 
			INNER JOIN sispacto3.identificacaousuario i ON i.picid = p.picid
			INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd AND t.pflcod=".PFL_COORDENADORLOCAL."
...
function condicaoComposicaoTurma( $picid, $uncid ) {
	...
	if($uncid) {
		...
		$sql = "SELECT i.iusnome, tu.turid, CASE WHEN tu.turid IS NOT NULL THEN (SELECT count(*) FROM sispacto3.orientadorturmaoutros WHERE turid=tu.turid) ELSE 0 END ntur  FROM sispacto3.identificacaousuario i 
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd ANd t.pflcod='".PFL_SUPERVISORIES."' 
				LEFT JOIN sispacto3.turmas tu ON tu.iusd = i.iusd 
				WHERE i.uncid='".$uncid."'";
		...
		$sql = "SELECT i.iusnome, p.pfldsc FROM sispacto3.identificacaousuario i 
				INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd ANd t.pflcod IN('".PFL_FORMADORIESP."','".PFL_FORMADORIES."','".PFL_COORDENADORLOCAL."') 
				INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod 
				LEFT JOIN sispacto3.orientadorturmaoutros ot ON ot.iusd = i.iusd  
...
function carregarTurmasOutros($dados) {
	...
	$sql = "SELECT
				...
				(SELECT COUNT(*) FROM sispacto3.orientadorestudoturmacl ot INNER JOIN sispacto3.tipoperfil tt ON tt.iusd = ot.iusd WHERE turid=t.turid) as nalunos
			FROM sispacto3.turmas t
			INNER JOIN sispacto3.identificacaousuario i ON i.iusd = t.iusd
			LEFT JOIN sispacto3.tipoperfil t1 ON t1.iusd = i.iusd
...
function abaTurmaCLs($dados) {
	...
	$qtd_cl = $db->pegaUm("SELECT count(*) FROM sispacto3.identificacaousuario i 
						   INNER JOIN sispacto3.tipoperfil t ON t.iusd = i.iusd 
						   WHERE t.pflcod='".PFL_COORDENADORLOCAL."' AND i.picid='".$_SESSION['sispacto3']['coordenadorlocal'][$_SESSION['sispacto3']['esfera']]['picid']."' AND i.iusstatus='A'");
...
function verificarInstrumentosPendentes($dados) {
	...
	$arr = $db->pegaLinha("SELECT it.intdsc, it.intid, it.inttitulo, it.inttextoperguntas, it.inttextoopcoes FROM sispacto3.instrumentostitulo it
						   INNER JOIN sispacto3.instrumentosperguntas i ON i.intid = it.intid
						   LEFT JOIN sispacto3.instrumentosrespostas ir ON ir.inpid = i.inpid AND ir.iusd='".$dados['iusd']."'
...
function carregarInstrumentos($dados) {
	...
	if($intline) {
		...
		$inpline = $db->carregar("SELECT i.inpid, i.inpdsc, i.inogrupo, ir.inoid FROM sispacto3.instrumentosperguntas i 
								  LEFT JOIN sispacto3.instrumentosrespostas ir ON ir.inpid = i.inpid AND ir.iusd='".$dados['iusd']."' 
		...
		if($inpline[0]) {
			...
			foreach($inpline as $inp) {
				...
				$sql = "(
              			...
              			SELECT '&inoid='||inoid||'&inpid={$inp['inpid']}' as codigo, inodsc as descricao FROM sispacto3.instrumentosopcoes WHERE inogrupo='{$inp['inogrupo']}' ORDER BY inoid
						)";
...
function gravarRespostaInstrumento($dados) {
	...
	if($dados['inpid']) {
		...
		$inrid = $db->pegaUm("SELECT inrid FROM sispacto3.instrumentosrespostas WHERE iusd='".$dados['iusd']."' AND inpid='".$dados['inpid']."'");
		
		if($inrid) {
	
			if($dados['inoid']) $sql = "UPDATE sispacto3.instrumentosrespostas SET inoid='".$dados['inoid']."' WHERE inrid='{$inrid}'";
			else $sql = "DELETE FROM sispacto3.instrumentosrespostas WHERE inrid='{$inrid}'";
			
		} else {
			
			if($dados['inoid']) {
				$sql = "INSERT INTO sispacto3.instrumentosrespostas(
				            iusd, inpid, inoid)
...
function montarGraficoPagamentos() {
	...
	$sql = "select pe.pfldsc as descricao, sum(p.pbovlrpagamento) as valor, '' as categoria from sispacto3.pagamentobolsista p
	...
	$sql = "select e.esddsc as descricao, count(*) as valor, '' as categoria
			from sispacto3.pagamentobolsista p
			inner join workflow.documento d on d.docid = p.docid
			inner join workflow.estadodocumento e on e.esdid = d.esdid
			where d.esdid!=".ESD_PAGAMENTO_EFETIVADO."
			group by e.esddsc";
...
function inserirComprovanteGRU($dados) {
	...
	$file = new FilesSimec( "comprovantegru", $campos, "sispacto3" );
	$file->setUpload( NULL, "arquivo" );
	
	$al = array("alert"=>"Comprovante GRU inserido com sucesso","location"=>"sispacto3.php?modulo=principal/comprovantegru&acao=A");
	alertlocation($al);
...
function excluirComprovanteGRU($dados) {
	...
	$sql = "UPDATE sispacto3.comprovantegru SET cogstatus='I' WHERE cogid='".$dados['cogid']."'";
	$db->executar($sql);
	$db->commit();
	
	$al = array("alert"=>"Comprovante GRU removido com sucesso","location"=>"sispacto3.php?modulo=principal/comprovantegru&acao=A");
	alertlocation($al);
...
function salvarInstrumentosAbertos($dados) {
	...
	$iraid = $db->pegaUm("SELECT iraid FROM sispacto3.instrumentosrespostasabertas WHERE iusd='".$dados['iusd']."'");
	
	if($iraid) {

		$sql = "UPDATE sispacto3.instrumentosrespostasabertas SET irapergunta1=".(($dados['irapergunta1'])?"'".$dados['irapergunta1']."'":"NULL").", 
		...
		$sql = "INSERT INTO sispacto3.instrumentosrespostasabertas(
	...
	$al = array("alert"=>"Pesquisa gravada com sucesso","location"=>"sispacto3.php?modulo=".$dados['modulo']."&acao=A&aba=principal");
	alertlocation($al);
	
}

function inserirLogRequisicao($dados) {
	global $db;
	
	$sql = "INSERT INTO sispacto3.logrequisicao(
            mnuid, usucpf, lordata, lorrequisicao, lordados)
    		VALUES ('".$_SESSION['mnuid']."', '".$_SESSION['usucpf']."', NOW(), '".$dados['requisicao']."', '".addslashes(simec_json_encode($dados))."');";
...
function mostraAbaEquipeIES($dados) {
	global $db;
	...
	$sql = "SELECT u.uncid FROM sispacto3.universidadecadastro u
			...
			WHERE d.esdid='".ESD_VALIDADO_COORDENADOR_IES."' AND u.uncid='".$_SESSION['sispacto3']['universidade']['uncid']."'";

	$uncid = $db->pegaUm($sql);

	if($uncid) return true;
	else return false;
}

function mostraAbaTurmaIES($dados) {
	global $db;

	$sql = "SELECT u.uncid FROM sispacto3.universidadecadastro u
			INNER JOIN workflow.documento d ON d.docid = u.docidestruturaformacao 
			INNER JOIN workflow.documento d2 ON d2.docid = u.docidequipeies
			WHERE d.esdid='".ESD_VALIDADO_COORDENADOR_IES."' AND d2.esdid='".ESD_VALIDADO_COORDENADOR_IES."' AND u.uncid='".$_SESSION['sispacto3']['universidade']['uncid']."'";
...
function mostraAbaVisualizacaoProjeto($dados) {
	global $db;
	...
	$sql = "SELECT u.uncid FROM sispacto3.universidadecadastro u 
			...
			WHERE d.esdid='".ESD_VALIDADO_COORDENADOR_IES."' AND 
				  ...
				  u.uncid='".$_SESSION['sispacto3']['universidade']['uncid']."'";
...
function exibirPeriodosReferenciaUniversidade($dados) {
	...
	$sql = "SELECT '<input type=checkbox name=\"rfuid[]\" value=\"'||fu.rfuid||'\" '||CASE WHEN fu.rfustatus='A' THEN '' ELSE 'checked' END||' onclick=\"atualizarFolhaPagamento('||fu.rfuid||', this);\">' as acao, un.unisigla||' - '||un.uninome as universidade,
				    ...
				    '<input type=checkbox onclick=\"inserirTipoAvaliacao('||fu.uncid||','||fu.pflcod||','||f.fpbid||',this);\" '||CASE WHEN (SELECT count(*) FROM sispacto3.tipoavaliacaoperfil WHERE uncid=fu.uncid AND pflcod=fu.pflcod AND fpbid=f.fpbid)=0 THEN '' ELSE 'checked' END||'>' as avalia
			FROM sispacto3.folhapagamentouniversidade fu
			INNER JOIN sispacto3.folhapagamento f ON f.fpbid = fu.fpbid
			INNER JOIN public.meses m ON m.mescod::integer = f.fpbmesreferencia
			INNER JOIN sispacto3.universidadecadastro u ON u.uncid = fu.uncid
			INNER JOIN sispacto3.universidade un ON un.uniid = u.uniid
...
function atualizarPeriodosReferenciaUniversidade($dados) {
	...
	$sql = "UPDATE sispacto3.folhapagamentouniversidade SET rfustatus='".$dados['status']."' WHERE rfuid='".$dados['rfuid']."'";
	$db->executar($sql);
	$db->commit();
}
...
function inserirTipoAvaliacao($dados) {
	...
	if($dados['tipo']=='inserir') {
		...
		$sql = "INSERT INTO sispacto3.tipoavaliacaoperfil(
	...
	if($dados['tipo']=='remover') {
		...
		$sql = "DELETE FROM sispacto3.tipoavaliacaoperfil WHERE fpbid='".$dados['fpbid']."' AND pflcod='".$dados['pflcod']."' AND uncid='".$dados['uncid']."'";
