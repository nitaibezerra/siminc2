...
var k = 0;
var t = opener.document.formulario.municipios;
...
function obterMarcados(Nome,Valor,Estado) {     
	...
	if ( checkBox.checked ) { 
		if((opener.document.formulario.municipios.options.length == 1) && (opener.document.formulario.municipios.options[0].value == "")){
			opener.document.formulario.municipios.options[0] = null;
		}
	  	var d=opener.document.formulario.municipios.options.length++;
		opener.document.formulario.municipios.options[d].text = Estado;
		opener.document.formulario.municipios.options[d].value = Valor;
		opener.document.formulario.municipios.options[d].setAttribute("selected","selected");
	}else{
		var listaOpcoes = opener.document.formulario.municipios.options;
		for(x = 0 ; x< listaOpcoes.length; x++){
			if(listaOpcoes[x].value == Valor ){
				opener.document.formulario.municipios.options[x] = null;
			}
			if(listaOpcoes.length == 0){
				var textocombogeral = "Duplo clique para selecionar da lista"; 
				var d=opener.document.formulario.municipios.options.length++;
				opener.document.formulario.municipios.options[d].text = textocombogeral;
				opener.document.formulario.municipios.options[d].value = "";
