...
function removerProfessorAlfabetizador($dados) {
	...
	$sql = "DELETE FROM sispacto.tipoperfil WHERE iusd='".$dados['iusd']."' AND pflcod='".PFL_PROFESSORALFABETIZADOR."'";
	$db->executar($sql);
...
function inserirProfessoresAlfabetizadores($dados) {
	...
	if(!$_SESSION['sispacto']['coordenadorlocal'][$_SESSION['sispacto']['esfera']]['picid'])
		die("<script>alert('As informações não foram gravadas. Houve perdas de informações internas. Você será direcionado para a tela principal.');window.location='sispacto.php?modulo=inicio&acao=C';</script>");
	...
	if($dados['cpf']) {
		foreach($dados['cpf'] as $cpf) {
			...
			$palid = $db->pegaUm("SELECT palid FROM sispacto.professoresalfabetizadores WHERE cpf='".str_replace(array(".","-"),array(""),$cpf)."'");
			...
			if($pfldsc) {
				...
				$al = array("alert"=>"Caso queira indicar este CPF(".$dados['nome'][$cpf].") como Professor Alfabetizador, é necessário antes remove-lo do perfil(".$pfldsc.",".$descricao.").","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
				alertlocation($al);
			}
	...
	$professoesalfabetizados = carregarDadosIdentificacaoUsuario(array("picid" => $_SESSION['sispacto']['coordenadorlocal'][$_SESSION['sispacto']['esfera']]['picid'],"pflcod" => PFL_PROFESSORALFABETIZADOR));
	...
	if($dados['cpf']) {
		foreach($dados['cpf'] as $cpf) {
			...
			if($pfldsc) {
				...
				$al = array("alert"=>"Caso queira indicar este CPF(".$dados['nome'][$cpf].") como Orientador de Estudo, é necessário antes remove-lo do perfil(".$pfldsc.",".$descricao.").","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
				alertlocation($al);
			}
...
function carregarCoordenadorLocal($dados) {
	...
	if($dados['muncod']) { // se tiver muncod, simular coordenador municipal
		...
		if($picid) {
			...
			$_SESSION['sispacto']['coordenadorlocal']['iusdesativado'] = $infprof['iusdesativado'];
...
function excluirDocumento($dados) {
	...
	$al = array("alert"=>"Documento excluído com sucesso","location"=>"sispacto.php?modulo=principal/coordenadorlocal/inseriranexos&acao=A&apaid=".$dados['apaid']);
	alertlocation($al);
...
function verificarCoordenadorLocalTermoCompromisso($dados) {
	...
	if($coordlocal['iustermocompromisso']!="t") {
		$al = array("alert"=>"Antes de cadastrar os Orientadores de Estudo, preencha todos os campos obrigatórios da tela “Dados do Coordenador”.","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=dados");
		alertlocation($al);
	}
}
	...
	if($iusd) {
		...
		if($pflcod) {
			if($picid!=$dados['picid']) {
	 			$al = array("alert"=>"Este CPF ja possui um perfil (".$identificacaousuario['pfldsc'].",".$identificacaousuario['descricao'].") no sistema e não pode ser cadastrado","location"=>"sispacto.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
	 			alertlocation($al);
			}
		} else {
...
function alterarStatusIndicacao($dados) {
	...
	$al = array("alert"=>"Situação alterada com sucesso","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	alertlocation($al);
...
function carregarTurmasOrientadores($dados) {
	...
	$sql = "SELECT 
				...
				CASE WHEN i.iusformacaoinicialorientador = TRUE THEN t.turdesc ELSE '<font color=blue;>Orientador não possui Formação Inicial</font>' END as turdesc,
				i.iuscpf,
				i.iusnome,
				i.iusemailprincipal,
...
function gravarJustificativasTroca($dados) {
	...
	$db->commit();
	$al = array("alert"=>"Justificativas para trocar os Orientadores de Estudo foram salvas com sucesso. Para submeter ao MEC, acesse a aba Resumo Orientadores de Estudo e clique em Enviar para análise da substituição do(s) Orientador(es) pelo MEC.","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	alertlocation($al);
...
function removerJustificativaTroca($dados) {
	...
	$al = array("alert"=>"Justificativa para substituição do(s) Orientadores de Estudo removida com sucesso","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	alertlocation($al);
...
function trocarOrientadorEstudoMunicipio($dados) {
	...
	if(!$_SESSION['sispacto']['coordenadorlocal']['uncid']) {
	 	$al = array("alert"=>"Coordenador Local não vinculado a nenhuma Universidade","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	 	alertlocation($al);
	}
	...
	if($retorno) {
		...
		$db->commit();
	 	...
	 	$al = array("alert"=>"Novo Usuário ja possui atribuções no SISPACTO, por isso não pode ser inserido","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=orientadorestudo");
	 	alertlocation($al);
	}
...
function verificarCenso2012($dados) {
	global $db;
	$sql = "SELECT m.estuf||'/'||m.mundescricao FROM sispacto.professoresalfabetizadores p INNER JOIN territorios.municipio m ON m.muncod = p.muncod WHERE cpf='".$dados['cpf']."'";
...
function gerenciarMateriais($dados) {
	...
	$al = array("alert"=>"Informações sobre materiais salvas com sucesso","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocal&acao=A&aba=materiais");
	alertlocation($al);
...
function excluirAtividadeCoordenadorLocal($dados) {
	...
	$al = array("alert"=>"Atividade excluída com sucesso","location"=>"sispacto.php?modulo=principal/coordenadorlocal/coordenadorlocalexecucao&acao=A&aba=gestaomobilizacao");
	alertlocation($al);
