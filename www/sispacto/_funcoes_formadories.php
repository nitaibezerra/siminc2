...
function carregarFormadorIES($dados) {
	...
	$_SESSION['sispacto']['formadories'] = array("descricao" => $arr['descricao']." ( ".$infprof['iusnome']." )",
												 "curid" => $arr['curid'], 
												 "uncid" => $arr['uncid'], 
												 "reiid" => $arr['reiid'], 
												 "estuf" => $arr['uniuf'], 
	...
	if($dados['direcionar']) {
		$al = array("location"=>"sispacto.php?modulo=principal/formadories/formadories&acao=A&aba=principal");
		alertlocation($al);
	}
