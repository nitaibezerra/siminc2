...
<tr>
	...
	<td valign="middle" style="font-size:15px;"><b>Consulta Pagamento/Avaliação no SISPACTO 2013</b></td>
</tr>
</table>
<?php
...
if($_POST['requisicao']=='consultarPagamento') :
	...
	if(strlen(trim($_POST['datanascimento']))==10 && strlen(trim($_POST['cpf']))==14) {
		$sql = "SELECT uncid, iusd FROM sispacto.identificacaousuario WHERE iuscpf='".str_replace(array(".","-"),array("",""),$_POST['cpf'])."' AND iusdatanascimento='".formata_data_sql($_POST['datanascimento'])."'";
		$identificacaousuario = $db->pegaLinha($sql);
	}
	
	if(!$identificacaousuario['iusd']) {
		$al = array("alert"=>"Usuário não encontrado no SISPACTO","location"=>"sispacto_consulta_pagamento.php");
		alertlocation($al);
	}
	...
	if($identificacaousuario['uncid']) {
		...
		$sql = "SELECT i2.iusnome||' ( '||p.pfldsc||' )' as iusnome,
			       CASE WHEN ma.mavrecomendadocertificacao='1' THEN '<font style=color:blue;><b>Recomendado '|| CASE WHEN (t2.pflcod='".PFL_FORMADORIES."' OR t2.pflcod='".PFL_ORIENTADORESTUDO."') THEN 'para certificação IES' WHEN t2.pflcod='".PFL_COORDENADORLOCAL."' THEN 'para SISPACTO 2014' END ||'</b></font>'
				    	WHEN ma.mavrecomendadocertificacao='2' THEN '<font style=color:red;><b>Não recomendado '|| CASE WHEN (t2.pflcod='".PFL_FORMADORIES."' OR t2.pflcod='".PFL_ORIENTADORESTUDO."') THEN 'para certificação IES' WHEN t2.pflcod='".PFL_COORDENADORLOCAL."' THEN 'para SISPACTO 2014' END ||'</b></font>' END as certificacao,
		...
		$sql = "SELECT f.fpbid as codigo, 
					   ...
					   COALESCE((
					   	SELECT e.esddsc || ' ( ' || to_char(h.htddata,'dd/mm/YYYY HH24:MI') || ' )' as s FROM sispacto.pagamentobolsista p 
		...
		if($folhapagamento[0]) {
			...
			foreach($folhapagamento as $fl) {
				...
				$sql = "SELECT * FROM sispacto.mensario WHERE fpbid='".$fl['codigo']."' AND iusd='".$identificacaousuario['iusd']."'";
				...
				$sql = "SELECT pboid FROM sispacto.pagamentobolsista WHERE fpbid='".$fl['codigo']."' AND iusd='".$identificacaousuario['iusd']."'";
				$pboid = $db->pegaUm($sql);
			...
			if($pagamentosrestantes[0]) {
				...
				foreach($pagamentosrestantes as $pr) {
					...
					$sql = "SELECT * FROM sispacto.mensario WHERE fpbid='".$pr['codigo']."' AND iusd='".$identificacaousuario['iusd']."'";
					$mensario = $db->pegaLinha($sql);
					...
					$sql = "SELECT pboid FROM sispacto.pagamentobolsista WHERE fpbid='".$pr['codigo']."' AND iusd='".$identificacaousuario['iusd']."'";
					$pboid = $db->pegaUm($sql);
			...
			echo '<tr><td class="SubTituloCentro" colspan="2"><input type=button value=Voltar onclick="window.location=\'sispacto_consulta_pagamento.php\';"></td></tr>';
			echo '</table>';
			...
			$al = array("alert"=>"A universidade do Usuário não possui período de referência atribuído","location"=>"sispacto_consulta_pagamento.php");
			alertlocation($al);
		}
		
		
	} else {
		$al = array("alert"=>"Usuário não esta vinculado a nenhuma Universidade","location"=>"sispacto_consulta_pagamento.php");
		alertlocation($al);
	}
