...
function definirAbrangencia($dados) {
	...
	$sql = "SELECT
			...
			COALESCE(array_to_string(array(SELECT iusnome FROM sispacto.identificacaousuario i INNER JOIN sispacto.tipoperfil t ON i.iusd=t.iusd WHERE i.picid=pp.picid AND t.pflcod=".PFL_COORDENADORLOCAL."), ','),'Coordenador Local não cadastrado') as coordenadorlocal,
...
function cadastrarMunicipioAbrangencia($dados) {
	...
	$municipio_abrangencia = $db->carregarColuna("SELECT 'Não foi possível vincular o município - '||m.mundescricao||' esta cadastrado na Universidade: '||un.uninome as descricao FROM sispacto.abrangencia a 
    ...
    $existe_usr = $db->pegaUm("select usucpf from sispacto.usuarioresponsabilidade where usucpf='".str_replace(array(".","-"),array(""),$dados['iuscpf'])."' and pflcod='".PFL_COORDENADORIES."' AND rpustatus='A' AND uncid='".$dados['uncid']."'");
     ...
     if(!$existe_tpf) {
     	...
     	if($existe_tpf!=PFL_COORDENADORIES) {
 			$al = array("alert"=>"Este CPF ja possui um perfil (".$arrTp['pfldsc'].",".$arrTp['descricao'].") no sistema e não pode ser cadastrado","location"=>"sispacto.php?modulo=principal/coordenadorlocal/gerenciarcoordenadorlocal&acao=A&picid=".$dados['picid']);
 			alertlocation($al);
     	}
...
function atualizarOrcamentoExecucao($dados) {
	...
	$al = array("alert"=>"Orçamento atualizado com sucesso.","location"=>"sispacto.php?modulo=principal/universidade/universidadeexecucao&acao=A&aba=orcamentoexecucao");
	alertlocation($al);
...
function inserirEquipeRecursosHumanos($dados) {
	...
	if($dados['iusd']) {
		...
		if($dados['pflcod']!=PFL_FORMADORIES) {
			...
			if($existe) {
				$al = array("alert"=>"Não é possível trocar o perfil. Este usuário possui turma associada. Remova a turma para altera o perfil","location"=>"sispacto.php?modulo=principal/universidade/inserirequipe&acao=A&iusd=".$dados['iusd']);
				alertlocation($al);
			}
		}
	...
	if($dados['pflcod']==PFL_COORDENADORADJUNTOIES) {
		if(!validarNumeroCoordenadorAdjuntoIES(array("uncid"=>$dados['uncid']))) {
			$al = array("alert"=>"Número de Coordenador Adjunto da IES está no máximo. Não é possível inserir o CPF","location"=>"sispacto.php?modulo=principal/universidade/inserirequipe&acao=A");
			alertlocation($al);
		}
	}
	...
	if(!$iusd) {
		...
		$sql = "SELECT 'Nome : '||i.iusnome||', Perfil : '||p.pfldsc||', CPF não pode ser cadastrado. É necessário remove-lo do perfil indicado.' as msg FROM sispacto.identificacaousuario i 
				INNER JOIN sispacto.tipoperfil t ON t.iusd = i.iusd 
				INNER JOIN seguranca.perfil p ON p.pflcod = t.pflcod
				WHERE i.iusd='".$iusd."'";
...
function validarFormadoresTurmas($dados) {
	global $db;
	$sql = "SELECT 'Formador ('||i.iusnome||') não foi vinculado a nenhuma turma.' as t FROM sispacto.identificacaousuario i 
...
function verificarCoordenadorIESTermoCompromisso($dados) {
	...
	if($coordies['iustermocompromisso']!="t") {
		$al = array("alert"=>"Por favor preencha todos os campos obrigatórios da tela “Dados Coordenador IES”.","location"=>"sispacto.php?modulo=principal/universidade/universidade&acao=A&aba=dados");
		alertlocation($al);
	}
}
...
function verificarValidacaoEstruturaFormacao($dados) {
	...
	if($tot_abrangencia == 0 || !$existe_sede) {
		$al = array("alert"=>"É necessário cadastrar as informações na aba Estrutura da Formação","location"=>"sispacto.php?modulo=principal/universidade/universidade&acao=A&aba=estrutura_curso");
		alertlocation($al);
	}
...
function atualizarRelatorioFinal($dados) {
	...
	$al = array("alert"=>"Relatório Final atualizado com sucesso","location"=>"sispacto.php?modulo=principal/universidade/universidadeexecucao&acao=A&aba=relatoriofinal");
	alertlocation($al);
	...
	if($dados['xls']) {
		...
		$cls = "replace(to_char(foo.iuscpf::numeric, '000:000:000-00'), ':', '.') as iuscpf,
			   ...
			   CASE WHEN foo.mavrecomendadocertificacao='1' THEN 'Recomendado'
					WHEN foo.mavrecomendadocertificacao='2' THEN 'Não recomendado'
					ELSE 'Não avaliado ' END as acao";
		...
		$cls = "'<font style=font-size:xx-small;>'||replace(to_char(foo.iuscpf::numeric, '000:000:000-00'), ':', '.')||'</font>' as iuscpf,
			   ...
			   CASE WHEN foo.mavrecomendadocertificacao='1' THEN '<font style=font-size:xx-small;color:blue;>Recomendado</font>'
					WHEN foo.mavrecomendadocertificacao='2' THEN '<font style=font-size:xx-small;color:red;>Não recomendado</font>'
					ELSE '<font style=font-size:xx-small;>Não avaliado</font> ' END || CASE WHEN foo.mavrecomendadocertificacaojustificativa IS NOT NULL THEN ' <img src=\"../imagens/page_attach.png\" align=absmiddle onclick=\"alert(\''||foo.mavrecomendadocertificacaojustificativa||'\');\" style=\"cursor:pointer;\">' ELSE '' END as acao";
	}
