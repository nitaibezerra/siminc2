...
and mun.muncod not in (select distinct muncod from sispacto.usuarioresponsabilidade where rpustatus='A' and pflcod='".PFL_COORDENADORLOCAL."' and muncod is not null)
order by
...
foreach($users as $us) {
	...
	$existe = $db->pegaUm("select * from sispacto.pactoidadecerta where muncod='".$us['muncod']."'");
	
	if($existe) {
		$erro[] = "Duplicidade nos cadastros - ".$us['cpf_coordenador'];
	} else {
		
		$sql = "INSERT INTO sispacto.pactoidadecerta(
            	estuf, muncod, docid, picselecaopublica, picstatus)
    			VALUES (NULL, '".$us['muncod']."', NULL, FALSE, 'A') returning picid;";
    	$picid = $db->pegaUm($sql);
    	
    	$sql = "SELECT iusd FROM sispacto.identificacaousuario WHERE iuscpf='".$us['cpf_coordenador']."'";
    	$iusd = $db->pegaUm($sql);
    	...
    	$existe_tipoperfil = $db->pegaUm("SELECT * FROM sispacto.tipoperfil WHERE iusd='".$iusd."'");
    	...
    	$existe_ur = $db->pegaUm("SELECT * from sispacto.usuarioresponsabilidade WHERE pflcod='".PFL_COORDENADORLOCAL."' and usucpf='".$us['cpf_coordenador']."'");
...
left join
par.instrumentounidade inu on inu.estuf = est.estuf
left join
par.pfadesaoprograma adp on adp.inuid = inu.inuid and adp.tapid in (13,14)
left join
workflow.documento doc on doc.docid = adp.docid
...
and
esd.esdid in( 484, 485, 489 ) 
and est.estuf not in (select distinct estuf from sispacto.usuarioresponsabilidade where rpustatus='A' and pflcod='".PFL_COORDENADORLOCAL."' and estuf is not null)
order by
est.estuf";
...
foreach($users as $us) {
	...
	$existe = $db->pegaUm("select * from sispacto.pactoidadecerta where estuf='".$us['estuf']."'");
	...
	if($existe) {
		...
		$sql = "INSERT INTO sispacto.pactoidadecerta(
            	estuf, muncod, docid, picselecaopublica, picstatus)
    			VALUES ('".$us['estuf']."', NULL, NULL, FALSE, 'A') returning picid;";
    	$picid = $db->pegaUm($sql);
    	...
    	if(!$iusd) {
	    	...
	    	$sql = "INSERT INTO sispacto.identificacaousuario(
		            picid, muncod, iuscpf, iusnome, iusemailprincipal,  
		            iusdatainclusao, iusstatus, tvpid, funid, foeid)
				    VALUES ('".$picid."', NULL, '".$us['cpf_coordenador']."', '".$us['coordenador']."', '".$us['email_coordenador']."',  
				            NOW(), 'A', '".$us['vinculoid']."', '".$us['funcao_id']."', '".$us['formacaoid']."') returning iusd;";
	    	$iusd = $db->pegaUm($sql);
    	...
    	if(!$existe_us) {
	    	...
	    	$sql = "INSERT INTO seguranca.usuario(
	            	usucpf, usunome, usuemail, usustatus, usufoneddd, usufonenum, ususenha, suscod)
	    			VALUES ('".$us['cpf_coordenador']."', '".$us['coordenador']."', '".$us['email_coordenador']."', 'A', '".trim(str_replace(array("(",")"),array("",""),$tel[0]))."', '".trim($tel[1])."', '".md5_encrypt_senha( 'simecdti', '' )."', 'A');";
	    	$db->executar($sql);
    	...
    	if(!$existe_ur) {
	    	$sql = "INSERT INTO sispacto.usuarioresponsabilidade(
					    ...
					    VALUES ('".PFL_COORDENADORLOCAL."', '".$us['cpf_coordenador']."', 'A', NOW(), '".$us['estuf']."');";
	    	$db->executar($sql);
    	}
