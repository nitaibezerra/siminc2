...
function enviaEmailCadSolicitacao( $scsid, $acao = null )
{
    ...
    //pega o emails dos perfis (especialista indra ou mba)
    $sqlx = "select ctrid from fabrica.analisesolicitacao where scsid={$scsid}";
    $ctrid = $db->pegaUm($sqlx);

    if($ctrid == 8){ //MBA

        $sqlx2 = "SELECT DISTINCT
                            u.usuemail
                        FROM
                            seguranca.usuario  AS u
                        INNER JOIN
                        ...
                        INNER JOIN
                            seguranca.usuario_sistema us ON us.usucpf = u.usucpf
                        LEFT JOIN
						    fabrica.usuarioresponsabilidade ur ON ur.usucpf = u.usucpf
                        WHERE
                            us.sisid = " . SISID_FABRICA . " AND
                            us.suscod = 'A' AND
                            ur.entid=872127 AND
                            p.pflcod in (" . PERFIL_ESPECIALISTA_SQUADRA . ")";

        $dadox2 = $db->carregarColuna($sqlx2);
        ...
        $sqlx2 = "SELECT DISTINCT
                        ...
                        WHERE
                            us.sisid = " . SISID_FABRICA . " AND
                            us.suscod = 'A' AND
                            (ur.entid is null or ur.entid=796705) AND
                            p.pflcod in (" . PERFIL_ESPECIALISTA_SQUADRA . ")";
...
function enviaEmailFluxoHistoricoOS( $odsid ) {
    ...
    */
    ...
    //pega o emails dos perfis (especialista indra ou mba)
    $sqlx = "select ctrid from fabrica.ordemservico where odsid={$odsid}";
    $ctrid = $db->pegaUm($sqlx);
