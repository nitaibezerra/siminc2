...
function listaEmpresas(){
	...
	$empresasExistentes = $db->carregar("SELECT distinct
										  ...
										  WHERE ent.entid in (872127,742798,796705,657076,657077)
...
function atribuiEmpresas($usucpf, $pflcod, $empresa){
	...
	$db->executar("UPDATE fabrica.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."'");
	if ($empresa[0]){
		foreach($empresa as $tipo) {
			$dadosur = $db->carregar("SELECT * FROM fabrica.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND entid = '". $tipo ."'");
			if($dadosur) {
...
<form name="formassocia" action="cadastro_responsabilidade_empresa.php" method="post">
