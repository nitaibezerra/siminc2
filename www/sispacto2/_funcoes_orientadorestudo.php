...
function carregarOrientadorEstudo($dados) {
	...
	$_SESSION['sispacto2']['orientadorestudo'] = array("descricao" => $arr['descricao']." ( ".$infprof['iusnome']." )",
													  "curid" 	  => $arr['curid'], 
													  "uncid" 	  => $arr['uncid'], 
													  "reiid" 	  => $arr['reiid'], 
													  "estuf" 	  => $arr['uniuf'], 
	...
	if($dados['direcionar']) {
		$al = array("location"=>"sispacto2.php?modulo=principal/orientadorestudo/orientadorestudo&acao=A&aba=principal");
		alertlocation($al);
	}
...
function mostrarAbaAvaliacaoComplementar($dados) {
	...
	$sql = "SELECT COUNT(*) as tot FROM sispacto2.grupoitensavaliacaocomplementar g 
			INNER JOIN sispacto2.grupoitensavaliacaocomplementarperfil p ON  p.gicid = g.gicid 
			WHERE pflcod IN(SELECT pflcod FROM sispacto2.tipoperfil WHERe iusd='".$_SESSION['sispacto2'][$dados['abapai']]['iusd']."')";
...
function exibirRelatoExperiencia($dados) {
	...
	if($dados['reeid']) {
		$relatoexperiencia = $db->pegaLinha("SELECT * FROM sispacto2.relatoexperiencia WHERE reeid='".$dados['reeid']."'");
...
function listaProfessoresRelatoExperiencia($dados) {
	...
	$sql = "SELECT '<img src=../imagens/'||CASE WHEN r.correcao1 IS NOT NULL AND r.correcao2 IS NOT NULL AND r.correcao3 IS NOT NULL AND r.correcao4 IS NOT NULL AND r.correcao5 IS NOT NULL THEN 'valida1.gif' ELSE 'valida2.gif' END||' onclick=\"exibirRelatoExperiencia('||reeid||')\" style=cursor:pointer;>' as acao, '<span style=font-size:x-small;>'||replace(to_char(i.iuscpf::numeric, '000:000:000-00'), ':', '.')||'</span>' as iuscpf, 
			...
			INNER JOIN sispacto2.relatoexperiencia r ON i.iusd = r.iusd
...
function atualizarCorrecaoRelatoExperiencia($dados) {
	...
	$sql = "UPDATE sispacto2.relatoexperiencia SET correcao1=".(($dados['correcao1'])?"'".$dados['correcao1']."'":"NULL").",
