...
function carregarProfessorAlfabetizador($dados) {
	...
	$_SESSION['sispacto2']['professoralfabetizador'] = array("descricao" => $arr['descricao']." ( ".$infprof['iusnome']." )",
															"curid" 	=> $arr['curid'], 
															"uncid" 	=> $arr['uncid'], 
															"reiid" 	=> $arr['reiid'], 
															"estuf" 	=> $arr['uniuf'], 
	...
	if($dados['direcionar']) {
		$al = array("location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=principal");
		alertlocation($al);
	}
...
function gerenciarMateriaisProfessores($dados) {
	...
	if(!$_SESSION['sispacto2']['professoralfabetizador']['iusd']) {
		$al = array("alert"=>"Não foi possível gravar as informações sobre materiais. Tente novamente mais tarde.","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=materiais");
		alertlocation($al);
	}
	
	$sql = "UPDATE sispacto2.materiaisprofessores
	 			...
	 			WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."'";
	
	$db->executar($sql);
	
	$sql = "INSERT INTO sispacto2.materiaisprofessores(
            	iusd, recebeumaterialpacto,  
            	criadocantinholeitura, mapstatus)
			 	SELECT '".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', 
			    		'".$dados['recebeumaterialpacto']."', 
			            '".$dados['criadocantinholeitura']."', 'A'
			    WHERE (SELECT mapid FROM sispacto2.materiaisprofessores WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."') IS NULL";
	
	$db->executar($sql);
	
	$db->commit();
			
	$sql = "SELECT mapid as t FROM sispacto2.materiaisprofessores WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."'";
	$mapid = $db->pegaUm($sql);
	...
	if($_FILES['arquivo']['error']=='0') {
		...
		$file = new FilesSimec("materiaisprofessoresfotos", $campos ,"sispacto2");
				
		$arquivoSalvo = $file->setUpload($dados['mafdsc']);
	}
	
	
	$al = array("alert"=>"Informações sobre materiais salvas com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function desvincularTurmaProfessor($dados) {
	...
	$sql = "UPDATE sispacto2.turmasprofessoresalfabetizadores SET tpastatus='I', tpajustificativadesvinculacao='".$dados['tpajustificativadesvinculacao']."' WHERE tpaid='".$dados['tpaid']."'";
	$db->executar($sql);
	$db->commit();
	
	$al = array("alert"=>"Desvinculação feita com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function confirmarRegenciaTurma($dados) {
	...
	if($dados['tpaconfirmaregencia']) {
		foreach($dados['tpaconfirmaregencia'] as $tpaid => $vl) {
			$db->executar("UPDATE sispacto2.turmasprofessoresalfabetizadores SET tpaconfirmaregencia=".$vl." WHERE tpaid='".$tpaid."'");
			$db->commit();			
		}
	}
	
	$al = array("alert"=>"Confirmação de regência feita com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function inserirTurmaProfessor($dados) {
	...
	if($inf_faltando) {
		$al = array("alert"=>"Estão faltando informações para o cadastramento da turma : ".'\n'.implode('\n',$inf_faltando).'\n'."Caso o erro persista (mesmo selecionando as informações necessárias), solicitamos que utilize outra máquina. Sugerimos Sistema Operacional: Window, Linux. Browser: Internet Explorer, Firefox, Google Chrome","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=dadosturmas");
		alertlocation($al);
	}
	
	$sql = "INSERT INTO sispacto2.turmasprofessoresalfabetizadores(
            ...
            SELECT pk_cod_entidade, no_entidade, fk_cod_municipio, no_email,
            	   ...
            	   '".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', 'A', false, (SELECT no_etapa_ensino FROM educacenso_2013.tab_etapa_ensino WHERE pk_cod_etapa_ensino='".$dados['pk_cod_etapa_ensino']."'), 
            	   true
	...
	$al = array("alert"=>"Turma do professor inserida com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function gerenciarInformacoesTurmasProfessor($dados) {
	...
	if($dados['tpaid']) {
		foreach($dados['tpaid'] as $tpaid) {
			...
			$sql = "UPDATE sispacto2.turmasprofessoresalfabetizadores SET 
	...
	$al = array("alert"=>"Informações das Turmas gravadas com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function excluirMateriaisProfessoresFoto($dados) {
	...
	$sql = "DELETE FROM sispacto2.materiaisprofessoresfotos WHERE mpfid='".$dados['mpfid']."'";
	$db->executar($sql);
	$db->commit();
	
	$al = array("alert"=>"Foto excluída com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=materiais");
	alertlocation($al);
...
function gravarAprendizagemTurma($dados) {
	...
	if($dados['tpaid']) {
		...
		$sql = "SELECT rfuparcela FROM sispacto2.folhapagamentouniversidade WHERE fpbid='".$dados['fpbid']."' AND pflcod='".PFL_PROFESSORALFABETIZADOR."' AND uncid='".$_SESSION['sispacto2']['professoralfabetizador']['uncid']."'";
	...
	if($dados['catid']) {
		...
		foreach($dados['catid'] as $tpaid => $arr) {
			...
			foreach($arr as $catid) {
				...
				$sql = "SELECT actid as id_aprendizagem FROM sispacto2.aprendizagemconhecimentoturma WHERE tpaid='".$tpaid."' AND catid='".$catid."'";
				$id_aprendizagem = $db->pegaUm($sql);
				
				if($id_aprendizagem) {
					$sql = "UPDATE sispacto2.aprendizagemconhecimentoturma SET actsim=".(($dados['actsim'][$tpaid][$catid])?"'".$dados['actsim'][$tpaid][$catid]."'":"NULL").", 
					...
					$sql = "SELECT count(*) as qtd FROM sispacto2.aprendizagemconhecimentoturma WHERE tpaid='".$tpaid."' AND catid='".$catid."'";
					$qtd = $db->pegaUm($sql);
					
					if(!$qtd) {
						
						$sql = "INSERT INTO sispacto2.aprendizagemconhecimentoturma(
								...
								FROM coalesce((SELECT actid::text FROM sispacto2.aprendizagemconhecimentoturma WHERE catid='".$catid."' AND tpaid='".$tpaid."'),NULL) as foo WHERE foo IS NULL";
	...
	$al = array("alert"=>"Dados gravados com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']);
	alertlocation($al);
...
function verificarAprendizagemTurma($dados) {
	...
	$tot = $db->pegaUm("SELECT count(actid) as t FROM sispacto2.aprendizagemconhecimentoturma a 
				 	    INNER JOIN sispacto2.turmasprofessoresalfabetizadores t ON a.tpaid = t.tpaid 
...
function gerenciarUsoMateriaisDidaticos($dados) {
	...
	if($dados['usomaterialdidatico']) {
		foreach($dados['usomaterialdidatico'] as $catid => $umdopcao) {
			...
			$umdid = $db->pegaUm("SELECT umdid FROM sispacto2.usomateriaisdidaticos WHERE catid='{$catid}' AND iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."'");
			
			if($umdid) {
				
				$sql = "UPDATE sispacto2.usomateriaisdidaticos SET umdopcao='{$umdopcao}' WHERE umdid='{$umdid}'";
				
				$db->executar($sql);
				
			} else {
				
				$sql = "INSERT INTO sispacto2.usomateriaisdidaticos(
            			iusd, catid, umdopcao)
    					VALUES ('".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', '{$catid}', '{$umdopcao}');";
...
function gravarRelatoExperiencia($dados) {
	...
	if(!$_SESSION['sispacto2']['professoralfabetizador']['iusd']) {
		$al = array("alert"=>"Foram encontrados problemas internos. Por favor tente novamente.","location"=>"sispacto2.php?modulo=inicio&acao=C");
		alertlocation($al);
	}
	
	$reeid = $db->pegaUm("SELECT reeid FROM sispacto2.relatoexperiencia WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."'");
	...
	if($reeid) {
		...
		$sql = "UPDATE sispacto2.relatoexperiencia SET 
		...
		$sql = "INSERT INTO sispacto2.relatoexperiencia(
			            iusd, 
						reeareatematica, 
						reeturma, 
			    ...
			    VALUES ('".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', 
...
function gravarImpressoesANA($dados) {
	...
	if($dados['tpaid']) {
		foreach($dados['tpaid'] as $tpaid) {
			$imaid = $db->pegaUm("SELECT imaid FROM sispacto2.impressoesana WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."' AND tpaid='{$tpaid}'");
			
			if($imaid) $sql = "UPDATE sispacto2.impressoesana SET imaparticiparam='".$dados['imaparticiparam'][$tpaid]."', imaacessoresultados='".$dados['imaacessoresultados'][$tpaid]."', imaresultadosescola='".$dados['imaresultadosescola'][$tpaid]."', 
					            ...
					            WHERE imaid='{$imaid}'";
			else $sql = "INSERT INTO sispacto2.impressoesana(
					            ...
					            tpaid)
					    VALUES ('".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', '".$dados['imaparticiparam'][$tpaid]."', '".$dados['imaacessoresultados'][$tpaid]."', '".$dados['imaresultadosescola'][$tpaid]."', 
...
function gravarQuestoesDiversas($dados) {
	...
	$qudid = $db->pegaUm("SELECT qudid FROM sispacto2.questoesdiversasatv8 WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."'");
	
	if($qudid) $sql = "UPDATE sispacto2.questoesdiversasatv8 SET qudformaapoia='".implode(";",$dados['qudformaapoia'])."', 
	...
	else $sql = "INSERT INTO sispacto2.questoesdiversasatv8(
            iusd, qudformaapoia, qudperiodicidade, qudformaparticipa, 
            qudmedidaparticipa, qudmedidacontribui)
    		VALUES ('".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', '".implode(";",$dados['qudformaapoia'])."', '".$dados['qudperiodicidade']."', '".implode(";",$dados['qudformaparticipa'])."', 
...
function gravarContribuicaoPacto($dados) {
	...
	$cpaid = $db->pegaUm("SELECT cpaid FROM sispacto2.contribuicaopacto WHERE iusd='".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."'");

	if($cpaid) $sql = "UPDATE sispacto2.contribuicaopacto SET cpacontribuicao1='".$dados['cpacontribuicao1']."',
	...
	else $sql = "INSERT INTO sispacto2.contribuicaopacto(
            ...
            cpadificuldade2)
    VALUES ('".$_SESSION['sispacto2']['professoralfabetizador']['iusd']."', '".$dados['cpacontribuicao1']."', '".$dados['cpacontribuicao2']."', '".$dados['cpacontribuicao3']."', 
...
function gravarAprendizagemTurma2($dados) {
	...
	if($dados['catid']) {
		...
		foreach($dados['catid'] as $tpaid => $arr) {
			...
			foreach($arr as $catid) {
				...
				$sql = "SELECT actid as id_aprendizagem FROM sispacto2.aprendizagemconhecimentoturma2 WHERE tpaid='".$tpaid."' AND catid='".$catid."'";
				$id_aprendizagem = $db->pegaUm($sql);

				if($id_aprendizagem) {
					$sql_e[] = "UPDATE sispacto2.aprendizagemconhecimentoturma2 SET actsim=".(($dados['actsim'][$tpaid][$catid])?"'".$dados['actsim'][$tpaid][$catid]."'":"NULL").",
							actparcialmente=".(($dados['actparcialmente'][$tpaid][$catid])?"'".$dados['actparcialmente'][$tpaid][$catid]."'":"NULL").",
																			   actnao=".(($dados['actnao'][$tpaid][$catid])?"'".$dados['actnao'][$tpaid][$catid]."'":"NULL")."
							WHERE actid='".$id_aprendizagem."';";
					...
					$sql = "SELECT count(*) as qtd FROM sispacto2.aprendizagemconhecimentoturma2 WHERE tpaid='".$tpaid."' AND catid='".$catid."'";
					$qtd = $db->pegaUm($sql);
						
					if(!$qtd) {

						$sql_e[] = "INSERT INTO sispacto2.aprendizagemconhecimentoturma2(
			            		catid, tpaid, actsim, actparcialmente, actnao)
								SELECT '".$catid."',
									   '".$tpaid."',
									   ".(($dados['actsim'][$tpaid][$catid])?"'".$dados['actsim'][$tpaid][$catid]."'":"NULL").",
									   ".(($dados['actparcialmente'][$tpaid][$catid])?"'".$dados['actparcialmente'][$tpaid][$catid]."'":"NULL").",
									   ".(($dados['actnao'][$tpaid][$catid])?"'".$dados['actnao'][$tpaid][$catid]."'":"NULL")."
									   		FROM coalesce((SELECT actid::text FROM sispacto2.aprendizagemconhecimentoturma2 WHERE catid='".$catid."' AND tpaid='".$tpaid."'),NULL) as foo WHERE foo IS NULL;";
	...
	$al = array("alert"=>"Dados gravados com sucesso","location"=>"sispacto2.php?modulo=principal/professoralfabetizador/professoralfabetizador&acao=A&aba=atividadesobrigatorias&fpbid=".$dados['fpbid']."&cattipo=".$dados['cattipo']);
	alertlocation($al);
