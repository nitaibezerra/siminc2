...
class Crypt_Blowfish
{
    ...
    public function encrypt($plainText)
    {
        if (!is_string($plainText)) {
            $type = gettype($plainText);
            die('Plain text must be a string, was ' . $type . ' instead.');
    ...
    public function decrypt($cipherText)
    {
        if (!is_string($cipherText)) {
            $type = gettype($cipherText);
            die('Cipher text must be a string, was ' . $type . ' instead.');
    ...
    public function setKey($key)
    {
        if (!is_string($key)) {
            $type = gettype($key);
            die('Key must be a string, was ' . $type . ' instead.');
        ...
        if ($len > 56 || $len == 0) {
            die('Key must be less than 56 characters and non-zero. Supplied key length: ' . $len);
        ...
        require_once 'default_key.php';
        // require_once 'Blowfish/default_key.php';
        $this->_init();
