...
$db = new cls_banco();
	...
	if($_REQUEST['chk_superior']) {
		if(in_array("1", $_REQUEST['chk_superior'])) $classif[] = "Preexistentes";			
		if(in_array("2", $_REQUEST['chk_superior']))	$classif[] = "Criados em 2003/2010";			
		if(in_array("3", $_REQUEST['chk_superior']))	$classif[] = "Novos 2011/2012";
		if(in_array("4", $_REQUEST['chk_superior']))	$classif[] = "Novos 2013/2014";
		if(in_array("5", $_REQUEST['chk_superior']))	$classif[] = "Novas expansões";
	...
	CASE
	     ...
	     WHEN exi.exidsc='Novas expansões' THEN '5'
	END as tipo
...
/*
	$sql = "SELECT
				...
				mun.mundescricao,
				mun.muncod,
				trim(ref.estuf) as estuf,
				e.entid,
				ref.rfsid,
				ref.rfstipo,
				ref.rfscaracteristica, 
				CASE
				     WHEN ref.rfstipo='1 - Câmpus Preexistentes' THEN  '1'
				     WHEN ref.rfstipo='2 - Criadas (2003/2010)' THEN   '2'
				     WHEN ref.rfstipo='3 - Previstos (2011/2012)' THEN '3'
				     WHEN ref.rfstipo='4 - Propostos (2013/2014)' THEN '4'
				     WHEN ref.rfstipo='5 - Universidades Previstas (2013/2014)' THEN '5'
				END as tipo
				
		FROM 
			academico.redefederalsuperior ref
		LEFT JOIN
		...
		ORDER BY
			 ref.rfstipo DESC, e.entsig, e.entnome";
*/
	...
	if($dados):
		...
		foreach($dados as $d):
			...
			$conteudo .= "rfscaracteristica=\"". 'Não carregado' ."\" "; // adiciona UF;
