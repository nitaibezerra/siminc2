...
function gravarResponsabilidadeAcao($dados) {
    global $db;
    $sql = "UPDATE acompanhamento.usuarioresponsabilidade SET rpustatus='I' WHERE usucpf='".$dados['usucpf']."' AND pflcod='".$dados['pflcod']."'";
    $db->executar($sql);
    ...
    if ($dados['usuacaresp']) {
        foreach($dados['usuacaresp'] as $ungcod) {
            ...
            $sql = <<<DML
INSERT INTO acompanhamento.usuarioresponsabilidade(pflcod, usucpf, rpustatus, rpudata_inc, ungcod)
        ...
        <?php
            ...
            $sql = "
                ...
                FROM public.vw_subunidadeorcamentaria ung
                    LEFT JOIN acompanhamento.usuarioresponsabilidade urp ON(
                ...
                <?php
                    $sql = "
                        SELECT DISTINCT
                            ...
                            ung.suocod || ' - ' || ung.suonome AS descricao
                        FROM acompanhamento.usuarioresponsabilidade ur
