...
function lista(){
	...
	$perfils = $db->carregarColuna("SELECT 
										pus.pflcod 
									FROM 
	...
	$reg = $db->carregar("SELECT 
							...
							WHERE
								coordstatus = 'A'
								".(count($arrCoords)>0 ? " AND coordid in (".implode(",",$arrCoords).")" : ""));
	$count = count($reg);
...
function atribui($usucpf, $pflcod, $resp){
	...
	$db->executar("UPDATE catalogocurso.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND coordid is not null");
	if ($resp[0]){
		foreach($resp as $tipo) {
			$dadosur = $db->carregar("SELECT * FROM catalogocurso.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND coordid = '". $tipo ."'");
			if($dadosur) {
...
function buscaAtribuido($usucpf, $pflcod){
	...
	$sql = "SELECT 
			...
			FROM 
				catalogocurso.coordenacao c
			INNER JOIN catalogocurso.usuarioresponsabilidade u ON u.coordid = c.coordid
			WHERE
				u.rpustatus = 'A' AND u.usucpf = '$usucpf' AND u.pflcod = $pflcod";
