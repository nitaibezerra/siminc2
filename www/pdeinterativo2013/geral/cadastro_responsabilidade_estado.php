...
if(is_array($_POST['usuunidresp']) && @count($_POST['usuunidresp'])>0) {
	...
	$sqlInsRpu = "INSERT INTO pdeinterativo2013.usuarioresponsabilidade 
				  (
				  	estuf, 
				  	usucpf, 
				  	rpustatus, 
	...
	}else if ( $confirmarestados && ($pflcod == 386 || $pflcod == 385) ){
		//dbg(1,1);
		$sql = $sqlUpdRpuUsu;
		$db->executar($sql);
		foreach ($_POST['usuunidresp'] as $respcod) {
...
if(is_array($RS)) {
	...
	$sql = "select
			...
			from
			 territorios.estado u 
			 inner join pdeinterativo2013.unidproposto up on 
...
function retorna(objeto)
{
	...
	if (document.formulario.estuf[objeto].checked == true){
		...
		if (!campoSelect.options[0]) {
			campoSelect.options[0] = new Option('Clique na estado.', '', false, false);
		}
		sortSelect(campoSelect);
	}
}
