...
$sqlResponsabilidadesPerfil = "SELECT
							    tr.*
							   FROM
							    pdeinterativo2013.tprperfil p
							    INNER JOIN pdeinterativo2013.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
							   WHERE
		...
		switch ($rp["tprsigla"]) {
			...
			case "M": // Municípios
				...
				$sqlRespUsuario = "select
									...
									from 
										pdeinterativo2013.usuarioresponsabilidade ur
										inner join territorios.municipio m on m.muncod::integer = ur.muncod::integer
									where
										ur.usucpf = '%s' and
										ur.pflcod = '%s' and
										ur.rpustatus = 'A'				
