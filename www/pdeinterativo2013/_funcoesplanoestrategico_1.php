...
function planoestrategico_0_1_grandesdesafios($dados) {
	...
	if($dados['respostaideb']) {
		...
		if(!$ridid) die("<script>alert('É necessário salvar o Diagnostico: 1.1. IDEB');window.location='pdeinterativo2013.php?modulo=principal/planoestrategico&acao=A&aba=planoestrategico_0_pdeescola&aba1=planoestrategico_0_1_grandesdesafios';</script>");
	...
	if($dados['respostataxarendimento']) {
		...
		if(!$rtrid) die("<script>alert('É necessário salvar o Diagnostico: 1.2. Taxas de rendimento');window.location='pdeinterativo2013.php?modulo=principal/planoestrategico&acao=A&aba=planoestrategico_0_pdeescola&aba1=planoestrategico_0_1_grandesdesafios';</script>");
	...
	if($dados['respostaprovabrasil']) {
		...
		if(!$rpbid) die("<script>alert('É necessário salvar o Diagnostico: 1.3. Prova Brasil');window.location='pdeinterativo2013.php?modulo=principal/planoestrategico&acao=A&aba=planoestrategico_0_pdeescola&aba1=planoestrategico_0_1_grandesdesafios';</script>");
		...
		?>
			...
			<tr>
				...
				<td style="background-color:#DCDCDC" colspan="2" class="bordapreto">
					<input style="margin-left:90px" type="button" value="Inserir estratégia" name="estrategia" onclick="gerenciarEstrategias('<?=$dados['abaid'] ?>','<?=$problema['codigo'] ?>');">
					...
					<?php if(!in_array(PDEESC_PERFIL_DIRETOR,$arrPerfil) && (in_array(PDEINT_PERFIL_COMITE_PAR_ESTADUAL, $arrPerfil)||in_array(PDEINT_PERFIL_COMITE_PAR_MUNICIPAL, $arrPerfil)||in_array(PDEINT_PERFIL_CONSULTA_ESTADUAL, $arrPerfil)||in_array(PDEINT_PERFIL_CONSULTA_MUNICIPAL, $arrPerfil)||in_array(PDEINT_PERFIL_COMITE_ESTADUAL, $arrPerfil)||in_array(PDEINT_PERFIL_COMITE_MUNICIPAL, $arrPerfil))): ?>
						<a href="#" onclick="gerenciarEstrategias('<?=$dados['abaid'] ?>','<?=$problema['codigo'] ?>');"><img src="../imagens/consultar.gif" > Visualizar estratégias</a>
					<?php endif; ?>
				</td>
			</tr>
			<?
...
function listametas_diagnostico_3_ensinoeaprendizagem($dados) {
	...
	$sql = "SELECT rp.repid, pe.prgdesc, LOWER(op.oppdesc) as oppdesc FROM pdeinterativo2013.respostapergunta rp 
			...
			WHERE rp.pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."' AND 
				  rp.prgid IN(select prgid from pdeinterativo2013.pergunta where prgmodulo = 'E' and prgsubmodulo = 'P' and prgstatus = 'A' and (prgdetalhe='Projeto Pedagógico' or prgdetalhe='Projeto Pedagógico - Educação Infantil')) AND
...
function listaproblemas_diagnostico_5_comunidadeescolar($dados) {
	...
	$sql = "SELECT rp.repid, pe.prgdesc, op.oppdesc FROM pdeinterativo2013.respostapergunta rp 
			...
			INNER JOIN pdeinterativo2013.opcaopergunta op ON op.oppid = rp.oppid
			WHERE pd.pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."' AND 
				  rp.prgid IN(select prgid from pdeinterativo2013.pergunta where prgmodulo = 'C' and prgsubmodulo = 'E' and prgstatus = 'A' and prgdetalhe='Protagonismo e Participação') AND
				  rp.oppid IN(5,6) AND rp.critico=true ORDER BY rp.repid";
	...
	$sql = "SELECT rp.repid, pe.prgdesc, op.oppdesc FROM pdeinterativo2013.respostapergunta rp 
			...
			INNER JOIN pdeinterativo2013.opcaopergunta op ON op.oppid = rp.oppid
			WHERE pd.pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."' AND 
				  rp.prgid IN(select prgid from pdeinterativo2013.pergunta where prgmodulo = 'C' and prgsubmodulo = 'E' and prgstatus = 'A' and prgdetalhe='Saúde e Bem-estar') AND
				  rp.oppid IN(5,6) AND rp.critico=true ORDER BY rp.repid";
