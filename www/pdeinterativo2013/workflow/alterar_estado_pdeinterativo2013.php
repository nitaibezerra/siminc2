...
{
	include "alterar_estado_comentario_pdeinterativo2013.php";
	exit();
}
...
if ( wf_alterarEstado( $docid, $aedid, $cmddsc, $dadosVerificacao ) )
{
	...
	//alterar o estado do documento na tabela listapdeinterativo, para mostrar o dado atualizado na lista de escolas
	$sql = "	select esddsc from  workflow.documento d  
	inner join workflow.estadodocumento e on d.esdid = e.esdid
	where  docid = $docid";
		
	$estado = $db->pegaUm($sql);
	
	$sql = "select max (htddata) from workflow.historicodocumento where docid = $docid ";
	$data = $db->pegaUm($sql);
	
	$sql = "update pdeinterativo2013.listapdeinterativo set realizado = '{$estado}', datatramitacao = '{$data}' , htddata = '{$data}' where pdeid = {$_SESSION['pdeinterativo2013_vars']['pdeid']}";
