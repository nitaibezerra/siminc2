...
function verificaRespostaAbaFormacao() {
	...
	$sql = "SELECT COUNT(*) as numero FROM pdeinterativo2013.abaresposta ar 
			INNER JOIN pdeinterativo2013.aba ab ON ab.abaid=ar.abaid 
			WHERE ar.pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."' AND 
...
function apagarCachePdeInterativo() {
	if(CACHE_FILE) {
		...
		$cache->apagarCache("planoestrategico_".$_SESSION['pdeinterativo2013_vars']['pdeid']);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_EQUIPE_FNDE);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_CONSULTA_ESTADUAL);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_CONSULTA_MUNICIPAL);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEESC_PERFIL_CONSULTA);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEESC_PERFIL_DIRETOR);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_ESTADUAL);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_MUNICIPAL);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_EQUIPE_MEC);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_SUPER_USUARIO);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_PAR_MUNICIPAL);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_PAR_ESTADUAL);
		$cache->apagarCache("planoestrategico_0_3_visualizarplanoacao_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_semperfil");
...
function apagarCachePdeDiagnotico() {
	...
	if($arrAbasCache) {
		foreach($arrAbasCache as $aba) {
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_EQUIPE_FNDE);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_CONSULTA_ESTADUAL);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_CONSULTA_MUNICIPAL);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEESC_PERFIL_CONSULTA);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEESC_PERFIL_DIRETOR);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_ESTADUAL);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_MUNICIPAL);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_EQUIPE_MEC);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_SUPER_USUARIO);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_PAR_MUNICIPAL);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_PAR_ESTADUAL);
			$cache->apagarCache($aba."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_semperfil");
		}
	}
...
function recuperaMembroPorCPF()
{
	...
	$sql = "select DISTINCT
			...
			left join
				pdeinterativo2013.pessoa pde ON pde.usucpf = usu.usucpf
			where 
				usu.usucpf = '$usucpf'";
...
function salvarMembroComissao()
{
	...
	if(!$usucpf || !$estuf || !muncod || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
		$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroComite&acao=A");
		exit;
	}
	
	if( !preg_match( "/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $usuemail)){
		$_SESSION['pdeinterativo2013']['msg'] = "E-mail inválido!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroComite&acao=A");
		exit;
	}
	if( $usuemail != $cousuemail ){
		$_SESSION['pdeinterativo2013']['msg'] = "Confirmação de E-mail inválida!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroComite&acao=A");
		exit;
	}
	...
	verificaAddPerfil($pflcod,$usucpf);
	
	if(verificaTotalUsuariosPDEInterativo($_POST,$pflcod,$entid,$tpocod) > 9){
		$_SESSION['pdeinterativo2013']['msg'] = "Apenas 10 membros podem ser cadastrados!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroComite&acao=A");
		exit;
	}
	
	if(!ativaUsuarioPDEInterativo($_POST,$pflcod,$entid,$tpocod)){
		$_SESSION['pdeinterativo2013']['msg'] = "Não foi possível cadastrar o usuário no SIMEC!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroComite&acao=A");
		exit;
	}
	
	registraHistoricoUsuario(array("usucpf"=>$usucpf,"hustipo"=>"I","pflcod"=>$pflcod,"husjustificativa"=>$justificativa));
	
	$usucpf = "'".substr(str_replace(array(".","-","/"),"",$usucpf),0,11)."'";
	$pesnome = "'$usunome'";
	$pesregional = $pesregional ? "'$pesregional'" : "null";
	
	$sql = "select pesid from pdeinterativo2013.pessoa where usucpf = $usucpf;";
	$existe = $db->pegaUm($sql);
	if(!$existe){
		$sql = "insert into 
					pdeinterativo2013.pessoa
				(usucpf,pesnome,pesstatus,pflcod,pesregional)
					values
				($usucpf,$pesnome,'$rdo_status',$pflcod,$pesregional)";
	}else{
		$sql = "update 
					pdeinterativo2013.pessoa
				set
	...
	$_SESSION['pdeinterativo2013']['msg'] = "Operação realizada com sucesso!";
	header("Location: pdeinterativo2013.php?modulo=principal/cadastroComite&acao=A");
	exit;
}
...
function salvarMembroConsulta()
{
	...
	if( !preg_match( "/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $usuemail)){
		$_SESSION['pdeinterativo2013']['msg'] = "E-mail inválido!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroConsulta&acao=A");
		exit;
	}
	if( $usuemail != $cousuemail ){
		$_SESSION['pdeinterativo2013']['msg'] = "Confirmação de E-mail inválida!";
...
function salvarDirigente()
{
	...
	if( !preg_match( "/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $usuemail)){
		$_SESSION['pdeinterativo2013']['msg'] = "E-mail inválido!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroDirigente&acao=A");
		exit;
	}
	if( $usuemail != $cousuemail ){
		$_SESSION['pdeinterativo2013']['msg'] = "Confirmação de E-mail inválida!";
	...
	if($tpocod == "3"){
		$entid = $muncod;
		$pflcod = PDEINT_PERFIL_DIRIGENTE_MUNICIPAL;
		if(!$usucpf || !$estuf || !$muncod || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
			$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
			header("Location: pdeinterativo2013.php?modulo=principal/cadastroDirigente&acao=A");
			exit;
	...
	if($tpocod == "2"){
		$entid = $estuf;
		$pflcod = PDEINT_PERFIL_DIRIGENTE_ESTADUAL;
		if(!$usucpf || !$estuf  || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
			$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
			header("Location: pdeinterativo2013.php?modulo=principal/cadastroDirigente&acao=A");
			exit;
...
function salvarEquipeApoio()
{
	...
	if( !preg_match( "/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $usuemail)){
		$_SESSION['pdeinterativo2013']['msg'] = "E-mail inválido!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroEquipeApoio&acao=A");
		exit;
	}
	if( $usuemail != $cousuemail ){
		$_SESSION['pdeinterativo2013']['msg'] = "Confirmação de E-mail inválida!";
	...
	if($tpocod == "3"){
		$entid = $muncod;
		$pflcod = PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL;
		if(!$usucpf || !$estuf || !$muncod || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
			$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
			header("Location: pdeinterativo2013.php?modulo=principal/cadastroEquipeApoio&acao=A");
			exit;
	...
	if($tpocod == "2"){
		$entid = $estuf;
		$pflcod = PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL;
		if(!$usucpf || !$estuf  || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
			$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
			header("Location: pdeinterativo2013.php?modulo=principal/cadastroEquipeApoio&acao=A");
			exit;
	...
	if(verificaTotalUsuariosPDEInterativo($_POST,$pflcod,$entid,$tpocod) > 9){
		$_SESSION['pdeinterativo2013']['msg'] = "Apenas 10 membros da Equipe de Apoio podem ser cadastrados!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroEquipeApoio&acao=A");
		exit;
	}
...
function salvarCoordenador()
{
	...
	if( !preg_match( "/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $usuemail)){
		$_SESSION['pdeinterativo2013']['msg'] = "E-mail inválido!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroCoordenador&acao=A");
		exit;
	}
	if( $usuemail != $cousuemail ){
		$_SESSION['pdeinterativo2013']['msg'] = "Confirmação de E-mail inválida!";
	...
	if($tpocod == "3"){
		$entid = $muncod;
		$pflcod = PDEINT_PERFIL_COORDENADOR_MUNICIPAL;
		if(!$usucpf || !$estuf || !$muncod || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
			$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
			header("Location: pdeinterativo2013.php?modulo=principal/cadastroCoordenador&acao=A");
			exit;
	...
	if($tpocod == "2"){
		$entid = $estuf;
		$pflcod = PDEINT_PERFIL_COORDENADOR_ESTADUAL;
		if(!$usucpf || !$estuf  || !$tpocod || !$usuemail || !$usufoneddd || !$usufonenum || !$rdo_status){
			$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
			header("Location: pdeinterativo2013.php?modulo=principal/cadastroCoordenador&acao=A");
			exit;
	...
	//Regra de apenas um coordenador
	if(verificaTotalUsuariosPDEInterativo($_POST,$pflcod,$entid,$tpocod) != 0){
		$_SESSION['pdeinterativo2013']['msg'] = "Apenas um coordenador pode ser inserido!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroCoordenador&acao=A");
		exit;
	}
		...
		foreach($arrCPF as $cpf){
			...
			if(!strstr($dados,"WS-ACS - Erro")){
				...
				$arrSql.= "delete from seguranca.historicousuario where usucpf = '".trim($cpf)."';
						   ...
						   delete from pdeinterativo2013.usuarioresponsabilidade where usucpf = '".trim($cpf)."';
...
function ativaUsuarioPDEInterativo( $dados, $pflcod, $entid = null , $tpocod = null){
	...
	if($dados['rdo_status'] == "A"){
		$sql = "select pesid from pdeinterativo2013.pessoa where usucpf = '$cpf' and pesstatus != 'A'";
		if($db->pegaUm($sql)){
			$reenvio_email = true;
		}
	}
	...
	if($entid){
		if($pflcod == PDEINT_PERFIL_COMITE_ESTADUAL || $pflcod == PDEINT_PERFIL_CONSULTA_ESTADUAL || $pflcod == PDEINT_PERFIL_DIRIGENTE_ESTADUAL  || $pflcod == PDEINT_PERFIL_COORDENADOR_ESTADUAL  || $pflcod == PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL){
			$campo = "estuf";
			$NaoDesativaOutros = true;
		}elseif($pflcod == PDEINT_PERFIL_COMITE_MUNICIPAL || $pflcod == PDEINT_PERFIL_CONSULTA_MUNICIPAL || $pflcod == PDEINT_PERFIL_DIRIGENTE_MUNICIPAL  || $pflcod == PDEINT_PERFIL_COORDENADOR_MUNICIPAL  || $pflcod == PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL){
			$campo = "muncod";
			$NaoDesativaOutros = true;
		}else{
			$campo = "entid";
		...
		$sql = "SELECT * FROM pdeinterativo2013.usuarioresponsabilidade WHERE rpustatus = 'A' and usucpf = '$cpf' and $campo = '$entid' and pflcod = $pflcod";
		$existeu = $db->pegaUm($sql);
		if(!$existeu){
			
			if($pflcod == PDEINT_PERFIL_COMITE_MUNICIPAL || $pflcod == PDEINT_PERFIL_COMITE_ESTADUAL || $pflcod == PDEINT_PERFIL_DIRIGENTE_ESTADUAL || $pflcod == PDEINT_PERFIL_DIRIGENTE_MUNICIPAL || $pflcod == PDEINT_PERFIL_COORDENADOR_MUNICIPAL || $pflcod == PDEINT_PERFIL_COORDENADOR_ESTADUAL  || $pflcod == PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL  || $pflcod == PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL){
				$sqlp = sprintf(
				   "INSERT INTO pdeinterativo2013.usuarioresponsabilidade ( pflcod, usucpf, $campo, rpustatus, rpudata_inc ) 
					VALUES ( %d, '%s', '%s', 'A', now() )",
				$pflcod,
				$cpf,
				$entid
				...
				$sqlp = sprintf(
				   (!$NaoDesativaOutros ? "UPDATE pdeinterativo2013.usuarioresponsabilidade set rpustatus = 'I' where pflcod = $pflcod and $campo = '$entid' ;" : "")."
					INSERT INTO pdeinterativo2013.usuarioresponsabilidade ( pflcod, usucpf, $campo, rpustatus, rpudata_inc ) 
					VALUES ( %d, '%s', '%s', 'A', now() )",
				$pflcod,
				$cpf,
...
function verificaTotalUsuariosPDEInterativo($dados, $pflcod, $valor = null , $tpocod = null)
{
	global $db;
	...
	$sql = "select distinct tprcampo 
			from pdeinterativo2013.tprperfil tpr
			inner join pdeinterativo2013.tiporesponsabilidade prf ON prf.tprcod = tpr.tprcod
			where pflcod = $pflcod";
	$tprcampo = $db->pegaUm($sql);
	
	$sql = "select 
				count(distinct usucpf) 
			from 
				pdeinterativo2013.usuarioresponsabilidade 
			where 
				pflcod = $pflcod
			".($tprcampo ? " and $tprcampo = '$valor' " : "")."
			and usucpf != '$cpf'
			and rpustatus = 'A'";
...
function ativaUsuarioPDEInterativo2012( $dados, $pflcod, $entid = null , $tpocod = null){
	global $db;

	//id modulo sistema
	$sisid = SISID_PDE_INTERATIVO_2012;
	...
	// verifica se o usuário já está cadastrado no módulo selecionado
	$sql = sprintf("SELECT usucpf FROM seguranca.usuario_sistema WHERE usucpf = '%s' and sisid = ".SISID_PDE_INTERATIVO_2012." and susstatus = 'A' ",
		$cpf);
	$modulo = $db->pegaLinha( $sql );
	...
	if($entid){
		if($pflcod == PDEINT_PERFIL_CONSULTA_DIRETOR_PDE2013){
			$campo = "entid";
			$NaoDesativaOutros = true;
		}
...
function listaComite()
{
	...
	if($usunome){
		...
		$arrWhereInativo[] = "removeacento(usu.usunome) ilike removeacento(('%$usunome%'))";
	}
	if($usucpf){
		$arrWhere[] = "usu.usucpf = '".str_replace(array("-","."),"",$usucpf)."'";
		$arrWhereInativo[] = "usu.usucpf = '".str_replace(array("-","."),"",$usucpf)."'";
	}
	if($estuf){
		$arrWhere[] = "usu.regcod = '$estuf'";
		$arrWhereInativo[] = "usu.regcod = '$estuf'";
	}
	if($muncod){
		$arrWhere[] = "usu.muncod = '$muncod'";
		$arrWhereInativo[] = "usu.muncod = '$muncod'";
	}
	if($carid){
	...
	if($usu_plfcod){
		switch($usu_plfcod){
			case "Estadual":
				...
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_COMITE_ESTADUAL;
				$arrWhere1[] = "ususis.pflcod = ".PDEINT_PERFIL_COMITE_ESTADUAL;
				break;
			case "Municipal":
				$arrWhere[] = "per.pflcod = ".PDEINT_PERFIL_COMITE_MUNICIPAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_COMITE_MUNICIPAL;
				$arrWhere1[] = "ususis.pflcod = ".PDEINT_PERFIL_COMITE_ESTADUAL;
				break;
		}
	}
	...
	if($rdo_status == "I"){
		$sql = "select acao,usucpf,usunome,usuemail,mundescricao,estuf,membro,status from (
				select
					'<img class=\"link\" onclick=\"editarMembroComite(\'' || usu.usucpf || '\',\'' || his.pflcod || '\')\" src=\"../imagens/alterar.gif\" />
					<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
					usu.usucpf,
					usu.usunome,
					usu.usuemail,
					mun.mundescricao,
				...
				END as membro,
				'Inativo' as status,
				MAX(husdata) as husdata,
				( select hus2.hustipo from pdeinterativo2013.historicousuario hus2 where hus2.usucpf = usu.usucpf and hus2.pflcod = his.pflcod order by husdata desc limit 1) as hustipo
				from
					pdeinterativo2013.historicousuario his
				inner join
					seguranca.usuario usu ON usu.usucpf = his.usucpf
				left join
					territorios.municipio mun ON mun.muncod = usu.muncod
				where
					his.pflcod in (".PDEINT_PERFIL_COMITE_ESTADUAL.",".PDEINT_PERFIL_COMITE_MUNICIPAL.")
				".($arrWhereInativo ? " and ".implode(" and ",$arrWhereInativo) : "")."
				group by 
					usu.usucpf,
					usu.usunome,
					usu.usuemail,
					mun.mundescricao,
					mun.estuf,
					his.pflcod ) as foo where hustipo = 'E'";
...
(
select DISTINCT
				'<img class=\"link\" onclick=\"editarMembroComite(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/alterar.gif\" /> 
				<img class=\"link\" onclick=\"excluirMembroComite(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/excluir.gif\" />
				<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
				usu.usucpf,
				usu.usunome,
				usu.usuemail,
			...
			left join
				territorios.municipio mun ON mun.muncod = usu.muncod
			left join
				pdeinterativo2013.pessoa pes ON pes.usucpf = usu.usucpf AND pesstatus <> 'I'
			left join
...
select DISTINCT
				'<img class=\"link\" onclick=\"editarMembroComite(\'' || usu.usucpf || '\',\'' || ususis.pflcod || '\')\" src=\"../imagens/alterar.gif\" /> 
				<img class=\"link\" onclick=\"excluirMembroComite(\'' || usu.usucpf || '\',\'' || ususis.pflcod || '\')\" src=\"../imagens/excluir.gif\" />
				<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
				usu.usucpf,
				usu.usunome,
				usu.usuemail,
			...
			where
				1=1
			".($arrWhere ? " and ".implode(" and ",$arrWhere) : "")."
			".($arrWhere1 ? " and ".implode(" and ",$arrWhere1) : "")."
			and
...
function listaMembroConsulta()
{
	...
	if($estuf){
		$inner = "inner join pdeinterativo2013.usuarioresponsabilidade ur ON ur.usucpf = usu.usucpf and ur.estuf='$estuf' AND rpustatus='A'";
		$arrWhereInativo[] = "usu.regcod = '$estuf'";
	}
	if($muncod){
		$inner = "inner join pdeinterativo2013.usuarioresponsabilidade ur ON ur.usucpf = usu.usucpf and ur.muncod='$muncod' AND rpustatus='A'";
		$arrWhereInativo[] = "usu.muncod = '$muncod'";
	}
	if($carid){
		$arrWhere[] = "usu.carid = $carid";
	...
	if($usu_plfcod){
		switch($usu_plfcod){
			case "Estadual":
				...
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_CONSULTA_ESTADUAL;
				break;
			case "Municipal":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_CONSULTA_MUNICIPAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_CONSULTA_MUNICIPAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_CONSULTA_MUNICIPAL;
				break;
		}
	}
	if($rdo_status){
	...
	if($rdo_status == "I"){
		$sql = "select acao,usucpf,usunome,usuemail,mundescricao,estuf,membro,status from (
				select
					'<img class=\"link\" onclick=\"editarMembroConsulta(\'' || usu.usucpf || '\',\'' || his.pflcod || '\')\" src=\"../imagens/alterar.gif\" />
					<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
					usu.usucpf,
					usu.usunome,
					usu.usuemail,
				...
				left join
					territorios.municipio mun ON mun.muncod = usu.muncod
				where
					his.pflcod in (".PDEINT_PERFIL_CONSULTA_ESTADUAL.",".PDEINT_PERFIL_CONSULTA_MUNICIPAL.")
				".($arrWhereInativo ? " and ".implode(" and ",$arrWhereInativo) : "")."
				group by 
					usu.usucpf,
					usu.usunome,
	...
	$sql = "select DISTINCT
				'<img class=\"link\" onclick=\"editarMembroConsulta(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/alterar.gif\" /> <img class=\"link\" onclick=\"excluirMembroConsulta(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/excluir.gif\" /> <img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
				usu.usucpf,
				usu.usunome,
				usu.usuemail,
				mun.mundescricao,
...
function listaDirigentes()
{
	...
	if($usu_plfcod){
		switch($usu_plfcod){
			case "Estadual":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_DIRIGENTE_ESTADUAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_DIRIGENTE_ESTADUAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_DIRIGENTE_ESTADUAL;
				break;
			case "Municipal":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL;
				break;
		}
	}
	if($rdo_status){
	...
	if($rdo_status == "I"){
		$sql = "select acao,usucpf,usunome,usuemail,mundescricao,estuf,membro,status from (
				select
					'<img class=\"link\" onclick=\"editarDirigente(\'' || usu.usucpf || '\',\'' || his.pflcod || '\')\" src=\"../imagens/alterar.gif\" />
					<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
					usu.usucpf,
					usu.usunome,
					usu.usuemail,
					...
					mun.estuf,
					CASE his.pflcod
					WHEN ".PDEINT_PERFIL_DIRIGENTE_ESTADUAL." THEN 'Dirigente Estadual'
					WHEN ".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL." THEN 'Dirigente Municipal'
					ELSE 'N/A'
				END as membro,
				'Inativo' as status,
				MAX(husdata) as husdata,
				...
				left join
					territorios.municipio mun ON mun.muncod = usu.muncod
				where
					his.pflcod in (".PDEINT_PERFIL_DIRIGENTE_ESTADUAL.",".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL.")
				".($arrWhereInativo ? " and ".implode(" and ",$arrWhereInativo) : "")."
				group by 
					usu.usucpf,
					usu.usunome,
	...
	$sql = "select * from (
			select DISTINCT
				'<img class=\"link\" onclick=\"editarDirigente(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/alterar.gif\" /> 
				<img class=\"link\" onclick=\"excluirDirigente(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/excluir.gif\" /> 
				<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
				usu.usucpf,
				usu.usunome,
				usu.usuemail,
				...
				pes.pesstatus,
				CASE per.pflcod
					WHEN ".PDEINT_PERFIL_DIRIGENTE_ESTADUAL." THEN 'Estadual'
					WHEN ".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL." THEN 'Municipal'
					ELSE 'N/A'
				END as membro,
				CASE (
						CASE ususis.suscod
			...
			".($arrWhere11 ? " and ".implode(" and ",$arrWhere11) : "")."
			and
				per.pflcod in (".PDEINT_PERFIL_DIRIGENTE_ESTADUAL.",".PDEINT_PERFIL_DIRIGENTE_MUNICIPAL.")
			and
				pfl.sisid  = ".SISID_PDE_INTERATIVO." ) as foo where 1=1 ".($arrWhere2 ? " and ".implode(" and ",$arrWhere2) : "");
	...
	$cabecalho = array("Ação","CPF","Nome","E-mail","Município","UF","Dirigente","Status");
	$db->monta_lista_array($arRegistro, $cabecalho, 50, 10, 'N','Center');
...
function listaCoordenador()
{
	...
	if($usu_plfcod){
		switch($usu_plfcod){
			case "Estadual":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_COORDENADOR_ESTADUAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_COORDENADOR_ESTADUAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_COORDENADOR_ESTADUAL;
				break;
			case "Municipal":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_COORDENADOR_MUNICIPAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_COORDENADOR_MUNICIPAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_COORDENADOR_MUNICIPAL;
				break;
		}
	}
	if($rdo_status){
	...
	if($rdo_status == "I"){
		$sql = "select acao,usucpf,usunome,usuemail,mundescricao,estuf,membro,status from (
				select
					'<img class=\"link\" onclick=\"editarCoordenador(\'' || usu.usucpf || '\',\'' || his.pflcod || '\')\" src=\"../imagens/alterar.gif\" />
					<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
					usu.usucpf,
					usu.usunome,
					usu.usuemail,
					...
					mun.estuf,
					CASE his.pflcod
					WHEN ".PDEINT_PERFIL_COORDENADOR_ESTADUAL." THEN 'Coordenador Estadual'
					WHEN ".PDEINT_PERFIL_COORDENADOR_MUNICIPAL." THEN 'Coordenador Municipal'
					ELSE 'N/A'
				END as membro,
				'Inativo' as status,
				MAX(husdata) as husdata,
				...
				left join
					territorios.municipio mun ON mun.muncod = usu.muncod
				where
					his.pflcod in (".PDEINT_PERFIL_COORDENADOR_ESTADUAL.",".PDEINT_PERFIL_COORDENADOR_MUNICIPAL.")
				".($arrWhereInativo ? " and ".implode(" and ",$arrWhereInativo) : "")."
				group by 
					usu.usucpf,
					usu.usunome,
	...
	$sql = "select * from (
			select DISTINCT
				'<img class=\"link\" onclick=\"editarCoordenador(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/alterar.gif\" /> 
				<img class=\"link\" onclick=\"excluirCoordenador(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/excluir.gif\" /> 
				<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
				usu.usucpf,
				usu.usunome,
				usu.usuemail,
				...
				pes.pesstatus,
				CASE per.pflcod
					WHEN ".PDEINT_PERFIL_COORDENADOR_ESTADUAL." THEN 'Estadual'
					WHEN ".PDEINT_PERFIL_COORDENADOR_MUNICIPAL." THEN 'Municipal'
					ELSE 'N/A'
				END as membro,
				CASE (
						CASE ususis.suscod
			...
			".($arrWhere11 ? " and ".implode(" and ",$arrWhere11) : "")."
			and
				per.pflcod in (".PDEINT_PERFIL_COORDENADOR_ESTADUAL.",".PDEINT_PERFIL_COORDENADOR_MUNICIPAL.")
			and
				pfl.sisid  = ".SISID_PDE_INTERATIVO." ) as foo where 1=1 ".($arrWhere2 ? " and ".implode(" and ",$arrWhere2) : "");
	...
	$cabecalho = array("Ação","CPF","Nome","E-mail","Município","UF","Coordenador","Status");
	$db->monta_lista_array($arRegistro, $cabecalho, 50, 10, 'N','Center');
	
}

function listaEquipeApoio()
{
	global $db;
	...
	if($usu_plfcod){
		switch($usu_plfcod){
			case "Estadual":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL;
				break;
			case "Municipal":
				$arrWhere11[] = "per.pflcod = ".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL;
				$arrWhere22[] = "ususis.pflcod = ".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL;
				$arrWhereInativo[] = "his.pflcod = ".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL;
				break;
		}
	}
	if($rdo_status){
	...
	if($rdo_status == "I"){
		$sql = "select acao,usucpf,usunome,usuemail,mundescricao,estuf,membro,status from (
				select
					'<img class=\"link\" onclick=\"editarEquipeApoio(\'' || usu.usucpf || '\',\'' || his.pflcod || '\')\" src=\"../imagens/alterar.gif\" />
					<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
					usu.usucpf,
					usu.usunome,
					usu.usuemail,
					...
					mun.estuf,
					CASE his.pflcod
					WHEN ".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL." THEN 'Coordenador Estadual'
					WHEN ".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL." THEN 'Coordenador Municipal'
					ELSE 'N/A'
				END as membro,
				'Inativo' as status,
				MAX(husdata) as husdata,
				...
				left join
					territorios.municipio mun ON mun.muncod = usu.muncod
				where
					his.pflcod in (".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL.",".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL.")
				".($arrWhereInativo ? " and ".implode(" and ",$arrWhereInativo) : "")."
				group by 
					usu.usucpf,
					usu.usunome,
	...
	$sql = "select * from (
			select DISTINCT
				'<img class=\"link\" onclick=\"editarEquipeApoio(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/alterar.gif\" /> 
				<img class=\"link\" onclick=\"excluirEquipeApoio(\'' || usu.usucpf || '\',\'' || per.pflcod || '\')\" src=\"../imagens/excluir.gif\" /> 
				<img class=\"link\" onclick=\"historicoUsuario(\'' || usu.usucpf || '\')\" src=\"../imagens/icone_historico.png\" />' as acao,
				usu.usucpf,
				usu.usunome,
				usu.usuemail,
				...
				pes.pesstatus,
				CASE per.pflcod
					WHEN ".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL." THEN 'Estadual'
					WHEN ".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL." THEN 'Municipal'
					ELSE 'N/A'
				END as membro,
				CASE (
						CASE ususis.suscod
			...
			".($arrWhere11 ? " and ".implode(" and ",$arrWhere11) : "")."
			and
				per.pflcod in (".PDEINT_PERFIL_EQUIPE_APOIO_ESTADUAL.",".PDEINT_PERFIL_EQUIPE_APOIO_MUNICIPAL.")
			and
				pfl.sisid  = ".SISID_PDE_INTERATIVO." ) as foo where 1=1 ".($arrWhere2 ? " and ".implode(" and ",$arrWhere2) : "");
	...
	$cabecalho = array("Ação","CPF","Nome","E-mail","Município","UF","Equipe de Apoio","Status");
	$db->monta_lista_array($arRegistro, $cabecalho, 50, 10, 'N','Center');
...
{
	return array("usucpf" => $_POST['usucpf'],"pflcod" => $_POST['pflcod']);
}
...
function carregarDirigente()
{
	return array("usucpf" => $_POST['usucpf'],"pflcod" => $_POST['pflcod']);
}
function carregarCoordenador()
{
	return array("usucpf" => $_POST['usucpf'],"pflcod" => $_POST['pflcod']);
}
function carregarEquipeApoio()
{
	return array("usucpf" => $_POST['usucpf'],"pflcod" => $_POST['pflcod']);
}
		...
		foreach($arrUsers as $user){
			if($user['muncod'] || $user['estuf']){
				...
				if($campo && $valor && $user['usucpf'] && $user['pflcod']){
					$sql = "SELECT * FROM pdeinterativo2013.usuarioresponsabilidade WHERE rpustatus = 'A' and usucpf = '{$user['usucpf']}' and $campo = '$valor' and pflcod = {$user['pflcod']}";
					$exite = $db->pegaUm($sql);
					if(!$exite){
						$sqlp= sprintf(
						   "INSERT INTO pdeinterativo2013.usuarioresponsabilidade ( pflcod, usucpf, $campo, rpustatus, rpudata_inc ) 
...
function excluirMembroConsulta()
{
	...
	if($pflcod){
		$sqlD = "delete from seguranca.perfilusuario where usucpf = '$usucpf' and pflcod = $pflcod;
				delete from pdeinterativo2013.usuarioresponsabilidade where usucpf = '$usucpf' and pflcod = '$pflcod';";
	}
	...
	$db->executar($sql);
	$db->commit();
	$_SESSION['pdeinterativo2013']['msg'] = "Usuário excluído com sucesso!";
	...
	registraHistoricoUsuario(array("usucpf"=>$usucpf,"hustipo"=>"E","pflcod"=>$pflcod));
	
	header("Location: pdeinterativo2013.php?modulo=principal/listaConsulta&acao=A");
	exit;
}
...
function excluirDirigente()
{
	...
	$arrUsucpf = pegaPerfilGeral($usucpf);
	
	if(count($arrUsucpf) == 1 && in_array($pflcod,$arrUsucpf)){
		$sql = "$sqlD
			delete from seguranca.usuario_sistema where usucpf = '$usucpf' and sisid = ".SISID_PDE_INTERATIVO.";
			update 
				pdeinterativo2013.pessoa
	...
	header("Location: pdeinterativo2013.php?modulo=principal/listaDirigente&acao=A");
	exit;
}

function excluirEquipeApoio()
{
	global $db;
	$usucpf = $_POST['usucpf'];
	$pflcod = $_POST['pflcod'];
	...
	header("Location: pdeinterativo2013.php?modulo=principal/listaEquipeApoio&acao=A");
	exit;
}
...
function excluirCoordenador()
{
	...
	header("Location: pdeinterativo2013.php?modulo=principal/listaCoordenador&acao=A");
	exit;
}
...
function excluirMembroComite()
{
	...
	header("Location: pdeinterativo2013.php?modulo=principal/listaComite&acao=A");
	exit;
...
function importarEscola()
{
	...
	if(!$_FILES['arquivo']){
		$_SESSION['pdeinterativo2013']['msg'] = "Favor selecionar o arquivo!";
		header("Location: pdeinterativo2013.php?modulo=sistema/importacao/importarEscola&acao=A");
		exit;	
	}
	if(!strstr($_FILES['arquivo']['name'],".csv")){
		$_SESSION['pdeinterativo2013']['msg'] = "Favor selecionar um arquivo '.csv'!";
		header("Location: pdeinterativo2013.php?modulo=sistema/importacao/importarEscola&acao=A");
		exit;
	}
	...
	if($csv){
		...
		if($arrEscola){
			$sql = "select pdicodinep from pdeinterativo2013.pdinterativo where pdicodinep in('".implode("','",$arrEscola)."') and pdistatus = 'A'";
			...
			foreach($arrEscolaNovo as $esc){
				...
				$sqlI.= "insert into pdeinterativo2013.pdinterativo (pdicodinep,pdistatus) values ('$esc','A');";
			}
		}
		...
		if($sqlI){
			...
			$_SESSION['pdeinterativo2013']['msg'] = "Foram importadas ".number_format(count($arrEscolasImportadas),0,".",".")." de ".number_format(count($csv),0,".",".")." escolas presentes no arquivo selecionado!";
			header("Location: pdeinterativo2013.php?modulo=sistema/importacao/importarEscola&acao=A");
			exit;
		}else{
			$_SESSION['pdeinterativo2013']['msg'] = "Não existem escolas para importação no arquivo selecionado!";
			header("Location: pdeinterativo2013.php?modulo=sistema/importacao/importarEscola&acao=A");
			exit;
		}
	}
...
function listaArquivosImportacao()
{
	global $db;
	 ...
	 $sql = "SELECT
                        ...
                        tpa.tpddsc,
                        '<a style=\"cursor: pointer; color: blue;\" onclick=\"window.location=\'pdeinterativo2013.php?modulo=sistema/importacao/importarEscola&acao=A&download=S&arqid=' || arq.arqid || '\';\" />' || arq.arqnome || '.'|| arq.arqextensao ||'</a>',
                        arq.arqdescricao,
                        usu.usunome
                    FROM
                        public.arquivo arq 
                    INNER JOIN 
                   		pdeinterativo2013.anexo a ON arq.arqid = a.arqid
                    INNER JOIN 
                   		pdeinterativo2013.tipodocumento tpa ON tpa.tpdid = a.tpdid
                    INNER JOIN
...
function listaDiretor()
{
	...
	if(!$db->testa_superuser()) {
		...
		if(in_array(PDEINT_PERFIL_COMITE_MUNICIPAL,$arrPerfil)){
			$sql = "select mun.muncod
					from pdeinterativo2013.usuarioresponsabilidade ur
	...
	$sql = "SELECT '<span style=\"white-space:nowrap;\" >".(($nao_exibir_icones)?"-":"<img class=\"link\" onclick=\"editarDiretor(\'' || COALESCE(diretor.usucpf,'') || '\',\'' || COALESCE(pdicodinep,'') || '\')\" src=\"../imagens/alterar.gif\" /> <img class=\"link\" onclick=\"excluirDiretor(\'' || COALESCE(diretor.usucpf,'') || '\',\'' || COALESCE(pdicodinep,'') || '\')\" src=\"../imagens/excluir.gif\" />")."</span>' as acao,
			...
			LEFT JOIN (SELECT pes.usucpf, pes.pesnome, ptp.pdeid, usu.usuemail, ususis.suscod, pes.pesstatus from pdeinterativo2013.pessoa pes 
...
function listarEscolas()
{
	...
	if($spasituacao){
		$arrWhere[] = "pagamento ilike '".trim($spasituacao)."%'";
	}
	if($usunome){
		$arrWhere[] = "removeacento(usunome) ilike removeacento(('%$usunome%'))";
	}
	if($usucpf){
		$arrWhere[] = "usucpfdiretor = '".str_replace(array("-","."),"",$usucpf)."'";
	}
	if($aedid) {
	...
	if($arrPdeid){
		$arrWhere[] = "pdeid in (".implode(",",$arrPdeid).")";
	}
	...
	if(!$db->testa_superuser()) {
		...
		if(in_array(PDEINT_PERFIL_CONSULTA_ESTADUAL, $arrPerfil)) {
			...
			if(in_array(PDEINT_PERFIL_COMITE_ESTADUAL, $arrPerfil)) {
				$paramperfil[] 		  = " (pdiesfera = 'Estadual' AND estuf='".$estuf."')";
			}
			if(in_array(PDEINT_PERFIL_COMITE_MUNICIPAL, $arrPerfil)) {
	...
	if($db->testa_superuser() || in_array(PDEINT_PERFIL_EQUIPE_MEC, $arrPerfil) || in_array(PDEINT_PERFIL_EQUIPE_FNDE, $arrPerfil) || in_array(PDEINT_PERFIL_COMITE_ESTADUAL, $arrPerfil) || in_array(PDEINT_PERFIL_COMITE_MUNICIPAL, $arrPerfil) ) {
		$acao = "(CASE WHEN esdid = ".WF_ESD_VALIDADO_MEC." THEN '<span style=\"white-space:nowrap;\" > <img class=\"link\" onclick=\"cadastrarDiretor(\'' || coalesce(usucpfdiretor,'') || '\',\'' || pdicodinep || '\',\'' || pdeid || '\')\" src=\"../imagens/usuario.gif\" /> <img class=\"link\" onclick=\"visualizarPDE(\'' || pdeid || '\')\" src=\"../imagens/consultar.gif\" /> <img class=\"link\" onclick=\"informarPagamento(\'' || usucpf || '\')\" src=\"../imagens/money.gif\" /> <img src=\"../imagens/editar_nome_vermelho.gif\" style=cursor:pointer; border=0 onclick=\"wf_exibirHistorico( '||docid||' );\" /></span>'
					   WHEN usucpfdiretor IS NULL THEN '<span style=\"white-space:nowrap;\" > <img class=\"link\" onclick=\"cadastrarDiretor(\'' || coalesce(usucpfdiretor,'') || '\',\'' || pdicodinep || '\',\'' || pdeid || '\')\" src=\"../imagens/usuario.gif\" /></span>'
					   ELSE '<span style=\"white-space:nowrap;\" > <img class=\"link\" onclick=\"cadastrarDiretor(\'' || coalesce(usucpfdiretor,'') || '\',\'' || pdicodinep || '\',\'' || pdeid || '\')\" src=\"../imagens/usuario.gif\" />  <img class=\"link\" onclick=\"visualizarPDE(\'' || pdeid || '\')\" src=\"../imagens/consultar.gif\" /> '|| CASE WHEN docid IS NULL THEN '' ELSE '<img src=\"../imagens/editar_nome_vermelho.gif\" border=0 style=cursor:pointer; onclick=\"wf_exibirHistorico( '||docid||' );\" />' END ||'</span>'
				  END) ";
		$campo = ", pagamento";
	}else{
		$acao = "(CASE WHEN usucpf IS NULL THEN '-'
					  ELSE '<span style=\"white-space:nowrap;\" ><img class=\"link\" onclick=\"visualizarPDE(\'' || pdeid || '\')\" src=\"../imagens/consultar.gif\" /> '|| CASE WHEN docid IS NULL THEN '' ELSE '<img src=\"../imagens/editar_nome_vermelho.gif\" border=0 style=cursor:pointer; onclick=\"wf_exibirHistorico( '||docid||' );\" />' END ||'</span>' END)";
		$campo = ", pagamento";
	}
	...
	$sql = "SELECT $acao as acao,
				...
				usunome, 
				usuemail
				".($pditempdeescola?",realizado,
				to_char(htddata,'dd/mm/YYYY HH24:MI') as datatramitacao,
				percent,
				pagamento
				":"")."
			FROM pdeinterativo2013.listapdeinterativo lp 
	...
	/*
			if ($usunome)
			{
				...
				$sql.= " usunome ilike REMOVEACENTO('%{$usunome}%') "; 
			}
			*/
			...
			if ($usucpf)
			{
				...
				$sql.= " usucpfdiretor = '".str_replace(array("-","."),"",$usucpf)."' ";
			}
			*/
	...
	if($pditempdeescola){
		$cabecalho = array("Ação","F","Código INEP","Nome da Escola", "Esfera", "Município", "UF", "CPF do Diretor","Nome do Diretor","E-mail","Situação (PDE)","Data de Tramitação (PDE)","Preenchimento do PDE (%)","Situação do Pagamento");
	}
	else
	{
		$cabecalho = array("Ação","F","Código INEP","Nome da Escola", "Esfera", "Município", "UF", "CPF do Diretor","Nome do Diretor","E-mail");
	}

	$align = array ("center","center","center","left","left","left", "center","right");
	
	if(CACHE_MEM) {
		$db->monta_lista($sql,$cabecalho,50,10,"N","center","N","","","",3600);
	} else {
		$db->monta_lista($sql,$cabecalho,50,10,"N","center","N","","",$align);
	}
}
...
function recuperaDiretorPorCPF()
{
	...
	$pdicodinep = $_REQUEST['pdicodinep'];
	
	//Se o CPF ja for diretor da escola não há necessidade de verificar os próximos itens
	$sql = "select 
				usu.*
			from
			...
			where
				ure.pflcod in (864) 			
			AND
				pdicodinep::integer = $pdicodinep";
	$arrDiretorAtual = $db->pegaLinha($sql);
	if($arrDiretorAtual['usucpf'] == $usucpf){
		echo simec_json_encode($arrDiretorAtual);
		die;
	}
	...
	$sql = "select 
			...
			from 
				seguranca.usuario usu
			inner join
				pdeinterativo2013.pessoa pes ON pes.usucpf = usu.usucpf 
			inner join 
				pdeinterativo2013.pessoatipoperfil ptp on ptp.pesid=pes.pesid
			inner join
				seguranca.perfilusuario per ON per.usucpf = usu.usucpf
			inner join
				seguranca.statususuario sus ON sus.suscod = usu.suscod
	...
	$sql = "select 
			...
			inner join 
				pdeinterativo2013.pessoatipoperfil ptp on ptp.pesid=pes.pesid
			inner join 
				pdeinterativo2013.pdinterativo pde on ptp.pdeid = pde.pdeid
			inner join
				seguranca.perfilusuario per ON per.usucpf = usu.usucpf
			inner join
				seguranca.statususuario sus ON sus.suscod = usu.suscod
			...
			and
				usu.usucpf = '$usucpf'
			".($pdicodinep ? " and ptp.pdeid != (select pdeid from pdeinterativo2013.pdinterativo where pdistatus = 'A' and pdicodinep::integer = $pdicodinep limit 1) " : " and pdeid is not null ")."
			and 
				ptp.tpeid=".TPE_DIRETOR."
			and
	...
	if($escolas){
		$mensagem = "Este Diretor já está vinculado à(s) escola ";
		
		foreach($escolas as $e) {
			$mensagem .= "\n* ". $e["pdenome"];	
		}
		//$confirm = ICONV( "ISO-8859-1", "UTF-8", "Este Diretor já está vinculado à(s) escola (...).Confirma que ele também será vinculado à escola(...)?" );
		$mensagem .= "\nConfirma que este também será vinculado a esta escola ?";
		$confirm = ICONV( "ISO-8859-1", "UTF-8", $mensagem);
	...
	if($pdicodinep){
		$sql = "select 
				...
				and
					pesstatus = 'A'
				and
					pdeid = (select pdeid from pdeinterativo2013.pdinterativo where pdistatus = 'A' and pdicodinep::integer = $pdicodinep limit 1)
				and
					per.pflcod in (".PDEESC_PERFIL_DIRETOR.") 
				and 
					ptp.tpeid=".TPE_DIRETOR."";
...
function listarDiretoresPorCodigoINEP($codigoinep)
{
	
	
	global $db;
	$acao = '\'<span style="white-space:nowrap;" ><img class="link" onclick="editarDiretor(  \'||pes.usucpf||\','.$codigoinep.')" src="../imagens/alterar.gif" /> <img class="link" onclick="excluirDiretor( \'\'\'||pes.usucpf||\'\'\', \'\'\'||pde.pdeid||\'\'\')" src="../imagens/excluir.gif" /></span>\' as acao' ;
	$sql = 'select '.$acao.', pes.usucpf, pesnome				
			from 
				seguranca.usuario usu
			inner join
				pdeinterativo2013.pessoa pes ON pes.usucpf = usu.usucpf 
			...
			where
				pesstatus = \'A\'
			and
				ptp.pdeid in (select pdeid from pdeinterativo2013.pdinterativo where pdistatus = \'A\' and pdicodinep::integer = '.$codigoinep.' ) 
			and 
				ptp.tpeid='.TPE_DIRETOR.'
			and
				per.pflcod in ('.PDEESC_PERFIL_DIRETOR.')';
	...
	$cabecalho = array("Ação", "CPF do Diretor","Nome do Diretor");
...
function recuperaEscolaPorCodigoINEP()
{
	...
	if(!$db->testa_superuser() && (in_array(PDEINT_PERFIL_COMITE_PAR_ESTADUAL,$arrPerfil) || in_array(PDEINT_PERFIL_COMITE_PAR_MUNICIPAL,$arrPerfil))){	
		if(in_array(PDEINT_PERFIL_COMITE_PAR_MUNICIPAL,$arrPerfil)){
			 $sql = "select 
						...
						mun.muncod
					from 
						pdeinterativo2013.usuarioresponsabilidade ur
					inner join
	...
	$sql = "select
				...
				pes.usucpf
			from
				pdeinterativo2013.pdinterativo pdi
			left join
				territorios.municipio mun ON mun.muncod = pdi.muncod
			left join
				pdeinterativo2013.pessoatipoperfil ptp ON ptp.pdeid = pdi.pdeid 
			left join
				pdeinterativo2013.pessoa pes ON pes.pesid = ptp.pesid 
			where 
	...
	if(!$arrDados['pdenome']){
		...
		$sql = "select
				...
				inner join
					pdeinterativo2013.pdinterativo pdi ON pdi.pdicodinep = ent.entcodent
				left join
					entidade.tipolocalizacao tpl ON ent.tplid = tpl.tplid
				left join
					educacenso_2012.tab_entidade ten ON ten.pk_cod_entidade = pdi.pdicodinep::bigint
				where
	...
	$sql = "select
		...
		left join
			pdeinterativo2013.pessoatipoperfil ptp ON ptp.pdeid = pdi.pdeid 
		left join
			pdeinterativo2013.pessoa pes ON pes.pesid = ptp.pesid and pes.pflcod = ".PDEESC_PERFIL_DIRETOR."
		where 
			pdicodinep = '$pdicodinep'
		".($arrWhere ? " and ".implode(" and ",$arrWhere) : "")."
		and
...
function AjaxrecuperaDadosDiretor()
{
	global $db;

	$pdeid = $_REQUEST['pdeid'];
	...
	$sql = "select
			...
			inner join
				pdeinterativo2013.pessoa pes ON pes.usucpf = usu.usucpf
			inner join
				pdeinterativo2013.pessoatipoperfil ptp ON ptp.pesid = pes.pesid and ptp.tpeid=".TPE_DIRETOR."
				inner join
				pdeinterativo2013.usuarioresponsabilidade rpu ON usu.usucpf = rpu.usucpf AND pfl.pflcod = rpu.pflcod AND rpustatus = 'A'
				inner join
				pdeinterativo2013.pdinterativo pde ON pde.pdeid = ptp.pdeid
				where
				pesstatus = 'A'
				and
				pde.pdeid = '$pdeid'
...
function salvarDiretor()
{
	...
	if(!$_REQUEST['entid'] || $_REQUEST['entid'] == null || $_REQUEST['entid'] == ""){
		$_SESSION['pdeinterativo2013']['msg'] = "Escola não encontrada!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroDiretor&acao=A");
	}
	
	//Atribui ao usuário UF e Município igual ao da escola
	$sql = "select estuf, muncod, pdiesfera from pdeinterativo2013.pdinterativo where entid = {$_REQUEST['entid']} and pdistatus = 'A'";
	...
	if($naopode){
		$_SESSION['pdeinterativo2013']['msg'] = $naopode;
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroDiretor&acao=A");
		exit;
	}
	...
	if($pdeid) {
		$sql = "update pdeinterativo2013.pessoa set pflcod=NULL where pesid in(select pesid from pdeinterativo2013.pessoatipoperfil where tpeid=".TPE_DIRETOR." and pdeid=".$pdeid.")";
		$db->executar($sql);
		$db->commit();
		
		$sql = "delete from pdeinterativo2013.pessoatipoperfil where tpeid=".TPE_DIRETOR." and pdeid=".$pdeid."";
		$db->executar($sql);
		$db->commit();
	} else {
		$_SESSION['pdeinterativo2013']['msg'] = "Não foi possível identificar o Código INEP da escola selecionada!";
		header("Location: pdeinterativo2013.php?modulo=principal/cadastroDiretor&acao=A");
		exit;
	}
	...
	$sql = "select 
				pesid 
			from 
				pdeinterativo2013.pessoatipoperfil
			where
				pesid = $pesid
			and
				tpeid = ".TPE_DIRETOR."
	...
	if(!$db->pegaUm($sql)){
		$sql = "insert into pdeinterativo2013.pessoatipoperfil (pesid,tpeid,pdeid) values ($pesid,".TPE_DIRETOR.",$pdeid);";
		$db->executar($sql);
		$db->commit();
	}
	
	$sql="update pdeinterativo2013.listapdeinterativo set usucpfdiretor = {$usucpf},
	usunome  = {$pesnome},
	usuemail = '{$usuemail}'
	where pdeid = {$pdeid}";
	...
	$sql = "INSERT INTO pdeinterativo2013.historicocadastrodiretor(
	...
	ativaUsuarioPDEInterativo2012($_POST,PDEINT_PERFIL_CONSULTA_DIRETOR_PDE2013,$entid,3);
	
	$_SESSION['pdeinterativo2013']['msg'] = "Operação realizada com sucesso.";
	header("Location: pdeinterativo2013.php?modulo=principal/cadastroDiretor&acao=A&requisicao=carregarDiretor&usucpf=".str_replace("'", "", $usucpf)."&pdicodinep={$pdicodinep}");
	exit;
}
...
function recuperaPdeidPorCodigoINEP($pdicodinep)
{
	...
	$sql = "select pdeid from pdeinterativo2013.pdinterativo where pdicodinep = '$pdicodinep' and pdistatus = 'A'";
	return $db->pegaUm($sql);
}
...
function carregarDiretor()
{
	...
	if($_REQUEST['usucpf'] && $_REQUEST['pdicodinep']){
		$sql = "select 
					pde.pdicodinep,
					pde.pdeid,
					( select htudsc from seguranca.historicousuario htu where htu.usucpf = usu.usucpf order by htuid desc limit 1) as justificativa,
				...
				inner join
					pdeinterativo2013.pdinterativo pde ON pde.entid = ure.entid  
				where 
					ure.usucpf = '{$_REQUEST['usucpf']}' 
				and 
					ure.pflcod = ".PDEESC_PERFIL_DIRETOR." 
				and 
					pde.pdicodinep = '{$_REQUEST['pdicodinep']}'";
		return $db->pegaLinha($sql);
	}else{
		return array("pdicodinep" => $_REQUEST['pdicodinep']);
	}
}
...
function excluirDiretor()
{
	...
	*/
	...
	$sql = "delete from pdeinterativo2013.pessoatipoperfil where pdeid = {$pdeid} and pesid in(select pesid from pdeinterativo2013.pessoa where usucpf='".$usucpf."') AND tpeid=".TPE_DIRETOR;
	...
	$sql = "delete from pdeinterativo2013.listadiretor where lstid = (select lstid from pdeinterativo2013.LISTAPDEINTERATIVO where pdeid = {$pdeid}) and usucpf='".$usucpf."'";
	
	$db->executar($sql);
	$db->commit();
	
	/*
	$sql = "update pdeinterativo2013.pessoa	set	pflcod=NULL where usucpf = '$usucpf'";
	$db->executar($sql);
	$db->commit();
	*/
	...
	//dbg($sql,1);
	$_SESSION['pdeinterativo2013']['msg'] = "Diretor excluído com sucesso!";
	
	header("Location: pdeinterativo2013.php?modulo=principal/cadastroDiretor&acao=A&requisicao=carregarDiretor&pdicodinep={$pdicodinep}");
	//exit;
}
...
function recuperaDadosDiretor($pdeid = null)
{
	...
	$sql = "select 
			...
			from 
				pdeinterativo2013.pdinterativo pde
			inner join 
				pdeinterativo2013.pessoatipoperfil ptp ON ptp.pdeid = pde.pdeid and ptp.tpeid=2 
			inner join
				pdeinterativo2013.pessoa pes ON ptp.pesid = pes.pesid
			inner join
				seguranca.usuario usu ON usu.usucpf = pes.usucpf
			inner join
				seguranca.perfilusuario pfl ON usu.usucpf = pfl.usucpf
			where 
				pesstatus = 'A'
			and
				pde.pdeid = $pdeid
			and
				pfl.pflcod = ".PDEESC_PERFIL_DIRETOR.";";
	//dbg($sql);
	return $db->pegaLinha($sql);
}
...
function confirmaDadosDiretor()
{
	...
	if(!$usucpf || !$usuemail || !$usufoneddd || !$usufonenum){
		$_SESSION['pdeinterativo2013']['msg'] = "Favor preencher todos os campos obrigatórios!";
		header("Location: pdeinterativo2013.php?modulo=principal/identificacao&acao=A");
		exit;
	}
...
//	}
	...
	if($usucelddd && $usucelnum) {
		...
		$uscid = $db->pegaUm("SELECT uscid FROM pdeinterativo2013.usuariocelular WHERE usucpf = {$usucpf}");
		
		if($uscid) {
			
			$sql = "UPDATE pdeinterativo2013.usuariocelular
					SET usucelddd='".$usucelddd."', usucelnum='".$usucelnum."'
					WHERE  usucpf={$usucpf}";
			
		} else {
			
			$sql = "INSERT INTO pdeinterativo2013.usuariocelular(
	...
	*/
	...
	$sql = "update 
				pdeinterativo2013.detalhepessoa 
			set
				dpeemail = '$usuemail',
				dpetelefone = '".str_replace("-","",$usufoneddd.$usufonenum)."'
			where 
				pesid in(select pesid from pdeinterativo2013.pessoa where usucpf=$usucpf);";
	...
	if($hdn_redirect == "C"){
		header("Location: pdeinterativo2013.php?modulo=principal/identificacao&acao=A&aba=Escola");
	}else{
		header("Location: pdeinterativo2013.php?modulo=principal/identificacao&acao=A&aba=Diretor");
	}
...
function recuperaDadosEscolaPorCPFDiretor($usucpf = null)
{
	...
	$sql = "select 
				...
				mun.mundescricao,
				mun.muncod,
				CASE WHEN (select distinct id_ens_fundamental_ciclos from educacenso_2012.tab_dado_escola where fk_cod_entidade::bigint = pdicodinep::bigint) = 1
					THEN 'Sim'
					ELSE 'Não'
				END as ciclo
			from 
				pdeinterativo2013.pdinterativo pdi 
			inner join
				pdeinterativo2013.pessoatipoperfil ptp ON ptp.pdeid = pdi.pdeid and ptp.tpeid=".TPE_DIRETOR."  
			inner join
				pdeinterativo2013.pessoa pes ON pes.pesid = ptp.pesid
			inner join
...
function recuperaDadosEscolaPorPdeid($pdeid = null)
{
	global $db;
...
function gerenciarCargaCapitalCusteio($dados) {
	...
	$cccid = $db->pegaUm("SELECT cccid FROM pdeinterativo2013.cargacapitalcusteio WHERE pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'");
	
	if($cccid) {
		
		$sql = "UPDATE pdeinterativo2013.cargacapitalcusteio
				...
				WHERE pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'";
		
	} else {
	
		$sql = "INSERT INTO pdeinterativo2013.cargacapitalcusteio(
	    		...
	    		VALUES (".$_SESSION['pdeinterativo2013_vars']['pdeid'].", 
	            		...
	            		'2011', 
	            		'2012', 
	            		'".$_SESSION['pdeinterativo2013_vars']['pdicodinep']."');";
	...
	echo "<script>
			...
			window.location='pdeinterativo2013.php?modulo=principal/planoestrategico&acao=A&aba=".$_REQUEST['aba']."&aba1=".$_REQUEST['aba1']."';
		  </script>";
...
function confirmaDadosEscola()
{
	...
	if($hdn_redirect == "C"){
		header("Location: pdeinterativo2013.php?modulo=principal/identificacao&acao=A&aba=Galeria");
	}else{
		header("Location: pdeinterativo2013.php?modulo=principal/identificacao&acao=A&aba=Escola");
	}
...
function recuperaModalidadesEnsinoPorCodigoINEP($inep)
{
	...
	$sql = "select distinct
			...
			from 
				educacenso_2012.tab_turma turma
			inner join
				educacenso_2012.tab_mod_ensino modalidade ON turma.fk_cod_mod_ensino = modalidade.pk_cod_mod_ensino
			where 
...
function recuperaNiveisEnsinoPorCodigoINEP($inep)
{
	...
	$sql = "select 
			...
			from 
				educacenso_2012.tab_matricula matricula
			inner join
				educacenso_2012.tab_etapa_ensino etapa ON etapa.pk_cod_etapa_ensino = matricula.fk_cod_etapa_ensino
			where 
...
function recuperaParametroCNE()
{
	...
	$sql = "select 
				...
				cnenumestturm
			from 
				pdeinterativo2013.parametrocne
			where 
				cnestatus = 'A'
			order by
				cneposicao,
...
function recuperaTurmasPorEscola($inep = null)
{
	...
	$inep = !$inep ? $_SESSION['pdeinterativo2013_vars']['pdicodinep'] : $inep;
	...
	$sql = "select 
				...
				(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where pk_cod_turma = fk_cod_turma and t.id_status = 1) as matricula,
				ten.id_localizacao as localizacao
			from 
				educacenso_2012.tab_turma turma
			inner join
				educacenso_2012.tab_etapa_ensino etapa ON etapa.pk_cod_etapa_ensino = turma.fk_cod_etapa_ensino
			left join
				educacenso_2012.tab_entidade ten ON ten.pk_cod_entidade::bigint = turma.fk_cod_entidade::bigint
			where 
			...
			if($dado['matricula'] != 0){
				if(strstr($dado['no_etapa_ensino'],"Ensino Fundamental")){
					...
					$arrEF["Ensino Fundamental"][] = array(
									...
									"serie" => substr($dado['no_etapa_ensino'],($inicio == 0 ? $inicio : $inicio+2),$fim - $inicio),
									"turma" => $dado['no_turma'],
...
function retornaparametroCNE($ensino,$serie = null,$numMatricula,$escola = null)
{
	...
	if($ensino == "Ensino Médio"){
		$sql = "select cnenumestturm from pdeinterativo2013.parametrocne where cnedesc = '$ensino'";
		...
		if(is_numeric($ano)){
			...
			if($tipo == "º"){
				if($ano <= 5){
					$sql = "select cnenumestturm from pdeinterativo2013.parametrocne where cnedesc ilike ('%{$escola}Anos iniciais%')";
					$cnenumestturm = $db->pegaUm($sql);
					if($numMatricula > $cnenumestturm){
						return true;
					}else{
					...
					$sql = "select cnenumestturm from pdeinterativo2013.parametrocne where cnedesc ilike ('%{$escola}Anos finais%')";
					$cnenumestturm = $db->pegaUm($sql);
					if($numMatricula > $cnenumestturm){
						return true;
					}else{
		...
		$sql = "select cnenumestturm from pdeinterativo2013.parametrocne where cnedesc ilike ('%{$serie}%')";
		$cnenumestturm = $db->pegaUm($sql);
		if($numMatricula > $cnenumestturm){
			return true;
		}else{
...
function salvarDistorcaoDiagnosticoMatricula()
{
	...
	$sql = "update pdeinterativo2013.distorcaoaproveitamento set diastatus = 'I' where pdeid = $pdeid and diasubmodulo = 'M';";
	$db->executar($sql);
	$db->commit();
	...
	if($hdn_redirect == "C"){
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_2_distorcaoidadeserie");
	}else{
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_1_matriculas");
	}
...
{
	global $db;
	if(!strstr($sql,"pdeinterativo2013.pergunta")){
	...
	if($arrPerguntas){
		...
		$sql = "select 
					...
					pro.prgid
				from 
					pdeinterativo2013.opcaopergunta opc
				inner join
					pdeinterativo2013.perguntaopcao pro ON opc.oppid = pro.oppid 
				where 
		...
		$sql = "select * from pdeinterativo2013.respostapergunta where prgid in (".implode(",",$arrPrgid).") AND pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'";
...
function salvarDistorcaoIdadeSerie()
{
	...
	$sql = "update pdeinterativo2013.distorcaoaproveitamento set diastatus = 'I' where pdeid = $pdeid and diasubmodulo = 'D';";
	$db->executar($sql);
	$db->commit();
	...
	if($hdn_redirect == "C"){
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_3_aproveitamentoescolar");
	}else{
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_2_distorcaoidadeserie");
	}
	exit;
...
function salvarDistorcaoAproveitamentoEscolar()
{
	...
	$sql = "update pdeinterativo2013.distorcaoaproveitamento set diastatus = 'I' where pdeid = $pdeid and diasubmodulo = 'A';";
	$db->executar($sql);
	$db->commit();
	...
	if($hdn_redirect == "C"){
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_4_areasdeconhecimento");
	}else{
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_3_aproveitamentoescolar");
	}
...
function salvarRespostasPorEscola($pdeid = null)
{
	...
	if($perg){
		foreach($perg as $prgid => $oppid){
			$sqlE= "delete from pdeinterativo2013.respostapergunta where pdeid = $pdeid and prgid = $prgid;";
			$db->executar($sqlE);
			$db->commit();
			if($oppid) {
				$sqlI= "insert into pdeinterativo2013.respostapergunta (pdeid,oppid,prgid) values ($pdeid,$oppid,$prgid);";
				$db->executar($sqlI);
				$db->commit();
			}
		}
	...
	$sql = "select
				...
				CASE 
					WHEN dianumdistorcao > 0
						THEN round(( (dianumdistorcao::numeric/(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where t.fk_cod_turma = p.fk_cod_turma  and t.id_status = 1)::numeric)*100))
					when dianumdistorcao = 0
						THEN 0
					ELSE null
				END as taxa
			from
				pdeinterativo2013.distorcaoaproveitamento p
			where
				pdeid = $pdeid
			and
				diasubmodulo = 'D'
	...
	$sql = "select
				...
				CASE 
					WHEN dianumabandono > 0
						THEN round(( (dianumabandono::numeric/(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where t.fk_cod_turma = p.fk_cod_turma and t.id_status = 1)::numeric)*100))
					when dianumabandono = 0
						THEN 0
					ELSE null
				END as taxaabandono,
				CASE 
					WHEN dianumreprovado > 0
						THEN round(( (dianumreprovado::numeric/(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where t.fk_cod_turma = p.fk_cod_turma and t.id_status = 1)::numeric)*100))
					when dianumreprovado = 0
						THEN 0
					ELSE null
...
function retornaDisciplinasTurma($pdeid = null)
{
	...
	$sql = "select distinct
			...
			inner join
				pdeinterativo2013.distorcaoaproveitamento dia ON dia.fk_cod_turma = turma.pk_cod_turma
			inner join
				educacenso_2012.tab_disciplina_turma disturma ON disturma.fk_cod_turma = turma.pk_cod_turma
			inner join
				educacenso_2012.tab_disciplina disc ON disc.pk_cod_disciplina = disturma.fk_cod_disciplina
			where 
				dia.pdeid = $pdeid
			and
				dia.diamarcado = 'R'
...
function salvarDistorcaoAreasConhecimento()
{
	...
	$sql = "update pdeinterativo2013.distorcaodisciplina set dtdstatus = 'I' where pdeid = $pdeid;";
	$db->executar($sql);
	$db->commit();
		...
		foreach($num_reprovacao_disciplina as $turma => $arrDisc){
			if($arrDisc){
				foreach($arrDisc as $disc => $rep){
					if($rep || $rep == "0"){
						$sql= "insert into pdeinterativo2013.distorcaodisciplina (fk_cod_disciplina,fk_cod_turma,dtdnumreprovado,dtdstatus,pdeid) values ($disc,$turma,$rep,'A',$pdeid);";
						$db->executar($sql);
						$db->commit();
					}
				}
	...
	if($hdn_redirect == "C"){
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_5_sintesedimensao2");
	}else{
		header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_4_areasdeconhecimento");
	}
	exit;
...
function exibeTurmasCriticas()
{
	...
	$sql = "select distinct
				...
				CASE 
					WHEN dianumdistorcao > 0
						THEN round(( (dianumdistorcao::numeric/(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where t.fk_cod_turma = turma.pk_cod_turma and t.id_status = 1)::numeric)*100))
					when dianumdistorcao = 0
						THEN 0
					ELSE null
				END || ' %' as taxa
		...
		foreach($arrDados as $dado){
			if(strstr($dado['no_etapa_ensino'],"Ensino Fundamental")){
				...
				$arrEF[] = array( 
									...
									"turma" => $dado['no_turma'],
									"taxa" => $dado['taxa'],
									"taxaBrasil" => $db->pegaUm("SELECT intvalor FROM pdeinterativo2013.indicadorestaxas it where intesfera = 'B' and intsubmodulo = 'D' and intano = 2010 and intensino = 'U'")."%"
								);
			}
			if(strstr($dado['no_etapa_ensino'],"Ensino Médio")){
				...
				$arrEM[] = array( 
									...
									"turma" => $dado['no_turma'],
									"taxa" => $dado['taxa'],
									"taxaBrasil" => $db->pegaUm("SELECT intvalor FROM pdeinterativo2013.indicadorestaxas it where intesfera = 'B' and intsubmodulo = 'D' and intano = 2010 and intensino = 'M'")."%"
								);
			}
...
function exibeTurmasCriticasReprovacao()
{
	...
	$sql = "select distinct
				...
				CASE 
					WHEN dianumreprovado > 0
						THEN round(( (dianumreprovado::numeric/(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where t.fk_cod_turma = turma.pk_cod_turma)::numeric)*100))
					when dianumreprovado = 0
						THEN 0
					ELSE null
				END || ' %' as taxa
		...
		foreach($arrDados as $dado){
			if(strstr($dado['no_etapa_ensino'],"Ensino Fundamental")){
				...
				$arrEF[] = array( 
									...
									"turma" => $dado['no_turma'],
									"taxa" => $dado['taxa'],
									"taxaBrasil" => $db->pegaUm("SELECT intvalor FROM pdeinterativo2013.indicadorestaxas it where intesfera = 'B' and intaprrepaba = 'R' and intano = 2010 and intensino = 'U'")."%"
								);
			}
			if(strstr($dado['no_etapa_ensino'],"Ensino Médio")){
				$inicio = strpos($dado['no_etapa_ensino'],"-");
				...
				$arrEM[] = array( 
									...
									"turma" => $dado['no_turma'],
									"taxa" => $dado['taxa'],
									"taxaBrasil" => $db->pegaUm("SELECT intvalor FROM pdeinterativo2013.indicadorestaxas it where intesfera = 'B' and intaprrepaba = 'R' and intano = 2010 and intensino = 'M'")."%"
								);
			}
			if(strstr($dado['no_etapa_ensino'],"Educação Infantil")){
				$inicio = strpos($dado['no_etapa_ensino'],"-");
...
function exibeTurmasCriticasAbandono()
{
	...
	$sql = "select distinct
				...
				CASE 
					WHEN dianumabandono > 0
						THEN round(( (dianumabandono::numeric/(select distinct count(pk_cod_matricula) from educacenso_2012.tab_matricula t where t.fk_cod_turma = turma.pk_cod_turma and t.id_status = 1)::numeric)*100))
					when dianumabandono = 0
						THEN 0
					ELSE null
				END || ' %' as taxa
		...
		foreach($arrDados as $dado){
			if(strstr($dado['no_etapa_ensino'],"Ensino Fundamental")){
				...
				$arrEF[] = array( 
									...
									"turma" => $dado['no_turma'],
									"taxa" => $dado['taxa'],
									"taxaBrasil" => $db->pegaUm("SELECT intvalor FROM pdeinterativo2013.indicadorestaxas it where intesfera = 'B' and intaprrepaba = 'B' and intano = 2010 and intensino = 'U'")."%"
								);
			}
			if(strstr($dado['no_etapa_ensino'],"Ensino Médio")){
				$inicio = strpos($dado['no_etapa_ensino'],"-");
				...
				$arrEM[] = array( 
									...
									"turma" => $dado['no_turma'],
									"taxa" => $dado['taxa'],
									"taxaBrasil" => $db->pegaUm("SELECT intvalor FROM pdeinterativo2013.indicadorestaxas it where intesfera = 'B' and intaprrepaba = 'B' and intano = 2010 and intensino = 'M'")."%"
								);
			}
			if(strstr($dado['no_etapa_ensino'],"Educação Infantil")){
				$inicio = strpos($dado['no_etapa_ensino'],"-");
...
function recuperaRespostasEscola($pdeid = null,$modulo = "D", $submodulo = "D", $detalhe = null, $arrWhere = array() )
{
	...
	$sql = "select
				...
				critico
			from
				pdeinterativo2013.respostapergunta rp
			inner join
				pdeinterativo2013.pergunta pe ON pe.prgid = rp.prgid AND prgstatus = 'A'
			inner join
				pdeinterativo2013.opcaopergunta op ON op.oppid = rp.oppid  AND oppstatus = 'A'
			where
				pdeid = $pdeid
			and
				prgmodulo = '$modulo'
...
function removeDiretoresAntigos()
{
	...
	$pdeid = $db->pegaUm("SELECT pdeid FROM pdeinterativo2013.pdinterativo WHERE pdicodinep='".$pdicodinep."' AND pdistatus='A'");
	$arrDados = recuperaDadosDiretor($pdeid);
	
	$sql = "delete from pdeinterativo2013.pessoatipoperfil where tpeid='".TPE_DIRETOR."' and pesid=(select pesid from pdeinterativo2013.pessoa where usucpf='".$arrDados['usucpf']."')";
	$db->executar($sql);
	$db->commit();
	...
	$sql = "
			update pdeinterativo2013.usuarioresponsabilidade set rpustatus = 'I' where entid = '$pdicodinep' and pflcod = '$pflcod';
			update 
				pdeinterativo2013.pessoa
			set
				pflcod = null
...
function carregaTaxa($taxa = "R", $esfera = "B", $submodulo = "T")
{
	...
	$sql = "SELECT it.* FROM pdeinterativo2013.indicadorestaxas it 
			 LEFT JOIN pdeinterativo2013.pdinterativo es ON es.estuf = it.intcoduf::character(2) and es.pdistatus = 'A' 
			 LEFT JOIN pdeinterativo2013.pdinterativo mu ON mu.muncod = it.intcodmun::character(7) and mu.pdistatus = 'A'
...
function salvarSinsteseDimensao2($salvaAba = true)
{
	global $db;
	...
	if($arrRepid){
		$sql = "update pdeinterativo2013.respostapergunta set critico = false where repid in (".implode(",",$arrRepid).");";
		$db->executar($sql);
		$db->commit();
	}
	if($chk_problemas['opcao']){
		foreach($chk_problemas['opcao'] as $repid => $valor){
			$sql = "update pdeinterativo2013.respostapergunta set critico = true where repid = $repid;";
			$db->executar($sql);
			$db->commit();
		}
	}
	...
	if($arrTurmasTaxa){
		$sql="update pdeinterativo2013.distorcaoaproveitamento set diacritico = false where fk_cod_turma in (".implode(",",$arrTurmasTaxa).") and pdeid = '$pdeid' and diastatus = 'A' and diamarcado in ('D','R','A');";
		$db->executar($sql);
		$db->commit();
	...
	$sql="update pdeinterativo2013.distorcaoaproveitamento set diacritico = false where diasubmodulo = 'M' and pdeid = '$pdeid' and diastatus = 'A';";
	$db->executar($sql);
	$db->commit();

	if($chk_problemas['matricula'] && $arrTurmasMatricula[0]){
		$sql="update pdeinterativo2013.distorcaoaproveitamento set diacritico = true where fk_cod_turma in (".$arrTurmasMatricula[0].") and pdeid = '$pdeid' and diastatus = 'A' and diasubmodulo = 'M';";
		$db->executar($sql);
		$db->commit();
	...
	if($arrTurmasDisciplina){
		$sql="update pdeinterativo2013.distorcaodisciplina set dtdcritico = false where fk_cod_turma in (".implode(",",$arrTurmasDisciplina).") and pdeid = '$pdeid' and dtdstatus = 'A';";
		$db->executar($sql);
		$db->commit();
	}
	...
	if($salvaAba){
		...
		if($hdn_redirect == "C"){
			header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_3_ensinoeaprendizagem&aba1=diagnostico_3_0_orientacoes");
		}else{
			header("Location: pdeinterativo2013.php?modulo=principal/diagnostico&acao=A&aba=diagnostico_2_distorcaoeaproveitamento&aba1=diagnostico_2_5_sintesedimensao2");
		}
...
function verificaCheckBoxPergunta($repid)
{
	...
	$sql = "select repid from pdeinterativo2013.respostapergunta where critico = true and repid = $repid;";
	if($db->pegaUm($sql)){
		return true;	
	}else{
		return false;
...
{
	$_SESSION['pdeinterativo2013_vars']['pdeid'] = $_POST['pdeid'];
	header("Location: pdeinterativo2013.php?modulo=principal/identificacao&acao=A");
	exit;
}

function cabecalhoPDEInterativo2013($arrDados = array())
{ global $db; ?>
	<table class="tabela" bgcolor="#f5f5f5" cellSpacing="1" cellPadding="3" align="center">
		...
		<tr>
			<td class="SubTituloDireita bold" >Ano do CENSO:</td>
			<td>2012</td>
		</tr>
		...
		<tr>
			...
			if($cargacapitalcusteio && !$_REQUEST['cccid']):
				...
				$sql = "SELECT SUM(pabvalorcapital) as pabvalorcapital, SUM(pabvalorcusteiro) as pabvalorcusteiro, pabparcela 
						...
						WHERE pap.pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."' AND pabstatus='A' AND papstatus='A' AND paestatus='A' AND paastatus='A' and ((pabfonte='O') or (pabfonte='P' and pabparcela in('S','P'))) 
						GROUP BY pabparcela";
			...
			<?
			if($_REQUEST['cccid']) {
				$sql = "SELECT * FROM pdeinterativo2013.cargacapitalcusteio WHERE cccid='".$_REQUEST['cccid']."' AND pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'";
...
{ global $db; ?>
	<script>
		function enviarAnalise( docid ){
			window.location = window.location.href+'&requisicao=tramitaFormacao&pdeid=<?=$_SESSION['pdeinterativo2013_vars']['pdeid'] ?>&docid='+docid+'&aedid=1146';
		}
	...
	<table class="tabela" bgcolor="#f5f5f5" cellSpacing="1" cellPadding="3" align="center">
		...
		<!--<tr>
			...
			<td rowspan="6">
				...
				$docid = pegaDocid( $_SESSION['pdeinterativo2013_vars']['pdeid'] );
				
				wf_desenhaBotoesNavegacao( $docid , array( 'pdeid' => $_SESSION['pdeinterativo2013_vars']['pdeid']));
...
function barraProgressoPDEInterativo2013($pdeid = null)
{ 
...
function progressBar($pdeid = null) {
	...
	$sql = "select count(distinct abaid) from pdeinterativo2013.aba where (abatipo != 'O' or abatipo is null) and abaidpai is not null and abaid not in (2,3,4,5,6,7,8,54,57,58);";
	...
	$sql = "select count(distinct abaid) from pdeinterativo2013.abaresposta where pdeid = $pdeid;";
...
function pegaPorcentagemPreechimento($pdeid)
{
	global $db;
...
function salvarAbaResposta($abacod,$pdeid = null)
{
	...
	if(!$pdeid) die("<script>alert('Problemas na identificação de variáveis. Tente novamente!');window.location='pdeinterativo2013.php?modulo=inicio&acao=C';</script>");
	...
	if(CACHE_FILE) {
		...
		if(in_array($abacod,$arrAbasCache)){
			include_once APPRAIZ.'includes/classes/cacheSimec.class.inc';
			$cache = new cache(false);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_EQUIPE_FNDE);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_CONSULTA_ESTADUAL);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_CONSULTA_MUNICIPAL);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEESC_PERFIL_CONSULTA);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEESC_PERFIL_DIRETOR);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_ESTADUAL);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_MUNICIPAL);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_EQUIPE_MEC);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_SUPER_USUARIO);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_PAR_ESTADUAL);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_".PDEINT_PERFIL_COMITE_PAR_MUNICIPAL);
			$cache->apagarCache($abacod."_".$_SESSION['pdeinterativo2013_vars']['pdeid']."_semperfil");
		}
		/* Fim - Cache em arquivo*/
	}
	...
	if(strstr($abacod,"diagnostico_") && !strstr($abacod,"_orientacoes")){
		...
		$sql = "select flaid from pdeinterativo2013.flag where pdeid = $pdeid";
		$flaid = $db->pegaUm($sql);
		if($flaid){
			$sql = "update pdeinterativo2013.flag set atualizaplano = true where flaid = $flaid";
		}else{
			$sql = "insert into pdeinterativo2013.flag (pdeid,atualizaplano) values ($pdeid,true)";
		}
		$db->executar($sql);
		$db->commit();
	...
	if($abacod != "planoestrategico_0_2_planoacao") {
		$sql = "delete from pdeinterativo2013.abaresposta where pdeid = $pdeid and abaid = (select abaid from pdeinterativo2013.aba where abacod = 'planoestrategico_0_2_planoacao' limit 1)";
		$db->executar($sql);
		$db->commit();
	}
	
	$arrInfoPde = $db->pegaLinha("SELECT docid, pditempdeescola, pdicodinep, pdeano, pdeid, pdiesfera FROM pdeinterativo2013.pdinterativo WHERE pdeid=$pdeid");
	...
	if(!$arrInfoPde['docid']) {
		...
		$db->executar("UPDATE pdeinterativo2013.pdinterativo SET docid='".$docid."' WHERE pdeid='$pdeid'");
		$db->commit();
	}
	 	
	$sql = "delete from pdeinterativo2013.abaresposta where pdeid = $pdeid and abaid = (select abaid from pdeinterativo2013.aba where abacod = '$abacod' limit 1);";
	$db->executar($sql);
	$db->commit();
	$sql = "insert into pdeinterativo2013.abaresposta (abaid,pdeid) values ( (select abaid from pdeinterativo2013.aba where abacod = '$abacod' limit 1) , $pdeid)";
	$db->executar($sql);
	$db->commit();


	$porcentagem = pegaPorcentagemPreechimento($pdeid);
	$sql = "update  pdeinterativo2013.listapdeinterativo  set percent = $porcentagem where pdeid = $pdeid";
...
function verificaFlagPDEInterativo($flag,$pdeid = null)
{
	global $db;
	$pdeid = !$pdeid ? $_SESSION['pdeinterativo2013_vars']['pdeid'] : $pdeid;
	$sql = "select $flag from pdeinterativo2013.flag where pdeid = $pdeid";
...
if($_GET['limparCache']){
	include_once APPRAIZ.'includes/classes/cacheSimec.class.inc';
	$cache = new cache(false);
	$cache->apagarCache($_GET['aba1']."_".$_SESSION['pdeinterativo2013_vars']['pdeid']);
	die("Cache excluído!");
}
...
function recuperaTelasPendentes($abacod,$pdeid = null)
{
	...
	$sql = "select 
				abadescricao 
			from 
				pdeinterativo2013.aba 
			where 
				abaidpai = (select abaid from pdeinterativo2013.aba where abacod = '$abacod' limit 1)
			and
				abaid not in (select abaid from pdeinterativo2013.abaresposta where pdeid = $pdeid)
			and
				abatipo is null 
			order by 
				abadescricao";
...
function verificaQtdMaxima()
{
	...
	$maxFotos = $db->pegaUm("SELECT coalesce(aefqtdfoto, 0) FROM pdeinterativo2013.ambienteescolafoto WHERE aefid = ".$_POST['aefid']);
...
function excluirFoto()
{
	...
	$db->executar("UPDATE pdeinterativo2013.galeriafoto SET gfostatus = 'I' WHERE arqid = ".$_POST['arqid']);
	$db->commit();
...
function verificaQtdMinima()
{
	...
	$sql = "SELECT aefid,aefdesc,coalesce(aefqtdfotoobrigatoria,0) as aefqtdfotoobrigatoria FROM pdeinterativo2013.ambienteescolafoto WHERE aefstatus = 'A'";
	$ambienteescolafoto = $db->carregar($sql);
	
	if($ambienteescolafoto[0]) {
		foreach($ambienteescolafoto as $ambiente)
		{
			$sql = "SELECT count(1) FROM pdeinterativo2013.galeriafoto WHERE aefid = ".$ambiente['aefid']." AND pdeid = ".$_POST['pdeid']." AND gfostatus = 'A'";
			$qtdFotos = $db->pegaUm($sql);
...
function verificaPermissao($arrPflcod,$usucpf = null)
{
	...
	$estado_documento = $db->pegaUm("SELECT d.esdid FROM pdeinterativo2013.pdinterativo p 
	...
	if(!$db->carregar($sql)) {
		$arrJS[] = "jQuery(\"[name^='inserirbensservicos'],[name^='btn_salvar'],[value='Inserir ação'],[value='Enviar para o comitê'],[value='Inserir estratégia'],[value='Inserir outros desafios'],[value='Inserir Membros do Conselho'],[id='btSalvar'],[value='Inserir Demais Profissionais'],[name^='fontesprogramas'],[src='../imagens/alterar.gif'],[src='../imagens/gif_inclui.gif'],[name^='salvar'],[value='Incluir Projeto'],[value='Incluir Programa'],[name^='continuar'], [name^='btn_continuar'],[class='btNovaFoto'],[id='btContinuar'],[src^='/imagens/exclui_p'],[src^='../imagens/exclui'],[src^='../imagens/alterar.gif'],[src^='/imagens/check_p'],[id='arquivo'],[id='btIncluirMembro']\").remove();";
		$arrJS[] = "jQuery(\"[name^='paarecurso'],[name^='ridfinal'],[name^='pesid_estrategias['],[name^='aoaid['],[name^='metas2['], [name^='metas['],[name^='resposta'],[name='rpcperiodicidade'],[name='rpcunidadeexecutora'],[name^='pessoas['],[name^='pgtcoordenador'],[name^='porque'],[name^='rpb'], [name^='rid'], [name^='opcaocoordenador'],[name^='opcaogrupo'], [name^='atende'],[name^='necessaria'],[name^='justificativa'],[name^='rpcpossuiconcelho'],[name^='fus'],[name^='rdo'],[name^='rdovinculo'],[name^='critico'],[name^='qtd'],[name^='atv'],[name^='rta'],[name^='rtacaso'],[name^='num_'],[name^='perg'],[type='checkbox'][name^='chk_'],[type='checkbox'][name^='resposta'],[name^='exibeprograma'],[name^='rtrfun'],[name^='rpbfinal'],[name^='exibeprojeto']\").attr('disabled','disabled');";
	}
...
function salvarInformePagamento()
{
	...
	$sql = "update pdeinterativo2013.situacaopagamento set spastatus = 'I' where pdeid = $pdeid;
			insert into 
				pdeinterativo2013.situacaopagamento 
	...
	$sql = "select ( case when s.spasituacao then 'Pago' else  'Pendente - ' || mopdesc end)  as result from pdeinterativo2013.motivopagamento m
	right join pdeinterativo2013.situacaopagamento s
	on m.mopid = s.mopid
	...
	$sql = "update pdeinterativo2013.listapdeinterativo set pagamento = '{$situacao}' where pdeid = $pdeid";
	$db->executar($sql);
	$db->commit();
...
function salvarJustificativaEvidencias($dados) {
	...
	$sql = "UPDATE pdeinterativo2013.justificativaevidencias SET juedescricao=NULL WHERE abacod='".$dados['abacod']."' AND pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'";
	
	if($dados['juedescricao']) {	
		
		$jueid = $db->pegaUm("SELECT jueid FROM pdeinterativo2013.justificativaevidencias 
							  WHERE abacod='".$dados['abacod']."' AND 
							  		pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'");
		
		if($jueid) {
			$sql = "UPDATE pdeinterativo2013.justificativaevidencias SET juedescricao='".substr(trim($dados['juedescricao']),0,1200)."' WHERE jueid='".$jueid."'";
		} else {
			$sql = "INSERT INTO pdeinterativo2013.justificativaevidencias(abacod, pdeid, juedescricao) VALUES ('".$dados['abacod']."', '".$_SESSION['pdeinterativo2013_vars']['pdeid']."', '".substr(trim($dados['juedescricao']),0,1200)."');";
		}
	}
...
function salvarPriorizacaoProblemas($dados)
{
	global $db;
	extract($dados);
	include_once '_funcoesdiagnostico_1.php';
	diagnostico_1_4_sintesedimensao1($dados,false);
	salvarSinsteseDimensao2(false);
	include_once '_funcoesdiagnostico_3.php';
	diagnostico_3_3_sintesedimensao3($dados,false);
	include_once '_funcoesdiagnostico_4.php';
	diagnostico_4_4_sintesedimensao4($dados,false);
	include_once '_funcoesdiagnostico_5.php';
	diagnostico_5_5_sintesedimensao5($dados,false);
	salvarSinteseInfraestrutura(false);
	salvarAbaResposta("diagnostico_2_5_sintesedimensao2");
	$db->executar("UPDATE pdeinterativo2013.flag SET atualizaplano=true WHERE pdeid='".$_SESSION['pdeinterativo2013_vars']['pdeid']."'");
	$db->commit();
	if($hdn_redirect == "C"){
		echo "<script>
		...
		echo "<script>
				alert('Dados gravados com sucesso.');
				window.location='pdeinterativo2013.php?modulo=principal/planoestrategico&acao=A&aba=planoestrategico_0_pdeescola&aba1=planoestrategico_0_1_priorizacao_problemas';
			  </script>";
	}
	exit;
}

function listaHistoricoUsuario($usucpf)
{
	global $db;
	monta_titulo("Histórico do Usuário","");
	$sql = "select
				resp.usunome as responsavel,
				case hustipo  
					when 'A' then 'Alterou'
					when 'I' then 'Atribuiu'
					when 'E' then 'Removeu'
				end as acao,
				usu.usunome,
				pfldsc,
				to_char(husdata,'DD/MM/YYYY') as data
			from pdeinterativo2013.historicousuario his
			inner join seguranca.perfil per on per.pflcod = his.pflcod
			inner join seguranca.usuario usu on usu.usucpf = his.usucpf
			inner join seguranca.usuario resp on resp.usucpf = his.huscpfresponsavel
			where usu.usucpf = '$usucpf' order by husid";
	$arrCab = array("Responsável","Ação","Usuário","Atribuição","Data");
	$db->monta_lista_simples($sql,$arrCab,100,10,"N");
}

function registraHistoricoUsuario($dados)
{
	global $db;
	extract($dados);
	$huscpfresponsavel = $_SESSION['usucpf'];
	$usucpf = str_replace(array("-","."),"",$usucpf);
	
	if($hustipo != 'E'){ //Excluir
		$sql = "select usucpf from pdeinterativo2013.historicousuario where usucpf = '$usucpf' and hustipo = 'I'";
		$existe = $db->pegaUm($sql);
		if($existe){
			$hustipo = "A"; //Alterar
		}else{
	...
	$sql = "insert into pdeinterativo2013.historicousuario (usucpf,husdata,hustipo,pflcod,husjustificativa,huscpfresponsavel) 
			values ('$usucpf',now(),'$hustipo',$pflcod,'$husjustificativa','$huscpfresponsavel') ";
	$db->executar($sql);
	$db->commit();
}

function verificaAddPerfil($pflcod,$usucpf = null)
{
	global $db;
	
	//O perfil só poderá acumular perfil de consulta (PDEINT_PERFIL_CONSULTA_ESTADUAL,PDEINT_PERFIL_CONSULTA_MUNICIPAL,PDEESC_PERFIL_CONSULTA,PDEINT_PERFIL_CONSULTA_DIRETOR_PDE2013)
	$arrPerfis = array($pflcod,PDEINT_PERFIL_CONSULTA_ESTADUAL,PDEINT_PERFIL_CONSULTA_MUNICIPAL,PDEINT_PERFIL_CONSULTA_ESTADUAL,PDEINT_PERFIL_SUPER_USUARIO);
	$usucpf = !$usucpf ? $_SESSION['usucpf'] : $usucpf;
	$usucpf = substr(str_replace(array(".","-","/"),"",$usucpf),0,11);	
	$sql = "select 
				count (distinct per.pflcod) as qtde 
			from 
				seguranca.perfilusuario per
			inner join
				seguranca.perfil pfl ON pfl.pflcod = per.pflcod
			where 
				usucpf = '$usucpf'
			and
				per.pflcod not in (".implode(",",$arrPerfis).")
			and
				sisid = ".SISID_PDE_INTERATIVO;
	...
	if($qtde == 0){
		...
		$_SESSION['pdeinterativo2013']['msg'] = "Não é possível atribuir este perfil ao usuário indicado, pois ele possui outros perfis atribuídos.";
...
function recuperaFiltroPerfil($usucpf = null)
{
	global $db;
	$usucpf = !$usucpf ? $_SESSION['usucpf'] : $usucpf;
	$usucpf = substr(str_replace(array(".","-","/"),"",$usucpf),0,11);
	$arrPerfil = pegaPerfilGeral($usucpf);
	
	if( in_array(PDEINT_PERFIL_SUPER_USUARIO,$arrPerfil) ){
		return false;
	}
	...
	$sql = "select 
			...
			from 
				pdeinterativo2013.usuarioresponsabilidade ure
			inner join
				pdeinterativo2013.tprperfil tpr ON tpr.pflcod = ure.pflcod
			inner join
				pdeinterativo2013.tiporesponsabilidade res ON res.tprcod = tpr.tprcod 
			where 
				ure.usucpf = '$usucpf'
			and
				ure.pflcod in (".implode(",",$arrPerfil).")";
	//dbg($sql);
	$arrDados = $db->pegaLinha($sql);
	if($arrDados){
		if(trim($arrDados['tprcampo']) == "muncod"){
			$sql = "select estuf from territorios.municipio where muncod = '{$arrDados['muncod']}'";
			$arrFiltro['estuf'] = $db->pegaUm($sql);
			$arrFiltro['muncod'] = $arrDados['muncod'];
			$arrFiltro['tpocod'] = "3";
			$arrFiltro['pdiesfera'] = "Municipal";
		}elseif(trim($arrDados['tprcampo']) == "estuf"){
			$arrFiltro['estuf'] = $arrDados['estuf'];
			$arrFiltro['tpocod'] = "2";
			$arrFiltro['pdiesfera'] = "Estadual";
		}elseif(trim($arrDados['tprcampo']) == "entid"){
			$arrFiltro['entid'] = $arrDados['entid'];
		}
		if($arrFiltro){
			return $arrFiltro;
		}else{
...
function pegarDocid($entid){
	...
	$sql = "
		SELECT docid
		FROM pdeinterativo2013.pdinterativo
		WHERE pdeano = ".ANO_EXERCICIO_PDE_INTERATIVO."
		AND   entid  = ".$entid;
	return $db->pegaUm($sql);
}

function cabecalhoGrandesDesafios($arrDados)
{
	monta_titulo("Grandes Desafios","&nbsp;");
}

function salvarOutrosDesafios($arrDados)
{
	global $db;
	if($arrDados){
		extract($arrDados);
	}
	$sql = "select otdid from pdeinterativo2013.outrosdesafios where otdstatus = 'A' and pdeid = ".$_SESSION['pdeinterativo2013_vars']['pdeid'];
	$otdid = $db->pegaUm($sql);
	if($otdid){
		$sql = "update pdeinterativo2013.outrosdesafios set otdtexto = '$otdtexto' where otdid = $otdid and pdeid = ".$_SESSION['pdeinterativo2013_vars']['pdeid'];
	}else{
		$sql = "insert into pdeinterativo2013.outrosdesafios (pdeid,otdtexto,otdstatus) values ({$_SESSION['pdeinterativo2013_vars']['pdeid']},'$otdtexto','A');";
	}
	$db->executar($sql);
	$db->commit();
}

function excluirOutrosDesafios($arrDados)
{
	global $db;
	$sql = "update pdeinterativo2013.outrosdesafios set otdstatus = 'I' where pdeid = ".$_SESSION['pdeinterativo2013_vars']['pdeid'];
	$db->executar($sql);
	$db->commit();
}