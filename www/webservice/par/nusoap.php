...
$cached = $GLOBALS['_transient']['static']['nusoap_base'];
$cached = is_object($cached) ? $cached : new stdClass();
$cached->globalDebugLevel = 9;

$GLOBALS['_transient']['static']['nusoap_base'] = $cached;
/**
*
* nusoap_base
*
...
class nusoap_base {
	...
	*/
	function debug($string){
		if ($this->debugLevel > 0) {
			$this->appendDebug($this->getmicrotime().' '.simec_get_class($this).": $string\n");
		}
	}
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
    	...
    	if(is_object($val) && simec_get_class($val) == 'soapval'){
		...
		switch(true) {
			...
			case is_object($val):
				if (! $name) {
					$name = simec_get_class($val);
					$this->debug("In serialize_val, used class name $name as element name");
				} else {
					$this->debug("In serialize_val, do not override name $name for element name for class " . simec_get_class($val));
				}
			...
			case (is_array($val) || $type):
                ...
                if($valueType=='arraySimple' || ereg('^ArrayOf',$type)){
					...
					if(is_array($val) && count($val)> 0){
						foreach($val as $v){
	                    	if(is_object($v) && simec_get_class($v) ==  'soapval'){
								$tt_ns = $v->type_ns;
								$tt = $v->type;
							} elseif (is_array($v)) {
    ...
    */
    ...
    function formatDump($str){
		$str = simec_htmlspecialchars($str);
		return nl2br($str);
    }
...
class soap_transport_http extends nusoap_base {
	...
	*/
	...
	function connect($connection_timeout=0,$response_timeout=10000){
...
//curl_setopt($this->ch, CURLOPT_CAINFO, 'f:\php-4.3.2-win32\extensions\curl-ca-bundle.crt');		
	...
	* @access   public
	*/
	function send($data, $timeout=0, $response_timeout=10000, $cookies=NULL) {
	...
	* @access   public
	*/
	function sendHTTPS($data, $timeout=0, $response_timeout=10000, $cookies) {
...
class soapservidor extends nusoap_base {
	...
	*/
	function soapservidor($wsdl=false){
		...
		if($wsdl){
			...
			if (is_object($wsdl) && (simec_get_class($wsdl) == 'wsdl')) {
				$this->wsdl = $wsdl;
	...
	*/
	function invoke_method() {
		...
		if ($class == '') {
			...
			if (!in_array($method_to_compare, simec_get_class_methods($class))) {
	...
	*/
	function serialize_return() {
		...
		// if fault
		if (isset($this->methodreturn) && (simec_get_class($this->methodreturn) == 'soap_fault')) {
...
class wsdl extends nusoap_base {
     ...
     * @access public 
     */
    function wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=10000){
		parent::nusoap_base();
        $this->wsdl = $wsdl;
	...
	*/
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					if(isset($opParts['documentation']) && $opParts['documentation'] != '') {
						$portType_xml .= '<documentation>' . simec_htmlspecialchars($opParts['documentation']) . '</documentation>';
					}
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
    	...
    	if (is_object($value) && simec_get_class($value) == 'soapval') {
    		if ($value->type_ns) {
...
//				}
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		// set name
		$this->message[$pos]['name'] = simec_htmlspecialchars($name);
		// set attrs
...
class soapcliente extends nusoap_base  {
	...
	var $timeout = 10000;				// HTTP connection timeout
	var $response_timeout = 10000;		// HTTP response timeout
	...
	* @access   public
	*/
	function soapcliente($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 10000){
		parent::nusoap_base();
		...
		// make values
		if($wsdl){
			if (is_object($endpoint) && (simec_get_class($endpoint) == 'wsdl')) {
				$this->wsdl = $endpoint;
	...
	* @access   private
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=10000) {
		$this->checkCookies();
		// detect transport
		switch(true){
			// http(s)
