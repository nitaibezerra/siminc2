...
function recuperaDocentesDiretor( $pdeid = null ){
	...
	$sql = "SELECT 
			...
			INNER JOIN pdeinterativo.pessoa 		 		 pes ON pes.usucpf = usu.usucpf
			INNER JOIN pdeinterativo.usuarioresponsabilidade rpu ON usu.usucpf = rpu.usucpf AND pfl.pflcod = rpu.pflcod AND rpustatus = 'A'
			INNER JOIN pdeinterativo.pdinterativo 		     pde ON pde.pdeid  = pes.pdeid
			WHERE 
				pesstatus = 'A'
				AND
				pde.pdeid = '$pdeid'
...
function recuperaDocentes(){
	...
	$sql = "SELECT DISTINCT
				rd.pk_cod_docente
			FROM 
				pdeinterativo.respostadocente rd 
			INNER JOIN pdeinterativo.pdinterativo pde ON pde.pdeid = rd.pdeid
			WHERE 
				pde.pdicodinep = '".$_SESSION['pdeinterativo_vars']['pdicodinep']."' AND rdoformapro = true AND rdovinculo in ('E','T')";
	//ver($_SESSION, $sql);
	$docentes = $db->carregarColuna($sql);
	
	if(count($docentes)>0){
		$db->executar('DELETE FROM pdeinterativo.planoformacaodocente WHERE pdeid = '.$_SESSION['pdeinterativo_vars']['pdeid'].' AND pk_cod_docente not in ('.implode(',',$docentes).')');
		$db->commit();
		$insere = '';
		$sql = "SELECT DISTINCT
					pk_cod_docente,
		...
		foreach( $docentes as $docente ){
			...
			$tes = "SELECT true FROM pdeinterativo.planoformacaodocente WHERE pk_cod_docente = '".$docente['pk_cod_docente']."' AND pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid']."";
			if(!$db->pegaUm($tes)){
				$insere = "INSERT INTO pdeinterativo.planoformacaodocente(pdeid, pk_cod_docente, pfdcpf) VALUES(".$_SESSION['pdeinterativo_vars']['pdeid'].",".$docente['pk_cod_docente'].",'".$docente['num_cpf']."');";
				$db->executar($insere);
				$db->commit();
			}
		}
	...
	/*$sql = "SELECT DISTINCT
			...
			LEFT  JOIN pdeinterativo.periodocursoformacao pe ON pe.pcfid		  = pf.pcfid
...
//	ver($sql);
	$sql = "
		...
		FROM educacenso_2011.tab_docente tdo 
			INNER JOIN pdeinterativo.planoformacaodocente pfd ON pfd.pk_cod_docente  = tdo.pk_cod_docente
			...
			LEFT  JOIN pdeinterativo.periodocursoformacao pcf ON pcf.pcfid           = pfd.pcfid
			LEFT  JOIN catalogocurso.modalidadecurso      mod ON mod.modid           = pfd.modid
			LEFT  JOIN pdeinterativo.pdinterativo         pde ON pde.pdeid           = pfd.pdeid
			LEFT  JOIN workflow.documento                 doc ON doc.docid           = pde.formacaodocid
			LEFT  JOIN workflow.estadodocumento           esd ON esd.esdid           = doc.esdid
			LEFT  JOIN (SELECT * FROM 
				dblink( 'host= user= password= port= dbname=',
		...
		WHERE pfd.pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid']."
		ORDER BY 2
	";
	//ver($sql);
...
function carregaCursos(){
	...
	if( $_REQUEST['pfdid'] ){
		...
		if( $db->pegaUm('SELECT true FROM pdeinterativo.planoformacaodocente WHERE pk_cod_docente IS NOT NULL AND pfdid = '.$_REQUEST['pfdid']) ){
			...
			$sql = "SELECT DISTINCT
					...
					INNER JOIN pdeinterativo.pessoatipoperfil      pt ON pt.pesid  = pfd.pesid
					INNER JOIN pdeinterativo.tipoperfil            tp ON tp.tpeid  = pt.tpeid AND tp.tpestatus = 'A'
					WHERE
						pfd.pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid']."
						AND pfdid = ".$_REQUEST['pfdid']."
					ORDER BY
						1";
...
//				INNER JOIN pdeinterativo.pdinterativo   pde ON pde.pdicodinep = dsh.dshcod
//				WHERE 
//					i.indid = 268
//					AND pde.pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid']."
...
function carregaCurso( $pfdid ){
	...
	$sql = "SELECT
			...
			LEFT  JOIN pdeinterativo.pessoa pes ON pes.pesid = pfd.pesid
			WHERE
				pfdid = $pfdid
				AND pfd.pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid'];
...
function verificaCurso( $pfdid ){
	...
	$pfdcpf = $db->pegaUm('SELECT pfdcpf FROM pdeinterativo.planoformacaodocente WHERE pfdid = '.$_REQUEST['pfdid']); 
...
//	if( $pk_docente ){
		...
		$sql = "SELECT
				...
				FROM
					pdeinterativo.planoformacaodocente pfd
				INNER JOIN pdeinterativo.pdinterativo pde ON pde.pdeid  = pfd.pdeid
...
//	}else{
//		
//		$pesid = $db->pegaUm('SELECT pesid FROM pdeinterativo.planoformacaodocente WHERE pesid IS NOT NULL AND pfdid = '.$_REQUEST['pfdid']);
//		
//		$sql = "SELECT
//					pde.pdenome as escola,
//					mundescricao||' - '||pde.estuf as municipio,
...
function recuperaDiretoriaEquipe(){
	...
	$sql = "SELECT DISTINCT
			    pe.pesid
			FROM
				pdeinterativo.pessoa pe
			    LEFT  JOIN seguranca.usuario usu ON usu.usucpf = pe.usucpf
			    INNER JOIN pdeinterativo.pessoatipoperfil ptp on ptp.pesid = pe.pesid
			WHERE
				ptp.pdeid = '".$_SESSION['pdeinterativo_vars']['pdeid']."'
			    AND pe.pesstatus = 'A'
	...
	if(count($arDiretoria)>0){
		$db->executar('DELETE FROM pdeinterativo.planoformacaodocente WHERE pdeid = '.$_SESSION['pdeinterativo_vars']['pdeid'].' AND pk_cod_docente IS NULL and pesid not in ('.implode(',',$arDiretoria).')');
		$db->commit();
		$insere = '';
		$sql = "SELECT DISTINCT
				    pe.pesid,
		...
		if( is_array($arDiretoria) ){
			foreach( $arDiretoria as $diretoria ){
				...
				$tes = "SELECT true FROM pdeinterativo.planoformacaodocente WHERE pesid = '".$diretoria['pesid']."' AND pdeid = '".$_SESSION['pdeinterativo_vars']['pdeid']."'";
				if(!$db->pegaUm($tes)){
					$insere = "INSERT INTO pdeinterativo.planoformacaodocente(pdeid, pesid, pfdcpf) VALUES(".$_SESSION['pdeinterativo_vars']['pdeid'].",".$diretoria['pesid'].",'".$diretoria['usucpf']."');";
					$db->executar($insere);
					$db->commit();
				}
			}
	...
	/*$sql = "SELECT DISTINCT
				...
				pfdcpf
			FROM 
				pdeinterativo.pessoa ps
                INNER JOIN pdeinterativo.pessoatipoperfil      pt ON pt.pesid  = ps.pesid
                INNER JOIN pdeinterativo.tipoperfil 		   tp ON tp.tpeid  = pt.tpeid and tp.tpestatus = 'A'
    			LEFT JOIN  seguranca.usuario 				   us ON us.usucpf = ps.usucpf 
                INNER JOIN  pdeinterativo.planoformacaodocente pf ON pf.pesid  = ps.pesid
                LEFT  JOIN catalogocurso.curso                 cr ON cr.curid  = pf.curid
                LEFT  JOIN catalogocurso.areatematica          at ON at.ateid  = cr.ateid
                LEFT  JOIN catalogocurso.nivelcurso			   nc ON nc.ncuid  = cr.ncuid
                LEFT  JOIN pdeinterativo.periodocursoformacao  pe ON pe.pcfid  = pf.pcfid
                LEFT  JOIN catalogocurso.modalidadecurso	   mo ON mo.modid  = pf.modid
			WHERE 
				pt.pdeid = {$_SESSION['pdeinterativo_vars']['pdeid']}
                AND ps.pesstatus = 'A'
	...
	$sql = "
		...
		FROM pdeinterativo.pessoa pes
			...
			LEFT  JOIN pdeinterativo.periodocursoformacao pcf  ON pcf.pcfid     = pfd.pcfid
			LEFT  JOIN catalogocurso.modalidadecurso      mod  ON mod.modid     = pfd.modid
			LEFT  JOIN pdeinterativo.pdinterativo         pde  ON pde.pdeid     = ptp.pdeid
			LEFT  JOIN workflow.documento                 doc  ON doc.docid     = pde.formacaodocid
			LEFT  JOIN workflow.estadodocumento           esd  ON esd.esdid     = doc.esdid
			LEFT  JOIN (SELECT * FROM 
				dblink( 'host= user= password= port= dbname=',
					'SELECT pri.pdicodinep,
					 ...
					 FROM snf.prioridadecursoescola pri  
						...
						LEFT JOIN workflow.estadodocumento esd ON esd.esdid = doc.esdid')
				AS resultado(pdicodinep varchar, esddsc varchar)) snf ON snf.pdicodinep = pde.pdicodinep
		WHERE ptp.pdeid     = {$_SESSION['pdeinterativo_vars']['pdeid']}
		AND   pes.pesstatus = 'A'
...
function detalheDocente( $dados ){
	...
	$sql = "SELECT DISTINCT
			...
			LEFT JOIN pdeinterativo.pessoa       	  		pes ON pes.pesid = pf.pesid 
			LEFT JOIN educacenso_".ANO_CENSO.".tab_docente 	   		 d  ON d.num_cpf = pf.pfdcpf
			LEFT JOIN pdeinterativo.pdinterativo 	  		pde ON pde.pdeid = pf.pdeid
			LEFT JOIN educacenso_".ANO_CENSO.".tab_entidade    		ent ON ent.cod_orgao_regional_inep = pde.pdicodinep
			LEFT JOIN educacenso_".ANO_CENSO.".tab_dado_escola 		des ON des.fk_cod_entidade = ent.pk_cod_entidade
			LEFT JOIN educacenso_".ANO_CENSO.".tab_docente_form_sup   bf ON bf.fk_cod_docente = d.pk_cod_docente
			LEFT JOIN educacenso_".ANO_CENSO.".tab_dado_docencia      dc ON dc.fk_cod_docente = d.pk_cod_docente
...
//	ver($sql);
	...
	$sql = "SELECT DISTINCT
			...
			FROM 
				pdeinterativo.planoformacaodocente     pf 
			INNER JOIN pdeinterativo.pdinterativo 	   pde ON pde.pdeid = pf.pdeid
...
function carregarOrdemPrioridade(){
	...
	$sql = "SELECT DISTINCT
		...
		SELECT DISTINCT
					...
					WHERE 
						id_tipo_docente = 1
						AND pt.pdeid = {$_SESSION['pdeinterativo_vars']['pdeid']}
					ORDER BY 
						2";
...
function manterCursoSugerido( $dados ){
	...
	if( empty($focid) ){
		$sql = "INSERT INTO pdeinterativo.formacaocursosugerido(foccurso, focdescricao, focjustificativa, focqtdprof, focformcontinuada, pdeid) 
				VALUES ( {$foccurso}, {$focdescricao}, {$focjustificativa}, {$focqtdprof}, {$focformcontinuada}, {$_SESSION['pdeinterativo_vars']['pdeid']} ) returning focid";
		
		$focid = $db->pegaUm( $sql );
		
		$sql = "DELETE FROM pdeinterativo.cursosugeridofuncao 	  WHERE focid = $focid;
				DELETE FROM pdeinterativo.cursosugeridoetapa  	  WHERE focid = $focid;
				DELETE FROM pdeinterativo.cursosugeridodisciplina WHERE focid = $focid;
				DELETE FROM pdeinterativo.cursosugeridomodalidade WHERE focid = $focid;";
		$db->executar( $sql );
		$db->commit();
		
		if( is_array( $fexid ) && !empty($fexid[0]) ){
			foreach ($fexid as $v) {			
				$sql = "INSERT INTO pdeinterativo.cursosugeridofuncao(focid, fexid) 
						VALUES ($focid, $v)";
				$db->executar( $sql );
				$db->commit();
			}
		...
		if( is_array( $pk_cod_etapa_ensino ) && !empty($pk_cod_etapa_ensino[0]) ){
			foreach ($pk_cod_etapa_ensino as $ensino) {			
				$sql = "INSERT INTO pdeinterativo.cursosugeridoetapa(focid, pk_cod_etapa_ensino) 
						VALUES ($focid, $ensino)";
				$db->executar( $sql );
				$db->commit();
			}
		...
		if( is_array( $pk_cod_disciplina ) && !empty($pk_cod_disciplina[0]) ){
			foreach ($pk_cod_disciplina as $disciplina) {			
				$sql = "INSERT INTO pdeinterativo.cursosugeridodisciplina(focid, pk_cod_disciplina) 
		...
		if( is_array( $pk_cod_mod_ensino ) && !empty($pk_cod_mod_ensino[0]) ){
			foreach ($pk_cod_mod_ensino as $mod) {			
				$sql = "INSERT INTO pdeinterativo.cursosugeridomodalidade(focid, pk_cod_mod_ensino) 
						VALUES ($focid, $mod)";
				$db->executar( $sql );
				$db->commit();
			}
		...
		$sql = "UPDATE pdeinterativo.formacaocursosugerido SET 
					foccurso 			= {$foccurso},
...
function carregaCursoSugerido(){
	...
	$sql = "SELECT focid, pdeid, focformcontinuada, foccurso, focdescricao, focjustificativa, focqtdprof
			FROM pdeinterativo.formacaocursosugerido WHERE pdeid = {$_SESSION['pdeinterativo_vars']['pdeid']}";
...
function desvincularCurso( $request ){
	...
	$db->commit();
	echo "<script>
			window.location = 'pdeinterativo.php?modulo=principal/planoestrategico&acao=A&aba=formacao_0_planoformacao&aba1=formacao_0_planoformacao&vv=".date("Ymdhis")."';
		  </script>";
}
	...
	if( $existe == 'true' ){
		...
		$sql = "SELECT DISTINCT
				...
				WHERE
					...
					AND pfdstatus = 'A'
					AND pfdcpf NOT IN (SELECT pfdcpf FROM pdeinterativo.planoformacaodocente WHERE curid IS NOT NULL)";
//ver($sql);
...
function tramitaFormacao($dados){
	...
	echo "<script>
			...
			window.location = 'pdeinterativo.php?modulo=principal/planoestrategico&acao=A&aba=formacao_0_planoformacao';
		  </script>";
...
function enviarEmail( $pdeid ){
	...
	$sql = "SELECT DISTINCT
			...
			WHERE 
				...
				AND pf.pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid'];
...
function recuperaModalidadeCurso( $curid ){
	...
	$sql = "SELECT DISTINCT
			...
			INNER JOIN pdeinterativo.planoformacaodocente  pfd ON pfd.modid = mod.modid
			WHERE 
				cur.curid = ".$curid."
				AND pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid'];
	return $db->carregarColuna($sql);
...
function gravarDemandaSocial( $request ){
	...
	$sql = "UPDATE catalogocurso.membrosdemandasocial SET cpdstatus = 'I' WHERE pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid'].";";
	if( is_array($request['mdsnome']) ){
		...
		if( $request['proximo'] == 'proximo' ){
			echo "<script>
					...
					window.location = 'pdeinterativo.php?modulo=principal/planoestrategico&acao=A&aba=formacao_0_planoformacao&aba1=formacao_1_cursosugerido';
				  </script>";
		}else{
			echo "<script>
					alert('Dados gravados com sucesso.');
					window.location = 'pdeinterativo.php?modulo=principal/planoestrategico&acao=A&aba=formacao_0_planoformacao&aba1=formacao_3_demanda_social';
				  </script>";
		}
	}
	echo "<script>
...
function pegaPermissao(){
	...
	$sql = "SELECT
			...
			WHERE
				pde.pdeid = ".$_SESSION['pdeinterativo_vars']['pdeid'];
