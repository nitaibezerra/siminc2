...
function excluirPessoa($dados) {
	...
	if( $dados['tpeid'] ){
		$sql = "DELETE FROM pdeinterativo.pessoatipoperfil WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND pesid=".$dados['pesid']." AND tpeid = ".$dados['tpeid'];
		$db->executar($sql);
		$sql = "SELECT count(tpeid) FROM pdeinterativo.pessoatipoperfil WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND pesid=".$dados['pesid'];
		$qtdFuncao = $db->pegaUm($sql);
	}
	if( $qtdFuncao < 1 ){
		$sql = "DELETE FROM pdeinterativo.detalhepessoa WHERE pesid='".$dados['pesid']."'";
		$db->executar($sql);
		$sql = "DELETE FROM pdeinterativo.direcao WHERE pesid='".$dados['pesid']."'";
		$db->executar($sql);
		$sql = "UPDATE pdeinterativo.pessoa SET pesstatus='I' WHERE pesid='".$dados['pesid']."'";
		$db->executar($sql);
	}
...
function atualizarPessoa($dados) {
	...
	$arrPessoa = $db->pegaLinha("SELECT pesid FROM pdeinterativo.pessoa WHERE usucpf='".str_replace(array(".","-"),array("",""),$dados['usucpf'])."'");
	...
	$sql = "UPDATE pdeinterativo.pessoa SET
	...
	if($dados['tpeid']) $peaid_n = $db->pegaUm("select peaid from pdeinterativo.perfilarea where tpeid='".$dados['tpeid']."'");
	if($peaid_n) $ptpid = $db->pegaUm("SELECT ptpid FROM pdeinterativo.pessoatipoperfil p INNER JOIN pdeinterativo.perfilarea a ON a.tpeid = p.tpeid WHERE pesid='".$dados['pesid']."' AND peaid='".$peaid_n."'");
	
	if($dados['tpeid']) {
		
		if($ptpid) {
			$sql = "UPDATE pdeinterativo.pessoatipoperfil SET tpeid='".$dados['tpeid']."' WHERE ptpid='{$ptpid}';";
			$db->executar($sql);
		} else {
			$sql = "INSERT INTO pdeinterativo.pessoatipoperfil(tpeid, pesid, pdeid)
	    			VALUES ('".$dados['tpeid']."', '".$dados['pesid']."', '".$_SESSION['pdeinterativo_vars']['pdeid']."');";
	...
	$dpeid = $db->pegaUm("SELECT dpeid FROM pdeinterativo.detalhepessoa WHERE pesid='".$dados['pesid']."'");
	if($dpeid) {
		$sql = "UPDATE pdeinterativo.detalhepessoa SET
		...
		$sql = "INSERT INTO pdeinterativo.detalhepessoa(
			            pesid, 
			            fgtid, 
	...
	$dirid = $db->pegaUm("SELECT dirid FROM pdeinterativo.direcao WHERE pesid='".$dados['pesid']."'");
	
	if($dirid) {
		$sql = "UPDATE pdeinterativo.direcao SET
		...
		if($dados['dirqtdanoexerce']   || 
		   ...
		   $dados['dircursodistan']) {
			...
			$sql = "INSERT INTO pdeinterativo.direcao(
				            pesid, 
...
function inserirPessoa($dados) {
	...
	$pesid = $db->pegaUm("SELECT pesid FROM pdeinterativo.pessoa WHERE usucpf='".trim(str_replace(array(".","-"," ","/"),array("","","",""),$dados['usucpf']))."'");
	
	if(!$pesid) {
		
		$sql = "INSERT INTO pdeinterativo.pessoa(
		...
		if($pesid && $dados['tpeid']) {
			...
			$sql = "INSERT INTO pdeinterativo.pessoatipoperfil(
            		tpeid, pesid, pdeid)
    				VALUES ('".$dados['tpeid']."', '".$pesid."', '".$_SESSION['pdeinterativo_vars']['pdeid']."');";
...
function verificaDadosDirecao()
{
	...
	$flag = $db->pegaLinha("SELECT naoexistevicediretor, naoexistesecretario FROM pdeinterativo.flag WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
	
	if($flag['naoexistevicediretor']!="t") {
		
		$c_vice = $db->pegaUm("SELECT count(p.pesid) FROM pdeinterativo.pessoa p 
								INNER JOIN pdeinterativo.pessoatipoperfil ptp ON ptp.pesid = p.pesid 
								WHERE ptp.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND ptp.tpeid='".TPE_VICEDIRETOR."'");
	...
	if($flag['naoexistesecretario']!="t") {
		...
		$c_secr = $db->pegaUm("SELECT count(p.pesid) FROM pdeinterativo.pessoa p 
								INNER JOIN pdeinterativo.pessoatipoperfil ptp ON ptp.pesid = p.pesid 
								WHERE ptp.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND ptp.tpeid='".TPE_SECRETARIO."'");
...
?>
	<table class="tabela" bgcolor="#f5f5f5" cellSpacing="1" cellPadding="3"	align="center" id="tbl_direcao">
		...
		$sql = "SELECT pe.pesid, tp.tpedesc, usu.usunome, pe.usucpf, dp.dpetelefone, dp.dpeemail, te.tendesc, di.dirqtdanoexerce||' ano(s) '||di.dirqtdmesesexerce||' mes(es)' as tempo_escola,
				...
				FROM pdeinterativo.pessoa pe
				INNER JOIN seguranca.usuario usu ON usu.usucpf = pe.usucpf 
				LEFT JOIN pdeinterativo.pessoatipoperfil ptp ON ptp.pesid = pe.pesid
				...
				WHERE pd.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND pe.pflcod = ".PDEESC_PERFIL_DIRETOR." AND pe.pesstatus='A' ORDER BY pe.pesnome";
		...
		$sql = "SELECT pe.pesid, tp.tpeid, tp.tpedesc, pe.pesnome, pe.usucpf, dp.dpetelefone, dp.dpeemail, te.tendesc, di.dirqtdanoexerce||' ano(s) '||di.dirqtdmesesexerce||' mes(es)' as tempo_escola,
				...
				CASE WHEN dircursodistan='S' THEN 'Sim'
					 ...
					 END as dircursodistan
				FROM pdeinterativo.pessoa pe 
				INNER JOIN pdeinterativo.pessoatipoperfil pt ON pt.pesid = pe.pesid 
				INNER JOIN pdeinterativo.detalhepessoa dp ON dp.pesid = pe.pesid 
				INNER JOIN pdeinterativo.pdinterativo pd ON pd.pdeid = pt.pdeid 
				INNER JOIN pdeinterativo.tipoperfil tp ON tp.tpeid = pt.tpeid 
				INNER JOIN pdeinterativo.tipoescolaridade te ON te.tenid = dp.tenid 
				INNER JOIN pdeinterativo.direcao di ON di.pesid = pe.pesid
				WHERE pd.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND tp.tpeid='".TPE_VICEDIRETOR."'";
		...
		$sql = "SELECT pe.pesid, tp.tpeid, tp.tpedesc, pe.pesnome, pe.usucpf, dp.dpetelefone, dp.dpeemail, te.tendesc, COALESCE(di.dirqtdanoexerce,0)||' ano(s) '||COALESCE(di.dirqtdmesesexerce,0)||' mes(es)' as tempo_escola,
				...
				INNER JOIN pdeinterativo.tipoescolaridade te ON te.tenid = dp.tenid 
				INNER JOIN pdeinterativo.direcao di ON di.pesid = pe.pesid
				WHERE pd.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND tp.tpeid='".TPE_SECRETARIO."'";
		...
		$sql = "SELECT pe.pesid, tp.tpeid, tp.tpedesc, pe.pesnome, pe.usucpf, dp.dpetelefone, dp.dpeemail, te.tendesc, COALESCE(di.dirqtdanoexerce,0)||' ano(s) '||COALESCE(di.dirqtdmesesexerce,0)||' mes(es)' as tempo_escola,
				...
				INNER JOIN pdeinterativo.tipoescolaridade te ON te.tenid = dp.tenid 
				INNER JOIN pdeinterativo.direcao di ON di.pesid = pe.pesid
				WHERE pd.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND tp.tpeid IN(SELECT tpeid FROM pdeinterativo.perfilarea WHERE apeid='".APE_EQUIPEPEDAGOGICA."')";
...
function naoExisteViceDiretor()
{
	...
	$sql = "SELECT pe.pesid, tp.tpedesc, pe.pesnome, pe.usucpf, dp.dpetelefone, dp.dpeemail, te.tendesc, di.dirqtdanoexerce||' ano(s) '||di.dirqtdmesesexerce||' mes(es)' as tempo_escola,
				...
				CASE WHEN dircursodistan='S' THEN 'Sim'
					 ...
					 END as dircursodistan
				FROM pdeinterativo.pessoa pe 
				INNER JOIN pdeinterativo.pessoatipoperfil pt ON pe.pesid = pt.pesid
				INNER JOIN pdeinterativo.detalhepessoa dp ON dp.pesid = pe.pesid 
				INNER JOIN pdeinterativo.pdinterativo pd ON pd.pdeid = pt.pdeid 
				INNER JOIN pdeinterativo.tipoperfil tp ON tp.tpeid = pt.tpeid 
				INNER JOIN pdeinterativo.tipoescolaridade te ON te.tenid = dp.tenid 
	...
	if($existe){
		$sql = "update pdeinterativo.flag set naoexistevicediretor = true where pdeid = {$_SESSION['pdeinterativo_vars']['pdeid']}";
	}else{
		$sql = "insert into pdeinterativo.flag (naoexistevicediretor,pdeid) values (true,{$_SESSION['pdeinterativo_vars']['pdeid']});";
	}
	$db->executar($sql);
	$db->commit();
...
function naoExisteEquipe()
{
	...
	if($existe){
		$sql = "update pdeinterativo.flag set naoexisteequipe = true where pdeid = {$_SESSION['pdeinterativo_vars']['pdeid']}";
	}else{
		$sql = "insert into pdeinterativo.flag (naoexisteequipe,pdeid) values (true,{$_SESSION['pdeinterativo_vars']['pdeid']});";
	}
	$db->executar($sql);
	$db->commit();
...
function naoExisteSecretario()
{
	...
	if($existe){
		$sql = "update pdeinterativo.flag set naoexistesecretario = true where pdeid = {$_SESSION['pdeinterativo_vars']['pdeid']}";
	}else{
		$sql = "insert into pdeinterativo.flag (naoexistesecretario,pdeid) values (true,{$_SESSION['pdeinterativo_vars']['pdeid']});";
	}
	$db->executar($sql);
	$db->commit();
...
function gerenciarProgramasFinancas($dados) {
	...
	if($dados['fprid']) {
		$programasfinancas = $db->pegaLinha("SELECT * FROM pdeinterativo.respostafinancaprograma WHERE fprid='".$dados['fprid']."'");
...
function pegarTdProgramaFinanca($dados) {
	...
	switch($dados['fprfonte']) {
		case 'F':
			$sql = "SELECT proid as codigo, prodesc as descricao FROM pdeinterativo.programa WHERE prostatus='A' and prorepassarec = 'S'";
...
function excluirProgramasFinancas($dados) {
	...
	$sql = "UPDATE pdeinterativo.respostafinancaprograma SET fprstatus='I' WHERE fprid='".$dados['fprid']."'";
	$db->executar($sql);
	$db->commit();
	
	$sql = "SELECT COUNT(*) as num FROM pdeinterativo.respostafinancaprograma WHERE fprid='".$dados['fprid']."' AND fprstatus='A'";
	$num = $db->pegaUm($sql);
...
function atualizarProgramasFinancas($dados) {
	...
	$sql = "UPDATE pdeinterativo.respostafinancaprograma
...
function inserirProgramasFinancas($dados) {
	...
	$sql = "INSERT INTO pdeinterativo.respostafinancaprograma(
...
function carregarProgramasFinancas($dados) {
	...
	$sql = "SELECT '<center><img src=../imagens/alterar.gif onclick=\"gerenciarProgramasFinancas(\''||rfp.fprid||'\');\" style=cursor:pointer;> <img src=../imagens/excluir.gif onclick=\"excluirProgramasFinancas(\''||rfp.fprid||'\');\" style=cursor:pointer;></center>' as acoes, 
				   ...
				   fprvalorrep									   
			FROM pdeinterativo.respostafinancaprograma rfp
			LEFT JOIN pdeinterativo.programa pro ON pro.proid = rfp.proid  
			WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND fprstatus='A'";
...
function pegarTotalProgramasFinancas($pdeid) {
	global $db;
	$total = $db->pegaUm("SELECT SUM(fprvalorrep) as fprvalorrep FROM pdeinterativo.respostafinancaprograma 
...
function carregarResumoProgramasFinancas($dados) {
	global $db;
	$total = pegarTotalProgramasFinancas($_SESSION['pdeinterativo_vars']['pdeid']);
	...
	$sql = "SELECT CASE WHEN fprfonte='E' THEN 'Estadual'
			...
			FROM pdeinterativo.respostafinancaprograma 
			WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND fprstatus='A'
			GROUP BY fprfonte";
	...
	switch($dados['tipo']) {
		case 'usos':
			$sql = "SELECT SUM(fuscapital) as capital, SUM(fuscusteio) as custeio 
					FROM pdeinterativo.respostafinancauso 
					WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND fusstatus='A'";
...
function carregarDiferencaFontesUsos($dados) {
	...
	$sql = "SELECT COALESCE(SUM(fustotal),0.00) as total  
			FROM pdeinterativo.financacategoria fc 
			INNER JOIN pdeinterativo.respostafinancauso rf ON rf.fcaid = fc.fcaid AND rf.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' 
			WHERE fc.fcastatus='A'";
	
	$total = $db->pegaUm($sql);
	
	$totalprogramas = pegarTotalProgramasFinancas($_SESSION['pdeinterativo_vars']['pdeid']);
...
function carregarUsosFinancas($dados) {
	...
	$sql = "SELECT fc.fcadesc, '<input onblur=\"gerenciarUsos();\" value=\"'|| COALESCE(trim(to_char(rf.fuscusteio,'999g999g999d99')),'0,00') ||'\" '|| CASE WHEN fcabloqueado='C' THEN 'disabled class=disabled' ELSE 'class=normal' END ||' name=fuscusteio['||fc.fcaid||'] type=text style=text-align:right; size=18 onKeyUp=\"this.value=mascaraglobal(\'###.###.###,##\',this.value);\">' as custeio, '<input onblur=\"gerenciarUsos();\" value=\"'|| COALESCE(trim(to_char(rf.fuscapital,'999g999g999d99')),'0,00') ||'\" '|| CASE WHEN fcabloqueado='P' THEN 'disabled class=disabled' ELSE 'class=normal' END ||' name=fuscapital['||fc.fcaid||'] type=text class=normal style=text-align:right; size=18 onKeyUp=\"this.value=mascaraglobal(\'###.###.###,##\',this.value);\">' as capital,
			...
			FROM pdeinterativo.financacategoria fc 
			LEFT JOIN pdeinterativo.respostafinancauso rf ON rf.fcaid = fc.fcaid AND rf.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' 
...
function gerenciarUsos($dados) {
	...
	if($dados['fuscusteio']) {
		foreach($dados['fuscusteio'] as $fcaid => $custeio) {
			if($custeio) {
				$sql = "SELECT fusid FROM pdeinterativo.respostafinancauso WHERE fcaid='".$fcaid."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'";
				$fusid = $db->pegaUm($sql);
				if($fusid) $sql = "UPDATE pdeinterativo.respostafinancauso SET fuscusteio=".((str_replace(array(".",","),array("","."),$custeio) > 0 && is_numeric(str_replace(array(".",","),array("","."),$custeio)))?"'".str_replace(array(".",","),array("","."),$custeio)."'":"NULL")." WHERE fusid='".$fusid."';";
				else $sql = "INSERT INTO pdeinterativo.respostafinancauso(pdeid, fuscusteio, fusstatus, fcaid) VALUES ('".$_SESSION['pdeinterativo_vars']['pdeid']."', ".((str_replace(array(".",","),array("","."),$custeio) > 0 && is_numeric(str_replace(array(".",","),array("","."),$custeio)))?"'".str_replace(array(".",","),array("","."),$custeio)."'":"NULL").", 'A', '".$fcaid."');";
				$db->executar($sql);
				$db->commit();
			}
		}
	...
	if($dados['fuscapital']) {
		foreach($dados['fuscapital'] as $fcaid => $capital) {
			if($capital) {
				$sql = "SELECT fusid FROM pdeinterativo.respostafinancauso WHERE fcaid='".$fcaid."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'";
				$fusid = $db->pegaUm($sql);
				if($fusid) $sql = "UPDATE pdeinterativo.respostafinancauso SET fuscapital=".((str_replace(array(".",","),array("","."),$capital) > 0 && is_numeric(str_replace(array(".",","),array("","."),$capital)))?"'".str_replace(array(".",","),array("","."),$capital)."'":"NULL")." WHERE fusid='".$fusid."';";
				else $sql = "INSERT INTO pdeinterativo.respostafinancauso(pdeid, fuscapital, fusstatus, fcaid) VALUES ('".$_SESSION['pdeinterativo_vars']['pdeid']."', ".((str_replace(array(".",","),array("","."),$capital) > 0 && is_numeric(str_replace(array(".",","),array("","."),$capital)))?"'".str_replace(array(".",","),array("","."),$capital)."'":"NULL").", 'A', '".$fcaid."');";
				$db->executar($sql);
				$db->commit();
			}
		}
	...
	$db->executar("UPDATE pdeinterativo.respostafinancauso SET fustotal=(coalesce(fuscapital,0.00)+coalesce(fuscusteio,0.00)) WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
	$db->commit();
...
function montarGraficoBarraFinancas($dados) {
	...
	$sql = "SELECT fustotal, SUBSTR(fcadesc,1,10) as fcadesc 
			FROM pdeinterativo.respostafinancauso u
			INNER JOIN pdeinterativo.financacategoria c ON c.fcaid = u.fcaid 
			WHERE u.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND u.fusstatus='A' 
			ORDER BY fcadesc";
...
function diagnostico_4_3_financas($dados) {
	...
	if($recebeurecursosanoanterior=="t") {
		...
		$fprid = $db->pegaUm("SELECT fprid FROM pdeinterativo.respostafinancaprograma WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND fprstatus='A'");
		$fusid = $db->pegaUm("SELECT fusid FROM pdeinterativo.respostafinancauso WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND fustotal>0");
...
function diagnostico_4_4_sintesedimensao4($dados) {
	global $db;
	if($dados['critico']) {
	...
	if($dados['pessoas']) {
		foreach($dados['pessoas'] as $indice => $valor) {
			...
			foreach($pesids as $pesid) {
				$sql = "UPDATE pdeinterativo.pessoa SET critico=".$valor." WHERE pesid='".$pesid."'";
				$db->executar($sql);
				$db->commit();
			}
		}
...
function gravarRecebimentoRecursosAnoAnterior($dados) {
	...
	if($flaid) {
		$db->executar("UPDATE pdeinterativo.flag SET recebeurecursosanoanterior=".(($dados['recebeurecursosanoanterior'])?$dados['recebeurecursosanoanterior']:"NULL")." WHERE flaid='".$flaid."'");
	} else {
		$db->executar("INSERT INTO pdeinterativo.flag(pdeid, recebeurecursosanoanterior)
    				   VALUES ('".$_SESSION['pdeinterativo_vars']['pdeid']."', ".(($dados['recebeurecursosanoanterior'])?$dados['recebeurecursosanoanterior']:"NULL").")");
	}
	$db->commit();
	
	if($dados['recebeurecursosanoanterior']=="FALSE") {
		$db->executar("UPDATE pdeinterativo.respostafinancaprograma SET fprstatus='I' WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
	}
	$db->commit();
