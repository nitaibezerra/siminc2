 ...
 */
function diagnostico_1_4_sintesedimensao1($dados) {
	global $db;
	
	if($dados['respostaideb']) {
		foreach($dados['respostaideb'] as $campo => $ideb) {
			$db->executar("UPDATE pdeinterativo.respostaideb SET ".$campo."=".$ideb." WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
			$db->commit();
		}
	}
	...
	if($dados['respostataxarendimento']) {
		foreach($dados['respostataxarendimento'] as $campo => $tx) {
			$db->executar("UPDATE pdeinterativo.respostataxarendimento SET ".$campo."=".$tx." WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
			$db->commit();
		}
	}
	...
	if($dados['respostaprovabrasil']) {
		foreach($dados['respostaprovabrasil'] as $campo => $pb) {
			$db->executar("UPDATE pdeinterativo.respostaprovabrasil SET ".$campo."=".$pb." WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
			$db->commit();
		}
	}
...
function existePrograma($dados) {
	global $db;
	$sql = "SELECT rppresposta FROM pdeinterativo.respostaprojetoprograma 
			WHERE rppmodulo='".$dados['rppmodulo']."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND rppstatus='A' AND rpptipo='G' LIMIT 1";
...
function existeProjeto($dados) {
	...
	$sql = "SELECT rppresposta FROM pdeinterativo.respostaprojetoprograma 
			WHERE rppmodulo='".$dados['rppmodulo']."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND rppstatus='A' AND rpptipo='J' LIMIT 1";
 ...
 * @author Alexandre Dourado
 * @return javascriptcode
 * @param $dados => array contendo todas as informações referentes a sintese do programa (pdeinterativo.sinteseprograma)
 * @global $db classe que instância o banco de dados 
 * @version v1.0 20/07/2011
 */
function atualizarPrograma($dados) {
	global $db;
	$sql = "UPDATE pdeinterativo.sinteseprograma
...
function inserirPrograma($dados) {
	...
	$sql = "INSERT INTO pdeinterativo.sinteseprograma(
            pdeid, spostatus, sprmodulo, spodesc, spoobjepro, sposituacao, 
            spoorgao, sposite, proid)
		    VALUES ('".$_SESSION['pdeinterativo_vars']['pdeid']."', 'A', '".$dados['sprmodulo']."', ".(($dados['spodesc'])?"'".$dados['spodesc']."'":"NULL").", ".(($dados['spoobjepro'])?"'".$dados['spoobjepro']."'":"NULL").", ".(($dados['sposituacao'])?"'".$dados['sposituacao']."'":"NULL").", 
...
function excluirPrograma($dados) {
	global $db;
	$sql = "UPDATE pdeinterativo.sinteseprograma SET spostatus='I' WHERE spoid='".$dados['spoid']."'";
	$db->executar($sql);
	$db->commit();
...
function excluirProjeto($dados) {
	...
	$sql = "UPDATE pdeinterativo.sinteseprojeto SET sprstatus='I' WHERE sprid='".$dados['sprid']."'";
	$db->executar($sql);
	$db->commit();
...
function atualizarProjeto($dados) {
	global $db;
	$sql = "UPDATE pdeinterativo.sinteseprojeto
...
function inserirProjeto($dados) {
	...
	$sql = "INSERT INTO pdeinterativo.sinteseprojeto(
            pdeid, sprmodulo, sprdesc, sprobjetivo, sprstatus)
    		VALUES ('".$_SESSION['pdeinterativo_vars']['pdeid']."', '".$dados['sprmodulo']."', '".$dados['sprdesc']."', '".$dados['sprobjetivo']."', 'A');";
...
function gerenciarProgramas($dados) {
	...
	echo "<script language=\"JavaScript\" src=\"./js/pdeinterativo.js\"></script>";
	...
	$programas = $db->carregar("SELECT * 
				   				FROM pdeinterativo.programa WHERE prostatus='A' order by prodesc");
...
";
	...
	if($dados['spoid']) {
		...
		$dadosprograma = $db->pegaLinha("SELECT * FROM pdeinterativo.sinteseprograma p
										 LEFT JOIN pdeinterativo.programa po ON po.proid = p.proid 
...
function gravarRespostaProgramasProjetos($dados) {
	...
	if($dados['tipo']=="G") {
		...
		$sql = "UPDATE pdeinterativo.sinteseprograma
   				SET spostatus='I'
 				WHERE sprmodulo='".$dados['smodulo']."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."';";
		$db->executar($sql);
	...
	$rppid = $db->pegaUm("SELECT rppid FROM pdeinterativo.respostaprojetoprograma WHERE rppmodulo='".$dados['smodulo']."' AND rpptipo='".$dados['tipo']."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'");
	
	if($rppid) {
		
		$sql = "UPDATE pdeinterativo.respostaprojetoprograma
   				SET rppresposta=".$dados['resposta']." 
 				WHERE rppid='".$rppid."';";
		
	} else {
		
		$sql = "INSERT INTO pdeinterativo.respostaprojetoprograma(
	            rppresposta, rppmodulo, pdeid, rpptipo, rppstatus)
	    		VALUES (".$dados['resposta'].", '".$dados['smodulo']."', '".$_SESSION['pdeinterativo_vars']['pdeid']."', '".$dados['tipo']."', 'A');";
...
function gerenciarProjetos($dados) {
	...
	if($dados['sprid']) {
		...
		$dadosprojeto = $db->pegaLinha("SELECT * FROM pdeinterativo.sinteseprojeto WHERE sprid='".$dados['sprid']."'");
...
function carregarProjetos($dados) {
	...
	$sql = "SELECT '<center><img src=../imagens/alterar.gif style=cursor:pointer; onclick=\"gerenciarProjetos(\'".$dados['sprmodulo']."\', \'' || sprid || '\');\"> <img src=../imagens/excluir.gif style=cursor:pointer; onclick=\"excluirProjeto(\'".$dados['sprmodulo']."\', \'' || sprid || '\');\"></center>' as acoes, sprdesc, sprobjetivo 
			FROM pdeinterativo.sinteseprojeto 
			WHERE sprmodulo='".$dados['sprmodulo']."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND sprstatus='A'";
...
function carregarProgramas($dados) {
	...
	$sql = "SELECT '<center><img src=../imagens/alterar.gif style=cursor:pointer; onclick=\"gerenciarProgramas(\'".$dados['sprmodulo']."\', \'' || spoid || '\');\"> <img src=../imagens/excluir.gif style=cursor:pointer; onclick=\"excluirPrograma(\'".$dados['sprmodulo']."\', \'' || spoid || '\');\"></center>' as acoes, 
				   ...
				   sposite
	FROM pdeinterativo.sinteseprograma p 
	LEFT JOIN pdeinterativo.programa po ON po.proid = p.proid 
	WHERE p.sprmodulo='".$dados['sprmodulo']."' AND pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND spostatus='A'";
...
function montaGraficoTaxasRendimento($dados) {
	...
	$filtros_por_esfera = array("B" => "",
								"E" => "es.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND it.intcoduf IS NOT NULL AND",
								"M" => "mu.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND it.intcodmun IS NOT NULL AND mu.estuf <> 'DF' AND",
								"S" => "it2.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND");
	
	
	foreach($arrEsf as $codesf => $esfera) {
		
		$dadosescensino = $db->carregar("SELECT it.* FROM pdeinterativo.indicadorestaxas it 
										 LEFT JOIN pdeinterativo.pdinterativo it2 ON it2.pdicodinep::numeric = it.intinep and it2.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."'
										 LEFT JOIN pdeinterativo.pdinterativo es ON es.estuf = it.intcoduf::character(2) and es.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND it.intcoduf IS NOT NULL
										 LEFT JOIN pdeinterativo.pdinterativo mu ON mu.muncod = it.intcodmun::character(7) and mu.pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND it.intcodmun IS NOT NULL
	...
	$graph->title->Set($arrEns[$dados['ensino']]."(".$arrTipo[$dados['tipo']].")");
...
function montaTabelaTaxasRendimento($ensino, $tipo) {
	...
	$arrEsf = array("B" => "Brasil",
					"E" => "Estado",
					"M" => "Município",
					"S" => "Escola (".$db->pegaUm("SELECT entnome FROM entidade.entidade WHERe entcodent='".$_SESSION['pdeinterativo_vars']['pdicodinep']."'").")");
	...
	$html .= "<tr>
				...
				<td class=SubTituloCentro colspan=".count($anosTX).">".$arrTipo[$tipo]."</td>
			  </tr>";
	...
	foreach($arrEsf as $codesfera => $esfera) {
		...
		foreach($anosTX as $anotx) {
			...
			$html .= "<td ".($codesfera == "S" ? "style=\"color:#006400;font-weight:bold\"" : "")." align=center>".(($arrEscEns[$codesfera][$anotx])?$arrEscEns[$codesfera][$anotx]:"0")."</td>";
		}
...
function montaTabelaProvaBrasil($ensino, $materia) {
	...
	foreach($arrEsf as $codesfera => $esfera) {
		...
		foreach($anosPB as $anopb) {
			...
			$html .= "<td ".($codesfera == "S" ? "style=\"color:#006400;font-weight:bold\"" : "")." align=center>".(($arrEscEns[$codesfera][$anopb])?$arrEscEns[$codesfera][$anopb]:"0")."</td>";
		}
...
function montaGraficoIDEB($dados) {
	...
	$anosIDEB = array("2005","2007","2009"); 
	$anosIDEBMeta = array("2013");
...
function diagnostico_1_3_provabrasil($dados) {
	global $db;
	$sql = "SELECT rpbid FROM pdeinterativo.respostaprovabrasil WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND rbdstatus='A'";
	$rpbid = $db->pegaUm($sql);
	
	if($rpbid) {
		
		$sql = "UPDATE pdeinterativo.respostaprovabrasil
		...
		$sql = "INSERT INTO pdeinterativo.respostaprovabrasil(
			            pdeid, 
			            rpbinicialport, 
			            rpbinicialmat, 
			            rpbfinalport, 
...
function diagnostico_1_1_ideb($dados) {
	...
	$sql = "SELECT ridid FROM pdeinterativo.respostaideb WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND ridsstatus='A'";
	$ridid = $db->pegaUm($sql);
	
	if($ridid) {
		$sql = "UPDATE pdeinterativo.respostaideb
		...
		$sql = "INSERT INTO pdeinterativo.respostaideb(
	            ridinicialum, 
	            ridinicialdois, 
	            riddinicialdesc, 
	            ridfinalum, 
...
function diagnostico_1_2_taxasderendimento($dados) {
	...
	$sql = "SELECT rtrid FROM pdeinterativo.respostataxarendimento WHERE pdeid='".$_SESSION['pdeinterativo_vars']['pdeid']."' AND rtrstatus='A'";
	$rtrid = $db->pegaUm($sql);
	
	if($rtrid) {
		
		$sql = "UPDATE pdeinterativo.respostataxarendimento
		...
		$sql = "INSERT INTO pdeinterativo.respostataxarendimento(
			            pdeid, 
			            rtrfunaprova, 
			            rtrfunreprova, 
			            rtrfunabandono, 
...
function possuiEnsino($ensino, $submodulo) {
	global $db;
	$existe = $db->pegaUm("SELECT intid FROM pdeinterativo.indicadorestaxas WHERE intinep='".$_SESSION['pdeinterativo_vars']['pdicodinep']."' AND intensino='".$ensino."' AND intsubmodulo='".$submodulo."' LIMIT 1");
...
function montaTabelaIDEB($ensino) {
	...
	$arrEsf = array("B" => "IDEB Brasil",
					"E" => "IDEB Estado",
					"M" => "IDEB Município",
					"S" => "IDEB Escola (".$db->pegaUm("SELECT entnome FROM entidade.entidade WHERE entcodent='".$_SESSION['pdeinterativo_vars']['pdicodinep']."'").")");
	...
	foreach($arrEsf as $codesfera => $esfera) {
		...
		foreach($anosIDEB as $key => $anoideb) {
			if($codesfera == "S") $valores_escola[] = $arrEscEns[$codesfera][$anoideb];
				$html .= "<td ".($codesfera == "S" ? "style=\"color:#006400;font-weight:bold\" " : "")." align=center>".(($arrEscEns[$codesfera][$anoideb])?$arrEscEns[$codesfera][$anoideb]:"0")."</td>";
		}
		
		foreach($anosIDEBMeta as $anoidebmeta) {
			$html .= "<td style=\"color:#006400;font-weight:bold\" align=center>".(($arrEscEnsMeta[$codesfera][$anoidebmeta])?$arrEscEnsMeta[$codesfera][$anoidebmeta]:"0")."</td>";
		}
	...
	// verificando se os valores do IDEB estão melhorando ou piorando (aplicação da regra)
	if(count($valores_escola) == 0 || count($valores_escola) == 1 || (!$valores_escola[(count($valores_escola)-1)] || !$valores_escola[(count($valores_escola)-2)])){
