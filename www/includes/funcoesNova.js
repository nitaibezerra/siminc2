...
abreconteudo = function (url, idobj) {
	...
	{
		if (document.getElementById('td'+idobj).innerHTML=='') {
...
//			alert( 'vou chamar o corretor');
//			IeGetThisEvents( jsNode );
			if (jsCode != '') {
				try	{
					eval( jsCode );
				}
...
function IeGetThisEvents( objElement , arrDones )
{
	if( arrDones == undefined ) arrDones = new Array();
	
	if( array_search( objElement, arrDones ) != -1 )
	{
		return;
	}
	arrDones.push( objElement );
	if( objElement.getAttribute )
	{
		if( objElement.onmouseover || objElement.getAttribute( 'onmouseover' ) != null )	
		{
			strCommand = objElement.getAttribute( 'onmouseover' );
			strCommand += '';
			
			if( IE )
			{
				strCommand = strCommand.substring( strCommand.indexOf( '{' ) + 1  );
				strCommand = trimString( strCommand , '}' , false , true  );

				alert( strCommand );
				objElement.parentNode.onmouseover = new Function( "s", strCommand );
			}
			else
			{
				/*
//				objElement.parentNode.parentNode.onmouseover = objElement.onmouseover;
//				objElement.parentNode.parentNode.onmouseout = objElement.onmouseout;
				objElement.onmouseover = new Function( "s", strCommand );
				strId = setId( "titleled" , objElement );
				objElement.parentNode.onmouseover	= new Function( "s", 'document.getElementById("' + strId + '").onmouseover()' );
				objElement.parentNode.onmouseout	= new Function( "s", 'document.getElementById("' + strId + '").onmouseout()' );
				
				objElement.parentNode.parentNode.onmouseover	= new Function( "s", 'document.getElementById("' + strId + '").onmouseover()' );
				objElement.parentNode.parentNode.onmouseout		= new Function( "s", 'document.getElementById("' + strId + '").onmouseout()' );
				
//				objElement.attachEvent( "onmouseout" new Function( "s", "SuperTitleOff();" ) );
//				objElement.setAttribute( "onmouseover" , strCommand );
//				objElement.onmouseout	= new Function( "SuperTitleOff();" );
				*/
				
			}
		}
		if( objElement.onmouseout ||  objElement.getAttribute( 'onmouseout' ) != null )	
		{
			//alert( objElement.getAttribute( 'onmouseout' ) );
		}
		if( objElement.onclick ||  objElement.getAttribute( 'onclick' ) != null )	
		{
			//alert( objElement.getAttribute( 'onclick' ) );
		}
		var arrChilds = objElement.childNodes;
		for( var i = 0 ; i < arrChilds.length ; ++i )
		{
			var objChild = arrChilds[ i ];
			IeGetThisEvents( objChild , arrDones ); 
		}
	}
}
