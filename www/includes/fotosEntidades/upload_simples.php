...
for($i=0;$i<count($_FILES['Filedata']['name']);$i++) {
	...
	if($extensaopermitida) {
        if(!trataErroFile($_FILES['Filedata']['name'][$i],$_FILES['Filedata']['error'][$i])){
            continue;
        }
...
function trataErroFile($nomeArquivo, $erro)
{
    switch ($erro) {
        case 0:
            return true;
        case 1:
            echo "
                <script>
                    alert('O arquivo $nomeArquivo enviado excede o limite definido na aplicação.');
                </script>";
            return false;
        case 2:
            echo "
        ...
        case 3:
            echo "
                <script>
                    alert('Falha ao efetuar upload do arquivo $nomeArquivo.');
                </script>";
            return false;
        case 4:
            echo "
