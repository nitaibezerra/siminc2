...
/*
 * Classe   Calculo
 * O obj c�lculo possibilita opera��es aritm�ticas e l�gicas em n�meros no formato monet�rio (###.###,##).
 *
...
function Calculo(){
	this.operacao = function (num1, num2, operador, decimal){
		...
		alert('A fun��o "operacao" n�o contempla AINDA o operador aritm�tico escolhido.');
	}
...
//		num2 = this.converteMonetario(num2); //new Number( replaceAll( replaceAll( num2, ".", "" ), ",", ".") );
		...
		alert('A fun��o "comparar" n�o contempla AINDA o operador l�gico escolhido.');
...
/*
 * Classe   Data
 * O obj Data possibilita opera��es aritm�ticas (Adicionar dias) e l�gicas em datas no formato Brasileiro (DD/MM/YYYY).
 *
 * @author  Felipe Tarchiani Cer�volo Chiavicatti
 * @since   07/04/2010
 * @exemple /simec/obras/modulos/principal/popUpInserirAditivo.inc
...
function Data(){
	 ...
	 * O terceiro parametro indica o operador l�gico de compara��o, o padr�o ">", Ex. data1 > data2.
	 * retorna o boleano resultante da opera��o de compara��o
	*/
	this.comparaData = function (data1, data2, operador){
		...
		alert('A fun��o "comparaData" n�o contempla AINDA o operador l�gico escolhido.');
	}
	...
	preparaData = function (data){
		...
		// � necess�rio diminuir 1, por que a contagem dos meses vai de 0 a 11
	...
	montaData = function (data){
		...
		// � necess�rio aumentar 1, para a formata��o ficar correta, pois os meses v�o de 0 a 11, n�o de 1 a 12.
 ...
 * Cria e insere no n� de elementos uma DIV transparente com uma imagem de carregando
 * P.S.: Para esconder essa DIV use a fun��o "divCarregado", ap�s a execu��o de suas opera��es.
 *
 * @author  Felipe Tarchiani Cer�volo Chiavicatti
 * @access  public
 * @since   11/05/2010
 * @param   string|object id - Deve conter o id ou objeto onde se est� clicando a fim de montar a imagem "carregando"
 							   tomando como refer�ncia o elemento encontrado por esse paramentro (n�o obrigat�rio).

 						  texto - No parametro texto pode ser passado o texto que aparecer� em tela. N�o � obrigat�rio,
 						  		aparecendo o texto "Carregando..." caso n�o passado.
 * @return  void
 ...
 * Retira a DIV que nubla a tela criada pela fun��o "divCarregando"
 *
 * @author  Felipe Tarchiani Cer�volo Chiavicatti
 * @access  public
 * @since   11/05/2010
...
function pegaRetornoAjax(tagIni, tagFim, html, somenteSeEncontrar){
	...
	var tagIniTam = (tagIni ? tagIni.length : 0);
   	...
   	// Se "somenteSeEncontrar" = true e caso n�o encontre a "tagIni" e "tagFim" retorna false;
 ...
 *					4� parametro: boleano indica se fun��o ir� subimeter o formul�rio ou n�o.
 *				  OBS: O parametro tipoCampos pode receber: numero, cpf, valor, data, checkbox, select, funcao, radio, texto
 *					   Sendo que:   se o tipo for numero ir� validar um campo numerico.
 *					   				se o tipo for textarea ir� validar um campo de texto grande.
 *					   				se o tipo for texto ir� verificar se o campos est� prenchido.
 *									se o tipo for cpf ir� validar se o cpf e valido.
 *									se o tipo for valor ir� validar valores numericos.
 * 									se o tipo for data ir� validar um campo data.
 * 									se o tipo for checkbox pelo menos 1 checkbox ter� que estar marcado.
 *									se o tipo for radio pelo menos 1 radio ter� que estar marcado.
 * 									se o tipo for select ir� validar se o select est� selecionado.
 *									se o tipo for funcao deve-se passar da seguinte forma funcao:nomeFuncao(paremetros).
 						O tipo fun��o server para validar um campo com uma fun��o exclusiva criada pelo desenvolvedor.
 *
...
function validaForm(nomeFormulario, campos, tipoCampos, submeter){
			...
			if(formulario.elements[i].name == dados[cont]){
				...
				if(formulario.elements[i].type == "textarea"){
					if(tiposDados[cont] == "textarea"){
						if(!strValor){
							existeErro = true;
							erro[contErro] = "O campo "+formulario.elements[i].title+" � obrigat�rio.";
							contErro++;
						}
					}
					...
					if(tiposDados[cont] == "numero"){
						if(!strValor){
							...
							existeErro = true;
							erro[contErro] = "O campo "+formulario.elements[i].title+" s� aceita valores num�ricos.";
							contErro++;
						}
						...
						if(!strValor){
							...
							existeErro = true;
							erro[contErro] = "O CPF do campo "+formulario.elements[i].title+" n�o � valido.";
							contErro++;
						}
					...
					if(tiposDados[cont] == "data"){
						if(!strValor){
							...
							existeErro = true;
							erro[contErro] = "O campo "+formulario.elements[i].title+" n�o e uma data.";
							contErro++;
						}
					}
	...
	if(existeErro){
		if(erroCheckbox && checarCheckbox ){
			mostraErroCheckbox = "� obrigat�rio selecionar pelo menos um dado do campo : "+nomeCheckbox;
		}
		for(conta=1; conta<campoRadio.length; conta++){
			if(marcadoRadio[conta] == false){
				erroRadio[contRadioCampo] = "� obrigat�rio selecionar pelo menos um dado do campo : "+campoRadio[conta];
				contRadioCampo++;
			}
		}
 ...
 * Abre a janela de intera��o do select_popup. Instru��es no arquivo funcoes.inc
 * @param string nome
...
/**
 * Abre a janela de intera��o do combo_popup. Veja restante dos
 * coment�rio do arquivo www/geral/combopopup.php
 *
 * @param string nome
 * @param integer height
 * @param integer width
...
function combo_popup_recebe_buscar_codigo( input, nome )
{
	...
	if ( httpRequest.readyState == 4 )
	{
		if ( httpRequest.status == 200 )
		{
			...
			{
				alert( 'Nenhum registro encontrado com o c�digo indicado.' );
			}
			input.focus();
		}
	}
...
function combo_popup_buscar_codigo( nome, cod )
{
	...
	if ( maximo > 0 ) {
		...
		if ( quantidade > 1 || input.options[0].value != '' ) {
			if ( quantidade >= maximo ) {
				alert( 'A quantidade mpaxima de itens que podem ser selecionados � ' + maximo );
				return;
			}
		}
	}
	...
	for ( i = input.options.length - 1; i >= 0; i-- )
	{
		...
		{
			alert( 'O item j� est� presenta lista' );
			return;
		}
	}
	*/
...
/**
 * Abre a janela de intera��o do texto_popup. Veja restante dos
 * coment�rio do arquivo www/geral/textopopup.php
 *
 * @param string nome
 * @param integer height
 * @param integer width
...
{	//Funcionalidade:	Valida a Data retornando True se for uma Data
        //					v�lida e False se n�o for.
        //					Antes de se usar esta fun��o deve-se garantir que os par�metros
        //					passados sejam num�ricos e inteiros.
        // PAR�METROS:
...
function validaInteiro(parametro)  //FUNCAO PARA VALIDACAO DE N�MEROS INTEIROS, E ESPA�OS EM BRANCO
{
...
// Inicio fun��es para preencher conte�dos dinamicamente (sem refresh)
var xmlhttp
var idobjeto
...
func_onload = function (idobj) {
	...
	if (jsNode._xmlHttp.readyState == 4) {
		...
		{
			alert("Erro! N�o foi poss�vel Recuperar os Dados!");
...
// Fim fun��es para preencher conte�dos dinamicamente (sem refresh)
...
/* ----------------------------------------------------------------------------
---------------------------------------------------------------------------- */
// m�scara para valores monet�rios. Usar passando como paramentro uma string ele retorna
// um string com a formata��o padr�o de valores com duas casa decimais.
...
/**
  *	M�todo adicionado ao objeto Array do javascript para ordena��o de array de objetos
  *
  * @param string campo
  ...
  */
  ...
  /**
  * M�todo adicionado ao objeto Array do javascript para convers�o de um array de objetos em string
  * Se campo for vazio ent�o ele gerar� um array contendo todos os campos do objeto no registro
  * separador de campo padr�o '#'
  * separador de registro padr�o ','
  *
  * @param string campo
  ...
  Array.prototype.objectToString = function( campo, separadorReg, separadorCampo )
  {
	...
	for( var i = 0 ; i < this.length ; i++ )
	{
		...
		else
		{
			...
			string = string.substr( 0, string.length - 1 );//Eliminar o �ltimo separador de campo
		}
		string += sepReg;
	}
	string = string.substr( 0, string.length - 1 );//Elimina o �ltimo separador de registro
  	return string;
  }
 ...
 * @param Object Select onde ser�o carregados os munic�pios da UF selecionada
 * @return void
 */
function carregarMunicipiosPorUf(regcod, munContainer, endid)
{
    ...
    munContainer.options[0] = new Option("Selecione um munic�pio", "0", false, true);
...
function getEnderecoPeloCEP(el)
{
    ...
    ajax.onreadystatechange = function()
    {
        ...
        /**
         * @cat Burlar diferen�a nas tabelas do IBGE e DNE
         */
        if (DNE['sg_uf'] == 'DF') {
    ...
    for (var z = 0; z < fields.length; z++) {
        for (var i = 0; i < inputList.length; i++) {
            ...
            if (!/hidden/.test(inputList[i].getAttribute('type'))) {
                if (inputList[i].value == '' && (fields == ALL || fields[z] == inputList[i].getAttribute('name'))) {
                    erros.push('O campo ' + inputList[i].getAttribute('label') + ' � obrigat�rio!');
                }
            }
        }
        ...
        for (var i = 0; i < selectList.length; i++) {
            ...
            if (selectList[i].value == '' && (fields == ALL || fields[z] == selectList[i].getAttribute('name'))) {
                erros.push('O campo ' + selectList[i].getAttribute('label') + ' � obrigat�rio!');
            }
        }
    }
...
function validarCpfCnpj(e)
{
    if (e.value.length != 14 && e.value.length != 18) {
        alert('O CPF ou CNPJ informado n�o � v�lido!');
        return false;
    }
...
/*
*	Fun��o que marca todos os checkboxes.
*	idComponente: id do input checkbox usado para selecionar todos.
*	nomeChecks: nome dos checkboxes a serem marcados/desmarcados(geralmente � um nome de array. Ex: nome[]);
*/
...
/**
 * Função que envia uma requisições em ajax para a própria página
 * para carregar uma unidade or�ament�ria
 *
 * entid: ID do Orgão selecionado
 */
...
/**
 * Fun��o que evia uma requisi��o em ajax para a pr�pria p�gina
 * para carregar uma unidade org�o
 *
 * tpocod: ID do tipo de org�o selecionado
 */
...
function HoraValidationRoundMore(formName, fieldName, event){
	...
	if(!flag)
	{
		alert("Valor inv�lido! O formato do Hor�rio deve ser hh:mm");
	...
	if(hh > 23)
	{
		if(mm > 59)
		{
			alert("Valor inv�lido! O valor da Hora deve ser no m�ximo 23 e o valor do Minuto deve ser no m�ximo 59");
			//document.forms[formName].elements[fieldName].value = '';
			setTimeout("document.forms['" + formName + "'].elements['" + fieldName + "'].focus()",50);
			return;
		}
		...
		{
			alert("Valor inv�lido! O valor da Hora deve ser no m�ximo 23 ");
			//document.forms[formName].elements[fieldName].value = '';
			setTimeout("document.forms['" + formName + "'].elements['" + fieldName + "'].focus()",50);
			return;
		}
	...
	if(mm > 59)
	{
		alert("Valor inv�lido! O valor do Minuto deve ser no m�ximo 59");
		//document.forms[formName].elements[fieldName].value = '';
		setTimeout("document.forms['" + formName + "'].elements['" + fieldName + "'].focus()",50);
		return;
	}
	if( (mm % 5) > 0 ){
		...
		{
		    alert("O hor�rio m�ximo permitido � 23:55!");
		    document.forms[formName].elements[fieldName].value = '';
		    return;
		}
...
/*
	Fun��o que, quando atingido seu maxlength, o focus o campo informado
	formName   = Nome do Formul�rio
	size       = maxlength do campo
	fieldName  = Nome do Campo
	campoFocus = Nome do campo que receber� o focus.
*/
		...
		for (i=0;i<formulario.elements.length;i++){
			if(formulario.elements[i].name == dados[cont]){
				if( Number(formulario.elements[i].value) ){
					existeErro = true;
					erro[contErro] = 'O campo "'+labelCampo[cont]+'" n�o aceita somente n�meros!';
					contErro++;
				}
			}
		}
...
/*
 * fun��o similar com a in_array() do php
 * */
...
/**
 * Faz a mudan�a de focus automatica de um elemento para o outro ap�s o usuario digitar um valor
 *
 * @param string nameCurrentElement atributo "name" do elemento atual que perder� o foco
 * @param string nameNextElement atributo "name" do elemento que receber� o foco
 * @returns VOID
 */
function autoTab(nameCurrentElement, nameNextElement){
    ...
    // Espera meio segundo para desencadear a pr�xima a��o
    setTimeout( function(){
        // Verifica se existe mais de 1 caractere no elemento atual
        if(NumChar >= 1){
            // Muda o foco para o pr�ximo elemento do formulario
...
/**
 * M�scaras ER
 * Mascaras compativeis com dispositivos mobile e vers�es recentes de navegadores
