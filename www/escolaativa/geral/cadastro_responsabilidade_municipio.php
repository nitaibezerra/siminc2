...
if(isset($_REQUEST['enviar'])) {
	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $muncod){
			$sql = "INSERT INTO escolaativa.usuarioresponsabilidade (muncod, usucpf, rpustatus, rpudata_inc, pflcod) 
...
";
		...
		$sql2 = "SELECT muncod FROM escolaativa.usuarioresponsabilidade where usucpf = '$usucpf' and rpustatus = 'A'";
		$arMunicipiosAssociados = $db->carregar($sql2);
		$arMunicipiosAssociados = ($arMunicipiosAssociados) ? $arMunicipiosAssociados : array();
		$arMuncod = array();
		foreach($arMunicipiosAssociados as $muncod){
			$arMuncod[] = $muncod['muncod'];
		}
		...
		<tr>
			...
			foreach ($municipios as $municipio) {
				...
				$checked = "";
				if(in_array($municipio['muncod'], $arMuncod)){
...
<form name="formassocia" style="margin: 0px;" method="POST"><input
	...
	$sql = "
			...
			from 
				escolaativa.usuarioresponsabilidade ur inner join territorios.municipio m on ur.muncod = m.muncod
