		...
		foreach($scope_list as $j => $list) {
			foreach($cache[$list] as $i => $entry) {
				...
				if($fieldkey=='info') {
					...
					echo simec_htmlspecialchars($output);
					echo "</pre></td></tr>\n";
				}
				break;
			}
...
EOB;
	...
	if (!$rss) {
		...
		while (list(,$v) = each($match[2])) {
			...
			echo "<b><a href=\"http://pecl.php.net/package/APC/$ver\">".simec_htmlspecialchars($v)."</a></b><br><blockquote>";
			echo nl2br(simec_htmlspecialchars(current($match[2])))."</blockquote>";
			next($match[2]);
		}
		echo '</td></tr>';
	}
