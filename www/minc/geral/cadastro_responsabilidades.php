...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM minc.tprperfil p
	INNER JOIN minc.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "E": // Estados
				...
				$sqlRespUsuario = "SELECT e.estuf AS codigo, e.estdescricao AS descricao, ur.rpustatus AS status
					FROM minc.usuarioresponsabilidade ur 
			...
			case "S": // escolas
				...
				/*$sqlRespUsuario = "
					...
					FROM
						 entidade.entidade e
					INNER JOIN 
						 minc.usuarioresponsabilidade ur ON ur.entid = e.entid AND pflcod = {$pflcod} AND usucpf = '{$usucpf}'
					INNER JOIN 
					...
					INNER JOIN
						 minc.mcemaiscultura maedu ON  maedu.entid = e.entid
					LEFT JOIN 
					...
					ORDER BY
						e.entnome
				";*/
				$sqlRespUsuario = " SELECT e.entid AS codigo, 
							       		   e.entnome AS descricao
							  		  FROM entidade.entidade e
							    INNER JOIN minc.usuarioresponsabilidade ur USING(entid)
