...
if(is_array($_POST['usuunidresp'])) {
	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $estuf){
			$sql = "INSERT INTO minc.usuarioresponsabilidade (estuf, usucpf, rpustatus, rpudata_inc, pflcod) 
...
<?
$sql = "select distinct u.estuf as codigo, u.estuf||' - '||u.estdescricao as descricao from minc.usuarioresponsabilidade ur inner join territorios.estado u on ur.estuf=u.estuf where ur.rpustatus='A' and ur.usucpf = '$usucpf' and ur.pflcod=$pflcod";
$RS = @$db->carregar($sql);
if(is_array($RS)) {
	$nlinhas = count($RS)-1;
	if ($nlinhas>=0) {
