...
/*
 * VERIFICANDO NOMES DUPLICADOS MAIS DE $_REQUEST['numero']X
 */
$sql = "select trim(ent.entnome) as entnome, entnumcpfcnpj, entcodent, count(ent.entid) from entidade.entidade ent 
		where entstatus='A' and entnome is not null
		group by trim(ent.entnome), entnumcpfcnpj, entcodent
		having count(ent.entid)>".$_REQUEST['numero'];
		...
		foreach($entidadesx as $ent) {
			if($ent['entnome']) {
				...
				for($i=0;$i<count($ents);$i++) {
					...
					//$t1 = $db->pegaUm("select * from pdeescola.memaiseducacao where entid='".$ents[$i]['entid']."'");
					...
					$db->executar("update obras.responsavelcontatos SET entid='".$ultent."' where entid='".$ents[$i]['entid']."'");
					$db->executar("update emenda.planotrabalho SET entid='".$ultent."' where entid='".$ents[$i]['entid']."'");
					$db->executar("update emenda.planotrabalho SET entiddirigente='".$ultent."' where entiddirigente='".$ents[$i]['entid']."'");
					$db->executar("update painel.responsavel SET entid='".$ultent."' where entid='".$ents[$i]['entid']."'");
