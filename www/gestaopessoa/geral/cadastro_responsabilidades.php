...
$sqlResponsabilidadesPerfil = "SELECT
							    tr.*
							   FROM
							    gestaopessoa.tprperfil p
							    INNER JOIN gestaopessoa.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
							   WHERE
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "S": 
				$aca_prg = "Situação Trabalhador";
				$sqlRespUsuario = "SELECT
									od.fstid AS codigo, 
									od.fstdescricao AS descricao, 
									ur.rpustatus AS status
								   FROM
								    gestaopessoa.usuarioresponsabilidade ur 
									INNER JOIN gestaopessoa.ftsituacaotrabalhador od ON od.fstid = ur.fstid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
				break;
			case "U": 
				$aca_prg = "Unidades Lotações";
				$sqlRespUsuario = "SELECT
								    sd.fulid AS codigo, 
								    sd.fuldescricao AS descricao, 
								    ur.rpustatus AS status
								   FROM
								    gestaopessoa.usuarioresponsabilidade ur 
									INNER JOIN gestaopessoa.ftunidadelotacao sd ON sd.fulid = ur.fulid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
			...
			case "O": 
				$aca_prg = "Unidades";
				$sqlRespUsuario = "
                                    SELECT  u.unicod AS codigo, 
                                            u.unidsc AS descricao, 
                                            ur.rpustatus AS status
                                    FROM gestaopessoa.usuarioresponsabilidade ur 
                                    JOIN public.unidade u ON u.unicod = ur.unicod
