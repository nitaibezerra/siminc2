...
if ( !in_array(PERFIL_SUPER_USER, $perfil) && !in_array(PERFIL_GESTOR, $perfil) ){	
	...
	$sql = "SELECT
				ur.fulid 
			FROM
				gestaopessoa.ftunidadelotacao od
				INNER JOIN gestaopessoa.usuarioresponsabilidade ur ON ur.fulid = od.fulid
			WHERE
				ur.rpustatus = 'A' AND
...
if ($_POST && $gravar == 1){
	$fulid = $_REQUEST["uniresp"];
	atribuiOrigemDemanda($usucpf, $pflcod, $fulid);
}
...
function retorna(objeto, vinculado) {
	...
	var elemento = document.formulario.fulid;
	
	if (!document.formulario.fulid.length){
		elemento 	= document.formulario.fulid;
		elementoDsc = document.formulario.fuldescricao;
	}else{
		elemento 	= document.formulario.fulid[objeto];
		elementoDsc = document.formulario.fuldescricao[objeto];
	}
 ...
 * @param string $fulid
 */
function atribuiOrigemDemanda($usucpf, $pflcod, $fulid){	
	global $db;
	...
	if (PERFIL_ADMINISTRADOR == $pflcod && $fulid[0]){
		$db->executar("UPDATE 
						gestaopessoa.usuarioresponsabilidade 
					   SET 
						rpustatus = 'I' 
					   WHERE  
						fulid IN (" . implode(',',$fulid) . ") AND
						pflcod = '{$pflcod}'"); 
	}	
	
	if (is_array($fulid) && !empty($fulid[0])){
		$count = count($fulid);
...
function listaOrigemDemanda(){
	...
	if (PERFIL_ADMINISTRADOR == $pflcod){
		$select[] = 'ur.rpuid';
		$from[]   = "LEFT JOIN gestaopessoa.usuarioresponsabilidade ur ON od.fulid = ur.fulid AND
																 	  ur.pflcod = ".PERFIL_ADMINISTRADOR." AND
																 	  ur.rpustatus = 'A' AND
																      ur.usucpf != '{$usucpf}'";
	}
	...
	if ( !in_array(PERFIL_SUPER_USER,$perfil) ){
		$from[] = "INNER JOIN gestaopessoa.usuarioresponsabilidade ur2 ON od.fulid = ur2.fulid AND
	...
	for ($i = 0; $i < $count; $i++){
		$codigo    = $origemDemanda[$i]["fulid"];
		$descricao = $origemDemanda[$i]["fuldescricao"];
		...
		echo "<tr bgcolor=\"".$cor."\">
				<td align=\"right\" width=\"10%\">
					<input type=\"Checkbox\" name=\"fulid\" id=\"".$codigo."\" value=\"$codigo\" onclick=\"retorna('".$i."','".$rpuid."');\">
					<input type=\"hidden\" name=\"fuldescricao\" value=\"". $codigo .' - '.$descricao ."\">
				</td>
				<td>
					".$descricao."
				</td>
