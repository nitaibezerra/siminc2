...
if(is_array($_POST['usuuniresp']) && @count($_POST['usuuniresp'])>0) {
	...
	$sqlInsRpu = "INSERT INTO evento.usuarioresponsabilidade (unicod, usucpf, rpustatus, rpudata_inc, pflcod, prsano) VALUES ('%s', '%s', '%s', '%s', '%s','".$_SESSION['exercicio']."')";

	$sqlUpdRpu = "UPDATE evento.usuarioresponsabilidade SET rpustatus = 'I' WHERE unicod = '%s' AND pflcod = ".$pflcod;

	$sqlUpdRpuUsu = "UPDATE evento.usuarioresponsabilidade SET rpustatus = 'I' WHERE unicod IS NOT NULL AND unicod != '' AND usucpf = '".$usucpf."' AND pflcod = ".$pflcod;
...
<?
	  ...
	  $sql = "select 
			  ...
			  from 
				entidade.entidade e
				INNER JOIN public.unidade u ON u.unicod = e.entunicod
...
if(is_array($RS)) {
	...
	?>
	<option value="">Selecione a(s) Instituição(ões)</option>
	<?		
...
/*	
	$sql = "
		...
		from public.unidade p
			inner join evento.progacaoproposto pp on
...
function retorna(objeto)
{
	...
	if (document.formulario.unicod[objeto].checked == true){
		...
		for(var i=0; i<=campoSelect.length-1; i++){
			...
			if (!campoSelect.options[0]){campoSelect.options[0] = new Option('Selecione a(s) Instituição(ões)', '', false, false);}
			sortSelect(campoSelect);
	}
}
