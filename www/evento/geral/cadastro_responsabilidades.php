...
$sqlResponsabilidadesPerfil = "SELECT 
								...
								FROM 
									evento.tprperfil p
									INNER JOIN 
										evento.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
								WHERE 
		...
		switch ($rp["tprsigla"]) {	
			...
			case "E": // projetos especiais 
				...
				$sqlRespUsuario = "SELECT p.pjecod AS codigo, p.pjedsc AS descricao, u.rpustatus AS status
					FROM evento.usuarioresponsabilidade u 
					INNER JOIN evento.projetoespecial p ON p.pjeid = u.pjeid
					WHERE p.prsano = '".$_SESSION['exercicio']."' and u.usucpf = '%s' AND u.pflcod = '%s' AND u.rpustatus='A'";
			break;
			case "I": // unidades
				$aca_prg = "UASG Associadas";
				$sqlRespUsuario = "
					SELECT ua.usgcod as codigo, usgdsc as descricao FROM evento.usuarioresponsabilidade ur 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	<tr onmouseover="this.bgColor='#ffffcc';" onmouseout="this.bgColor='F7F7F7';" bgcolor="F7F7F7">
	  ...
	  <td valign="top" width="90" style="border-top: 1px solid #cccccc; padding:2px; color:#003366;" nowrap><?if ($rp["tprsigla"]=='A'){?><a href="evento.php?modulo=principal/acao/cadacao&acao=C&acaid=<?=$ru["acaid"]?>&prgid=<?=$ru["prgid"]?>"><?=$ru["codigo"]?></a><?} else {print $ru["codigo"];}?></td>
