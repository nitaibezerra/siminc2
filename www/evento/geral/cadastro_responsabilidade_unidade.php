...
//	}
	...
	if(!$confirmarunidades || $_REQUEST["pflcod"] == "113" || $_REQUEST["pflcod"] == "411" || $_REQUEST["pflcod"] == "413") {
		$sql = $sqlUpdRpuUsu;
		$db->executar($sql);
		foreach ($_POST['usuuniresp'] as $respcod) {
		   $sql = "";
...
//			FROM 
//				public.unidade p
//			INNER JOIN evento.progacaoproposto pp ON p.unicod = pp.unicod AND pp.usucpf = '" . $usucpf . "'
//			WHERE 
//				pp.unicod is not null";
//	$RS = @$db->carregar($sql);
//	if(is_array($RS)) {
//		$nlinhas = count($RS)-1;
//		if ($nlinhas>=0) {
