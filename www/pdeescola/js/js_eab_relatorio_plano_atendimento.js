	...
	$("#bt_gerar_relatorio").click(function()
	{
		if( confirm('Deseja gerar o relatório para todas as escolas?') )
		{
			...
			({
			  ...
			  data: "ajax=1",
			  dataType: "json",
			  url: 'pdeescola.php?modulo=eabrelatorio/relatorio_plano_atendimento&acao=A',
			  success: function(data)
			  {
				 ...
				 if(data.valida)
				 {
					 ...
					 var janela = window.open('pdeescola.php?modulo=eabrelatorio/pop_relatorio_plano_atendimento&acao=A&lote=' + data.lote, 'relatorio', 'width=780,height=460,status=1,menubar=1,toolbar=0,scrollbars=1,resizable=1');
					 janela.focus();
					 /*** Recarrega a tela ***/
...
{
	var janela = window.open('pdeescola.php?modulo=eabrelatorio/pop_relatorio_plano_atendimento&acao=A&lote='+lote+'&muncod='+muncod+'&estuf='+estuf, 'relatorio', 'width=780,height=460,status=1,menubar=1,toolbar=0,scrollbars=1,resizable=1');
	janela.focus();
}