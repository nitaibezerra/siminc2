...
$sqlResponsabilidadesPerfil = "SELECT 
								...
								FROM 
									assint.tprperfil p
								INNER JOIN 
									assint.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
								WHERE 
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		$sqlRespUsuario = "";
		switch ($rp["tprsigla"]) {	
			case "U": // Universidades
				$aca_prg = "'Universidades'";
				$sqlRespUsuario = "SELECT 
									...
									INNER JOIN
										entidade.entidade ent on ent.entid = eai.entid			
									INNER JOIN 
										assint.usuarioresponsabilidade ur ON ur.entid = eai.entid
									WHERE
									  eai.entstatus = 'A'
									  AND ent.entstatus = 'A'
									  AND ur.rpustatus = 'A'
			...
			case "I": // Universidades
				...
				$sqlRespUsuario = "SELECT 
									...
									FROM 
										emec.ies ies		
									INNER JOIN 
										assint.usuarioresponsabilidade ur ON ur.co_ies = ies.co_ies
									WHERE
									  ur.co_ies IS NOT NULL
									  AND ur.rpustatus = 'A'
									  AND ur.usucpf = '%s' 
					  				  AND ur.pflcod = '%s'";
			break;
