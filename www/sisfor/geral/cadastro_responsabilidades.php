...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM sisfor.tprperfil p
							   INNER JOIN sisfor.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "C": // Unidades 
				...
				$sqlRespUsuario = "SELECT 
									DISTINCT 
									e.coordid AS codigo, 
									e.coorddesc AS descricao, 
									ur.rpustatus AS status
								   FROM
								    sisfor.usuarioresponsabilidade ur 
								    INNER JOIN catalogocurso2014.coordenacao e ON e.coordid = ur.coordid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
			...
			case "E": // Estados
				...
				$sqlRespUsuario = "SELECT DISTINCT 
								   ...
								   FROM 
									...
									INNER JOIN obras.orgao o ON o.orgid = ur.orgid
								   WHERE 
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM sisfor.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
