...
if ($_REQUEST["estufresp"]){
	$estufresp = $_REQUEST["estufresp"];
	atribuiEstuf($usucpf, $pflcod, $estufresp);
}
...
function listaEstuf(){
	...
	$estufExistentes = $db->carregar("SELECT 
											estuf,
											estdescricao
										  FROM territorios.estado  
										  ORDER BY estuf");
	$count = count($estufExistentes);

	// Monta as TR e TD com as unidades
	for ($i = 0; $i < $count; $i++){
		$codigo    = $estufExistentes[$i]["estuf"];
		$descricao = $estufExistentes[$i]["estdescricao"];
...
function atribuiEstuf($usucpf, $pflcod, $estuf){
	$db = new cls_banco();
	...
	if ($estuf[0]){
		foreach($estuf as $tipo) {
			$dadosur = $db->carregar("SELECT * FROM sisfor.usuarioresponsabilidade WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND estuf = '". $tipo ."'");
			if($dadosur) {
				// Se existir registro atualizar para ativo
				$db->carregar("UPDATE sisfor.usuarioresponsabilidade
   							   SET rpustatus = 'A', rpudata_inc= NOW()
 							   WHERE usucpf = '". $usucpf ."' AND pflcod = '". $pflcod ."' AND estuf = '". $tipo ."'");
			} else {
				// Se não existir, inserir novo
				$db->executar("INSERT INTO sisfor.usuarioresponsabilidade(
            				   pflcod, usucpf, estuf, rpustatus, rpudata_inc)
...
function buscaEstufAtribuido($usucpf, $pflcod){
	...
	if(is_array($RS)) {
		...
		print '<option value="">Clique na UF selecionar.</option>';
	}
}
...
<html>
	...
	<body leftmargin="0" topmargin="5" bottommargin="5" marginwidth="0" marginheight="0" bgcolor="#ffffff">
		...
		<form name="formassocia" action="cadastro_responsabilidade_estuf.php" method="post">
			<input type="hidden" name="usucpf" value="<?=$usucpf?>">
			<input type="hidden" name="pflcod" value="<?=$pflcod?>">
			<select multiple size="8" name="estufresp[]" id="estresp" style="width:500px;" class="CampoEstilo">
				<?php 
					buscaEstufAtribuido($usucpf, $pflcod);
				?>
			</select>
		</form>
