...
if(isset($_REQUEST['enviar'])) {
	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $muncod){
			$sql = "INSERT INTO projovemcampo.usuarioresponsabilidade (muncod, usucpf, rpustatus, rpudata_inc, pflcod) 
...
document.getElementById('tabela').style.display  = "none";
	...
	<thead>
		...
		$sql = "
			select
					e.estuf, e.estuf, e.estdescricao
				from
					projovemcampo.adesaoprojovemcampo c 
				inner join 
					territorios.municipio m on c.apccodibge::character(7) = m.muncod  
				inner join 
					territorios.estado e on e.estuf = m.estuf 
				where apcesfera in('C','M')
				group by e.estuf, e.estdescricao
				order by e.estuf, e.estdescricao
";
		$RS = @$db->carregar($sql);
		$nlinhas = count($RS)-1;
		$j = 0 ;
		...
		<tr>
			...
			<?
			$sql = "select mundescricao, muncod from territorios.municipio m
					inner join projovemcampo.adesaoprojovemcampo c on c.apccodibge::character(7) = m.muncod and apcesfera in('C','M')
					where estuf = '$estuf' order by mundescricao";
			...
			foreach ($municipios as $value) {
				...
				<tr bgcolor="<?=$cor2?>">
					<td align="left" style="border: 0"><input type="checkbox"
						name="muncod" id="<?=$value['muncod']?>"
						value="<?=$value['muncod']?>"
						onclick="retorna( this, '<?= $value['muncod'] ?>', '<?= $estuf.' - '. addslashes( $value['mundescricao'] ) ?>' );" />
						<?=$value['mundescricao']?></td>
				</tr>
				<?
}
?>
...
<form name="formassocia" style="margin: 0px;" method="POST"><input
	...
	$sql = "
			...
			from 
				projovemcampo.usuarioresponsabilidade ur inner join territorios.municipio m on ur.muncod = m.muncod
