...
$sqlResponsabilidadesPerfil = "SELECT tr.*
	FROM projovemcampo.tprperfil p
	INNER JOIN projovemcampo.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			...
			case "S": // Escolas
				...
				break;
			case "T": // Turmas
				$aca_prg = "Turmas Associadas";
				$sqlRespUsuario = "SELECT DISTINCT
										ur.turid as codigo, 
										entnome||'-'||turdescricao as descricao
									FROM 
										projovemcampo.usuarioresponsabilidade ur
									INNER JOIN projovemcampo.turma tur ON tur.turid = ur.turid
