...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM rehuf.tprperfil p
							   INNER JOIN rehuf.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
		...
		switch ($rp["tprsigla"]) {
			case "U": // Unidades 
				...
				$sqlRespUsuario = "SELECT 
								   ...
								   FROM
								    rehuf.usuarioresponsabilidade ur 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM rehuf.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
