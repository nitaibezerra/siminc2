...
function submeteFormCadastroProcesso() {
	...
	if(document.formulario.elements['prcnumsidoc'].value == "") {
		alert('"Número do Processo SIDOC" é obrigatório');
	...
	if(document.formulario.elements['tprid'].value == "") {
		...
		alert('"Processo na PROFE/INEP" é obrigatório');
		document.getElementById('botaosubmeter').disabled=false;
		return false;
	}
...
function submeteFormExpediente() {
	...
	if(document.formulario.elements['expdtinclusaoprofeinep'].value != '') {
		if(!validaData(document.formulario.elements['expdtinclusaoprofeinep'])) {
			alert('"Data do parecer da profeinep" é inválida');
			document.getElementById('botaosubmeter').disabled=false;
			return false;
		}
	}
...
function deletarlinha(t) {
	if(confirm("Deseja realmente excluir o processo vinculado?")) {
