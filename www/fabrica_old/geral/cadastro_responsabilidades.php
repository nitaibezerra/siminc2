...
$sqlResponsabilidadesPerfil = "SELECT tr.*
							   FROM fabrica.tprperfil p
							   INNER JOIN fabrica.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		$sqlRespUsuario = "";
		$aca_prg = "Tipo de execução";
		switch ($rp["tprsigla"]) {
			case "X": // Tipo de execução 
				$aca_prg = "Tipo de execução";
				$sqlRespUsuario = "SELECT 
									DISTINCT 
									e.tpeid AS codigo, 
									e.tpedsc AS descricao, 
									ur.rpustatus AS status
								   FROM
								    fabrica.usuarioresponsabilidade ur 
								    INNER JOIN fabrica.tipoexecucao e ON e.tpeid = ur.tpeid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
				break;
			case "E": // Estados
				$aca_prg = "Especialização";
				$sqlRespUsuario = "SELECT DISTINCT 
									e.espid AS codigo, 
									e.espdsc AS descricao  
								   FROM 
								    fabrica.usuarioresponsabilidade ur 
									INNER JOIN fabrica.especializacao e ON e.espid = ur.espid
								   WHERE 
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="width:100%; border: 0px; color:#006600;">
	...
	<tr onmouseover="this.bgColor='#ffffcc';" onmouseout="this.bgColor='F7F7F7';" bgcolor="F7F7F7">
	  ...
	  <td valign="top" width="90" style="border-top: 1px solid #cccccc; padding:2px; color:#003366;" nowrap><? print $ru["codigo"]; ?></td>
	...
	$teste = $db->carregar("SELECT DISTINCT * FROM fabrica.usuarioresponsabilidade WHERE usucpf = '{$usucpf}' AND pflcod = {$pflcod} AND rpustatus = 'A'");
	if (!$teste) {
		print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Não existem associações a este Perfil.</font>";
	}
}
