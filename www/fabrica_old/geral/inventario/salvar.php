...
if( empty($co_inventario) )
{
    ...
    if($inventarioRepositorio->possuiInventarioAtivo( $sidid )) {
        echo simec_json_encode( array( 'status' => false,'msg' => utf8_encode('Não é possível gerar um inventário de um sistema já inventariado')
            ) 
        );
    ...
    if($co_inventario == false) {
        echo simec_json_encode( array( 'status' => false,'msg' => utf8_encode('Não é possível gerar um novo inventário')
            ) 
        );
        exit;
    }
...
if( empty( $co_agrupador ) && !empty( $no_agrupador ) )
{
    ...
    if($co_agrupador == false) {
        echo simec_json_encode( array( 'status' => false,'msg' => utf8_encode('Não é possível gerar um novo agrupador de funcionalidade')
            ) 
        );
        exit;
    }
...
if( $funcionalidade->verificaSeExisteFuncionalidade( $ds_funcionalidade, $co_agrupador, $no_agrupador, $co_funcionalidade ))
    {
        echo simec_json_encode( array( 'status' => false,'msg' => utf8_encode('Funcionalidade já cadastrada para esse sistema e agrupador')
            ) 
        );
        exit;
    }
...
if( !empty( $co_funcionalidade ) ) 
{
    ...
    if( !$funcionalidade->salvar() ) {
        echo simec_json_encode( array( 'status' => false,'msg' => utf8_encode('Não foi possível atualizar a funcionalidade')
            ) 
        );
        exit;
    }
...
else 
{
    ...
    if($co_funcionalidade == false) {
        echo simec_json_encode( array( 'status' => false,'msg' => utf8_encode('Não foi possível gerar a funcionalidade')
            ) 
        );
        exit;
    }
...
echo simec_json_encode( array( 'status' => true,'dados' => array(
