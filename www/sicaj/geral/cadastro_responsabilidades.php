...
$sqlResponsabilidadesPerfil = "SELECT tr.*
                                 FROM sicaj.tprperfil p
                                   INNER JOIN sicaj.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
...
DML;
        ...
        if (!$respUsuario || @count($respUsuario) < 1) {
            ...
            <? foreach ($respUsuario as $ru): ?>
                ...
                <!--            <? //if ($rp["tprsigla"]=='A'){ ?><a href="simec_er.php?modulo=principal/acao/cadacao&acao=C&acaid=<? //=$ru["acaid"] ?>&prgid=<? //=$ru["prgid"] ?>"><? //=$ru["codigo"] ?></a><? //} else {print $ru["codigo"];} ?></td> -->
