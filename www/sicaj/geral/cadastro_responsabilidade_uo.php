...
require (APPRAIZ . 'www/sicaj/_constantes.php');
$db = new cls_banco();

function gravarResponsabilidadeAcao($dados) {
    global $db;
    $sql = "UPDATE sicaj.usuarioresponsabilidade SET rpustatus='I' WHERE usucpf='".$dados['usucpf']."' AND pflcod='".$dados['pflcod']."'";
    $db->executar($sql);
    ...
    if ($dados['usuacaresp']) {
        foreach($dados['usuacaresp'] as $unicod) {
            ...
            $sql = <<<DML
INSERT INTO sicaj.usuarioresponsabilidade(pflcod, usucpf, rpustatus, rpudata_inc, unicod)
...
SELECT '<input type=\"checkbox\" name=\"unicod\" id=\"chk_' || uni.unicod || '\" value=\"' || uni.unicod || '\" '
           ...
           || case WHEN (SELECT count(urp.rpuid) FROM sicaj.usuarioresponsabilidade urp WHERE urp.unicod = uni.unicod AND urp.usucpf = '{$usucpf}' AND urp.pflcod = '{$pflcod}' AND rpustatus = 'A') > 0 THEN ' checked' ELSE '' END || '>' AS unicod,
       uni.unicod || ' - ' || uni.unidsc AS descricao
  FROM public.unidade uni
    LEFT JOIN sicaj.usuarioresponsabilidade urp
...
SELECT uni.unicod AS codigo,
       uni.unicod || ' - ' || uni.unidsc AS descricao
  FROM sicaj.usuarioresponsabilidade ur
