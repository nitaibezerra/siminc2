...
include_once APPRAIZ . "includes/envia_sms_sis_geral_funcoes.inc";
...
if ( $_REQUEST[ "enviar" ] || $_REQUEST[ "sql" ]){
	...
	if( is_array( $_REQUEST["arDestinatarios"] ) ){
		foreach( $_REQUEST["arDestinatarios"] as $indice => $stDados ){
			...
			// Celular
			$arDestinatariosSelecao[$indice]['celular'] = $aCelularEnvio[] = '55' . str_replace('-', '', trim(substr($restante, 0, strpos( $restante, "||" ))));
	...
	if(trim($_REQUEST['sql'])) {
		
		$arrInfo = $db->carregar(trim(str_replace(array("\'"),array("'"),$_REQUEST['sql'])));
		
		if(!$arrInfo[0]['email'] || !$arrInfo[0]['nome']) {
			die('<script type="text/javascript">alert( "Na consulta SQL deve conter colunas \"email\" e \"nome\"");history.go(-2);</script>');
		}
...
//	array_push( $arDestinatariosSelecao, $remetente );
	...
	/*
	 * Trecho feito por Alexandre Dourado - 12/11/2012
	 * - Envio a partir do SQL, somente para Super Usuário
	 */
	if($arrInfo[0]) {
...
//
//			$mensagem->From     = $remetente["usuemail"];
//			$mensagem->FromName = $remetente["usunome"];
//
//			$mensagem->AddAddress( $info['email'], $info['nome'] );
...
//
//		}
		die('<script type="text/javascript">alert( "Destinatários da consulta SQL foram enviados com sucesso");history.go(-1);</script>');
    ...
    if($_REQUEST['mensagem']){
        $sms = new Sms();
        $sms->enviarSms($aCelularEnvio, $_REQUEST['mensagem']);
    }
...
$destinatarios = SmsSistema::identificar_destinatarios( $orgao, $tipoEnsino, $uo, $ug, $perfis, $outros, $statusUsuario, $ideb, $arMunicipios, $cargo );
?>
<html>
	<head>
	...
	<body leftmargin="0" topmargin="0" bottommargin="0" marginwidth="0" marginheight="0" bgcolor="#ffffff">
		<?php if( !empty( $destinatarios ) ): ?>
			...
			<form id="formulario" method="post" name="formulario" enctype="multipart/form-data">
					...
					<?php foreach( $destinatarios as $indice => $destinatario ): ?>
						...
						<tr style="vertical-align:top; background-color: <?php echo $cor; ?>;" onmouseover="this.style.backgroundColor='#ffffcc';" onmouseout="this.style.backgroundColor='<?php echo $cor; ?>';">
							<td style="text-align:left">
                                <?php if (in_array(substr(trim($destinatario['usufonenum']), 0, 1), array(8,9))) {  ?>
								    <input 	name="arDestinatarios[]" type="checkbox" checked="checked" value="<?php echo $destinatario['usunome']. '||' . $destinatario['usufoneddd']. $destinatario['usufonenum']. '||' .$destinatario['usucpf'] ?>" />
                                <?php } ?>
							</td>
							<td style="text-align:left"><?= $destinatario['usunome']?></td>
							<td style="text-align:left"><?= '(' . $destinatario['usufoneddd'] . ') ' . $destinatario['usufonenum']; ?></td>
				...
				<input type="hidden" value="<?php echo $_REQUEST["pessoas"]; ?>" name="pessoas" id="pessoas" />
