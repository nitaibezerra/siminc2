...
if(is_array($_POST['edtSelecionadas'])) {
	...
	if($_POST['edtSelecionadas'][0]){
		foreach($_POST['edtSelecionadas'] as $cedid){
			$sql = "insert into livro.usuarioresponsabilidade (pflcod, usucpf,  rpustatus, rpudata_inc, cedid)
						   								values ($pflcod, '$usucpf', 'A', now(), '$cedid')";
			$db->executar($sql);
		}		
	}
	$db->commit();
...
<?
	  ...
	  $sql = "SELECT 
					...
					e.cedrazaosocial 
				FROM 
				  	livro.campoeditora e	
				order by e.cedrazaosocial";
	  ...
	  if($RS){
		  ...
		  for ($i=0; $i<=$nlinhas;$i++){
		   ...
		   ?>
			   		...
			   		<tr bgcolor="<?=$cor?>">
						<td align="right"><input type="checkbox" name="usgid" id="<?=$cedid?>" value="<?=$cedid?>" onclick="retorna(<?=$i?>);">
										  <input type="Hidden" name="edinome" value="<?=$cedid.' - '.$cedrazaosocial?>"></td>
						<td align="right" style="color:blue;"><?=$cedid?></td>
						<td><?=$cedrazaosocial?></td>
					</tr>
...
$sql = "SELECT DISTINCT 
		...
		FROM 
		  livro.usuarioresponsabilidade ur	INNER JOIN livro.campoeditora e 
		  ON (ur.cedid = e.cedid)
		WHERE 
		  ur.rpustatus='A' 
