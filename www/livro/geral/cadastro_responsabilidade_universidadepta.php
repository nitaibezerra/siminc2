...
if(is_array($_POST['edtSelecionadas'])) {
	...
	if($_POST['edtSelecionadas'][0]){
		foreach($_POST['edtSelecionadas'] as $unvid){
			$sql = "insert into livro.usuarioresponsabilidade (pflcod, usucpf,  rpustatus, rpudata_inc, unvid)
						   								values ($pflcod, '$usucpf', 'A', now(), '$unvid')";
			$db->executar($sql);
		}		
	}
	$db->commit();
...
<?
	  ...
	  $sql = "SELECT 
					...
					e.univdsc,
					case
						when c.comdsc is null then '-'
						else c.comdsc
					end AS comdsc
				FROM 
				  	livro.ptauniversidade e	
				LEFT JOIN livro.componente c ON e.comid = c.comid
				order by e.univdsc";
	  ...
	  if($RS){
		  ...
		  for ($i=0; $i<=$nlinhas;$i++){
		   ...
		   ?>
			   		...
			   		<tr bgcolor="<?=$cor?>">
						<td align="right"><input type="checkbox" name="usgid" id="<?=$unvid?>" value="<?=$unvid?>" onclick="retorna(<?=$i?>);">
										  <input type="Hidden" name="edinome" value="<?=$unvid.' - '.$univdsc?>"></td>
						<td align="right" style="color:blue;"><?=$unvid?></td>
						<td><?=$univdsc?></td>
						<td><?=$comdsc?></td>
					</tr>
...
$sql = "SELECT DISTINCT 
		...
		FROM 
		  livro.usuarioresponsabilidade ur	INNER JOIN livro.ptauniversidade e 
		  ON (ur.unvid = e.unvid)
		WHERE 
		  ur.rpustatus='A' 
...
function retorna(objeto)
{
	...
	var aredinome = document.getElementsByName( 'edinome' );
	
	
	
	if (arUsgid[objeto].checked == true){
		campoSelect.options[tamanho] = new Option(aredinome[objeto].value, arUsgid[objeto].value, false, false);
		...
		for(var i=0; i<=campoSelect.length-1; i++){
			...
			if (!campoSelect.options[0]){campoSelect.options[0] = new Option('Clique na Universidade.', '', false, false);}
			sortSelect(campoSelect);
	}
}
