...
if(is_array($_POST['edtSelecionadas'])) {
	...
	if($_POST['edtSelecionadas'][0]){
		foreach($_POST['edtSelecionadas'] as $ediid){
			$sql = "insert into livro.usuarioresponsabilidade (pflcod, usucpf,  rpustatus, rpudata_inc, ediid)
						   								values ($pflcod, '$usucpf', 'A', now(), '$ediid')";
			$db->executar($sql);
		}		
	}
	$db->commit();
		   ...
		   ?>
			   		...
			   		<tr bgcolor="<?=$cor?>">
						<td align="right"><input type="checkbox" name="usgid" id="<?=$ediid?>" value="<?=$ediid?>" onclick="retorna(<?=$i?>);">
										  <input type="Hidden" name="edinome" value="<?=$ediid.' - '.$edinome?>"></td>
...
$sql = "SELECT DISTINCT 
		...
		FROM 
		  livro.usuarioresponsabilidade ur	INNER JOIN livro.ejaeditora e 
		  ON (ur.ediid = e.ediid)
		WHERE 
		  ur.rpustatus='A' 
