...
if(is_array($_POST['edtSelecionadas'])) {
	...
	if($_POST['edtSelecionadas'][0]){
		foreach($_POST['edtSelecionadas'] as $edpid){
			$sql = "insert into livro.usuarioresponsabilidade (pflcod, usucpf,  rpustatus, rpudata_inc, edpid)
						   								values ($pflcod, '$usucpf', 'A', now(), '$edpid')";
			$db->executar($sql);
		}		
	}
	$db->commit();
...
<?
	  ...
	  $sql = "SELECT 
					...
					e.edpnomefantasia 
				FROM 
				  	livro.pnaiceditora e	
				order by e.edpnomefantasia";
	  ...
	  if($RS){
		  ...
		  for ($i=0; $i<=$nlinhas;$i++){
		   ...
		   ?>
			   		...
			   		<tr bgcolor="<?=$cor?>">
						<td align="right"><input type="checkbox" name="usgid" id="<?=$edpid?>" value="<?=$edpid?>" onclick="retorna(<?=$i?>);">
										  <input type="Hidden" name="edinome" value="<?=$edpid.' - '.$edpnomefantasia?>"></td>
						<td align="right" style="color:blue;"><?=$edpid?></td>
						<td><?=$edpnomefantasia?></td>
					</tr>
...
$sql = "SELECT DISTINCT 
		...
		FROM 
		  livro.usuarioresponsabilidade ur	INNER JOIN livro.pnaiceditora e 
		  ON (ur.edpid = e.edpid)
		WHERE 
		  ur.rpustatus='A' 
