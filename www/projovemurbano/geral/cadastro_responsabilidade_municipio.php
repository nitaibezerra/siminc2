...
if(isset($_REQUEST['enviar'])) {
	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $muncod){
			$sql = "INSERT INTO projovemurbano.usuarioresponsabilidade (muncod, usucpf, rpustatus, rpudata_inc, pflcod) 
	...
	<thead>
		...
		$sql = "
			select
				...
				from
					projovemurbano.cargameta c 
				inner join 
					territorios.municipio m on c.cmecodibge::character(7) = m.muncod  
				inner join 
					territorios.estado e on e.estuf = m.estuf 
				where cmetipo in('C','M')
...
<form name="formassocia" style="margin: 0px;" method="POST"><input
	...
	$sql = "
			...
			from 
				projovemurbano.usuarioresponsabilidade ur inner join territorios.municipio m on ur.muncod = m.muncod
