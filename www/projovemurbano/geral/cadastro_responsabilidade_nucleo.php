...
if(isset($_REQUEST['enviar'])) {
	...
	if($_POST['usuunidresp'][0]){
		foreach($_POST['usuunidresp'] as $nucid){
			$sql = "INSERT INTO projovemurbano.usuarioresponsabilidade (nucid, usucpf, rpustatus, rpudata_inc, pflcod) 
...
document.getElementById('tabela').style.display  = "none";
	...
	<thead>
		...
		<tr>
			...
			$sql = "SELECT 
						nuc.nucid as codigo, 
						'NÚCLEO '||nuc.nucid||COALESCE(', SEDE: '||(SELECT entnome FROM entidade.entidade ent INNER JOIN projovemurbano.nucleoescola nes ON nes.entid = ent.entid WHERE nes.nucid=nuc.nucid AND nes.nuetipo='S'),'')||COALESCE(', ANEXO: '||(SELECT entnome FROM entidade.entidade ent INNER JOIN projovemurbano.nucleoescola nes ON nes.entid = ent.entid WHERE nes.nucid=nuc.nucid AND nes.nuetipo='A'),'') as descricao 
...
<form name="formassocia" style="margin: 0px;" method="POST"><input
	...
	$sql = "
			select 
				distinct nuc.nucid as codigo, 'NÚCLEO '||nuc.nucid||COALESCE(', SEDE: '||(SELECT entnome FROM entidade.entidade ent INNER JOIN projovemurbano.nucleoescola nes ON nes.entid = ent.entid WHERE nes.nucid=nuc.nucid AND nes.nuetipo='S'),'')||COALESCE(', ANEXO: '||(SELECT entnome FROM entidade.entidade ent INNER JOIN projovemurbano.nucleoescola nes ON nes.entid = ent.entid WHERE nes.nucid=nuc.nucid AND nes.nuetipo='A'),'') as descricao 
			from 
				projovemurbano.usuarioresponsabilidade ur 
			inner join 
				projovemurbano.nucleo nuc on ur.nucid = nuc.nucid
