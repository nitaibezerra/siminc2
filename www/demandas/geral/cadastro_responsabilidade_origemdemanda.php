...
if ( !in_array(DEMANDA_PERFIL_SUPERUSUARIO, $perfil) && !in_array(DEMANDA_PERFIL_GESTOR_MEC, $perfil) ){	
...
if ($_POST && $gravar == 1){
	$ordid = $_REQUEST["uniresp"];
	atribuiOrigemDemanda($usucpf, $pflcod, $ordid);
}
 ...
 * @param string $ordid
 */
function atribuiOrigemDemanda($usucpf, $pflcod, $ordid){	
	global $db;
	...
	if (DEMANDA_PERFIL_COORDENADOR == $pflcod && $ordid[0]){
		$db->executar("UPDATE 
						demandas.usuarioresponsabilidade 
					   SET 
						rpustatus = 'I' 
					   WHERE  
						ordid IN (" . implode(',',$ordid) . ") AND
						pflcod = '{$pflcod}'"); 
	}	
	
	if (is_array($ordid) && !empty($ordid[0])){
		$count = count($ordid);
...
function listaOrigemDemanda(){
	...
	if (DEMANDA_PERFIL_COORDENADOR == $pflcod){
		$select[] = 'ur.rpuid';
		$from[]   = "LEFT JOIN demandas.usuarioresponsabilidade ur ON od.ordid = ur.ordid AND
																 	  ur.pflcod = ".DEMANDA_PERFIL_COORDENADOR." AND
																 	  ur.rpustatus = 'A' AND
																      ur.usucpf != '{$usucpf}'";
	}
	
	if ( !in_array(DEMANDA_PERFIL_SUPERUSUARIO,$perfil) ){
		$from[] = "INNER JOIN demandas.usuarioresponsabilidade ur2 ON od.ordid = ur2.ordid AND
	...
	for ($i = 0; $i < $count; $i++){
		$codigo    = $origemDemanda[$i]["ordid"];
		$descricao = $origemDemanda[$i]["orddescricao"];
