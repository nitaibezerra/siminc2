...
$sqlResponsabilidadesPerfil = "SELECT
							    tr.*
							   FROM
							    demandas.tprperfil p
							    INNER JOIN demandas.tiporesponsabilidade tr ON p.tprcod = tr.tprcod
							   WHERE
...
if (!$responsabilidadesPerfil || @count($responsabilidadesPerfil)<1) {
	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		$sqlRespUsuario = "";
		switch ($rp["tprsigla"]) {
			case "A": // Áreas das Demandas
				$aca_prg = "Áreas Associadas";
				$sqlRespUsuario = "SELECT
									od.ordid AS codigo, 
									od.orddescricao AS descricao, 
									ur.rpustatus AS status
								   FROM
								    demandas.usuarioresponsabilidade ur 
									INNER JOIN demandas.origemdemanda od ON od.ordid = ur.ordid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
				break;
			case "S": // Sistemas
				$aca_prg = "Sistemas Associados";
				$sqlRespUsuario = "SELECT
								    sd.sidid AS codigo, 
								    sd.siddescricao AS descricao, 
								    ur.rpustatus AS status
								   FROM
								    demandas.usuarioresponsabilidade ur 
									INNER JOIN demandas.sistemadetalhe sd ON sd.sidid = ur.sidid
								   WHERE
								    ur.usucpf = '%s' AND 
								    ur.pflcod = '%s' AND 
								    ur.rpustatus='A'";
