...
class Sidor {
	...
	public function login($user, $pass) {
		...
		$this->s->setcookies();
		if(strpos($this->s->results, "<DIV class='erro' id='div_erro'><script>Mostra('Usuário ou Senha inválido!');</script>")!==false) {
			return new SidorLoginException();
		}
	}
...
//			dbg($this->s->results,1);
	...
	public function enviarDadosFinanceiros($momento, $unidade, $dados) {
		...
		do {
			...
			$tentativas++;
			echo "<pre>$tentativas :: $codresp</pre>";
