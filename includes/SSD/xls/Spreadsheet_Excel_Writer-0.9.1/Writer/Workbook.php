...
require_once DIR_COMPONENT . "pear/OLE/OLE-0.5/PPS/Root.php";
require_once DIR_COMPONENT . "pear/OLE/OLE-0.5/PPS/File.php";
...
class Spreadsheet_Excel_Writer_Workbook extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    */
    function Spreadsheet_Excel_Writer_Workbook($filename)
    {
        ...
        $this->_tmp_dir         = ( defined( "DIR_ATTACHMENT_TEMP" ) ) ? DIR_ATTACHMENT_TEMP : "";
    }
    ...
    */
    function close( $strFlagOutput = "L" , $strNameOfFile = "Arquivo" )
    {
        if( $strFlagOutput == "D" ) 
        {
        	$this->send( $strNameOfFile ); 
        }
        if ($this->_fileclosed) { // Prevent close() from being called twice.
            return true;
        }
        $res = $this->_storeWorkbook( $strFlagOutput , $strNameOfFile );
        if ($this->isError($res)) {
    ...
    */
    ...
    {
        include_once DIR_EXTENSION . "pear/xls/Spreadsheet_Excel_Writer-0.9.1/Writer/Validator.php";
    ...
    * @return mixed true on success. PEAR_Error on failure
    */
    function _storeWorkbook( $strFlagOutput = "L" , $strNameOfFile = "Arquivo" )
    {
        ...
        $res = $this->_storeOLEFile( $strFlagOutput , $strNameOfFile );
        if ($this->isError($res)) {
            return $this->raiseError($res->getMessage());
        }
        return( $res );
    ...
    * @return mixed true on success. PEAR_Error on failure
    */
    function _storeOLEFile( $strFlagOutput = "L" , $strNameOfFile = "Arquivo" )
    {
        ...
        $res = $root->save( $this->_filename , $strFlagOutput , $strNameOfFile );
        if ($this->isError($res)) {
            return $this->raiseError("OLE Error: ".$res->getMessage());
        }
        return $res;
