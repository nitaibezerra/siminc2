...
// 						window.html = '<center>'+
        ...
        /**
         * Exibe aviso com o conteúdo de um elemento HTML.
         *
         * @param {string} idCampoDescricao
         * @returns {VOID}
         */
        function exibirAlerta(idCampoDescricao){
            alert($('#'+idCampoDescricao).html());
        }
    </script>
        ...
        <?php if ( count( $estados ) ) : ?>
            ...
            <?php else :?>
                ...
                <?php if($action === false) : ?>
                    ...
                    <?php #Oculta linha contendo a ação cuja a condição para tramitação não esteja atendida
                    if( $estado['aedcodicaonegativa'] == 't') : ?>
                        <?php $nenhumaacao = false; ?>
                        <tr>
                            <td id="td_acao_<?=$estado['aedid'] ?>" style="font-size: 7pt; color: #909090; border-top: 2px solid #d0d0d0; text-align: center;" onclick="exibirAlerta('div_wf_aedobs');">
                                <div style="display: none;" id="div_wf_aedobs"><?php echo $estado['aeddescregracondicao']; ?></div>
                                <a href='#' title='<?php echo $estado['aeddscrealizar']; ?>' onclick="exibirAlerta('div_wf_aedobs')">
                                    <?php echo (($estado['aedicone'])? "<img align=absmiddle src=../imagens/workflow/".$estado['aedicone']." border=0><br/>":""); ?>
                                </a>
